...
main(int argc, char **argv)
{
  ...
  47:	e8 4f 07 00 00       	call   79b <atoi>
  4c:	89 04 24             	mov    %eax,(%esp)
  4f:	e8 08 08 00 00       	call   85c <kill>
  ...
  5f:	7c d1                	jl     32 <main+0x32>
    kill(atoi(argv[i]));
  exit();
  61:	e8 c6 07 00 00       	call   82c <exit>
  66:	90                   	nop
  67:	90                   	nop
...
int copy(char *inputfile, char *outputfile, int used_disk, int max_disk){
  ...
  cf:	e8 98 07 00 00       	call   86c <open>
  d4:	89 45 f0             	mov    %eax,-0x10(%ebp)
  d7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
  db:	79 20                	jns    fd <copy+0x42>
        printf(1, "Cannot open inputfile: %s\n", inputfile);
  dd:	8b 45 08             	mov    0x8(%ebp),%eax
  e0:	89 44 24 08          	mov    %eax,0x8(%esp)
  e4:	c7 44 24 04 83 0d 00 	movl   $0xd83,0x4(%esp)
  eb:	00 
  ec:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
  f3:	e8 b1 08 00 00       	call   9a9 <printf>
        exit();
  f8:	e8 2f 07 00 00       	call   82c <exit>
    }
    if ( (fd2 = open(outputfile, O_CREATE | O_WRONLY)) < 0) {
  fd:	c7 44 24 04 01 02 00 	movl   $0x201,0x4(%esp)
 104:	00 
 105:	8b 45 0c             	mov    0xc(%ebp),%eax
 108:	89 04 24             	mov    %eax,(%esp)
 10b:	e8 5c 07 00 00       	call   86c <open>
 110:	89 45 ec             	mov    %eax,-0x14(%ebp)
 113:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
 117:	79 20                	jns    139 <copy+0x7e>
        printf(1, "Cannot open outputfile: %s\n", outputfile);
 119:	8b 45 0c             	mov    0xc(%ebp),%eax
 11c:	89 44 24 08          	mov    %eax,0x8(%esp)
 120:	c7 44 24 04 9e 0d 00 	movl   $0xd9e,0x4(%esp)
 127:	00 
 128:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 12f:	e8 75 08 00 00       	call   9a9 <printf>
        exit();
 134:	e8 f3 06 00 00       	call   82c <exit>
    }

    while ( (count = read(fd1, buffer, 32)) > 0 ) {
 139:	eb 56                	jmp    191 <copy+0xd6>
        int max = used_disk+=count;
 13b:	8b 45 e8             	mov    -0x18(%ebp),%eax
 13e:	01 45 10             	add    %eax,0x10(%ebp)
 141:	8b 45 10             	mov    0x10(%ebp),%eax
 144:	89 45 e4             	mov    %eax,-0x1c(%ebp)
        printf(1, "This is max: %d\n", max);
 147:	8b 45 e4             	mov    -0x1c(%ebp),%eax
 14a:	89 44 24 08          	mov    %eax,0x8(%esp)
 14e:	c7 44 24 04 ba 0d 00 	movl   $0xdba,0x4(%esp)
 155:	00 
 156:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
 15d:	e8 47 08 00 00       	call   9a9 <printf>
        if(max > max_disk){
 162:	8b 45 e4             	mov    -0x1c(%ebp),%eax
 165:	3b 45 14             	cmp    0x14(%ebp),%eax
 168:	7e 07                	jle    171 <copy+0xb6>
          return -1;
 16a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
 16f:	eb 5c                	jmp    1cd <copy+0x112>
        }
        bytes = bytes + count;
...
memset(void *dst, int c, uint n)
{
 ...
 294:	e8 cf fd ff ff       	call   68 <stosb>
  return dst;
...
printf(int fd, char *fmt, ...)
{
 ...
 a9e:	75 09                	jne    aa9 <printf+0x100>
          s = "(null)";
 aa0:	c7 45 f4 cb 0d 00 00 	movl   $0xdcb,-0xc(%ebp)
        while(*s != 0){
