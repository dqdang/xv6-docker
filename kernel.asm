
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:

# Entering xv6 on boot processor, with paging off.
.globl entry
entry:
  # Turn on page size extension for 4Mbyte pages
  movl    %cr4, %eax
8010000c:	0f 20 e0             	mov    %cr4,%eax
  orl     $(CR4_PSE), %eax
8010000f:	83 c8 10             	or     $0x10,%eax
  movl    %eax, %cr4
80100012:	0f 22 e0             	mov    %eax,%cr4
  # Set page directory
  movl    $(V2P_WO(entrypgdir)), %eax
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
  movl    %eax, %cr3
8010001a:	0f 22 d8             	mov    %eax,%cr3
  # Turn on paging.
  movl    %cr0, %eax
8010001d:	0f 20 c0             	mov    %cr0,%eax
  orl     $(CR0_PG|CR0_WP), %eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
  movl    %eax, %cr0
80100025:	0f 22 c0             	mov    %eax,%cr0

  # Set up the stack pointer.
  movl $(stack + KSTACKSIZE), %esp
80100028:	bc 30 c6 10 80       	mov    $0x8010c630,%esp

  # Jump to main(), and switch to executing at
  # high addresses. The indirect call is needed because
  # the assembler produces a PC-relative instruction
  # for a direct jump.
  mov $main, %eax
8010002d:	b8 0e 38 10 80       	mov    $0x8010380e,%eax
  jmp *%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 ec 81 10 	movl   $0x801081ec,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
80100049:	e8 c4 4d 00 00       	call   80104e12 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 8c 0d 11 80 3c 	movl   $0x80110d3c,0x80110d8c
80100055:	0d 11 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 90 0d 11 80 3c 	movl   $0x80110d3c,0x80110d90
8010005f:	0d 11 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 74 c6 10 80 	movl   $0x8010c674,-0xc(%ebp)
80100069:	eb 46                	jmp    801000b1 <binit+0x7d>
    b->next = bcache.head.next;
8010006b:	8b 15 90 0d 11 80    	mov    0x80110d90,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 54             	mov    %edx,0x54(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 50 3c 0d 11 80 	movl   $0x80110d3c,0x50(%eax)
    initsleeplock(&b->lock, "buffer");
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	83 c0 0c             	add    $0xc,%eax
80100087:	c7 44 24 04 f3 81 10 	movl   $0x801081f3,0x4(%esp)
8010008e:	80 
8010008f:	89 04 24             	mov    %eax,(%esp)
80100092:	e8 3d 4c 00 00       	call   80104cd4 <initsleeplock>
    bcache.head.next->prev = b;
80100097:	a1 90 0d 11 80       	mov    0x80110d90,%eax
8010009c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010009f:	89 50 50             	mov    %edx,0x50(%eax)
    bcache.head.next = b;
801000a2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000a5:	a3 90 0d 11 80       	mov    %eax,0x80110d90

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
801000aa:	81 45 f4 5c 02 00 00 	addl   $0x25c,-0xc(%ebp)
801000b1:	81 7d f4 3c 0d 11 80 	cmpl   $0x80110d3c,-0xc(%ebp)
801000b8:	72 b1                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    initsleeplock(&b->lock, "buffer");
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ba:	c9                   	leave  
801000bb:	c3                   	ret    

801000bc <bget>:
// Look through buffer cache for block on device dev.
// If not found, allocate a buffer.
// In either case, return locked buffer.
static struct buf*
bget(uint dev, uint blockno)
{
801000bc:	55                   	push   %ebp
801000bd:	89 e5                	mov    %esp,%ebp
801000bf:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000c2:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
801000c9:	e8 65 4d 00 00       	call   80104e33 <acquire>

  // Is the block already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000ce:	a1 90 0d 11 80       	mov    0x80110d90,%eax
801000d3:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000d6:	eb 50                	jmp    80100128 <bget+0x6c>
    if(b->dev == dev && b->blockno == blockno){
801000d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000db:	8b 40 04             	mov    0x4(%eax),%eax
801000de:	3b 45 08             	cmp    0x8(%ebp),%eax
801000e1:	75 3c                	jne    8010011f <bget+0x63>
801000e3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e6:	8b 40 08             	mov    0x8(%eax),%eax
801000e9:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000ec:	75 31                	jne    8010011f <bget+0x63>
      b->refcnt++;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 40 4c             	mov    0x4c(%eax),%eax
801000f4:	8d 50 01             	lea    0x1(%eax),%edx
801000f7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fa:	89 50 4c             	mov    %edx,0x4c(%eax)
      release(&bcache.lock);
801000fd:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
80100104:	e8 94 4d 00 00       	call   80104e9d <release>
      acquiresleep(&b->lock);
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	83 c0 0c             	add    $0xc,%eax
8010010f:	89 04 24             	mov    %eax,(%esp)
80100112:	e8 f7 4b 00 00       	call   80104d0e <acquiresleep>
      return b;
80100117:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011a:	e9 94 00 00 00       	jmp    801001b3 <bget+0xf7>
  struct buf *b;

  acquire(&bcache.lock);

  // Is the block already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
8010011f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100122:	8b 40 54             	mov    0x54(%eax),%eax
80100125:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100128:	81 7d f4 3c 0d 11 80 	cmpl   $0x80110d3c,-0xc(%ebp)
8010012f:	75 a7                	jne    801000d8 <bget+0x1c>
  }

  // Not cached; recycle an unused buffer.
  // Even if refcnt==0, B_DIRTY indicates a buffer is in use
  // because log.c has modified it but not yet committed it.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100131:	a1 8c 0d 11 80       	mov    0x80110d8c,%eax
80100136:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100139:	eb 63                	jmp    8010019e <bget+0xe2>
    if(b->refcnt == 0 && (b->flags & B_DIRTY) == 0) {
8010013b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010013e:	8b 40 4c             	mov    0x4c(%eax),%eax
80100141:	85 c0                	test   %eax,%eax
80100143:	75 50                	jne    80100195 <bget+0xd9>
80100145:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100148:	8b 00                	mov    (%eax),%eax
8010014a:	83 e0 04             	and    $0x4,%eax
8010014d:	85 c0                	test   %eax,%eax
8010014f:	75 44                	jne    80100195 <bget+0xd9>
      b->dev = dev;
80100151:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100154:	8b 55 08             	mov    0x8(%ebp),%edx
80100157:	89 50 04             	mov    %edx,0x4(%eax)
      b->blockno = blockno;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 0c             	mov    0xc(%ebp),%edx
80100160:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = 0;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
      b->refcnt = 1;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 40 4c 01 00 00 00 	movl   $0x1,0x4c(%eax)
      release(&bcache.lock);
80100176:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
8010017d:	e8 1b 4d 00 00       	call   80104e9d <release>
      acquiresleep(&b->lock);
80100182:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100185:	83 c0 0c             	add    $0xc,%eax
80100188:	89 04 24             	mov    %eax,(%esp)
8010018b:	e8 7e 4b 00 00       	call   80104d0e <acquiresleep>
      return b;
80100190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100193:	eb 1e                	jmp    801001b3 <bget+0xf7>
  }

  // Not cached; recycle an unused buffer.
  // Even if refcnt==0, B_DIRTY indicates a buffer is in use
  // because log.c has modified it but not yet committed it.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100195:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100198:	8b 40 50             	mov    0x50(%eax),%eax
8010019b:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010019e:	81 7d f4 3c 0d 11 80 	cmpl   $0x80110d3c,-0xc(%ebp)
801001a5:	75 94                	jne    8010013b <bget+0x7f>
      release(&bcache.lock);
      acquiresleep(&b->lock);
      return b;
    }
  }
  panic("bget: no buffers");
801001a7:	c7 04 24 fa 81 10 80 	movl   $0x801081fa,(%esp)
801001ae:	e8 a1 03 00 00       	call   80100554 <panic>
}
801001b3:	c9                   	leave  
801001b4:	c3                   	ret    

801001b5 <bread>:

// Return a locked buf with the contents of the indicated block.
struct buf*
bread(uint dev, uint blockno)
{
801001b5:	55                   	push   %ebp
801001b6:	89 e5                	mov    %esp,%ebp
801001b8:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, blockno);
801001bb:	8b 45 0c             	mov    0xc(%ebp),%eax
801001be:	89 44 24 04          	mov    %eax,0x4(%esp)
801001c2:	8b 45 08             	mov    0x8(%ebp),%eax
801001c5:	89 04 24             	mov    %eax,(%esp)
801001c8:	e8 ef fe ff ff       	call   801000bc <bget>
801001cd:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((b->flags & B_VALID) == 0) {
801001d0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d3:	8b 00                	mov    (%eax),%eax
801001d5:	83 e0 02             	and    $0x2,%eax
801001d8:	85 c0                	test   %eax,%eax
801001da:	75 0b                	jne    801001e7 <bread+0x32>
    iderw(b);
801001dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001df:	89 04 24             	mov    %eax,(%esp)
801001e2:	e8 5e 27 00 00       	call   80102945 <iderw>
  }
  return b;
801001e7:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001ea:	c9                   	leave  
801001eb:	c3                   	ret    

801001ec <bwrite>:

// Write b's contents to disk.  Must be locked.
void
bwrite(struct buf *b)
{
801001ec:	55                   	push   %ebp
801001ed:	89 e5                	mov    %esp,%ebp
801001ef:	83 ec 18             	sub    $0x18,%esp
  if(!holdingsleep(&b->lock))
801001f2:	8b 45 08             	mov    0x8(%ebp),%eax
801001f5:	83 c0 0c             	add    $0xc,%eax
801001f8:	89 04 24             	mov    %eax,(%esp)
801001fb:	e8 ab 4b 00 00       	call   80104dab <holdingsleep>
80100200:	85 c0                	test   %eax,%eax
80100202:	75 0c                	jne    80100210 <bwrite+0x24>
    panic("bwrite");
80100204:	c7 04 24 0b 82 10 80 	movl   $0x8010820b,(%esp)
8010020b:	e8 44 03 00 00       	call   80100554 <panic>
  b->flags |= B_DIRTY;
80100210:	8b 45 08             	mov    0x8(%ebp),%eax
80100213:	8b 00                	mov    (%eax),%eax
80100215:	83 c8 04             	or     $0x4,%eax
80100218:	89 c2                	mov    %eax,%edx
8010021a:	8b 45 08             	mov    0x8(%ebp),%eax
8010021d:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010021f:	8b 45 08             	mov    0x8(%ebp),%eax
80100222:	89 04 24             	mov    %eax,(%esp)
80100225:	e8 1b 27 00 00       	call   80102945 <iderw>
}
8010022a:	c9                   	leave  
8010022b:	c3                   	ret    

8010022c <brelse>:

// Release a locked buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
8010022c:	55                   	push   %ebp
8010022d:	89 e5                	mov    %esp,%ebp
8010022f:	83 ec 18             	sub    $0x18,%esp
  if(!holdingsleep(&b->lock))
80100232:	8b 45 08             	mov    0x8(%ebp),%eax
80100235:	83 c0 0c             	add    $0xc,%eax
80100238:	89 04 24             	mov    %eax,(%esp)
8010023b:	e8 6b 4b 00 00       	call   80104dab <holdingsleep>
80100240:	85 c0                	test   %eax,%eax
80100242:	75 0c                	jne    80100250 <brelse+0x24>
    panic("brelse");
80100244:	c7 04 24 12 82 10 80 	movl   $0x80108212,(%esp)
8010024b:	e8 04 03 00 00       	call   80100554 <panic>

  releasesleep(&b->lock);
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	83 c0 0c             	add    $0xc,%eax
80100256:	89 04 24             	mov    %eax,(%esp)
80100259:	e8 0b 4b 00 00       	call   80104d69 <releasesleep>

  acquire(&bcache.lock);
8010025e:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
80100265:	e8 c9 4b 00 00       	call   80104e33 <acquire>
  b->refcnt--;
8010026a:	8b 45 08             	mov    0x8(%ebp),%eax
8010026d:	8b 40 4c             	mov    0x4c(%eax),%eax
80100270:	8d 50 ff             	lea    -0x1(%eax),%edx
80100273:	8b 45 08             	mov    0x8(%ebp),%eax
80100276:	89 50 4c             	mov    %edx,0x4c(%eax)
  if (b->refcnt == 0) {
80100279:	8b 45 08             	mov    0x8(%ebp),%eax
8010027c:	8b 40 4c             	mov    0x4c(%eax),%eax
8010027f:	85 c0                	test   %eax,%eax
80100281:	75 47                	jne    801002ca <brelse+0x9e>
    // no one is waiting for it.
    b->next->prev = b->prev;
80100283:	8b 45 08             	mov    0x8(%ebp),%eax
80100286:	8b 40 54             	mov    0x54(%eax),%eax
80100289:	8b 55 08             	mov    0x8(%ebp),%edx
8010028c:	8b 52 50             	mov    0x50(%edx),%edx
8010028f:	89 50 50             	mov    %edx,0x50(%eax)
    b->prev->next = b->next;
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	8b 40 50             	mov    0x50(%eax),%eax
80100298:	8b 55 08             	mov    0x8(%ebp),%edx
8010029b:	8b 52 54             	mov    0x54(%edx),%edx
8010029e:	89 50 54             	mov    %edx,0x54(%eax)
    b->next = bcache.head.next;
801002a1:	8b 15 90 0d 11 80    	mov    0x80110d90,%edx
801002a7:	8b 45 08             	mov    0x8(%ebp),%eax
801002aa:	89 50 54             	mov    %edx,0x54(%eax)
    b->prev = &bcache.head;
801002ad:	8b 45 08             	mov    0x8(%ebp),%eax
801002b0:	c7 40 50 3c 0d 11 80 	movl   $0x80110d3c,0x50(%eax)
    bcache.head.next->prev = b;
801002b7:	a1 90 0d 11 80       	mov    0x80110d90,%eax
801002bc:	8b 55 08             	mov    0x8(%ebp),%edx
801002bf:	89 50 50             	mov    %edx,0x50(%eax)
    bcache.head.next = b;
801002c2:	8b 45 08             	mov    0x8(%ebp),%eax
801002c5:	a3 90 0d 11 80       	mov    %eax,0x80110d90
  }
  
  release(&bcache.lock);
801002ca:	c7 04 24 40 c6 10 80 	movl   $0x8010c640,(%esp)
801002d1:	e8 c7 4b 00 00       	call   80104e9d <release>
}
801002d6:	c9                   	leave  
801002d7:	c3                   	ret    

801002d8 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002d8:	55                   	push   %ebp
801002d9:	89 e5                	mov    %esp,%ebp
801002db:	83 ec 14             	sub    $0x14,%esp
801002de:	8b 45 08             	mov    0x8(%ebp),%eax
801002e1:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002e5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801002e8:	89 c2                	mov    %eax,%edx
801002ea:	ec                   	in     (%dx),%al
801002eb:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801002ee:	8a 45 ff             	mov    -0x1(%ebp),%al
}
801002f1:	c9                   	leave  
801002f2:	c3                   	ret    

801002f3 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002f3:	55                   	push   %ebp
801002f4:	89 e5                	mov    %esp,%ebp
801002f6:	83 ec 08             	sub    $0x8,%esp
801002f9:	8b 45 08             	mov    0x8(%ebp),%eax
801002fc:	8b 55 0c             	mov    0xc(%ebp),%edx
801002ff:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
80100303:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80100306:	8a 45 f8             	mov    -0x8(%ebp),%al
80100309:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010030c:	ee                   	out    %al,(%dx)
}
8010030d:	c9                   	leave  
8010030e:	c3                   	ret    

8010030f <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
8010030f:	55                   	push   %ebp
80100310:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80100312:	fa                   	cli    
}
80100313:	5d                   	pop    %ebp
80100314:	c3                   	ret    

80100315 <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
80100315:	55                   	push   %ebp
80100316:	89 e5                	mov    %esp,%ebp
80100318:	56                   	push   %esi
80100319:	53                   	push   %ebx
8010031a:	83 ec 30             	sub    $0x30,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
8010031d:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100321:	74 1c                	je     8010033f <printint+0x2a>
80100323:	8b 45 08             	mov    0x8(%ebp),%eax
80100326:	c1 e8 1f             	shr    $0x1f,%eax
80100329:	0f b6 c0             	movzbl %al,%eax
8010032c:	89 45 10             	mov    %eax,0x10(%ebp)
8010032f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100333:	74 0a                	je     8010033f <printint+0x2a>
    x = -xx;
80100335:	8b 45 08             	mov    0x8(%ebp),%eax
80100338:	f7 d8                	neg    %eax
8010033a:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010033d:	eb 06                	jmp    80100345 <printint+0x30>
  else
    x = xx;
8010033f:	8b 45 08             	mov    0x8(%ebp),%eax
80100342:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100345:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
8010034c:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010034f:	8d 41 01             	lea    0x1(%ecx),%eax
80100352:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100355:	8b 5d 0c             	mov    0xc(%ebp),%ebx
80100358:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010035b:	ba 00 00 00 00       	mov    $0x0,%edx
80100360:	f7 f3                	div    %ebx
80100362:	89 d0                	mov    %edx,%eax
80100364:	8a 80 04 90 10 80    	mov    -0x7fef6ffc(%eax),%al
8010036a:	88 44 0d e0          	mov    %al,-0x20(%ebp,%ecx,1)
  }while((x /= base) != 0);
8010036e:	8b 75 0c             	mov    0xc(%ebp),%esi
80100371:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100374:	ba 00 00 00 00       	mov    $0x0,%edx
80100379:	f7 f6                	div    %esi
8010037b:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010037e:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80100382:	75 c8                	jne    8010034c <printint+0x37>

  if(sign)
80100384:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100388:	74 10                	je     8010039a <printint+0x85>
    buf[i++] = '-';
8010038a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010038d:	8d 50 01             	lea    0x1(%eax),%edx
80100390:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100393:	c6 44 05 e0 2d       	movb   $0x2d,-0x20(%ebp,%eax,1)

  while(--i >= 0)
80100398:	eb 17                	jmp    801003b1 <printint+0x9c>
8010039a:	eb 15                	jmp    801003b1 <printint+0x9c>
    consputc(buf[i]);
8010039c:	8d 55 e0             	lea    -0x20(%ebp),%edx
8010039f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801003a2:	01 d0                	add    %edx,%eax
801003a4:	8a 00                	mov    (%eax),%al
801003a6:	0f be c0             	movsbl %al,%eax
801003a9:	89 04 24             	mov    %eax,(%esp)
801003ac:	e8 b7 03 00 00       	call   80100768 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
801003b1:	ff 4d f4             	decl   -0xc(%ebp)
801003b4:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801003b8:	79 e2                	jns    8010039c <printint+0x87>
    consputc(buf[i]);
}
801003ba:	83 c4 30             	add    $0x30,%esp
801003bd:	5b                   	pop    %ebx
801003be:	5e                   	pop    %esi
801003bf:	5d                   	pop    %ebp
801003c0:	c3                   	ret    

801003c1 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003c1:	55                   	push   %ebp
801003c2:	89 e5                	mov    %esp,%ebp
801003c4:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003c7:	a1 d4 b5 10 80       	mov    0x8010b5d4,%eax
801003cc:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003cf:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003d3:	74 0c                	je     801003e1 <cprintf+0x20>
    acquire(&cons.lock);
801003d5:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
801003dc:	e8 52 4a 00 00       	call   80104e33 <acquire>

  if (fmt == 0)
801003e1:	8b 45 08             	mov    0x8(%ebp),%eax
801003e4:	85 c0                	test   %eax,%eax
801003e6:	75 0c                	jne    801003f4 <cprintf+0x33>
    panic("null fmt");
801003e8:	c7 04 24 19 82 10 80 	movl   $0x80108219,(%esp)
801003ef:	e8 60 01 00 00       	call   80100554 <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003f4:	8d 45 0c             	lea    0xc(%ebp),%eax
801003f7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003fa:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100401:	e9 1b 01 00 00       	jmp    80100521 <cprintf+0x160>
    if(c != '%'){
80100406:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
8010040a:	74 10                	je     8010041c <cprintf+0x5b>
      consputc(c);
8010040c:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010040f:	89 04 24             	mov    %eax,(%esp)
80100412:	e8 51 03 00 00       	call   80100768 <consputc>
      continue;
80100417:	e9 02 01 00 00       	jmp    8010051e <cprintf+0x15d>
    }
    c = fmt[++i] & 0xff;
8010041c:	8b 55 08             	mov    0x8(%ebp),%edx
8010041f:	ff 45 f4             	incl   -0xc(%ebp)
80100422:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100425:	01 d0                	add    %edx,%eax
80100427:	8a 00                	mov    (%eax),%al
80100429:	0f be c0             	movsbl %al,%eax
8010042c:	25 ff 00 00 00       	and    $0xff,%eax
80100431:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100434:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100438:	75 05                	jne    8010043f <cprintf+0x7e>
      break;
8010043a:	e9 01 01 00 00       	jmp    80100540 <cprintf+0x17f>
    switch(c){
8010043f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100442:	83 f8 70             	cmp    $0x70,%eax
80100445:	74 4f                	je     80100496 <cprintf+0xd5>
80100447:	83 f8 70             	cmp    $0x70,%eax
8010044a:	7f 13                	jg     8010045f <cprintf+0x9e>
8010044c:	83 f8 25             	cmp    $0x25,%eax
8010044f:	0f 84 a3 00 00 00    	je     801004f8 <cprintf+0x137>
80100455:	83 f8 64             	cmp    $0x64,%eax
80100458:	74 14                	je     8010046e <cprintf+0xad>
8010045a:	e9 a7 00 00 00       	jmp    80100506 <cprintf+0x145>
8010045f:	83 f8 73             	cmp    $0x73,%eax
80100462:	74 57                	je     801004bb <cprintf+0xfa>
80100464:	83 f8 78             	cmp    $0x78,%eax
80100467:	74 2d                	je     80100496 <cprintf+0xd5>
80100469:	e9 98 00 00 00       	jmp    80100506 <cprintf+0x145>
    case 'd':
      printint(*argp++, 10, 1);
8010046e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100471:	8d 50 04             	lea    0x4(%eax),%edx
80100474:	89 55 f0             	mov    %edx,-0x10(%ebp)
80100477:	8b 00                	mov    (%eax),%eax
80100479:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80100480:	00 
80100481:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100488:	00 
80100489:	89 04 24             	mov    %eax,(%esp)
8010048c:	e8 84 fe ff ff       	call   80100315 <printint>
      break;
80100491:	e9 88 00 00 00       	jmp    8010051e <cprintf+0x15d>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100496:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100499:	8d 50 04             	lea    0x4(%eax),%edx
8010049c:	89 55 f0             	mov    %edx,-0x10(%ebp)
8010049f:	8b 00                	mov    (%eax),%eax
801004a1:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801004a8:	00 
801004a9:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801004b0:	00 
801004b1:	89 04 24             	mov    %eax,(%esp)
801004b4:	e8 5c fe ff ff       	call   80100315 <printint>
      break;
801004b9:	eb 63                	jmp    8010051e <cprintf+0x15d>
    case 's':
      if((s = (char*)*argp++) == 0)
801004bb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801004be:	8d 50 04             	lea    0x4(%eax),%edx
801004c1:	89 55 f0             	mov    %edx,-0x10(%ebp)
801004c4:	8b 00                	mov    (%eax),%eax
801004c6:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004c9:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004cd:	75 09                	jne    801004d8 <cprintf+0x117>
        s = "(null)";
801004cf:	c7 45 ec 22 82 10 80 	movl   $0x80108222,-0x14(%ebp)
      for(; *s; s++)
801004d6:	eb 15                	jmp    801004ed <cprintf+0x12c>
801004d8:	eb 13                	jmp    801004ed <cprintf+0x12c>
        consputc(*s);
801004da:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004dd:	8a 00                	mov    (%eax),%al
801004df:	0f be c0             	movsbl %al,%eax
801004e2:	89 04 24             	mov    %eax,(%esp)
801004e5:	e8 7e 02 00 00       	call   80100768 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004ea:	ff 45 ec             	incl   -0x14(%ebp)
801004ed:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004f0:	8a 00                	mov    (%eax),%al
801004f2:	84 c0                	test   %al,%al
801004f4:	75 e4                	jne    801004da <cprintf+0x119>
        consputc(*s);
      break;
801004f6:	eb 26                	jmp    8010051e <cprintf+0x15d>
    case '%':
      consputc('%');
801004f8:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004ff:	e8 64 02 00 00       	call   80100768 <consputc>
      break;
80100504:	eb 18                	jmp    8010051e <cprintf+0x15d>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
80100506:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
8010050d:	e8 56 02 00 00       	call   80100768 <consputc>
      consputc(c);
80100512:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100515:	89 04 24             	mov    %eax,(%esp)
80100518:	e8 4b 02 00 00       	call   80100768 <consputc>
      break;
8010051d:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
8010051e:	ff 45 f4             	incl   -0xc(%ebp)
80100521:	8b 55 08             	mov    0x8(%ebp),%edx
80100524:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100527:	01 d0                	add    %edx,%eax
80100529:	8a 00                	mov    (%eax),%al
8010052b:	0f be c0             	movsbl %al,%eax
8010052e:	25 ff 00 00 00       	and    $0xff,%eax
80100533:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80100536:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010053a:	0f 85 c6 fe ff ff    	jne    80100406 <cprintf+0x45>
      consputc(c);
      break;
    }
  }

  if(locking)
80100540:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80100544:	74 0c                	je     80100552 <cprintf+0x191>
    release(&cons.lock);
80100546:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
8010054d:	e8 4b 49 00 00       	call   80104e9d <release>
}
80100552:	c9                   	leave  
80100553:	c3                   	ret    

80100554 <panic>:

void
panic(char *s)
{
80100554:	55                   	push   %ebp
80100555:	89 e5                	mov    %esp,%ebp
80100557:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];

  cli();
8010055a:	e8 b0 fd ff ff       	call   8010030f <cli>
  cons.locking = 0;
8010055f:	c7 05 d4 b5 10 80 00 	movl   $0x0,0x8010b5d4
80100566:	00 00 00 
  // use lapiccpunum so that we can call panic from mycpu()
  cprintf("lapicid %d: panic: ", lapicid());
80100569:	e8 73 2a 00 00       	call   80102fe1 <lapicid>
8010056e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100572:	c7 04 24 29 82 10 80 	movl   $0x80108229,(%esp)
80100579:	e8 43 fe ff ff       	call   801003c1 <cprintf>
  cprintf(s);
8010057e:	8b 45 08             	mov    0x8(%ebp),%eax
80100581:	89 04 24             	mov    %eax,(%esp)
80100584:	e8 38 fe ff ff       	call   801003c1 <cprintf>
  cprintf("\n");
80100589:	c7 04 24 3d 82 10 80 	movl   $0x8010823d,(%esp)
80100590:	e8 2c fe ff ff       	call   801003c1 <cprintf>
  getcallerpcs(&s, pcs);
80100595:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100598:	89 44 24 04          	mov    %eax,0x4(%esp)
8010059c:	8d 45 08             	lea    0x8(%ebp),%eax
8010059f:	89 04 24             	mov    %eax,(%esp)
801005a2:	e8 43 49 00 00       	call   80104eea <getcallerpcs>
  for(i=0; i<10; i++)
801005a7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801005ae:	eb 1a                	jmp    801005ca <panic+0x76>
    cprintf(" %p", pcs[i]);
801005b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005b3:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801005bb:	c7 04 24 3f 82 10 80 	movl   $0x8010823f,(%esp)
801005c2:	e8 fa fd ff ff       	call   801003c1 <cprintf>
  // use lapiccpunum so that we can call panic from mycpu()
  cprintf("lapicid %d: panic: ", lapicid());
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005c7:	ff 45 f4             	incl   -0xc(%ebp)
801005ca:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005ce:	7e e0                	jle    801005b0 <panic+0x5c>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005d0:	c7 05 80 b5 10 80 01 	movl   $0x1,0x8010b580
801005d7:	00 00 00 
  for(;;)
    ;
801005da:	eb fe                	jmp    801005da <panic+0x86>

801005dc <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005dc:	55                   	push   %ebp
801005dd:	89 e5                	mov    %esp,%ebp
801005df:	83 ec 28             	sub    $0x28,%esp
  int pos;

  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005e2:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005e9:	00 
801005ea:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005f1:	e8 fd fc ff ff       	call   801002f3 <outb>
  pos = inb(CRTPORT+1) << 8;
801005f6:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005fd:	e8 d6 fc ff ff       	call   801002d8 <inb>
80100602:	0f b6 c0             	movzbl %al,%eax
80100605:	c1 e0 08             	shl    $0x8,%eax
80100608:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
8010060b:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100612:	00 
80100613:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010061a:	e8 d4 fc ff ff       	call   801002f3 <outb>
  pos |= inb(CRTPORT+1);
8010061f:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100626:	e8 ad fc ff ff       	call   801002d8 <inb>
8010062b:	0f b6 c0             	movzbl %al,%eax
8010062e:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
80100631:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100635:	75 1b                	jne    80100652 <cgaputc+0x76>
    pos += 80 - pos%80;
80100637:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010063a:	b9 50 00 00 00       	mov    $0x50,%ecx
8010063f:	99                   	cltd   
80100640:	f7 f9                	idiv   %ecx
80100642:	89 d0                	mov    %edx,%eax
80100644:	ba 50 00 00 00       	mov    $0x50,%edx
80100649:	29 c2                	sub    %eax,%edx
8010064b:	89 d0                	mov    %edx,%eax
8010064d:	01 45 f4             	add    %eax,-0xc(%ebp)
80100650:	eb 34                	jmp    80100686 <cgaputc+0xaa>
  else if(c == BACKSPACE){
80100652:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
80100659:	75 0b                	jne    80100666 <cgaputc+0x8a>
    if(pos > 0) --pos;
8010065b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010065f:	7e 25                	jle    80100686 <cgaputc+0xaa>
80100661:	ff 4d f4             	decl   -0xc(%ebp)
80100664:	eb 20                	jmp    80100686 <cgaputc+0xaa>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
80100666:	8b 0d 00 90 10 80    	mov    0x80109000,%ecx
8010066c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010066f:	8d 50 01             	lea    0x1(%eax),%edx
80100672:	89 55 f4             	mov    %edx,-0xc(%ebp)
80100675:	01 c0                	add    %eax,%eax
80100677:	8d 14 01             	lea    (%ecx,%eax,1),%edx
8010067a:	8b 45 08             	mov    0x8(%ebp),%eax
8010067d:	0f b6 c0             	movzbl %al,%eax
80100680:	80 cc 07             	or     $0x7,%ah
80100683:	66 89 02             	mov    %ax,(%edx)

  if(pos < 0 || pos > 25*80)
80100686:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010068a:	78 09                	js     80100695 <cgaputc+0xb9>
8010068c:	81 7d f4 d0 07 00 00 	cmpl   $0x7d0,-0xc(%ebp)
80100693:	7e 0c                	jle    801006a1 <cgaputc+0xc5>
    panic("pos under/overflow");
80100695:	c7 04 24 43 82 10 80 	movl   $0x80108243,(%esp)
8010069c:	e8 b3 fe ff ff       	call   80100554 <panic>

  if((pos/80) >= 24){  // Scroll up.
801006a1:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
801006a8:	7e 53                	jle    801006fd <cgaputc+0x121>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
801006aa:	a1 00 90 10 80       	mov    0x80109000,%eax
801006af:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
801006b5:	a1 00 90 10 80       	mov    0x80109000,%eax
801006ba:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006c1:	00 
801006c2:	89 54 24 04          	mov    %edx,0x4(%esp)
801006c6:	89 04 24             	mov    %eax,(%esp)
801006c9:	e8 91 4a 00 00       	call   8010515f <memmove>
    pos -= 80;
801006ce:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006d2:	b8 80 07 00 00       	mov    $0x780,%eax
801006d7:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006da:	01 c0                	add    %eax,%eax
801006dc:	8b 0d 00 90 10 80    	mov    0x80109000,%ecx
801006e2:	8b 55 f4             	mov    -0xc(%ebp),%edx
801006e5:	01 d2                	add    %edx,%edx
801006e7:	01 ca                	add    %ecx,%edx
801006e9:	89 44 24 08          	mov    %eax,0x8(%esp)
801006ed:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006f4:	00 
801006f5:	89 14 24             	mov    %edx,(%esp)
801006f8:	e8 99 49 00 00       	call   80105096 <memset>
  }

  outb(CRTPORT, 14);
801006fd:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
80100704:	00 
80100705:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010070c:	e8 e2 fb ff ff       	call   801002f3 <outb>
  outb(CRTPORT+1, pos>>8);
80100711:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100714:	c1 f8 08             	sar    $0x8,%eax
80100717:	0f b6 c0             	movzbl %al,%eax
8010071a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010071e:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100725:	e8 c9 fb ff ff       	call   801002f3 <outb>
  outb(CRTPORT, 15);
8010072a:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100731:	00 
80100732:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100739:	e8 b5 fb ff ff       	call   801002f3 <outb>
  outb(CRTPORT+1, pos);
8010073e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100741:	0f b6 c0             	movzbl %al,%eax
80100744:	89 44 24 04          	mov    %eax,0x4(%esp)
80100748:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010074f:	e8 9f fb ff ff       	call   801002f3 <outb>
  crt[pos] = ' ' | 0x0700;
80100754:	8b 15 00 90 10 80    	mov    0x80109000,%edx
8010075a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010075d:	01 c0                	add    %eax,%eax
8010075f:	01 d0                	add    %edx,%eax
80100761:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
80100766:	c9                   	leave  
80100767:	c3                   	ret    

80100768 <consputc>:

void
consputc(int c)
{
80100768:	55                   	push   %ebp
80100769:	89 e5                	mov    %esp,%ebp
8010076b:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
8010076e:	a1 80 b5 10 80       	mov    0x8010b580,%eax
80100773:	85 c0                	test   %eax,%eax
80100775:	74 07                	je     8010077e <consputc+0x16>
    cli();
80100777:	e8 93 fb ff ff       	call   8010030f <cli>
    for(;;)
      ;
8010077c:	eb fe                	jmp    8010077c <consputc+0x14>
  }

  if(c == BACKSPACE){
8010077e:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
80100785:	75 26                	jne    801007ad <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 d9 61 00 00       	call   8010696c <uartputc>
80100793:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
8010079a:	e8 cd 61 00 00       	call   8010696c <uartputc>
8010079f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
801007a6:	e8 c1 61 00 00       	call   8010696c <uartputc>
801007ab:	eb 0b                	jmp    801007b8 <consputc+0x50>
  } else
    uartputc(c);
801007ad:	8b 45 08             	mov    0x8(%ebp),%eax
801007b0:	89 04 24             	mov    %eax,(%esp)
801007b3:	e8 b4 61 00 00       	call   8010696c <uartputc>
  cgaputc(c);
801007b8:	8b 45 08             	mov    0x8(%ebp),%eax
801007bb:	89 04 24             	mov    %eax,(%esp)
801007be:	e8 19 fe ff ff       	call   801005dc <cgaputc>
}
801007c3:	c9                   	leave  
801007c4:	c3                   	ret    

801007c5 <copy_buf>:

#define C(x)  ((x)-'@')  // Control-x


void copy_buf(char *dst, char *src, int len)
{
801007c5:	55                   	push   %ebp
801007c6:	89 e5                	mov    %esp,%ebp
801007c8:	83 ec 10             	sub    $0x10,%esp
  int i;

  for (i = 0; i < len; i++) {
801007cb:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801007d2:	eb 17                	jmp    801007eb <copy_buf+0x26>
    dst[i] = src[i];
801007d4:	8b 55 fc             	mov    -0x4(%ebp),%edx
801007d7:	8b 45 08             	mov    0x8(%ebp),%eax
801007da:	01 c2                	add    %eax,%edx
801007dc:	8b 4d fc             	mov    -0x4(%ebp),%ecx
801007df:	8b 45 0c             	mov    0xc(%ebp),%eax
801007e2:	01 c8                	add    %ecx,%eax
801007e4:	8a 00                	mov    (%eax),%al
801007e6:	88 02                	mov    %al,(%edx)

void copy_buf(char *dst, char *src, int len)
{
  int i;

  for (i = 0; i < len; i++) {
801007e8:	ff 45 fc             	incl   -0x4(%ebp)
801007eb:	8b 45 fc             	mov    -0x4(%ebp),%eax
801007ee:	3b 45 10             	cmp    0x10(%ebp),%eax
801007f1:	7c e1                	jl     801007d4 <copy_buf+0xf>
    dst[i] = src[i];
  }
}
801007f3:	c9                   	leave  
801007f4:	c3                   	ret    

801007f5 <consoleintr>:

void
consoleintr(int (*getc)(void))
{
801007f5:	55                   	push   %ebp
801007f6:	89 e5                	mov    %esp,%ebp
801007f8:	57                   	push   %edi
801007f9:	56                   	push   %esi
801007fa:	53                   	push   %ebx
801007fb:	83 ec 2c             	sub    $0x2c,%esp
  int c, doprocdump = 0, doconsoleswitch = 0;
801007fe:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100805:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)

  acquire(&cons.lock);
8010080c:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100813:	e8 1b 46 00 00       	call   80104e33 <acquire>
  while((c = getc()) >= 0){
80100818:	e9 ca 01 00 00       	jmp    801009e7 <consoleintr+0x1f2>
    switch(c){
8010081d:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100820:	83 f8 14             	cmp    $0x14,%eax
80100823:	74 3b                	je     80100860 <consoleintr+0x6b>
80100825:	83 f8 14             	cmp    $0x14,%eax
80100828:	7f 13                	jg     8010083d <consoleintr+0x48>
8010082a:	83 f8 08             	cmp    $0x8,%eax
8010082d:	0f 84 f5 00 00 00    	je     80100928 <consoleintr+0x133>
80100833:	83 f8 10             	cmp    $0x10,%eax
80100836:	74 1c                	je     80100854 <consoleintr+0x5f>
80100838:	e9 1b 01 00 00       	jmp    80100958 <consoleintr+0x163>
8010083d:	83 f8 15             	cmp    $0x15,%eax
80100840:	0f 84 ba 00 00 00    	je     80100900 <consoleintr+0x10b>
80100846:	83 f8 7f             	cmp    $0x7f,%eax
80100849:	0f 84 d9 00 00 00    	je     80100928 <consoleintr+0x133>
8010084f:	e9 04 01 00 00       	jmp    80100958 <consoleintr+0x163>
    case C('P'):  // Process listing.
      // procdump() locks cons.lock indirectly; invoke later
      doprocdump = 1;
80100854:	c7 45 e4 01 00 00 00 	movl   $0x1,-0x1c(%ebp)
      break;
8010085b:	e9 87 01 00 00       	jmp    801009e7 <consoleintr+0x1f2>
    case C('T'):  // Process listing.
      inputs[active] = input;
80100860:	8b 15 84 b5 10 80    	mov    0x8010b584,%edx
80100866:	89 d0                	mov    %edx,%eax
80100868:	c1 e0 02             	shl    $0x2,%eax
8010086b:	01 d0                	add    %edx,%eax
8010086d:	c1 e0 02             	shl    $0x2,%eax
80100870:	8d 14 c5 00 00 00 00 	lea    0x0(,%eax,8),%edx
80100877:	29 c2                	sub    %eax,%edx
80100879:	8d 82 40 10 11 80    	lea    -0x7feeefc0(%edx),%eax
8010087f:	89 c2                	mov    %eax,%edx
80100881:	bb a0 0f 11 80       	mov    $0x80110fa0,%ebx
80100886:	b8 23 00 00 00       	mov    $0x23,%eax
8010088b:	89 d7                	mov    %edx,%edi
8010088d:	89 de                	mov    %ebx,%esi
8010088f:	89 c1                	mov    %eax,%ecx
80100891:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
      active = (active + 1) % (NUM_VCS + 1);
80100893:	a1 84 b5 10 80       	mov    0x8010b584,%eax
80100898:	40                   	inc    %eax
80100899:	b9 05 00 00 00       	mov    $0x5,%ecx
8010089e:	99                   	cltd   
8010089f:	f7 f9                	idiv   %ecx
801008a1:	89 d0                	mov    %edx,%eax
801008a3:	a3 84 b5 10 80       	mov    %eax,0x8010b584
      input = inputs[active];
801008a8:	8b 15 84 b5 10 80    	mov    0x8010b584,%edx
801008ae:	89 d0                	mov    %edx,%eax
801008b0:	c1 e0 02             	shl    $0x2,%eax
801008b3:	01 d0                	add    %edx,%eax
801008b5:	c1 e0 02             	shl    $0x2,%eax
801008b8:	8d 14 c5 00 00 00 00 	lea    0x0(,%eax,8),%edx
801008bf:	29 c2                	sub    %eax,%edx
801008c1:	8d 82 40 10 11 80    	lea    -0x7feeefc0(%edx),%eax
801008c7:	ba a0 0f 11 80       	mov    $0x80110fa0,%edx
801008cc:	89 c3                	mov    %eax,%ebx
801008ce:	b8 23 00 00 00       	mov    $0x23,%eax
801008d3:	89 d7                	mov    %edx,%edi
801008d5:	89 de                	mov    %ebx,%esi
801008d7:	89 c1                	mov    %eax,%ecx
801008d9:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
      doconsoleswitch = 1;
801008db:	c7 45 e0 01 00 00 00 	movl   $0x1,-0x20(%ebp)
      break;
801008e2:	e9 00 01 00 00       	jmp    801009e7 <consoleintr+0x1f2>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801008e7:	a1 28 10 11 80       	mov    0x80111028,%eax
801008ec:	48                   	dec    %eax
801008ed:	a3 28 10 11 80       	mov    %eax,0x80111028
        consputc(BACKSPACE);
801008f2:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
801008f9:	e8 6a fe ff ff       	call   80100768 <consputc>
801008fe:	eb 01                	jmp    80100901 <consoleintr+0x10c>
      active = (active + 1) % (NUM_VCS + 1);
      input = inputs[active];
      doconsoleswitch = 1;
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100900:	90                   	nop
80100901:	8b 15 28 10 11 80    	mov    0x80111028,%edx
80100907:	a1 24 10 11 80       	mov    0x80111024,%eax
8010090c:	39 c2                	cmp    %eax,%edx
8010090e:	74 13                	je     80100923 <consoleintr+0x12e>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100910:	a1 28 10 11 80       	mov    0x80111028,%eax
80100915:	48                   	dec    %eax
80100916:	83 e0 7f             	and    $0x7f,%eax
80100919:	8a 80 a0 0f 11 80    	mov    -0x7feef060(%eax),%al
      active = (active + 1) % (NUM_VCS + 1);
      input = inputs[active];
      doconsoleswitch = 1;
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010091f:	3c 0a                	cmp    $0xa,%al
80100921:	75 c4                	jne    801008e7 <consoleintr+0xf2>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100923:	e9 bf 00 00 00       	jmp    801009e7 <consoleintr+0x1f2>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
80100928:	8b 15 28 10 11 80    	mov    0x80111028,%edx
8010092e:	a1 24 10 11 80       	mov    0x80111024,%eax
80100933:	39 c2                	cmp    %eax,%edx
80100935:	74 1c                	je     80100953 <consoleintr+0x15e>
        input.e--;
80100937:	a1 28 10 11 80       	mov    0x80111028,%eax
8010093c:	48                   	dec    %eax
8010093d:	a3 28 10 11 80       	mov    %eax,0x80111028
        consputc(BACKSPACE);
80100942:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100949:	e8 1a fe ff ff       	call   80100768 <consputc>
      }
      break;
8010094e:	e9 94 00 00 00       	jmp    801009e7 <consoleintr+0x1f2>
80100953:	e9 8f 00 00 00       	jmp    801009e7 <consoleintr+0x1f2>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
80100958:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
8010095c:	0f 84 84 00 00 00    	je     801009e6 <consoleintr+0x1f1>
80100962:	8b 15 28 10 11 80    	mov    0x80111028,%edx
80100968:	a1 20 10 11 80       	mov    0x80111020,%eax
8010096d:	29 c2                	sub    %eax,%edx
8010096f:	89 d0                	mov    %edx,%eax
80100971:	83 f8 7f             	cmp    $0x7f,%eax
80100974:	77 70                	ja     801009e6 <consoleintr+0x1f1>
        c = (c == '\r') ? '\n' : c;
80100976:	83 7d dc 0d          	cmpl   $0xd,-0x24(%ebp)
8010097a:	74 05                	je     80100981 <consoleintr+0x18c>
8010097c:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010097f:	eb 05                	jmp    80100986 <consoleintr+0x191>
80100981:	b8 0a 00 00 00       	mov    $0xa,%eax
80100986:	89 45 dc             	mov    %eax,-0x24(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
80100989:	a1 28 10 11 80       	mov    0x80111028,%eax
8010098e:	8d 50 01             	lea    0x1(%eax),%edx
80100991:	89 15 28 10 11 80    	mov    %edx,0x80111028
80100997:	83 e0 7f             	and    $0x7f,%eax
8010099a:	89 c2                	mov    %eax,%edx
8010099c:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010099f:	88 82 a0 0f 11 80    	mov    %al,-0x7feef060(%edx)
        consputc(c);
801009a5:	8b 45 dc             	mov    -0x24(%ebp),%eax
801009a8:	89 04 24             	mov    %eax,(%esp)
801009ab:	e8 b8 fd ff ff       	call   80100768 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801009b0:	83 7d dc 0a          	cmpl   $0xa,-0x24(%ebp)
801009b4:	74 18                	je     801009ce <consoleintr+0x1d9>
801009b6:	83 7d dc 04          	cmpl   $0x4,-0x24(%ebp)
801009ba:	74 12                	je     801009ce <consoleintr+0x1d9>
801009bc:	a1 28 10 11 80       	mov    0x80111028,%eax
801009c1:	8b 15 20 10 11 80    	mov    0x80111020,%edx
801009c7:	83 ea 80             	sub    $0xffffff80,%edx
801009ca:	39 d0                	cmp    %edx,%eax
801009cc:	75 18                	jne    801009e6 <consoleintr+0x1f1>
          input.w = input.e;
801009ce:	a1 28 10 11 80       	mov    0x80111028,%eax
801009d3:	a3 24 10 11 80       	mov    %eax,0x80111024
          wakeup(&input.r);
801009d8:	c7 04 24 20 10 11 80 	movl   $0x80111020,(%esp)
801009df:	e8 54 41 00 00       	call   80104b38 <wakeup>
        }
      }
      break;
801009e4:	eb 00                	jmp    801009e6 <consoleintr+0x1f1>
801009e6:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c, doprocdump = 0, doconsoleswitch = 0;

  acquire(&cons.lock);
  while((c = getc()) >= 0){
801009e7:	8b 45 08             	mov    0x8(%ebp),%eax
801009ea:	ff d0                	call   *%eax
801009ec:	89 45 dc             	mov    %eax,-0x24(%ebp)
801009ef:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
801009f3:	0f 89 24 fe ff ff    	jns    8010081d <consoleintr+0x28>
        }
      }
      break;
    }
  }
  release(&cons.lock);
801009f9:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100a00:	e8 98 44 00 00       	call   80104e9d <release>
  if(doprocdump){
80100a05:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100a09:	74 05                	je     80100a10 <consoleintr+0x21b>
    procdump();  // now call procdump() wo. cons.lock held
80100a0b:	e8 cb 41 00 00       	call   80104bdb <procdump>
  }
  if(doconsoleswitch){
80100a10:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100a14:	74 21                	je     80100a37 <consoleintr+0x242>
    cprintf("\nActive console now: %d\n", active);
80100a16:	a1 84 b5 10 80       	mov    0x8010b584,%eax
80100a1b:	89 44 24 04          	mov    %eax,0x4(%esp)
80100a1f:	c7 04 24 56 82 10 80 	movl   $0x80108256,(%esp)
80100a26:	e8 96 f9 ff ff       	call   801003c1 <cprintf>
    consputc('\n');
80100a2b:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80100a32:	e8 31 fd ff ff       	call   80100768 <consputc>
  }
}
80100a37:	83 c4 2c             	add    $0x2c,%esp
80100a3a:	5b                   	pop    %ebx
80100a3b:	5e                   	pop    %esi
80100a3c:	5f                   	pop    %edi
80100a3d:	5d                   	pop    %ebp
80100a3e:	c3                   	ret    

80100a3f <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
80100a3f:	55                   	push   %ebp
80100a40:	89 e5                	mov    %esp,%ebp
80100a42:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
80100a45:	8b 45 08             	mov    0x8(%ebp),%eax
80100a48:	89 04 24             	mov    %eax,(%esp)
80100a4b:	e8 ec 10 00 00       	call   80101b3c <iunlock>
  target = n;
80100a50:	8b 45 10             	mov    0x10(%ebp),%eax
80100a53:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&cons.lock);
80100a56:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100a5d:	e8 d1 43 00 00       	call   80104e33 <acquire>
  while(n > 0){
80100a62:	e9 b8 00 00 00       	jmp    80100b1f <consoleread+0xe0>
    while((input.r == input.w) || (active != ip->minor-1)){
80100a67:	eb 41                	jmp    80100aaa <consoleread+0x6b>
      if(myproc()->killed){
80100a69:	e8 b5 37 00 00       	call   80104223 <myproc>
80100a6e:	8b 40 24             	mov    0x24(%eax),%eax
80100a71:	85 c0                	test   %eax,%eax
80100a73:	74 21                	je     80100a96 <consoleread+0x57>
        release(&cons.lock);
80100a75:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100a7c:	e8 1c 44 00 00       	call   80104e9d <release>
        ilock(ip);
80100a81:	8b 45 08             	mov    0x8(%ebp),%eax
80100a84:	89 04 24             	mov    %eax,(%esp)
80100a87:	e8 a6 0f 00 00       	call   80101a32 <ilock>
        return -1;
80100a8c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100a91:	e9 b4 00 00 00       	jmp    80100b4a <consoleread+0x10b>
      }
      sleep(&input.r, &cons.lock);
80100a96:	c7 44 24 04 a0 b5 10 	movl   $0x8010b5a0,0x4(%esp)
80100a9d:	80 
80100a9e:	c7 04 24 20 10 11 80 	movl   $0x80111020,(%esp)
80100aa5:	e8 ba 3f 00 00       	call   80104a64 <sleep>

  iunlock(ip);
  target = n;
  acquire(&cons.lock);
  while(n > 0){
    while((input.r == input.w) || (active != ip->minor-1)){
80100aaa:	8b 15 20 10 11 80    	mov    0x80111020,%edx
80100ab0:	a1 24 10 11 80       	mov    0x80111024,%eax
80100ab5:	39 c2                	cmp    %eax,%edx
80100ab7:	74 b0                	je     80100a69 <consoleread+0x2a>
80100ab9:	8b 45 08             	mov    0x8(%ebp),%eax
80100abc:	8b 40 54             	mov    0x54(%eax),%eax
80100abf:	98                   	cwtl   
80100ac0:	8d 50 ff             	lea    -0x1(%eax),%edx
80100ac3:	a1 84 b5 10 80       	mov    0x8010b584,%eax
80100ac8:	39 c2                	cmp    %eax,%edx
80100aca:	75 9d                	jne    80100a69 <consoleread+0x2a>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &cons.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
80100acc:	a1 20 10 11 80       	mov    0x80111020,%eax
80100ad1:	8d 50 01             	lea    0x1(%eax),%edx
80100ad4:	89 15 20 10 11 80    	mov    %edx,0x80111020
80100ada:	83 e0 7f             	and    $0x7f,%eax
80100add:	8a 80 a0 0f 11 80    	mov    -0x7feef060(%eax),%al
80100ae3:	0f be c0             	movsbl %al,%eax
80100ae6:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(c == C('D')){  // EOF
80100ae9:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
80100aed:	75 17                	jne    80100b06 <consoleread+0xc7>
      if(n < target){
80100aef:	8b 45 10             	mov    0x10(%ebp),%eax
80100af2:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80100af5:	73 0d                	jae    80100b04 <consoleread+0xc5>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
80100af7:	a1 20 10 11 80       	mov    0x80111020,%eax
80100afc:	48                   	dec    %eax
80100afd:	a3 20 10 11 80       	mov    %eax,0x80111020
      }
      break;
80100b02:	eb 25                	jmp    80100b29 <consoleread+0xea>
80100b04:	eb 23                	jmp    80100b29 <consoleread+0xea>
    }
    *dst++ = c;
80100b06:	8b 45 0c             	mov    0xc(%ebp),%eax
80100b09:	8d 50 01             	lea    0x1(%eax),%edx
80100b0c:	89 55 0c             	mov    %edx,0xc(%ebp)
80100b0f:	8b 55 f0             	mov    -0x10(%ebp),%edx
80100b12:	88 10                	mov    %dl,(%eax)
    --n;
80100b14:	ff 4d 10             	decl   0x10(%ebp)
    if(c == '\n')
80100b17:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
80100b1b:	75 02                	jne    80100b1f <consoleread+0xe0>
      break;
80100b1d:	eb 0a                	jmp    80100b29 <consoleread+0xea>
  int c;

  iunlock(ip);
  target = n;
  acquire(&cons.lock);
  while(n > 0){
80100b1f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100b23:	0f 8f 3e ff ff ff    	jg     80100a67 <consoleread+0x28>
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
  }
  release(&cons.lock);
80100b29:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100b30:	e8 68 43 00 00       	call   80104e9d <release>
  ilock(ip);
80100b35:	8b 45 08             	mov    0x8(%ebp),%eax
80100b38:	89 04 24             	mov    %eax,(%esp)
80100b3b:	e8 f2 0e 00 00       	call   80101a32 <ilock>

  return target - n;
80100b40:	8b 45 10             	mov    0x10(%ebp),%eax
80100b43:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100b46:	29 c2                	sub    %eax,%edx
80100b48:	89 d0                	mov    %edx,%eax
}
80100b4a:	c9                   	leave  
80100b4b:	c3                   	ret    

80100b4c <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100b4c:	55                   	push   %ebp
80100b4d:	89 e5                	mov    %esp,%ebp
80100b4f:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (active == ip->minor-1){
80100b52:	8b 45 08             	mov    0x8(%ebp),%eax
80100b55:	8b 40 54             	mov    0x54(%eax),%eax
80100b58:	98                   	cwtl   
80100b59:	8d 50 ff             	lea    -0x1(%eax),%edx
80100b5c:	a1 84 b5 10 80       	mov    0x8010b584,%eax
80100b61:	39 c2                	cmp    %eax,%edx
80100b63:	75 5a                	jne    80100bbf <consolewrite+0x73>
    iunlock(ip);
80100b65:	8b 45 08             	mov    0x8(%ebp),%eax
80100b68:	89 04 24             	mov    %eax,(%esp)
80100b6b:	e8 cc 0f 00 00       	call   80101b3c <iunlock>
    acquire(&cons.lock);
80100b70:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100b77:	e8 b7 42 00 00       	call   80104e33 <acquire>
    for(i = 0; i < n; i++)
80100b7c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100b83:	eb 1b                	jmp    80100ba0 <consolewrite+0x54>
      consputc(buf[i] & 0xff);
80100b85:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100b88:	8b 45 0c             	mov    0xc(%ebp),%eax
80100b8b:	01 d0                	add    %edx,%eax
80100b8d:	8a 00                	mov    (%eax),%al
80100b8f:	0f be c0             	movsbl %al,%eax
80100b92:	0f b6 c0             	movzbl %al,%eax
80100b95:	89 04 24             	mov    %eax,(%esp)
80100b98:	e8 cb fb ff ff       	call   80100768 <consputc>
  int i;

  if (active == ip->minor-1){
    iunlock(ip);
    acquire(&cons.lock);
    for(i = 0; i < n; i++)
80100b9d:	ff 45 f4             	incl   -0xc(%ebp)
80100ba0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100ba3:	3b 45 10             	cmp    0x10(%ebp),%eax
80100ba6:	7c dd                	jl     80100b85 <consolewrite+0x39>
      consputc(buf[i] & 0xff);
    release(&cons.lock);
80100ba8:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100baf:	e8 e9 42 00 00       	call   80104e9d <release>
    ilock(ip);
80100bb4:	8b 45 08             	mov    0x8(%ebp),%eax
80100bb7:	89 04 24             	mov    %eax,(%esp)
80100bba:	e8 73 0e 00 00       	call   80101a32 <ilock>
  }
  return n;
80100bbf:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100bc2:	c9                   	leave  
80100bc3:	c3                   	ret    

80100bc4 <consoleinit>:

void
consoleinit(void)
{
80100bc4:	55                   	push   %ebp
80100bc5:	89 e5                	mov    %esp,%ebp
80100bc7:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100bca:	c7 44 24 04 6f 82 10 	movl   $0x8010826f,0x4(%esp)
80100bd1:	80 
80100bd2:	c7 04 24 a0 b5 10 80 	movl   $0x8010b5a0,(%esp)
80100bd9:	e8 34 42 00 00       	call   80104e12 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100bde:	c7 05 ac 1c 11 80 4c 	movl   $0x80100b4c,0x80111cac
80100be5:	0b 10 80 
  devsw[CONSOLE].read = consoleread;
80100be8:	c7 05 a8 1c 11 80 3f 	movl   $0x80100a3f,0x80111ca8
80100bef:	0a 10 80 
  cons.locking = 1;
80100bf2:	c7 05 d4 b5 10 80 01 	movl   $0x1,0x8010b5d4
80100bf9:	00 00 00 

  ioapicenable(IRQ_KBD, 0);
80100bfc:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100c03:	00 
80100c04:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100c0b:	e8 e7 1e 00 00       	call   80102af7 <ioapicenable>
}
80100c10:	c9                   	leave  
80100c11:	c3                   	ret    
	...

80100c14 <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100c14:	55                   	push   %ebp
80100c15:	89 e5                	mov    %esp,%ebp
80100c17:	81 ec 38 01 00 00    	sub    $0x138,%esp
  uint argc, sz, sp, ustack[3+MAXARG+1];
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;
  struct proc *curproc = myproc();
80100c1d:	e8 01 36 00 00       	call   80104223 <myproc>
80100c22:	89 45 d0             	mov    %eax,-0x30(%ebp)

  begin_op();
80100c25:	e8 01 29 00 00       	call   8010352b <begin_op>

  if((ip = namei(path)) == 0){
80100c2a:	8b 45 08             	mov    0x8(%ebp),%eax
80100c2d:	89 04 24             	mov    %eax,(%esp)
80100c30:	e8 22 19 00 00       	call   80102557 <namei>
80100c35:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100c38:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100c3c:	75 1b                	jne    80100c59 <exec+0x45>
    end_op();
80100c3e:	e8 6a 29 00 00       	call   801035ad <end_op>
    cprintf("exec: fail\n");
80100c43:	c7 04 24 77 82 10 80 	movl   $0x80108277,(%esp)
80100c4a:	e8 72 f7 ff ff       	call   801003c1 <cprintf>
    return -1;
80100c4f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100c54:	e9 f6 03 00 00       	jmp    8010104f <exec+0x43b>
  }
  ilock(ip);
80100c59:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c5c:	89 04 24             	mov    %eax,(%esp)
80100c5f:	e8 ce 0d 00 00       	call   80101a32 <ilock>
  pgdir = 0;
80100c64:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) != sizeof(elf))
80100c6b:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100c72:	00 
80100c73:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100c7a:	00 
80100c7b:	8d 85 08 ff ff ff    	lea    -0xf8(%ebp),%eax
80100c81:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c85:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c88:	89 04 24             	mov    %eax,(%esp)
80100c8b:	e8 39 12 00 00       	call   80101ec9 <readi>
80100c90:	83 f8 34             	cmp    $0x34,%eax
80100c93:	74 05                	je     80100c9a <exec+0x86>
    goto bad;
80100c95:	e9 89 03 00 00       	jmp    80101023 <exec+0x40f>
  if(elf.magic != ELF_MAGIC)
80100c9a:	8b 85 08 ff ff ff    	mov    -0xf8(%ebp),%eax
80100ca0:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100ca5:	74 05                	je     80100cac <exec+0x98>
    goto bad;
80100ca7:	e9 77 03 00 00       	jmp    80101023 <exec+0x40f>

  if((pgdir = setupkvm()) == 0)
80100cac:	e8 9d 6c 00 00       	call   8010794e <setupkvm>
80100cb1:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100cb4:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100cb8:	75 05                	jne    80100cbf <exec+0xab>
    goto bad;
80100cba:	e9 64 03 00 00       	jmp    80101023 <exec+0x40f>

  // Load program into memory.
  sz = 0;
80100cbf:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100cc6:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100ccd:	8b 85 24 ff ff ff    	mov    -0xdc(%ebp),%eax
80100cd3:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100cd6:	e9 fb 00 00 00       	jmp    80100dd6 <exec+0x1c2>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100cdb:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100cde:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100ce5:	00 
80100ce6:	89 44 24 08          	mov    %eax,0x8(%esp)
80100cea:	8d 85 e8 fe ff ff    	lea    -0x118(%ebp),%eax
80100cf0:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cf4:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100cf7:	89 04 24             	mov    %eax,(%esp)
80100cfa:	e8 ca 11 00 00       	call   80101ec9 <readi>
80100cff:	83 f8 20             	cmp    $0x20,%eax
80100d02:	74 05                	je     80100d09 <exec+0xf5>
      goto bad;
80100d04:	e9 1a 03 00 00       	jmp    80101023 <exec+0x40f>
    if(ph.type != ELF_PROG_LOAD)
80100d09:	8b 85 e8 fe ff ff    	mov    -0x118(%ebp),%eax
80100d0f:	83 f8 01             	cmp    $0x1,%eax
80100d12:	74 05                	je     80100d19 <exec+0x105>
      continue;
80100d14:	e9 b1 00 00 00       	jmp    80100dca <exec+0x1b6>
    if(ph.memsz < ph.filesz)
80100d19:	8b 95 fc fe ff ff    	mov    -0x104(%ebp),%edx
80100d1f:	8b 85 f8 fe ff ff    	mov    -0x108(%ebp),%eax
80100d25:	39 c2                	cmp    %eax,%edx
80100d27:	73 05                	jae    80100d2e <exec+0x11a>
      goto bad;
80100d29:	e9 f5 02 00 00       	jmp    80101023 <exec+0x40f>
    if(ph.vaddr + ph.memsz < ph.vaddr)
80100d2e:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100d34:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100d3a:	01 c2                	add    %eax,%edx
80100d3c:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
80100d42:	39 c2                	cmp    %eax,%edx
80100d44:	73 05                	jae    80100d4b <exec+0x137>
      goto bad;
80100d46:	e9 d8 02 00 00       	jmp    80101023 <exec+0x40f>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100d4b:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100d51:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100d57:	01 d0                	add    %edx,%eax
80100d59:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d5d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100d60:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d64:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d67:	89 04 24             	mov    %eax,(%esp)
80100d6a:	e8 ab 6f 00 00       	call   80107d1a <allocuvm>
80100d6f:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100d72:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100d76:	75 05                	jne    80100d7d <exec+0x169>
      goto bad;
80100d78:	e9 a6 02 00 00       	jmp    80101023 <exec+0x40f>
    if(ph.vaddr % PGSIZE != 0)
80100d7d:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
80100d83:	25 ff 0f 00 00       	and    $0xfff,%eax
80100d88:	85 c0                	test   %eax,%eax
80100d8a:	74 05                	je     80100d91 <exec+0x17d>
      goto bad;
80100d8c:	e9 92 02 00 00       	jmp    80101023 <exec+0x40f>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100d91:	8b 8d f8 fe ff ff    	mov    -0x108(%ebp),%ecx
80100d97:	8b 95 ec fe ff ff    	mov    -0x114(%ebp),%edx
80100d9d:	8b 85 f0 fe ff ff    	mov    -0x110(%ebp),%eax
80100da3:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100da7:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100dab:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100dae:	89 54 24 08          	mov    %edx,0x8(%esp)
80100db2:	89 44 24 04          	mov    %eax,0x4(%esp)
80100db6:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100db9:	89 04 24             	mov    %eax,(%esp)
80100dbc:	e8 76 6e 00 00       	call   80107c37 <loaduvm>
80100dc1:	85 c0                	test   %eax,%eax
80100dc3:	79 05                	jns    80100dca <exec+0x1b6>
      goto bad;
80100dc5:	e9 59 02 00 00       	jmp    80101023 <exec+0x40f>
  if((pgdir = setupkvm()) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100dca:	ff 45 ec             	incl   -0x14(%ebp)
80100dcd:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100dd0:	83 c0 20             	add    $0x20,%eax
80100dd3:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100dd6:	8b 85 34 ff ff ff    	mov    -0xcc(%ebp),%eax
80100ddc:	0f b7 c0             	movzwl %ax,%eax
80100ddf:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100de2:	0f 8f f3 fe ff ff    	jg     80100cdb <exec+0xc7>
    if(ph.vaddr % PGSIZE != 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100de8:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100deb:	89 04 24             	mov    %eax,(%esp)
80100dee:	e8 3e 0e 00 00       	call   80101c31 <iunlockput>
  end_op();
80100df3:	e8 b5 27 00 00       	call   801035ad <end_op>
  ip = 0;
80100df8:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100dff:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100e02:	05 ff 0f 00 00       	add    $0xfff,%eax
80100e07:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100e0c:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100e0f:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100e12:	05 00 20 00 00       	add    $0x2000,%eax
80100e17:	89 44 24 08          	mov    %eax,0x8(%esp)
80100e1b:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100e1e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e22:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100e25:	89 04 24             	mov    %eax,(%esp)
80100e28:	e8 ed 6e 00 00       	call   80107d1a <allocuvm>
80100e2d:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100e30:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100e34:	75 05                	jne    80100e3b <exec+0x227>
    goto bad;
80100e36:	e9 e8 01 00 00       	jmp    80101023 <exec+0x40f>
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100e3b:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100e3e:	2d 00 20 00 00       	sub    $0x2000,%eax
80100e43:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e47:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100e4a:	89 04 24             	mov    %eax,(%esp)
80100e4d:	e8 38 71 00 00       	call   80107f8a <clearpteu>
  sp = sz;
80100e52:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100e55:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100e58:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100e5f:	e9 95 00 00 00       	jmp    80100ef9 <exec+0x2e5>
    if(argc >= MAXARG)
80100e64:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100e68:	76 05                	jbe    80100e6f <exec+0x25b>
      goto bad;
80100e6a:	e9 b4 01 00 00       	jmp    80101023 <exec+0x40f>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100e6f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100e72:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100e79:	8b 45 0c             	mov    0xc(%ebp),%eax
80100e7c:	01 d0                	add    %edx,%eax
80100e7e:	8b 00                	mov    (%eax),%eax
80100e80:	89 04 24             	mov    %eax,(%esp)
80100e83:	e8 61 44 00 00       	call   801052e9 <strlen>
80100e88:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100e8b:	29 c2                	sub    %eax,%edx
80100e8d:	89 d0                	mov    %edx,%eax
80100e8f:	48                   	dec    %eax
80100e90:	83 e0 fc             	and    $0xfffffffc,%eax
80100e93:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100e96:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100e99:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100ea0:	8b 45 0c             	mov    0xc(%ebp),%eax
80100ea3:	01 d0                	add    %edx,%eax
80100ea5:	8b 00                	mov    (%eax),%eax
80100ea7:	89 04 24             	mov    %eax,(%esp)
80100eaa:	e8 3a 44 00 00       	call   801052e9 <strlen>
80100eaf:	40                   	inc    %eax
80100eb0:	89 c2                	mov    %eax,%edx
80100eb2:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100eb5:	8d 0c 85 00 00 00 00 	lea    0x0(,%eax,4),%ecx
80100ebc:	8b 45 0c             	mov    0xc(%ebp),%eax
80100ebf:	01 c8                	add    %ecx,%eax
80100ec1:	8b 00                	mov    (%eax),%eax
80100ec3:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100ec7:	89 44 24 08          	mov    %eax,0x8(%esp)
80100ecb:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100ece:	89 44 24 04          	mov    %eax,0x4(%esp)
80100ed2:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ed5:	89 04 24             	mov    %eax,(%esp)
80100ed8:	e8 65 72 00 00       	call   80108142 <copyout>
80100edd:	85 c0                	test   %eax,%eax
80100edf:	79 05                	jns    80100ee6 <exec+0x2d2>
      goto bad;
80100ee1:	e9 3d 01 00 00       	jmp    80101023 <exec+0x40f>
    ustack[3+argc] = sp;
80100ee6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100ee9:	8d 50 03             	lea    0x3(%eax),%edx
80100eec:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100eef:	89 84 95 3c ff ff ff 	mov    %eax,-0xc4(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100ef6:	ff 45 e4             	incl   -0x1c(%ebp)
80100ef9:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100efc:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100f03:	8b 45 0c             	mov    0xc(%ebp),%eax
80100f06:	01 d0                	add    %edx,%eax
80100f08:	8b 00                	mov    (%eax),%eax
80100f0a:	85 c0                	test   %eax,%eax
80100f0c:	0f 85 52 ff ff ff    	jne    80100e64 <exec+0x250>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100f12:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100f15:	83 c0 03             	add    $0x3,%eax
80100f18:	c7 84 85 3c ff ff ff 	movl   $0x0,-0xc4(%ebp,%eax,4)
80100f1f:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100f23:	c7 85 3c ff ff ff ff 	movl   $0xffffffff,-0xc4(%ebp)
80100f2a:	ff ff ff 
  ustack[1] = argc;
80100f2d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100f30:	89 85 40 ff ff ff    	mov    %eax,-0xc0(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100f36:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100f39:	40                   	inc    %eax
80100f3a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100f41:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100f44:	29 d0                	sub    %edx,%eax
80100f46:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)

  sp -= (3+argc+1) * 4;
80100f4c:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100f4f:	83 c0 04             	add    $0x4,%eax
80100f52:	c1 e0 02             	shl    $0x2,%eax
80100f55:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100f58:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100f5b:	83 c0 04             	add    $0x4,%eax
80100f5e:	c1 e0 02             	shl    $0x2,%eax
80100f61:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100f65:	8d 85 3c ff ff ff    	lea    -0xc4(%ebp),%eax
80100f6b:	89 44 24 08          	mov    %eax,0x8(%esp)
80100f6f:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100f72:	89 44 24 04          	mov    %eax,0x4(%esp)
80100f76:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100f79:	89 04 24             	mov    %eax,(%esp)
80100f7c:	e8 c1 71 00 00       	call   80108142 <copyout>
80100f81:	85 c0                	test   %eax,%eax
80100f83:	79 05                	jns    80100f8a <exec+0x376>
    goto bad;
80100f85:	e9 99 00 00 00       	jmp    80101023 <exec+0x40f>

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100f8a:	8b 45 08             	mov    0x8(%ebp),%eax
80100f8d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100f90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f93:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100f96:	eb 13                	jmp    80100fab <exec+0x397>
    if(*s == '/')
80100f98:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f9b:	8a 00                	mov    (%eax),%al
80100f9d:	3c 2f                	cmp    $0x2f,%al
80100f9f:	75 07                	jne    80100fa8 <exec+0x394>
      last = s+1;
80100fa1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100fa4:	40                   	inc    %eax
80100fa5:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100fa8:	ff 45 f4             	incl   -0xc(%ebp)
80100fab:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100fae:	8a 00                	mov    (%eax),%al
80100fb0:	84 c0                	test   %al,%al
80100fb2:	75 e4                	jne    80100f98 <exec+0x384>
    if(*s == '/')
      last = s+1;
  safestrcpy(curproc->name, last, sizeof(curproc->name));
80100fb4:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100fb7:	8d 50 6c             	lea    0x6c(%eax),%edx
80100fba:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100fc1:	00 
80100fc2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100fc5:	89 44 24 04          	mov    %eax,0x4(%esp)
80100fc9:	89 14 24             	mov    %edx,(%esp)
80100fcc:	e8 d1 42 00 00       	call   801052a2 <safestrcpy>

  // Commit to the user image.
  oldpgdir = curproc->pgdir;
80100fd1:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100fd4:	8b 40 04             	mov    0x4(%eax),%eax
80100fd7:	89 45 cc             	mov    %eax,-0x34(%ebp)
  curproc->pgdir = pgdir;
80100fda:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100fdd:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100fe0:	89 50 04             	mov    %edx,0x4(%eax)
  curproc->sz = sz;
80100fe3:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100fe6:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100fe9:	89 10                	mov    %edx,(%eax)
  curproc->tf->eip = elf.entry;  // main
80100feb:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100fee:	8b 40 18             	mov    0x18(%eax),%eax
80100ff1:	8b 95 20 ff ff ff    	mov    -0xe0(%ebp),%edx
80100ff7:	89 50 38             	mov    %edx,0x38(%eax)
  curproc->tf->esp = sp;
80100ffa:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ffd:	8b 40 18             	mov    0x18(%eax),%eax
80101000:	8b 55 dc             	mov    -0x24(%ebp),%edx
80101003:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(curproc);
80101006:	8b 45 d0             	mov    -0x30(%ebp),%eax
80101009:	89 04 24             	mov    %eax,(%esp)
8010100c:	e8 17 6a 00 00       	call   80107a28 <switchuvm>
  freevm(oldpgdir);
80101011:	8b 45 cc             	mov    -0x34(%ebp),%eax
80101014:	89 04 24             	mov    %eax,(%esp)
80101017:	e8 d8 6e 00 00       	call   80107ef4 <freevm>
  return 0;
8010101c:	b8 00 00 00 00       	mov    $0x0,%eax
80101021:	eb 2c                	jmp    8010104f <exec+0x43b>

 bad:
  if(pgdir)
80101023:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80101027:	74 0b                	je     80101034 <exec+0x420>
    freevm(pgdir);
80101029:	8b 45 d4             	mov    -0x2c(%ebp),%eax
8010102c:	89 04 24             	mov    %eax,(%esp)
8010102f:	e8 c0 6e 00 00       	call   80107ef4 <freevm>
  if(ip){
80101034:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80101038:	74 10                	je     8010104a <exec+0x436>
    iunlockput(ip);
8010103a:	8b 45 d8             	mov    -0x28(%ebp),%eax
8010103d:	89 04 24             	mov    %eax,(%esp)
80101040:	e8 ec 0b 00 00       	call   80101c31 <iunlockput>
    end_op();
80101045:	e8 63 25 00 00       	call   801035ad <end_op>
  }
  return -1;
8010104a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010104f:	c9                   	leave  
80101050:	c3                   	ret    
80101051:	00 00                	add    %al,(%eax)
	...

80101054 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80101054:	55                   	push   %ebp
80101055:	89 e5                	mov    %esp,%ebp
80101057:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
8010105a:	c7 44 24 04 83 82 10 	movl   $0x80108283,0x4(%esp)
80101061:	80 
80101062:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
80101069:	e8 a4 3d 00 00       	call   80104e12 <initlock>
}
8010106e:	c9                   	leave  
8010106f:	c3                   	ret    

80101070 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80101070:	55                   	push   %ebp
80101071:	89 e5                	mov    %esp,%ebp
80101073:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80101076:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
8010107d:	e8 b1 3d 00 00       	call   80104e33 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80101082:	c7 45 f4 34 13 11 80 	movl   $0x80111334,-0xc(%ebp)
80101089:	eb 29                	jmp    801010b4 <filealloc+0x44>
    if(f->ref == 0){
8010108b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010108e:	8b 40 04             	mov    0x4(%eax),%eax
80101091:	85 c0                	test   %eax,%eax
80101093:	75 1b                	jne    801010b0 <filealloc+0x40>
      f->ref = 1;
80101095:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101098:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
8010109f:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
801010a6:	e8 f2 3d 00 00       	call   80104e9d <release>
      return f;
801010ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801010ae:	eb 1e                	jmp    801010ce <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
801010b0:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
801010b4:	81 7d f4 94 1c 11 80 	cmpl   $0x80111c94,-0xc(%ebp)
801010bb:	72 ce                	jb     8010108b <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
801010bd:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
801010c4:	e8 d4 3d 00 00       	call   80104e9d <release>
  return 0;
801010c9:	b8 00 00 00 00       	mov    $0x0,%eax
}
801010ce:	c9                   	leave  
801010cf:	c3                   	ret    

801010d0 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
801010d0:	55                   	push   %ebp
801010d1:	89 e5                	mov    %esp,%ebp
801010d3:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
801010d6:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
801010dd:	e8 51 3d 00 00       	call   80104e33 <acquire>
  if(f->ref < 1)
801010e2:	8b 45 08             	mov    0x8(%ebp),%eax
801010e5:	8b 40 04             	mov    0x4(%eax),%eax
801010e8:	85 c0                	test   %eax,%eax
801010ea:	7f 0c                	jg     801010f8 <filedup+0x28>
    panic("filedup");
801010ec:	c7 04 24 8a 82 10 80 	movl   $0x8010828a,(%esp)
801010f3:	e8 5c f4 ff ff       	call   80100554 <panic>
  f->ref++;
801010f8:	8b 45 08             	mov    0x8(%ebp),%eax
801010fb:	8b 40 04             	mov    0x4(%eax),%eax
801010fe:	8d 50 01             	lea    0x1(%eax),%edx
80101101:	8b 45 08             	mov    0x8(%ebp),%eax
80101104:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80101107:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
8010110e:	e8 8a 3d 00 00       	call   80104e9d <release>
  return f;
80101113:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101116:	c9                   	leave  
80101117:	c3                   	ret    

80101118 <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80101118:	55                   	push   %ebp
80101119:	89 e5                	mov    %esp,%ebp
8010111b:	57                   	push   %edi
8010111c:	56                   	push   %esi
8010111d:	53                   	push   %ebx
8010111e:	83 ec 3c             	sub    $0x3c,%esp
  struct file ff;

  acquire(&ftable.lock);
80101121:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
80101128:	e8 06 3d 00 00       	call   80104e33 <acquire>
  if(f->ref < 1)
8010112d:	8b 45 08             	mov    0x8(%ebp),%eax
80101130:	8b 40 04             	mov    0x4(%eax),%eax
80101133:	85 c0                	test   %eax,%eax
80101135:	7f 0c                	jg     80101143 <fileclose+0x2b>
    panic("fileclose");
80101137:	c7 04 24 92 82 10 80 	movl   $0x80108292,(%esp)
8010113e:	e8 11 f4 ff ff       	call   80100554 <panic>
  if(--f->ref > 0){
80101143:	8b 45 08             	mov    0x8(%ebp),%eax
80101146:	8b 40 04             	mov    0x4(%eax),%eax
80101149:	8d 50 ff             	lea    -0x1(%eax),%edx
8010114c:	8b 45 08             	mov    0x8(%ebp),%eax
8010114f:	89 50 04             	mov    %edx,0x4(%eax)
80101152:	8b 45 08             	mov    0x8(%ebp),%eax
80101155:	8b 40 04             	mov    0x4(%eax),%eax
80101158:	85 c0                	test   %eax,%eax
8010115a:	7e 0e                	jle    8010116a <fileclose+0x52>
    release(&ftable.lock);
8010115c:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
80101163:	e8 35 3d 00 00       	call   80104e9d <release>
80101168:	eb 70                	jmp    801011da <fileclose+0xc2>
    return;
  }
  ff = *f;
8010116a:	8b 45 08             	mov    0x8(%ebp),%eax
8010116d:	8d 55 d0             	lea    -0x30(%ebp),%edx
80101170:	89 c3                	mov    %eax,%ebx
80101172:	b8 06 00 00 00       	mov    $0x6,%eax
80101177:	89 d7                	mov    %edx,%edi
80101179:	89 de                	mov    %ebx,%esi
8010117b:	89 c1                	mov    %eax,%ecx
8010117d:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  f->ref = 0;
8010117f:	8b 45 08             	mov    0x8(%ebp),%eax
80101182:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
80101189:	8b 45 08             	mov    0x8(%ebp),%eax
8010118c:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
80101192:	c7 04 24 00 13 11 80 	movl   $0x80111300,(%esp)
80101199:	e8 ff 3c 00 00       	call   80104e9d <release>

  if(ff.type == FD_PIPE)
8010119e:	8b 45 d0             	mov    -0x30(%ebp),%eax
801011a1:	83 f8 01             	cmp    $0x1,%eax
801011a4:	75 17                	jne    801011bd <fileclose+0xa5>
    pipeclose(ff.pipe, ff.writable);
801011a6:	8a 45 d9             	mov    -0x27(%ebp),%al
801011a9:	0f be d0             	movsbl %al,%edx
801011ac:	8b 45 dc             	mov    -0x24(%ebp),%eax
801011af:	89 54 24 04          	mov    %edx,0x4(%esp)
801011b3:	89 04 24             	mov    %eax,(%esp)
801011b6:	e8 00 2d 00 00       	call   80103ebb <pipeclose>
801011bb:	eb 1d                	jmp    801011da <fileclose+0xc2>
  else if(ff.type == FD_INODE){
801011bd:	8b 45 d0             	mov    -0x30(%ebp),%eax
801011c0:	83 f8 02             	cmp    $0x2,%eax
801011c3:	75 15                	jne    801011da <fileclose+0xc2>
    begin_op();
801011c5:	e8 61 23 00 00       	call   8010352b <begin_op>
    iput(ff.ip);
801011ca:	8b 45 e0             	mov    -0x20(%ebp),%eax
801011cd:	89 04 24             	mov    %eax,(%esp)
801011d0:	e8 ab 09 00 00       	call   80101b80 <iput>
    end_op();
801011d5:	e8 d3 23 00 00       	call   801035ad <end_op>
  }
}
801011da:	83 c4 3c             	add    $0x3c,%esp
801011dd:	5b                   	pop    %ebx
801011de:	5e                   	pop    %esi
801011df:	5f                   	pop    %edi
801011e0:	5d                   	pop    %ebp
801011e1:	c3                   	ret    

801011e2 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
801011e2:	55                   	push   %ebp
801011e3:	89 e5                	mov    %esp,%ebp
801011e5:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
801011e8:	8b 45 08             	mov    0x8(%ebp),%eax
801011eb:	8b 00                	mov    (%eax),%eax
801011ed:	83 f8 02             	cmp    $0x2,%eax
801011f0:	75 38                	jne    8010122a <filestat+0x48>
    ilock(f->ip);
801011f2:	8b 45 08             	mov    0x8(%ebp),%eax
801011f5:	8b 40 10             	mov    0x10(%eax),%eax
801011f8:	89 04 24             	mov    %eax,(%esp)
801011fb:	e8 32 08 00 00       	call   80101a32 <ilock>
    stati(f->ip, st);
80101200:	8b 45 08             	mov    0x8(%ebp),%eax
80101203:	8b 40 10             	mov    0x10(%eax),%eax
80101206:	8b 55 0c             	mov    0xc(%ebp),%edx
80101209:	89 54 24 04          	mov    %edx,0x4(%esp)
8010120d:	89 04 24             	mov    %eax,(%esp)
80101210:	e8 70 0c 00 00       	call   80101e85 <stati>
    iunlock(f->ip);
80101215:	8b 45 08             	mov    0x8(%ebp),%eax
80101218:	8b 40 10             	mov    0x10(%eax),%eax
8010121b:	89 04 24             	mov    %eax,(%esp)
8010121e:	e8 19 09 00 00       	call   80101b3c <iunlock>
    return 0;
80101223:	b8 00 00 00 00       	mov    $0x0,%eax
80101228:	eb 05                	jmp    8010122f <filestat+0x4d>
  }
  return -1;
8010122a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010122f:	c9                   	leave  
80101230:	c3                   	ret    

80101231 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
80101231:	55                   	push   %ebp
80101232:	89 e5                	mov    %esp,%ebp
80101234:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
80101237:	8b 45 08             	mov    0x8(%ebp),%eax
8010123a:	8a 40 08             	mov    0x8(%eax),%al
8010123d:	84 c0                	test   %al,%al
8010123f:	75 0a                	jne    8010124b <fileread+0x1a>
    return -1;
80101241:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101246:	e9 9f 00 00 00       	jmp    801012ea <fileread+0xb9>
  if(f->type == FD_PIPE)
8010124b:	8b 45 08             	mov    0x8(%ebp),%eax
8010124e:	8b 00                	mov    (%eax),%eax
80101250:	83 f8 01             	cmp    $0x1,%eax
80101253:	75 1e                	jne    80101273 <fileread+0x42>
    return piperead(f->pipe, addr, n);
80101255:	8b 45 08             	mov    0x8(%ebp),%eax
80101258:	8b 40 0c             	mov    0xc(%eax),%eax
8010125b:	8b 55 10             	mov    0x10(%ebp),%edx
8010125e:	89 54 24 08          	mov    %edx,0x8(%esp)
80101262:	8b 55 0c             	mov    0xc(%ebp),%edx
80101265:	89 54 24 04          	mov    %edx,0x4(%esp)
80101269:	89 04 24             	mov    %eax,(%esp)
8010126c:	e8 c8 2d 00 00       	call   80104039 <piperead>
80101271:	eb 77                	jmp    801012ea <fileread+0xb9>
  if(f->type == FD_INODE){
80101273:	8b 45 08             	mov    0x8(%ebp),%eax
80101276:	8b 00                	mov    (%eax),%eax
80101278:	83 f8 02             	cmp    $0x2,%eax
8010127b:	75 61                	jne    801012de <fileread+0xad>
    ilock(f->ip);
8010127d:	8b 45 08             	mov    0x8(%ebp),%eax
80101280:	8b 40 10             	mov    0x10(%eax),%eax
80101283:	89 04 24             	mov    %eax,(%esp)
80101286:	e8 a7 07 00 00       	call   80101a32 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
8010128b:	8b 4d 10             	mov    0x10(%ebp),%ecx
8010128e:	8b 45 08             	mov    0x8(%ebp),%eax
80101291:	8b 50 14             	mov    0x14(%eax),%edx
80101294:	8b 45 08             	mov    0x8(%ebp),%eax
80101297:	8b 40 10             	mov    0x10(%eax),%eax
8010129a:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
8010129e:	89 54 24 08          	mov    %edx,0x8(%esp)
801012a2:	8b 55 0c             	mov    0xc(%ebp),%edx
801012a5:	89 54 24 04          	mov    %edx,0x4(%esp)
801012a9:	89 04 24             	mov    %eax,(%esp)
801012ac:	e8 18 0c 00 00       	call   80101ec9 <readi>
801012b1:	89 45 f4             	mov    %eax,-0xc(%ebp)
801012b4:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801012b8:	7e 11                	jle    801012cb <fileread+0x9a>
      f->off += r;
801012ba:	8b 45 08             	mov    0x8(%ebp),%eax
801012bd:	8b 50 14             	mov    0x14(%eax),%edx
801012c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012c3:	01 c2                	add    %eax,%edx
801012c5:	8b 45 08             	mov    0x8(%ebp),%eax
801012c8:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
801012cb:	8b 45 08             	mov    0x8(%ebp),%eax
801012ce:	8b 40 10             	mov    0x10(%eax),%eax
801012d1:	89 04 24             	mov    %eax,(%esp)
801012d4:	e8 63 08 00 00       	call   80101b3c <iunlock>
    return r;
801012d9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012dc:	eb 0c                	jmp    801012ea <fileread+0xb9>
  }
  panic("fileread");
801012de:	c7 04 24 9c 82 10 80 	movl   $0x8010829c,(%esp)
801012e5:	e8 6a f2 ff ff       	call   80100554 <panic>
}
801012ea:	c9                   	leave  
801012eb:	c3                   	ret    

801012ec <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
801012ec:	55                   	push   %ebp
801012ed:	89 e5                	mov    %esp,%ebp
801012ef:	53                   	push   %ebx
801012f0:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801012f3:	8b 45 08             	mov    0x8(%ebp),%eax
801012f6:	8a 40 09             	mov    0x9(%eax),%al
801012f9:	84 c0                	test   %al,%al
801012fb:	75 0a                	jne    80101307 <filewrite+0x1b>
    return -1;
801012fd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101302:	e9 20 01 00 00       	jmp    80101427 <filewrite+0x13b>
  if(f->type == FD_PIPE)
80101307:	8b 45 08             	mov    0x8(%ebp),%eax
8010130a:	8b 00                	mov    (%eax),%eax
8010130c:	83 f8 01             	cmp    $0x1,%eax
8010130f:	75 21                	jne    80101332 <filewrite+0x46>
    return pipewrite(f->pipe, addr, n);
80101311:	8b 45 08             	mov    0x8(%ebp),%eax
80101314:	8b 40 0c             	mov    0xc(%eax),%eax
80101317:	8b 55 10             	mov    0x10(%ebp),%edx
8010131a:	89 54 24 08          	mov    %edx,0x8(%esp)
8010131e:	8b 55 0c             	mov    0xc(%ebp),%edx
80101321:	89 54 24 04          	mov    %edx,0x4(%esp)
80101325:	89 04 24             	mov    %eax,(%esp)
80101328:	e8 20 2c 00 00       	call   80103f4d <pipewrite>
8010132d:	e9 f5 00 00 00       	jmp    80101427 <filewrite+0x13b>
  if(f->type == FD_INODE){
80101332:	8b 45 08             	mov    0x8(%ebp),%eax
80101335:	8b 00                	mov    (%eax),%eax
80101337:	83 f8 02             	cmp    $0x2,%eax
8010133a:	0f 85 db 00 00 00    	jne    8010141b <filewrite+0x12f>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
80101340:	c7 45 ec 00 1a 00 00 	movl   $0x1a00,-0x14(%ebp)
    int i = 0;
80101347:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
8010134e:	e9 a8 00 00 00       	jmp    801013fb <filewrite+0x10f>
      int n1 = n - i;
80101353:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101356:	8b 55 10             	mov    0x10(%ebp),%edx
80101359:	29 c2                	sub    %eax,%edx
8010135b:	89 d0                	mov    %edx,%eax
8010135d:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
80101360:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101363:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80101366:	7e 06                	jle    8010136e <filewrite+0x82>
        n1 = max;
80101368:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010136b:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_op();
8010136e:	e8 b8 21 00 00       	call   8010352b <begin_op>
      ilock(f->ip);
80101373:	8b 45 08             	mov    0x8(%ebp),%eax
80101376:	8b 40 10             	mov    0x10(%eax),%eax
80101379:	89 04 24             	mov    %eax,(%esp)
8010137c:	e8 b1 06 00 00       	call   80101a32 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
80101381:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80101384:	8b 45 08             	mov    0x8(%ebp),%eax
80101387:	8b 50 14             	mov    0x14(%eax),%edx
8010138a:	8b 5d f4             	mov    -0xc(%ebp),%ebx
8010138d:	8b 45 0c             	mov    0xc(%ebp),%eax
80101390:	01 c3                	add    %eax,%ebx
80101392:	8b 45 08             	mov    0x8(%ebp),%eax
80101395:	8b 40 10             	mov    0x10(%eax),%eax
80101398:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
8010139c:	89 54 24 08          	mov    %edx,0x8(%esp)
801013a0:	89 5c 24 04          	mov    %ebx,0x4(%esp)
801013a4:	89 04 24             	mov    %eax,(%esp)
801013a7:	e8 81 0c 00 00       	call   8010202d <writei>
801013ac:	89 45 e8             	mov    %eax,-0x18(%ebp)
801013af:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801013b3:	7e 11                	jle    801013c6 <filewrite+0xda>
        f->off += r;
801013b5:	8b 45 08             	mov    0x8(%ebp),%eax
801013b8:	8b 50 14             	mov    0x14(%eax),%edx
801013bb:	8b 45 e8             	mov    -0x18(%ebp),%eax
801013be:	01 c2                	add    %eax,%edx
801013c0:	8b 45 08             	mov    0x8(%ebp),%eax
801013c3:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
801013c6:	8b 45 08             	mov    0x8(%ebp),%eax
801013c9:	8b 40 10             	mov    0x10(%eax),%eax
801013cc:	89 04 24             	mov    %eax,(%esp)
801013cf:	e8 68 07 00 00       	call   80101b3c <iunlock>
      end_op();
801013d4:	e8 d4 21 00 00       	call   801035ad <end_op>

      if(r < 0)
801013d9:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801013dd:	79 02                	jns    801013e1 <filewrite+0xf5>
        break;
801013df:	eb 26                	jmp    80101407 <filewrite+0x11b>
      if(r != n1)
801013e1:	8b 45 e8             	mov    -0x18(%ebp),%eax
801013e4:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801013e7:	74 0c                	je     801013f5 <filewrite+0x109>
        panic("short filewrite");
801013e9:	c7 04 24 a5 82 10 80 	movl   $0x801082a5,(%esp)
801013f0:	e8 5f f1 ff ff       	call   80100554 <panic>
      i += r;
801013f5:	8b 45 e8             	mov    -0x18(%ebp),%eax
801013f8:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801013fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013fe:	3b 45 10             	cmp    0x10(%ebp),%eax
80101401:	0f 8c 4c ff ff ff    	jl     80101353 <filewrite+0x67>
        break;
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
80101407:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010140a:	3b 45 10             	cmp    0x10(%ebp),%eax
8010140d:	75 05                	jne    80101414 <filewrite+0x128>
8010140f:	8b 45 10             	mov    0x10(%ebp),%eax
80101412:	eb 05                	jmp    80101419 <filewrite+0x12d>
80101414:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101419:	eb 0c                	jmp    80101427 <filewrite+0x13b>
  }
  panic("filewrite");
8010141b:	c7 04 24 b5 82 10 80 	movl   $0x801082b5,(%esp)
80101422:	e8 2d f1 ff ff       	call   80100554 <panic>
}
80101427:	83 c4 24             	add    $0x24,%esp
8010142a:	5b                   	pop    %ebx
8010142b:	5d                   	pop    %ebp
8010142c:	c3                   	ret    
8010142d:	00 00                	add    %al,(%eax)
	...

80101430 <readsb>:
struct superblock sb; 

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
80101430:	55                   	push   %ebp
80101431:	89 e5                	mov    %esp,%ebp
80101433:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;

  bp = bread(dev, 1);
80101436:	8b 45 08             	mov    0x8(%ebp),%eax
80101439:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80101440:	00 
80101441:	89 04 24             	mov    %eax,(%esp)
80101444:	e8 6c ed ff ff       	call   801001b5 <bread>
80101449:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
8010144c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010144f:	83 c0 5c             	add    $0x5c,%eax
80101452:	c7 44 24 08 1c 00 00 	movl   $0x1c,0x8(%esp)
80101459:	00 
8010145a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010145e:	8b 45 0c             	mov    0xc(%ebp),%eax
80101461:	89 04 24             	mov    %eax,(%esp)
80101464:	e8 f6 3c 00 00       	call   8010515f <memmove>
  brelse(bp);
80101469:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010146c:	89 04 24             	mov    %eax,(%esp)
8010146f:	e8 b8 ed ff ff       	call   8010022c <brelse>
}
80101474:	c9                   	leave  
80101475:	c3                   	ret    

80101476 <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
80101476:	55                   	push   %ebp
80101477:	89 e5                	mov    %esp,%ebp
80101479:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;

  bp = bread(dev, bno);
8010147c:	8b 55 0c             	mov    0xc(%ebp),%edx
8010147f:	8b 45 08             	mov    0x8(%ebp),%eax
80101482:	89 54 24 04          	mov    %edx,0x4(%esp)
80101486:	89 04 24             	mov    %eax,(%esp)
80101489:	e8 27 ed ff ff       	call   801001b5 <bread>
8010148e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
80101491:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101494:	83 c0 5c             	add    $0x5c,%eax
80101497:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
8010149e:	00 
8010149f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801014a6:	00 
801014a7:	89 04 24             	mov    %eax,(%esp)
801014aa:	e8 e7 3b 00 00       	call   80105096 <memset>
  log_write(bp);
801014af:	8b 45 f4             	mov    -0xc(%ebp),%eax
801014b2:	89 04 24             	mov    %eax,(%esp)
801014b5:	e8 75 22 00 00       	call   8010372f <log_write>
  brelse(bp);
801014ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801014bd:	89 04 24             	mov    %eax,(%esp)
801014c0:	e8 67 ed ff ff       	call   8010022c <brelse>
}
801014c5:	c9                   	leave  
801014c6:	c3                   	ret    

801014c7 <balloc>:
// Blocks.

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
801014c7:	55                   	push   %ebp
801014c8:	89 e5                	mov    %esp,%ebp
801014ca:	83 ec 28             	sub    $0x28,%esp
  int b, bi, m;
  struct buf *bp;

  bp = 0;
801014cd:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  for(b = 0; b < sb.size; b += BPB){
801014d4:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801014db:	e9 03 01 00 00       	jmp    801015e3 <balloc+0x11c>
    bp = bread(dev, BBLOCK(b, sb));
801014e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801014e3:	85 c0                	test   %eax,%eax
801014e5:	79 05                	jns    801014ec <balloc+0x25>
801014e7:	05 ff 0f 00 00       	add    $0xfff,%eax
801014ec:	c1 f8 0c             	sar    $0xc,%eax
801014ef:	89 c2                	mov    %eax,%edx
801014f1:	a1 18 1d 11 80       	mov    0x80111d18,%eax
801014f6:	01 d0                	add    %edx,%eax
801014f8:	89 44 24 04          	mov    %eax,0x4(%esp)
801014fc:	8b 45 08             	mov    0x8(%ebp),%eax
801014ff:	89 04 24             	mov    %eax,(%esp)
80101502:	e8 ae ec ff ff       	call   801001b5 <bread>
80101507:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
8010150a:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101511:	e9 9b 00 00 00       	jmp    801015b1 <balloc+0xea>
      m = 1 << (bi % 8);
80101516:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101519:	25 07 00 00 80       	and    $0x80000007,%eax
8010151e:	85 c0                	test   %eax,%eax
80101520:	79 05                	jns    80101527 <balloc+0x60>
80101522:	48                   	dec    %eax
80101523:	83 c8 f8             	or     $0xfffffff8,%eax
80101526:	40                   	inc    %eax
80101527:	ba 01 00 00 00       	mov    $0x1,%edx
8010152c:	88 c1                	mov    %al,%cl
8010152e:	d3 e2                	shl    %cl,%edx
80101530:	89 d0                	mov    %edx,%eax
80101532:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101535:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101538:	85 c0                	test   %eax,%eax
8010153a:	79 03                	jns    8010153f <balloc+0x78>
8010153c:	83 c0 07             	add    $0x7,%eax
8010153f:	c1 f8 03             	sar    $0x3,%eax
80101542:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101545:	8a 44 02 5c          	mov    0x5c(%edx,%eax,1),%al
80101549:	0f b6 c0             	movzbl %al,%eax
8010154c:	23 45 e8             	and    -0x18(%ebp),%eax
8010154f:	85 c0                	test   %eax,%eax
80101551:	75 5b                	jne    801015ae <balloc+0xe7>
        bp->data[bi/8] |= m;  // Mark block in use.
80101553:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101556:	85 c0                	test   %eax,%eax
80101558:	79 03                	jns    8010155d <balloc+0x96>
8010155a:	83 c0 07             	add    $0x7,%eax
8010155d:	c1 f8 03             	sar    $0x3,%eax
80101560:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101563:	8a 54 02 5c          	mov    0x5c(%edx,%eax,1),%dl
80101567:	88 d1                	mov    %dl,%cl
80101569:	8b 55 e8             	mov    -0x18(%ebp),%edx
8010156c:	09 ca                	or     %ecx,%edx
8010156e:	88 d1                	mov    %dl,%cl
80101570:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101573:	88 4c 02 5c          	mov    %cl,0x5c(%edx,%eax,1)
        log_write(bp);
80101577:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010157a:	89 04 24             	mov    %eax,(%esp)
8010157d:	e8 ad 21 00 00       	call   8010372f <log_write>
        brelse(bp);
80101582:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101585:	89 04 24             	mov    %eax,(%esp)
80101588:	e8 9f ec ff ff       	call   8010022c <brelse>
        bzero(dev, b + bi);
8010158d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101590:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101593:	01 c2                	add    %eax,%edx
80101595:	8b 45 08             	mov    0x8(%ebp),%eax
80101598:	89 54 24 04          	mov    %edx,0x4(%esp)
8010159c:	89 04 24             	mov    %eax,(%esp)
8010159f:	e8 d2 fe ff ff       	call   80101476 <bzero>
        return b + bi;
801015a4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801015a7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801015aa:	01 d0                	add    %edx,%eax
801015ac:	eb 51                	jmp    801015ff <balloc+0x138>
  struct buf *bp;

  bp = 0;
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801015ae:	ff 45 f0             	incl   -0x10(%ebp)
801015b1:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
801015b8:	7f 17                	jg     801015d1 <balloc+0x10a>
801015ba:	8b 45 f0             	mov    -0x10(%ebp),%eax
801015bd:	8b 55 f4             	mov    -0xc(%ebp),%edx
801015c0:	01 d0                	add    %edx,%eax
801015c2:	89 c2                	mov    %eax,%edx
801015c4:	a1 00 1d 11 80       	mov    0x80111d00,%eax
801015c9:	39 c2                	cmp    %eax,%edx
801015cb:	0f 82 45 ff ff ff    	jb     80101516 <balloc+0x4f>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801015d1:	8b 45 ec             	mov    -0x14(%ebp),%eax
801015d4:	89 04 24             	mov    %eax,(%esp)
801015d7:	e8 50 ec ff ff       	call   8010022c <brelse>
{
  int b, bi, m;
  struct buf *bp;

  bp = 0;
  for(b = 0; b < sb.size; b += BPB){
801015dc:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801015e3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801015e6:	a1 00 1d 11 80       	mov    0x80111d00,%eax
801015eb:	39 c2                	cmp    %eax,%edx
801015ed:	0f 82 ed fe ff ff    	jb     801014e0 <balloc+0x19>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801015f3:	c7 04 24 c0 82 10 80 	movl   $0x801082c0,(%esp)
801015fa:	e8 55 ef ff ff       	call   80100554 <panic>
}
801015ff:	c9                   	leave  
80101600:	c3                   	ret    

80101601 <bfree>:

// Free a disk block.
static void
bfree(int dev, uint b)
{
80101601:	55                   	push   %ebp
80101602:	89 e5                	mov    %esp,%ebp
80101604:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  int bi, m;

  readsb(dev, &sb);
80101607:	c7 44 24 04 00 1d 11 	movl   $0x80111d00,0x4(%esp)
8010160e:	80 
8010160f:	8b 45 08             	mov    0x8(%ebp),%eax
80101612:	89 04 24             	mov    %eax,(%esp)
80101615:	e8 16 fe ff ff       	call   80101430 <readsb>
  bp = bread(dev, BBLOCK(b, sb));
8010161a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010161d:	c1 e8 0c             	shr    $0xc,%eax
80101620:	89 c2                	mov    %eax,%edx
80101622:	a1 18 1d 11 80       	mov    0x80111d18,%eax
80101627:	01 c2                	add    %eax,%edx
80101629:	8b 45 08             	mov    0x8(%ebp),%eax
8010162c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101630:	89 04 24             	mov    %eax,(%esp)
80101633:	e8 7d eb ff ff       	call   801001b5 <bread>
80101638:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
8010163b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010163e:	25 ff 0f 00 00       	and    $0xfff,%eax
80101643:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
80101646:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101649:	25 07 00 00 80       	and    $0x80000007,%eax
8010164e:	85 c0                	test   %eax,%eax
80101650:	79 05                	jns    80101657 <bfree+0x56>
80101652:	48                   	dec    %eax
80101653:	83 c8 f8             	or     $0xfffffff8,%eax
80101656:	40                   	inc    %eax
80101657:	ba 01 00 00 00       	mov    $0x1,%edx
8010165c:	88 c1                	mov    %al,%cl
8010165e:	d3 e2                	shl    %cl,%edx
80101660:	89 d0                	mov    %edx,%eax
80101662:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
80101665:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101668:	85 c0                	test   %eax,%eax
8010166a:	79 03                	jns    8010166f <bfree+0x6e>
8010166c:	83 c0 07             	add    $0x7,%eax
8010166f:	c1 f8 03             	sar    $0x3,%eax
80101672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101675:	8a 44 02 5c          	mov    0x5c(%edx,%eax,1),%al
80101679:	0f b6 c0             	movzbl %al,%eax
8010167c:	23 45 ec             	and    -0x14(%ebp),%eax
8010167f:	85 c0                	test   %eax,%eax
80101681:	75 0c                	jne    8010168f <bfree+0x8e>
    panic("freeing free block");
80101683:	c7 04 24 d6 82 10 80 	movl   $0x801082d6,(%esp)
8010168a:	e8 c5 ee ff ff       	call   80100554 <panic>
  bp->data[bi/8] &= ~m;
8010168f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101692:	85 c0                	test   %eax,%eax
80101694:	79 03                	jns    80101699 <bfree+0x98>
80101696:	83 c0 07             	add    $0x7,%eax
80101699:	c1 f8 03             	sar    $0x3,%eax
8010169c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010169f:	8a 54 02 5c          	mov    0x5c(%edx,%eax,1),%dl
801016a3:	8b 4d ec             	mov    -0x14(%ebp),%ecx
801016a6:	f7 d1                	not    %ecx
801016a8:	21 ca                	and    %ecx,%edx
801016aa:	88 d1                	mov    %dl,%cl
801016ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801016af:	88 4c 02 5c          	mov    %cl,0x5c(%edx,%eax,1)
  log_write(bp);
801016b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016b6:	89 04 24             	mov    %eax,(%esp)
801016b9:	e8 71 20 00 00       	call   8010372f <log_write>
  brelse(bp);
801016be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016c1:	89 04 24             	mov    %eax,(%esp)
801016c4:	e8 63 eb ff ff       	call   8010022c <brelse>
}
801016c9:	c9                   	leave  
801016ca:	c3                   	ret    

801016cb <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(int dev)
{
801016cb:	55                   	push   %ebp
801016cc:	89 e5                	mov    %esp,%ebp
801016ce:	57                   	push   %edi
801016cf:	56                   	push   %esi
801016d0:	53                   	push   %ebx
801016d1:	83 ec 4c             	sub    $0x4c,%esp
  int i = 0;
801016d4:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
  
  initlock(&icache.lock, "icache");
801016db:	c7 44 24 04 e9 82 10 	movl   $0x801082e9,0x4(%esp)
801016e2:	80 
801016e3:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
801016ea:	e8 23 37 00 00       	call   80104e12 <initlock>
  for(i = 0; i < NINODE; i++) {
801016ef:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801016f6:	eb 2b                	jmp    80101723 <iinit+0x58>
    initsleeplock(&icache.inode[i].lock, "inode");
801016f8:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801016fb:	89 d0                	mov    %edx,%eax
801016fd:	c1 e0 03             	shl    $0x3,%eax
80101700:	01 d0                	add    %edx,%eax
80101702:	c1 e0 04             	shl    $0x4,%eax
80101705:	83 c0 30             	add    $0x30,%eax
80101708:	05 20 1d 11 80       	add    $0x80111d20,%eax
8010170d:	83 c0 10             	add    $0x10,%eax
80101710:	c7 44 24 04 f0 82 10 	movl   $0x801082f0,0x4(%esp)
80101717:	80 
80101718:	89 04 24             	mov    %eax,(%esp)
8010171b:	e8 b4 35 00 00       	call   80104cd4 <initsleeplock>
iinit(int dev)
{
  int i = 0;
  
  initlock(&icache.lock, "icache");
  for(i = 0; i < NINODE; i++) {
80101720:	ff 45 e4             	incl   -0x1c(%ebp)
80101723:	83 7d e4 31          	cmpl   $0x31,-0x1c(%ebp)
80101727:	7e cf                	jle    801016f8 <iinit+0x2d>
    initsleeplock(&icache.inode[i].lock, "inode");
  }

  readsb(dev, &sb);
80101729:	c7 44 24 04 00 1d 11 	movl   $0x80111d00,0x4(%esp)
80101730:	80 
80101731:	8b 45 08             	mov    0x8(%ebp),%eax
80101734:	89 04 24             	mov    %eax,(%esp)
80101737:	e8 f4 fc ff ff       	call   80101430 <readsb>
  cprintf("sb: size %d nblocks %d ninodes %d nlog %d logstart %d\
8010173c:	a1 18 1d 11 80       	mov    0x80111d18,%eax
80101741:	8b 3d 14 1d 11 80    	mov    0x80111d14,%edi
80101747:	8b 35 10 1d 11 80    	mov    0x80111d10,%esi
8010174d:	8b 1d 0c 1d 11 80    	mov    0x80111d0c,%ebx
80101753:	8b 0d 08 1d 11 80    	mov    0x80111d08,%ecx
80101759:	8b 15 04 1d 11 80    	mov    0x80111d04,%edx
8010175f:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80101762:	8b 15 00 1d 11 80    	mov    0x80111d00,%edx
80101768:	89 44 24 1c          	mov    %eax,0x1c(%esp)
8010176c:	89 7c 24 18          	mov    %edi,0x18(%esp)
80101770:	89 74 24 14          	mov    %esi,0x14(%esp)
80101774:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80101778:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
8010177c:	8b 45 d4             	mov    -0x2c(%ebp),%eax
8010177f:	89 44 24 08          	mov    %eax,0x8(%esp)
80101783:	89 d0                	mov    %edx,%eax
80101785:	89 44 24 04          	mov    %eax,0x4(%esp)
80101789:	c7 04 24 f8 82 10 80 	movl   $0x801082f8,(%esp)
80101790:	e8 2c ec ff ff       	call   801003c1 <cprintf>
 inodestart %d bmap start %d\n", sb.size, sb.nblocks,
          sb.ninodes, sb.nlog, sb.logstart, sb.inodestart,
          sb.bmapstart);
}
80101795:	83 c4 4c             	add    $0x4c,%esp
80101798:	5b                   	pop    %ebx
80101799:	5e                   	pop    %esi
8010179a:	5f                   	pop    %edi
8010179b:	5d                   	pop    %ebp
8010179c:	c3                   	ret    

8010179d <ialloc>:
// Allocate an inode on device dev.
// Mark it as allocated by  giving it type type.
// Returns an unlocked but allocated and referenced inode.
struct inode*
ialloc(uint dev, short type)
{
8010179d:	55                   	push   %ebp
8010179e:	89 e5                	mov    %esp,%ebp
801017a0:	83 ec 28             	sub    $0x28,%esp
801017a3:	8b 45 0c             	mov    0xc(%ebp),%eax
801017a6:	66 89 45 e4          	mov    %ax,-0x1c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;

  for(inum = 1; inum < sb.ninodes; inum++){
801017aa:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801017b1:	e9 9b 00 00 00       	jmp    80101851 <ialloc+0xb4>
    bp = bread(dev, IBLOCK(inum, sb));
801017b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017b9:	c1 e8 03             	shr    $0x3,%eax
801017bc:	89 c2                	mov    %eax,%edx
801017be:	a1 14 1d 11 80       	mov    0x80111d14,%eax
801017c3:	01 d0                	add    %edx,%eax
801017c5:	89 44 24 04          	mov    %eax,0x4(%esp)
801017c9:	8b 45 08             	mov    0x8(%ebp),%eax
801017cc:	89 04 24             	mov    %eax,(%esp)
801017cf:	e8 e1 e9 ff ff       	call   801001b5 <bread>
801017d4:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
801017d7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017da:	8d 50 5c             	lea    0x5c(%eax),%edx
801017dd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017e0:	83 e0 07             	and    $0x7,%eax
801017e3:	c1 e0 06             	shl    $0x6,%eax
801017e6:	01 d0                	add    %edx,%eax
801017e8:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
801017eb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801017ee:	8b 00                	mov    (%eax),%eax
801017f0:	66 85 c0             	test   %ax,%ax
801017f3:	75 4e                	jne    80101843 <ialloc+0xa6>
      memset(dip, 0, sizeof(*dip));
801017f5:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
801017fc:	00 
801017fd:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80101804:	00 
80101805:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101808:	89 04 24             	mov    %eax,(%esp)
8010180b:	e8 86 38 00 00       	call   80105096 <memset>
      dip->type = type;
80101810:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101813:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101816:	66 89 02             	mov    %ax,(%edx)
      log_write(bp);   // mark it allocated on the disk
80101819:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010181c:	89 04 24             	mov    %eax,(%esp)
8010181f:	e8 0b 1f 00 00       	call   8010372f <log_write>
      brelse(bp);
80101824:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101827:	89 04 24             	mov    %eax,(%esp)
8010182a:	e8 fd e9 ff ff       	call   8010022c <brelse>
      return iget(dev, inum);
8010182f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101832:	89 44 24 04          	mov    %eax,0x4(%esp)
80101836:	8b 45 08             	mov    0x8(%ebp),%eax
80101839:	89 04 24             	mov    %eax,(%esp)
8010183c:	e8 ea 00 00 00       	call   8010192b <iget>
80101841:	eb 2a                	jmp    8010186d <ialloc+0xd0>
    }
    brelse(bp);
80101843:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101846:	89 04 24             	mov    %eax,(%esp)
80101849:	e8 de e9 ff ff       	call   8010022c <brelse>
{
  int inum;
  struct buf *bp;
  struct dinode *dip;

  for(inum = 1; inum < sb.ninodes; inum++){
8010184e:	ff 45 f4             	incl   -0xc(%ebp)
80101851:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101854:	a1 08 1d 11 80       	mov    0x80111d08,%eax
80101859:	39 c2                	cmp    %eax,%edx
8010185b:	0f 82 55 ff ff ff    	jb     801017b6 <ialloc+0x19>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
80101861:	c7 04 24 4b 83 10 80 	movl   $0x8010834b,(%esp)
80101868:	e8 e7 ec ff ff       	call   80100554 <panic>
}
8010186d:	c9                   	leave  
8010186e:	c3                   	ret    

8010186f <iupdate>:
// Must be called after every change to an ip->xxx field
// that lives on disk, since i-node cache is write-through.
// Caller must hold ip->lock.
void
iupdate(struct inode *ip)
{
8010186f:	55                   	push   %ebp
80101870:	89 e5                	mov    %esp,%ebp
80101872:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101875:	8b 45 08             	mov    0x8(%ebp),%eax
80101878:	8b 40 04             	mov    0x4(%eax),%eax
8010187b:	c1 e8 03             	shr    $0x3,%eax
8010187e:	89 c2                	mov    %eax,%edx
80101880:	a1 14 1d 11 80       	mov    0x80111d14,%eax
80101885:	01 c2                	add    %eax,%edx
80101887:	8b 45 08             	mov    0x8(%ebp),%eax
8010188a:	8b 00                	mov    (%eax),%eax
8010188c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101890:	89 04 24             	mov    %eax,(%esp)
80101893:	e8 1d e9 ff ff       	call   801001b5 <bread>
80101898:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
8010189b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010189e:	8d 50 5c             	lea    0x5c(%eax),%edx
801018a1:	8b 45 08             	mov    0x8(%ebp),%eax
801018a4:	8b 40 04             	mov    0x4(%eax),%eax
801018a7:	83 e0 07             	and    $0x7,%eax
801018aa:	c1 e0 06             	shl    $0x6,%eax
801018ad:	01 d0                	add    %edx,%eax
801018af:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801018b2:	8b 45 08             	mov    0x8(%ebp),%eax
801018b5:	8b 40 50             	mov    0x50(%eax),%eax
801018b8:	8b 55 f0             	mov    -0x10(%ebp),%edx
801018bb:	66 89 02             	mov    %ax,(%edx)
  dip->major = ip->major;
801018be:	8b 45 08             	mov    0x8(%ebp),%eax
801018c1:	66 8b 40 52          	mov    0x52(%eax),%ax
801018c5:	8b 55 f0             	mov    -0x10(%ebp),%edx
801018c8:	66 89 42 02          	mov    %ax,0x2(%edx)
  dip->minor = ip->minor;
801018cc:	8b 45 08             	mov    0x8(%ebp),%eax
801018cf:	8b 40 54             	mov    0x54(%eax),%eax
801018d2:	8b 55 f0             	mov    -0x10(%ebp),%edx
801018d5:	66 89 42 04          	mov    %ax,0x4(%edx)
  dip->nlink = ip->nlink;
801018d9:	8b 45 08             	mov    0x8(%ebp),%eax
801018dc:	66 8b 40 56          	mov    0x56(%eax),%ax
801018e0:	8b 55 f0             	mov    -0x10(%ebp),%edx
801018e3:	66 89 42 06          	mov    %ax,0x6(%edx)
  dip->size = ip->size;
801018e7:	8b 45 08             	mov    0x8(%ebp),%eax
801018ea:	8b 50 58             	mov    0x58(%eax),%edx
801018ed:	8b 45 f0             	mov    -0x10(%ebp),%eax
801018f0:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
801018f3:	8b 45 08             	mov    0x8(%ebp),%eax
801018f6:	8d 50 5c             	lea    0x5c(%eax),%edx
801018f9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801018fc:	83 c0 0c             	add    $0xc,%eax
801018ff:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101906:	00 
80101907:	89 54 24 04          	mov    %edx,0x4(%esp)
8010190b:	89 04 24             	mov    %eax,(%esp)
8010190e:	e8 4c 38 00 00       	call   8010515f <memmove>
  log_write(bp);
80101913:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101916:	89 04 24             	mov    %eax,(%esp)
80101919:	e8 11 1e 00 00       	call   8010372f <log_write>
  brelse(bp);
8010191e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101921:	89 04 24             	mov    %eax,(%esp)
80101924:	e8 03 e9 ff ff       	call   8010022c <brelse>
}
80101929:	c9                   	leave  
8010192a:	c3                   	ret    

8010192b <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
8010192b:	55                   	push   %ebp
8010192c:	89 e5                	mov    %esp,%ebp
8010192e:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
80101931:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101938:	e8 f6 34 00 00       	call   80104e33 <acquire>

  // Is the inode already cached?
  empty = 0;
8010193d:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101944:	c7 45 f4 54 1d 11 80 	movl   $0x80111d54,-0xc(%ebp)
8010194b:	eb 5c                	jmp    801019a9 <iget+0x7e>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
8010194d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101950:	8b 40 08             	mov    0x8(%eax),%eax
80101953:	85 c0                	test   %eax,%eax
80101955:	7e 35                	jle    8010198c <iget+0x61>
80101957:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010195a:	8b 00                	mov    (%eax),%eax
8010195c:	3b 45 08             	cmp    0x8(%ebp),%eax
8010195f:	75 2b                	jne    8010198c <iget+0x61>
80101961:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101964:	8b 40 04             	mov    0x4(%eax),%eax
80101967:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010196a:	75 20                	jne    8010198c <iget+0x61>
      ip->ref++;
8010196c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010196f:	8b 40 08             	mov    0x8(%eax),%eax
80101972:	8d 50 01             	lea    0x1(%eax),%edx
80101975:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101978:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
8010197b:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101982:	e8 16 35 00 00       	call   80104e9d <release>
      return ip;
80101987:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010198a:	eb 72                	jmp    801019fe <iget+0xd3>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
8010198c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80101990:	75 10                	jne    801019a2 <iget+0x77>
80101992:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101995:	8b 40 08             	mov    0x8(%eax),%eax
80101998:	85 c0                	test   %eax,%eax
8010199a:	75 06                	jne    801019a2 <iget+0x77>
      empty = ip;
8010199c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010199f:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801019a2:	81 45 f4 90 00 00 00 	addl   $0x90,-0xc(%ebp)
801019a9:	81 7d f4 74 39 11 80 	cmpl   $0x80113974,-0xc(%ebp)
801019b0:	72 9b                	jb     8010194d <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801019b2:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801019b6:	75 0c                	jne    801019c4 <iget+0x99>
    panic("iget: no inodes");
801019b8:	c7 04 24 5d 83 10 80 	movl   $0x8010835d,(%esp)
801019bf:	e8 90 eb ff ff       	call   80100554 <panic>

  ip = empty;
801019c4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801019c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
801019ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801019cd:	8b 55 08             	mov    0x8(%ebp),%edx
801019d0:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
801019d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801019d5:	8b 55 0c             	mov    0xc(%ebp),%edx
801019d8:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
801019db:	8b 45 f4             	mov    -0xc(%ebp),%eax
801019de:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->valid = 0;
801019e5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801019e8:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  release(&icache.lock);
801019ef:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
801019f6:	e8 a2 34 00 00       	call   80104e9d <release>

  return ip;
801019fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801019fe:	c9                   	leave  
801019ff:	c3                   	ret    

80101a00 <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
80101a00:	55                   	push   %ebp
80101a01:	89 e5                	mov    %esp,%ebp
80101a03:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a06:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101a0d:	e8 21 34 00 00       	call   80104e33 <acquire>
  ip->ref++;
80101a12:	8b 45 08             	mov    0x8(%ebp),%eax
80101a15:	8b 40 08             	mov    0x8(%eax),%eax
80101a18:	8d 50 01             	lea    0x1(%eax),%edx
80101a1b:	8b 45 08             	mov    0x8(%ebp),%eax
80101a1e:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101a21:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101a28:	e8 70 34 00 00       	call   80104e9d <release>
  return ip;
80101a2d:	8b 45 08             	mov    0x8(%ebp),%eax
}
80101a30:	c9                   	leave  
80101a31:	c3                   	ret    

80101a32 <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
80101a32:	55                   	push   %ebp
80101a33:	89 e5                	mov    %esp,%ebp
80101a35:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
80101a38:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101a3c:	74 0a                	je     80101a48 <ilock+0x16>
80101a3e:	8b 45 08             	mov    0x8(%ebp),%eax
80101a41:	8b 40 08             	mov    0x8(%eax),%eax
80101a44:	85 c0                	test   %eax,%eax
80101a46:	7f 0c                	jg     80101a54 <ilock+0x22>
    panic("ilock");
80101a48:	c7 04 24 6d 83 10 80 	movl   $0x8010836d,(%esp)
80101a4f:	e8 00 eb ff ff       	call   80100554 <panic>

  acquiresleep(&ip->lock);
80101a54:	8b 45 08             	mov    0x8(%ebp),%eax
80101a57:	83 c0 0c             	add    $0xc,%eax
80101a5a:	89 04 24             	mov    %eax,(%esp)
80101a5d:	e8 ac 32 00 00       	call   80104d0e <acquiresleep>

  if(ip->valid == 0){
80101a62:	8b 45 08             	mov    0x8(%ebp),%eax
80101a65:	8b 40 4c             	mov    0x4c(%eax),%eax
80101a68:	85 c0                	test   %eax,%eax
80101a6a:	0f 85 ca 00 00 00    	jne    80101b3a <ilock+0x108>
    bp = bread(ip->dev, IBLOCK(ip->inum, sb));
80101a70:	8b 45 08             	mov    0x8(%ebp),%eax
80101a73:	8b 40 04             	mov    0x4(%eax),%eax
80101a76:	c1 e8 03             	shr    $0x3,%eax
80101a79:	89 c2                	mov    %eax,%edx
80101a7b:	a1 14 1d 11 80       	mov    0x80111d14,%eax
80101a80:	01 c2                	add    %eax,%edx
80101a82:	8b 45 08             	mov    0x8(%ebp),%eax
80101a85:	8b 00                	mov    (%eax),%eax
80101a87:	89 54 24 04          	mov    %edx,0x4(%esp)
80101a8b:	89 04 24             	mov    %eax,(%esp)
80101a8e:	e8 22 e7 ff ff       	call   801001b5 <bread>
80101a93:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
80101a96:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101a99:	8d 50 5c             	lea    0x5c(%eax),%edx
80101a9c:	8b 45 08             	mov    0x8(%ebp),%eax
80101a9f:	8b 40 04             	mov    0x4(%eax),%eax
80101aa2:	83 e0 07             	and    $0x7,%eax
80101aa5:	c1 e0 06             	shl    $0x6,%eax
80101aa8:	01 d0                	add    %edx,%eax
80101aaa:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
80101aad:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ab0:	8b 00                	mov    (%eax),%eax
80101ab2:	8b 55 08             	mov    0x8(%ebp),%edx
80101ab5:	66 89 42 50          	mov    %ax,0x50(%edx)
    ip->major = dip->major;
80101ab9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101abc:	66 8b 40 02          	mov    0x2(%eax),%ax
80101ac0:	8b 55 08             	mov    0x8(%ebp),%edx
80101ac3:	66 89 42 52          	mov    %ax,0x52(%edx)
    ip->minor = dip->minor;
80101ac7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101aca:	8b 40 04             	mov    0x4(%eax),%eax
80101acd:	8b 55 08             	mov    0x8(%ebp),%edx
80101ad0:	66 89 42 54          	mov    %ax,0x54(%edx)
    ip->nlink = dip->nlink;
80101ad4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ad7:	66 8b 40 06          	mov    0x6(%eax),%ax
80101adb:	8b 55 08             	mov    0x8(%ebp),%edx
80101ade:	66 89 42 56          	mov    %ax,0x56(%edx)
    ip->size = dip->size;
80101ae2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ae5:	8b 50 08             	mov    0x8(%eax),%edx
80101ae8:	8b 45 08             	mov    0x8(%ebp),%eax
80101aeb:	89 50 58             	mov    %edx,0x58(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101aee:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101af1:	8d 50 0c             	lea    0xc(%eax),%edx
80101af4:	8b 45 08             	mov    0x8(%ebp),%eax
80101af7:	83 c0 5c             	add    $0x5c,%eax
80101afa:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101b01:	00 
80101b02:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b06:	89 04 24             	mov    %eax,(%esp)
80101b09:	e8 51 36 00 00       	call   8010515f <memmove>
    brelse(bp);
80101b0e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b11:	89 04 24             	mov    %eax,(%esp)
80101b14:	e8 13 e7 ff ff       	call   8010022c <brelse>
    ip->valid = 1;
80101b19:	8b 45 08             	mov    0x8(%ebp),%eax
80101b1c:	c7 40 4c 01 00 00 00 	movl   $0x1,0x4c(%eax)
    if(ip->type == 0)
80101b23:	8b 45 08             	mov    0x8(%ebp),%eax
80101b26:	8b 40 50             	mov    0x50(%eax),%eax
80101b29:	66 85 c0             	test   %ax,%ax
80101b2c:	75 0c                	jne    80101b3a <ilock+0x108>
      panic("ilock: no type");
80101b2e:	c7 04 24 73 83 10 80 	movl   $0x80108373,(%esp)
80101b35:	e8 1a ea ff ff       	call   80100554 <panic>
  }
}
80101b3a:	c9                   	leave  
80101b3b:	c3                   	ret    

80101b3c <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
80101b3c:	55                   	push   %ebp
80101b3d:	89 e5                	mov    %esp,%ebp
80101b3f:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !holdingsleep(&ip->lock) || ip->ref < 1)
80101b42:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80101b46:	74 1c                	je     80101b64 <iunlock+0x28>
80101b48:	8b 45 08             	mov    0x8(%ebp),%eax
80101b4b:	83 c0 0c             	add    $0xc,%eax
80101b4e:	89 04 24             	mov    %eax,(%esp)
80101b51:	e8 55 32 00 00       	call   80104dab <holdingsleep>
80101b56:	85 c0                	test   %eax,%eax
80101b58:	74 0a                	je     80101b64 <iunlock+0x28>
80101b5a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b5d:	8b 40 08             	mov    0x8(%eax),%eax
80101b60:	85 c0                	test   %eax,%eax
80101b62:	7f 0c                	jg     80101b70 <iunlock+0x34>
    panic("iunlock");
80101b64:	c7 04 24 82 83 10 80 	movl   $0x80108382,(%esp)
80101b6b:	e8 e4 e9 ff ff       	call   80100554 <panic>

  releasesleep(&ip->lock);
80101b70:	8b 45 08             	mov    0x8(%ebp),%eax
80101b73:	83 c0 0c             	add    $0xc,%eax
80101b76:	89 04 24             	mov    %eax,(%esp)
80101b79:	e8 eb 31 00 00       	call   80104d69 <releasesleep>
}
80101b7e:	c9                   	leave  
80101b7f:	c3                   	ret    

80101b80 <iput>:
// to it, free the inode (and its content) on disk.
// All calls to iput() must be inside a transaction in
// case it has to free the inode.
void
iput(struct inode *ip)
{
80101b80:	55                   	push   %ebp
80101b81:	89 e5                	mov    %esp,%ebp
80101b83:	83 ec 28             	sub    $0x28,%esp
  acquiresleep(&ip->lock);
80101b86:	8b 45 08             	mov    0x8(%ebp),%eax
80101b89:	83 c0 0c             	add    $0xc,%eax
80101b8c:	89 04 24             	mov    %eax,(%esp)
80101b8f:	e8 7a 31 00 00       	call   80104d0e <acquiresleep>
  if(ip->valid && ip->nlink == 0){
80101b94:	8b 45 08             	mov    0x8(%ebp),%eax
80101b97:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b9a:	85 c0                	test   %eax,%eax
80101b9c:	74 5c                	je     80101bfa <iput+0x7a>
80101b9e:	8b 45 08             	mov    0x8(%ebp),%eax
80101ba1:	66 8b 40 56          	mov    0x56(%eax),%ax
80101ba5:	66 85 c0             	test   %ax,%ax
80101ba8:	75 50                	jne    80101bfa <iput+0x7a>
    acquire(&icache.lock);
80101baa:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101bb1:	e8 7d 32 00 00       	call   80104e33 <acquire>
    int r = ip->ref;
80101bb6:	8b 45 08             	mov    0x8(%ebp),%eax
80101bb9:	8b 40 08             	mov    0x8(%eax),%eax
80101bbc:	89 45 f4             	mov    %eax,-0xc(%ebp)
    release(&icache.lock);
80101bbf:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101bc6:	e8 d2 32 00 00       	call   80104e9d <release>
    if(r == 1){
80101bcb:	83 7d f4 01          	cmpl   $0x1,-0xc(%ebp)
80101bcf:	75 29                	jne    80101bfa <iput+0x7a>
      // inode has no links and no other references: truncate and free.
      itrunc(ip);
80101bd1:	8b 45 08             	mov    0x8(%ebp),%eax
80101bd4:	89 04 24             	mov    %eax,(%esp)
80101bd7:	e8 86 01 00 00       	call   80101d62 <itrunc>
      ip->type = 0;
80101bdc:	8b 45 08             	mov    0x8(%ebp),%eax
80101bdf:	66 c7 40 50 00 00    	movw   $0x0,0x50(%eax)
      iupdate(ip);
80101be5:	8b 45 08             	mov    0x8(%ebp),%eax
80101be8:	89 04 24             	mov    %eax,(%esp)
80101beb:	e8 7f fc ff ff       	call   8010186f <iupdate>
      ip->valid = 0;
80101bf0:	8b 45 08             	mov    0x8(%ebp),%eax
80101bf3:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
    }
  }
  releasesleep(&ip->lock);
80101bfa:	8b 45 08             	mov    0x8(%ebp),%eax
80101bfd:	83 c0 0c             	add    $0xc,%eax
80101c00:	89 04 24             	mov    %eax,(%esp)
80101c03:	e8 61 31 00 00       	call   80104d69 <releasesleep>

  acquire(&icache.lock);
80101c08:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101c0f:	e8 1f 32 00 00       	call   80104e33 <acquire>
  ip->ref--;
80101c14:	8b 45 08             	mov    0x8(%ebp),%eax
80101c17:	8b 40 08             	mov    0x8(%eax),%eax
80101c1a:	8d 50 ff             	lea    -0x1(%eax),%edx
80101c1d:	8b 45 08             	mov    0x8(%ebp),%eax
80101c20:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101c23:	c7 04 24 20 1d 11 80 	movl   $0x80111d20,(%esp)
80101c2a:	e8 6e 32 00 00       	call   80104e9d <release>
}
80101c2f:	c9                   	leave  
80101c30:	c3                   	ret    

80101c31 <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101c31:	55                   	push   %ebp
80101c32:	89 e5                	mov    %esp,%ebp
80101c34:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101c37:	8b 45 08             	mov    0x8(%ebp),%eax
80101c3a:	89 04 24             	mov    %eax,(%esp)
80101c3d:	e8 fa fe ff ff       	call   80101b3c <iunlock>
  iput(ip);
80101c42:	8b 45 08             	mov    0x8(%ebp),%eax
80101c45:	89 04 24             	mov    %eax,(%esp)
80101c48:	e8 33 ff ff ff       	call   80101b80 <iput>
}
80101c4d:	c9                   	leave  
80101c4e:	c3                   	ret    

80101c4f <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101c4f:	55                   	push   %ebp
80101c50:	89 e5                	mov    %esp,%ebp
80101c52:	53                   	push   %ebx
80101c53:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101c56:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101c5a:	77 3e                	ja     80101c9a <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101c5c:	8b 45 08             	mov    0x8(%ebp),%eax
80101c5f:	8b 55 0c             	mov    0xc(%ebp),%edx
80101c62:	83 c2 14             	add    $0x14,%edx
80101c65:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c69:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101c6c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101c70:	75 20                	jne    80101c92 <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101c72:	8b 45 08             	mov    0x8(%ebp),%eax
80101c75:	8b 00                	mov    (%eax),%eax
80101c77:	89 04 24             	mov    %eax,(%esp)
80101c7a:	e8 48 f8 ff ff       	call   801014c7 <balloc>
80101c7f:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101c82:	8b 45 08             	mov    0x8(%ebp),%eax
80101c85:	8b 55 0c             	mov    0xc(%ebp),%edx
80101c88:	8d 4a 14             	lea    0x14(%edx),%ecx
80101c8b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c8e:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101c92:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101c95:	e9 c2 00 00 00       	jmp    80101d5c <bmap+0x10d>
  }
  bn -= NDIRECT;
80101c9a:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101c9e:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101ca2:	0f 87 a8 00 00 00    	ja     80101d50 <bmap+0x101>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101ca8:	8b 45 08             	mov    0x8(%ebp),%eax
80101cab:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80101cb1:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101cb4:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101cb8:	75 1c                	jne    80101cd6 <bmap+0x87>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101cba:	8b 45 08             	mov    0x8(%ebp),%eax
80101cbd:	8b 00                	mov    (%eax),%eax
80101cbf:	89 04 24             	mov    %eax,(%esp)
80101cc2:	e8 00 f8 ff ff       	call   801014c7 <balloc>
80101cc7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101cca:	8b 45 08             	mov    0x8(%ebp),%eax
80101ccd:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101cd0:	89 90 8c 00 00 00    	mov    %edx,0x8c(%eax)
    bp = bread(ip->dev, addr);
80101cd6:	8b 45 08             	mov    0x8(%ebp),%eax
80101cd9:	8b 00                	mov    (%eax),%eax
80101cdb:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101cde:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ce2:	89 04 24             	mov    %eax,(%esp)
80101ce5:	e8 cb e4 ff ff       	call   801001b5 <bread>
80101cea:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101ced:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cf0:	83 c0 5c             	add    $0x5c,%eax
80101cf3:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101cf6:	8b 45 0c             	mov    0xc(%ebp),%eax
80101cf9:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101d00:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101d03:	01 d0                	add    %edx,%eax
80101d05:	8b 00                	mov    (%eax),%eax
80101d07:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101d0a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101d0e:	75 30                	jne    80101d40 <bmap+0xf1>
      a[bn] = addr = balloc(ip->dev);
80101d10:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d13:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101d1a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101d1d:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
80101d20:	8b 45 08             	mov    0x8(%ebp),%eax
80101d23:	8b 00                	mov    (%eax),%eax
80101d25:	89 04 24             	mov    %eax,(%esp)
80101d28:	e8 9a f7 ff ff       	call   801014c7 <balloc>
80101d2d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101d30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101d33:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101d35:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101d38:	89 04 24             	mov    %eax,(%esp)
80101d3b:	e8 ef 19 00 00       	call   8010372f <log_write>
    }
    brelse(bp);
80101d40:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101d43:	89 04 24             	mov    %eax,(%esp)
80101d46:	e8 e1 e4 ff ff       	call   8010022c <brelse>
    return addr;
80101d4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101d4e:	eb 0c                	jmp    80101d5c <bmap+0x10d>
  }

  panic("bmap: out of range");
80101d50:	c7 04 24 8a 83 10 80 	movl   $0x8010838a,(%esp)
80101d57:	e8 f8 e7 ff ff       	call   80100554 <panic>
}
80101d5c:	83 c4 24             	add    $0x24,%esp
80101d5f:	5b                   	pop    %ebx
80101d60:	5d                   	pop    %ebp
80101d61:	c3                   	ret    

80101d62 <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101d62:	55                   	push   %ebp
80101d63:	89 e5                	mov    %esp,%ebp
80101d65:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101d68:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101d6f:	eb 43                	jmp    80101db4 <itrunc+0x52>
    if(ip->addrs[i]){
80101d71:	8b 45 08             	mov    0x8(%ebp),%eax
80101d74:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101d77:	83 c2 14             	add    $0x14,%edx
80101d7a:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101d7e:	85 c0                	test   %eax,%eax
80101d80:	74 2f                	je     80101db1 <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101d82:	8b 45 08             	mov    0x8(%ebp),%eax
80101d85:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101d88:	83 c2 14             	add    $0x14,%edx
80101d8b:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101d8f:	8b 45 08             	mov    0x8(%ebp),%eax
80101d92:	8b 00                	mov    (%eax),%eax
80101d94:	89 54 24 04          	mov    %edx,0x4(%esp)
80101d98:	89 04 24             	mov    %eax,(%esp)
80101d9b:	e8 61 f8 ff ff       	call   80101601 <bfree>
      ip->addrs[i] = 0;
80101da0:	8b 45 08             	mov    0x8(%ebp),%eax
80101da3:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101da6:	83 c2 14             	add    $0x14,%edx
80101da9:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101db0:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101db1:	ff 45 f4             	incl   -0xc(%ebp)
80101db4:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101db8:	7e b7                	jle    80101d71 <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }

  if(ip->addrs[NDIRECT]){
80101dba:	8b 45 08             	mov    0x8(%ebp),%eax
80101dbd:	8b 80 8c 00 00 00    	mov    0x8c(%eax),%eax
80101dc3:	85 c0                	test   %eax,%eax
80101dc5:	0f 84 a3 00 00 00    	je     80101e6e <itrunc+0x10c>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101dcb:	8b 45 08             	mov    0x8(%ebp),%eax
80101dce:	8b 90 8c 00 00 00    	mov    0x8c(%eax),%edx
80101dd4:	8b 45 08             	mov    0x8(%ebp),%eax
80101dd7:	8b 00                	mov    (%eax),%eax
80101dd9:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ddd:	89 04 24             	mov    %eax,(%esp)
80101de0:	e8 d0 e3 ff ff       	call   801001b5 <bread>
80101de5:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101de8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101deb:	83 c0 5c             	add    $0x5c,%eax
80101dee:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101df1:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101df8:	eb 3a                	jmp    80101e34 <itrunc+0xd2>
      if(a[j])
80101dfa:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101dfd:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101e04:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101e07:	01 d0                	add    %edx,%eax
80101e09:	8b 00                	mov    (%eax),%eax
80101e0b:	85 c0                	test   %eax,%eax
80101e0d:	74 22                	je     80101e31 <itrunc+0xcf>
        bfree(ip->dev, a[j]);
80101e0f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e12:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80101e19:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101e1c:	01 d0                	add    %edx,%eax
80101e1e:	8b 10                	mov    (%eax),%edx
80101e20:	8b 45 08             	mov    0x8(%ebp),%eax
80101e23:	8b 00                	mov    (%eax),%eax
80101e25:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e29:	89 04 24             	mov    %eax,(%esp)
80101e2c:	e8 d0 f7 ff ff       	call   80101601 <bfree>
  }

  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101e31:	ff 45 f0             	incl   -0x10(%ebp)
80101e34:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e37:	83 f8 7f             	cmp    $0x7f,%eax
80101e3a:	76 be                	jbe    80101dfa <itrunc+0x98>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101e3c:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e3f:	89 04 24             	mov    %eax,(%esp)
80101e42:	e8 e5 e3 ff ff       	call   8010022c <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101e47:	8b 45 08             	mov    0x8(%ebp),%eax
80101e4a:	8b 90 8c 00 00 00    	mov    0x8c(%eax),%edx
80101e50:	8b 45 08             	mov    0x8(%ebp),%eax
80101e53:	8b 00                	mov    (%eax),%eax
80101e55:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e59:	89 04 24             	mov    %eax,(%esp)
80101e5c:	e8 a0 f7 ff ff       	call   80101601 <bfree>
    ip->addrs[NDIRECT] = 0;
80101e61:	8b 45 08             	mov    0x8(%ebp),%eax
80101e64:	c7 80 8c 00 00 00 00 	movl   $0x0,0x8c(%eax)
80101e6b:	00 00 00 
  }

  ip->size = 0;
80101e6e:	8b 45 08             	mov    0x8(%ebp),%eax
80101e71:	c7 40 58 00 00 00 00 	movl   $0x0,0x58(%eax)
  iupdate(ip);
80101e78:	8b 45 08             	mov    0x8(%ebp),%eax
80101e7b:	89 04 24             	mov    %eax,(%esp)
80101e7e:	e8 ec f9 ff ff       	call   8010186f <iupdate>
}
80101e83:	c9                   	leave  
80101e84:	c3                   	ret    

80101e85 <stati>:

// Copy stat information from inode.
// Caller must hold ip->lock.
void
stati(struct inode *ip, struct stat *st)
{
80101e85:	55                   	push   %ebp
80101e86:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101e88:	8b 45 08             	mov    0x8(%ebp),%eax
80101e8b:	8b 00                	mov    (%eax),%eax
80101e8d:	89 c2                	mov    %eax,%edx
80101e8f:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e92:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101e95:	8b 45 08             	mov    0x8(%ebp),%eax
80101e98:	8b 50 04             	mov    0x4(%eax),%edx
80101e9b:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e9e:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101ea1:	8b 45 08             	mov    0x8(%ebp),%eax
80101ea4:	8b 40 50             	mov    0x50(%eax),%eax
80101ea7:	8b 55 0c             	mov    0xc(%ebp),%edx
80101eaa:	66 89 02             	mov    %ax,(%edx)
  st->nlink = ip->nlink;
80101ead:	8b 45 08             	mov    0x8(%ebp),%eax
80101eb0:	66 8b 40 56          	mov    0x56(%eax),%ax
80101eb4:	8b 55 0c             	mov    0xc(%ebp),%edx
80101eb7:	66 89 42 0c          	mov    %ax,0xc(%edx)
  st->size = ip->size;
80101ebb:	8b 45 08             	mov    0x8(%ebp),%eax
80101ebe:	8b 50 58             	mov    0x58(%eax),%edx
80101ec1:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ec4:	89 50 10             	mov    %edx,0x10(%eax)
}
80101ec7:	5d                   	pop    %ebp
80101ec8:	c3                   	ret    

80101ec9 <readi>:
//PAGEBREAK!
// Read data from inode.
// Caller must hold ip->lock.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101ec9:	55                   	push   %ebp
80101eca:	89 e5                	mov    %esp,%ebp
80101ecc:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ecf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed2:	8b 40 50             	mov    0x50(%eax),%eax
80101ed5:	66 83 f8 03          	cmp    $0x3,%ax
80101ed9:	75 60                	jne    80101f3b <readi+0x72>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101edb:	8b 45 08             	mov    0x8(%ebp),%eax
80101ede:	66 8b 40 52          	mov    0x52(%eax),%ax
80101ee2:	66 85 c0             	test   %ax,%ax
80101ee5:	78 20                	js     80101f07 <readi+0x3e>
80101ee7:	8b 45 08             	mov    0x8(%ebp),%eax
80101eea:	66 8b 40 52          	mov    0x52(%eax),%ax
80101eee:	66 83 f8 09          	cmp    $0x9,%ax
80101ef2:	7f 13                	jg     80101f07 <readi+0x3e>
80101ef4:	8b 45 08             	mov    0x8(%ebp),%eax
80101ef7:	66 8b 40 52          	mov    0x52(%eax),%ax
80101efb:	98                   	cwtl   
80101efc:	8b 04 c5 a0 1c 11 80 	mov    -0x7feee360(,%eax,8),%eax
80101f03:	85 c0                	test   %eax,%eax
80101f05:	75 0a                	jne    80101f11 <readi+0x48>
      return -1;
80101f07:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f0c:	e9 1a 01 00 00       	jmp    8010202b <readi+0x162>
    return devsw[ip->major].read(ip, dst, n);
80101f11:	8b 45 08             	mov    0x8(%ebp),%eax
80101f14:	66 8b 40 52          	mov    0x52(%eax),%ax
80101f18:	98                   	cwtl   
80101f19:	8b 04 c5 a0 1c 11 80 	mov    -0x7feee360(,%eax,8),%eax
80101f20:	8b 55 14             	mov    0x14(%ebp),%edx
80101f23:	89 54 24 08          	mov    %edx,0x8(%esp)
80101f27:	8b 55 0c             	mov    0xc(%ebp),%edx
80101f2a:	89 54 24 04          	mov    %edx,0x4(%esp)
80101f2e:	8b 55 08             	mov    0x8(%ebp),%edx
80101f31:	89 14 24             	mov    %edx,(%esp)
80101f34:	ff d0                	call   *%eax
80101f36:	e9 f0 00 00 00       	jmp    8010202b <readi+0x162>
  }

  if(off > ip->size || off + n < off)
80101f3b:	8b 45 08             	mov    0x8(%ebp),%eax
80101f3e:	8b 40 58             	mov    0x58(%eax),%eax
80101f41:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f44:	72 0d                	jb     80101f53 <readi+0x8a>
80101f46:	8b 45 14             	mov    0x14(%ebp),%eax
80101f49:	8b 55 10             	mov    0x10(%ebp),%edx
80101f4c:	01 d0                	add    %edx,%eax
80101f4e:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f51:	73 0a                	jae    80101f5d <readi+0x94>
    return -1;
80101f53:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f58:	e9 ce 00 00 00       	jmp    8010202b <readi+0x162>
  if(off + n > ip->size)
80101f5d:	8b 45 14             	mov    0x14(%ebp),%eax
80101f60:	8b 55 10             	mov    0x10(%ebp),%edx
80101f63:	01 c2                	add    %eax,%edx
80101f65:	8b 45 08             	mov    0x8(%ebp),%eax
80101f68:	8b 40 58             	mov    0x58(%eax),%eax
80101f6b:	39 c2                	cmp    %eax,%edx
80101f6d:	76 0c                	jbe    80101f7b <readi+0xb2>
    n = ip->size - off;
80101f6f:	8b 45 08             	mov    0x8(%ebp),%eax
80101f72:	8b 40 58             	mov    0x58(%eax),%eax
80101f75:	2b 45 10             	sub    0x10(%ebp),%eax
80101f78:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101f7b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f82:	e9 95 00 00 00       	jmp    8010201c <readi+0x153>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f87:	8b 45 10             	mov    0x10(%ebp),%eax
80101f8a:	c1 e8 09             	shr    $0x9,%eax
80101f8d:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f91:	8b 45 08             	mov    0x8(%ebp),%eax
80101f94:	89 04 24             	mov    %eax,(%esp)
80101f97:	e8 b3 fc ff ff       	call   80101c4f <bmap>
80101f9c:	8b 55 08             	mov    0x8(%ebp),%edx
80101f9f:	8b 12                	mov    (%edx),%edx
80101fa1:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fa5:	89 14 24             	mov    %edx,(%esp)
80101fa8:	e8 08 e2 ff ff       	call   801001b5 <bread>
80101fad:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fb0:	8b 45 10             	mov    0x10(%ebp),%eax
80101fb3:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fb8:	89 c2                	mov    %eax,%edx
80101fba:	b8 00 02 00 00       	mov    $0x200,%eax
80101fbf:	29 d0                	sub    %edx,%eax
80101fc1:	89 c1                	mov    %eax,%ecx
80101fc3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fc6:	8b 55 14             	mov    0x14(%ebp),%edx
80101fc9:	29 c2                	sub    %eax,%edx
80101fcb:	89 c8                	mov    %ecx,%eax
80101fcd:	39 d0                	cmp    %edx,%eax
80101fcf:	76 02                	jbe    80101fd3 <readi+0x10a>
80101fd1:	89 d0                	mov    %edx,%eax
80101fd3:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101fd6:	8b 45 10             	mov    0x10(%ebp),%eax
80101fd9:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fde:	8d 50 50             	lea    0x50(%eax),%edx
80101fe1:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fe4:	01 d0                	add    %edx,%eax
80101fe6:	8d 50 0c             	lea    0xc(%eax),%edx
80101fe9:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fec:	89 44 24 08          	mov    %eax,0x8(%esp)
80101ff0:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ff4:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ff7:	89 04 24             	mov    %eax,(%esp)
80101ffa:	e8 60 31 00 00       	call   8010515f <memmove>
    brelse(bp);
80101fff:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102002:	89 04 24             	mov    %eax,(%esp)
80102005:	e8 22 e2 ff ff       	call   8010022c <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
8010200a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010200d:	01 45 f4             	add    %eax,-0xc(%ebp)
80102010:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102013:	01 45 10             	add    %eax,0x10(%ebp)
80102016:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102019:	01 45 0c             	add    %eax,0xc(%ebp)
8010201c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010201f:	3b 45 14             	cmp    0x14(%ebp),%eax
80102022:	0f 82 5f ff ff ff    	jb     80101f87 <readi+0xbe>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80102028:	8b 45 14             	mov    0x14(%ebp),%eax
}
8010202b:	c9                   	leave  
8010202c:	c3                   	ret    

8010202d <writei>:
// PAGEBREAK!
// Write data to inode.
// Caller must hold ip->lock.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
8010202d:	55                   	push   %ebp
8010202e:	89 e5                	mov    %esp,%ebp
80102030:	83 ec 28             	sub    $0x28,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80102033:	8b 45 08             	mov    0x8(%ebp),%eax
80102036:	8b 40 50             	mov    0x50(%eax),%eax
80102039:	66 83 f8 03          	cmp    $0x3,%ax
8010203d:	75 60                	jne    8010209f <writei+0x72>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
8010203f:	8b 45 08             	mov    0x8(%ebp),%eax
80102042:	66 8b 40 52          	mov    0x52(%eax),%ax
80102046:	66 85 c0             	test   %ax,%ax
80102049:	78 20                	js     8010206b <writei+0x3e>
8010204b:	8b 45 08             	mov    0x8(%ebp),%eax
8010204e:	66 8b 40 52          	mov    0x52(%eax),%ax
80102052:	66 83 f8 09          	cmp    $0x9,%ax
80102056:	7f 13                	jg     8010206b <writei+0x3e>
80102058:	8b 45 08             	mov    0x8(%ebp),%eax
8010205b:	66 8b 40 52          	mov    0x52(%eax),%ax
8010205f:	98                   	cwtl   
80102060:	8b 04 c5 a4 1c 11 80 	mov    -0x7feee35c(,%eax,8),%eax
80102067:	85 c0                	test   %eax,%eax
80102069:	75 0a                	jne    80102075 <writei+0x48>
      return -1;
8010206b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102070:	e9 45 01 00 00       	jmp    801021ba <writei+0x18d>
    return devsw[ip->major].write(ip, src, n);
80102075:	8b 45 08             	mov    0x8(%ebp),%eax
80102078:	66 8b 40 52          	mov    0x52(%eax),%ax
8010207c:	98                   	cwtl   
8010207d:	8b 04 c5 a4 1c 11 80 	mov    -0x7feee35c(,%eax,8),%eax
80102084:	8b 55 14             	mov    0x14(%ebp),%edx
80102087:	89 54 24 08          	mov    %edx,0x8(%esp)
8010208b:	8b 55 0c             	mov    0xc(%ebp),%edx
8010208e:	89 54 24 04          	mov    %edx,0x4(%esp)
80102092:	8b 55 08             	mov    0x8(%ebp),%edx
80102095:	89 14 24             	mov    %edx,(%esp)
80102098:	ff d0                	call   *%eax
8010209a:	e9 1b 01 00 00       	jmp    801021ba <writei+0x18d>
  }

  if(off > ip->size || off + n < off)
8010209f:	8b 45 08             	mov    0x8(%ebp),%eax
801020a2:	8b 40 58             	mov    0x58(%eax),%eax
801020a5:	3b 45 10             	cmp    0x10(%ebp),%eax
801020a8:	72 0d                	jb     801020b7 <writei+0x8a>
801020aa:	8b 45 14             	mov    0x14(%ebp),%eax
801020ad:	8b 55 10             	mov    0x10(%ebp),%edx
801020b0:	01 d0                	add    %edx,%eax
801020b2:	3b 45 10             	cmp    0x10(%ebp),%eax
801020b5:	73 0a                	jae    801020c1 <writei+0x94>
    return -1;
801020b7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801020bc:	e9 f9 00 00 00       	jmp    801021ba <writei+0x18d>
  if(off + n > MAXFILE*BSIZE)
801020c1:	8b 45 14             	mov    0x14(%ebp),%eax
801020c4:	8b 55 10             	mov    0x10(%ebp),%edx
801020c7:	01 d0                	add    %edx,%eax
801020c9:	3d 00 18 01 00       	cmp    $0x11800,%eax
801020ce:	76 0a                	jbe    801020da <writei+0xad>
    return -1;
801020d0:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801020d5:	e9 e0 00 00 00       	jmp    801021ba <writei+0x18d>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
801020da:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801020e1:	e9 a0 00 00 00       	jmp    80102186 <writei+0x159>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
801020e6:	8b 45 10             	mov    0x10(%ebp),%eax
801020e9:	c1 e8 09             	shr    $0x9,%eax
801020ec:	89 44 24 04          	mov    %eax,0x4(%esp)
801020f0:	8b 45 08             	mov    0x8(%ebp),%eax
801020f3:	89 04 24             	mov    %eax,(%esp)
801020f6:	e8 54 fb ff ff       	call   80101c4f <bmap>
801020fb:	8b 55 08             	mov    0x8(%ebp),%edx
801020fe:	8b 12                	mov    (%edx),%edx
80102100:	89 44 24 04          	mov    %eax,0x4(%esp)
80102104:	89 14 24             	mov    %edx,(%esp)
80102107:	e8 a9 e0 ff ff       	call   801001b5 <bread>
8010210c:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
8010210f:	8b 45 10             	mov    0x10(%ebp),%eax
80102112:	25 ff 01 00 00       	and    $0x1ff,%eax
80102117:	89 c2                	mov    %eax,%edx
80102119:	b8 00 02 00 00       	mov    $0x200,%eax
8010211e:	29 d0                	sub    %edx,%eax
80102120:	89 c1                	mov    %eax,%ecx
80102122:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102125:	8b 55 14             	mov    0x14(%ebp),%edx
80102128:	29 c2                	sub    %eax,%edx
8010212a:	89 c8                	mov    %ecx,%eax
8010212c:	39 d0                	cmp    %edx,%eax
8010212e:	76 02                	jbe    80102132 <writei+0x105>
80102130:	89 d0                	mov    %edx,%eax
80102132:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80102135:	8b 45 10             	mov    0x10(%ebp),%eax
80102138:	25 ff 01 00 00       	and    $0x1ff,%eax
8010213d:	8d 50 50             	lea    0x50(%eax),%edx
80102140:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102143:	01 d0                	add    %edx,%eax
80102145:	8d 50 0c             	lea    0xc(%eax),%edx
80102148:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010214b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010214f:	8b 45 0c             	mov    0xc(%ebp),%eax
80102152:	89 44 24 04          	mov    %eax,0x4(%esp)
80102156:	89 14 24             	mov    %edx,(%esp)
80102159:	e8 01 30 00 00       	call   8010515f <memmove>
    log_write(bp);
8010215e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102161:	89 04 24             	mov    %eax,(%esp)
80102164:	e8 c6 15 00 00       	call   8010372f <log_write>
    brelse(bp);
80102169:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010216c:	89 04 24             	mov    %eax,(%esp)
8010216f:	e8 b8 e0 ff ff       	call   8010022c <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80102174:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102177:	01 45 f4             	add    %eax,-0xc(%ebp)
8010217a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010217d:	01 45 10             	add    %eax,0x10(%ebp)
80102180:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102183:	01 45 0c             	add    %eax,0xc(%ebp)
80102186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102189:	3b 45 14             	cmp    0x14(%ebp),%eax
8010218c:	0f 82 54 ff ff ff    	jb     801020e6 <writei+0xb9>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102192:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
80102196:	74 1f                	je     801021b7 <writei+0x18a>
80102198:	8b 45 08             	mov    0x8(%ebp),%eax
8010219b:	8b 40 58             	mov    0x58(%eax),%eax
8010219e:	3b 45 10             	cmp    0x10(%ebp),%eax
801021a1:	73 14                	jae    801021b7 <writei+0x18a>
    ip->size = off;
801021a3:	8b 45 08             	mov    0x8(%ebp),%eax
801021a6:	8b 55 10             	mov    0x10(%ebp),%edx
801021a9:	89 50 58             	mov    %edx,0x58(%eax)
    iupdate(ip);
801021ac:	8b 45 08             	mov    0x8(%ebp),%eax
801021af:	89 04 24             	mov    %eax,(%esp)
801021b2:	e8 b8 f6 ff ff       	call   8010186f <iupdate>
  }
  return n;
801021b7:	8b 45 14             	mov    0x14(%ebp),%eax
}
801021ba:	c9                   	leave  
801021bb:	c3                   	ret    

801021bc <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
801021bc:	55                   	push   %ebp
801021bd:	89 e5                	mov    %esp,%ebp
801021bf:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
801021c2:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021c9:	00 
801021ca:	8b 45 0c             	mov    0xc(%ebp),%eax
801021cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801021d1:	8b 45 08             	mov    0x8(%ebp),%eax
801021d4:	89 04 24             	mov    %eax,(%esp)
801021d7:	e8 22 30 00 00       	call   801051fe <strncmp>
}
801021dc:	c9                   	leave  
801021dd:	c3                   	ret    

801021de <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
801021de:	55                   	push   %ebp
801021df:	89 e5                	mov    %esp,%ebp
801021e1:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
801021e4:	8b 45 08             	mov    0x8(%ebp),%eax
801021e7:	8b 40 50             	mov    0x50(%eax),%eax
801021ea:	66 83 f8 01          	cmp    $0x1,%ax
801021ee:	74 0c                	je     801021fc <dirlookup+0x1e>
    panic("dirlookup not DIR");
801021f0:	c7 04 24 9d 83 10 80 	movl   $0x8010839d,(%esp)
801021f7:	e8 58 e3 ff ff       	call   80100554 <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
801021fc:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102203:	e9 86 00 00 00       	jmp    8010228e <dirlookup+0xb0>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102208:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010220f:	00 
80102210:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102213:	89 44 24 08          	mov    %eax,0x8(%esp)
80102217:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010221a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010221e:	8b 45 08             	mov    0x8(%ebp),%eax
80102221:	89 04 24             	mov    %eax,(%esp)
80102224:	e8 a0 fc ff ff       	call   80101ec9 <readi>
80102229:	83 f8 10             	cmp    $0x10,%eax
8010222c:	74 0c                	je     8010223a <dirlookup+0x5c>
      panic("dirlookup read");
8010222e:	c7 04 24 af 83 10 80 	movl   $0x801083af,(%esp)
80102235:	e8 1a e3 ff ff       	call   80100554 <panic>
    if(de.inum == 0)
8010223a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010223d:	66 85 c0             	test   %ax,%ax
80102240:	75 02                	jne    80102244 <dirlookup+0x66>
      continue;
80102242:	eb 46                	jmp    8010228a <dirlookup+0xac>
    if(namecmp(name, de.name) == 0){
80102244:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102247:	83 c0 02             	add    $0x2,%eax
8010224a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010224e:	8b 45 0c             	mov    0xc(%ebp),%eax
80102251:	89 04 24             	mov    %eax,(%esp)
80102254:	e8 63 ff ff ff       	call   801021bc <namecmp>
80102259:	85 c0                	test   %eax,%eax
8010225b:	75 2d                	jne    8010228a <dirlookup+0xac>
      // entry matches path element
      if(poff)
8010225d:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80102261:	74 08                	je     8010226b <dirlookup+0x8d>
        *poff = off;
80102263:	8b 45 10             	mov    0x10(%ebp),%eax
80102266:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102269:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
8010226b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010226e:	0f b7 c0             	movzwl %ax,%eax
80102271:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102274:	8b 45 08             	mov    0x8(%ebp),%eax
80102277:	8b 00                	mov    (%eax),%eax
80102279:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010227c:	89 54 24 04          	mov    %edx,0x4(%esp)
80102280:	89 04 24             	mov    %eax,(%esp)
80102283:	e8 a3 f6 ff ff       	call   8010192b <iget>
80102288:	eb 18                	jmp    801022a2 <dirlookup+0xc4>
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
8010228a:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
8010228e:	8b 45 08             	mov    0x8(%ebp),%eax
80102291:	8b 40 58             	mov    0x58(%eax),%eax
80102294:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102297:	0f 87 6b ff ff ff    	ja     80102208 <dirlookup+0x2a>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
8010229d:	b8 00 00 00 00       	mov    $0x0,%eax
}
801022a2:	c9                   	leave  
801022a3:	c3                   	ret    

801022a4 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
801022a4:	55                   	push   %ebp
801022a5:	89 e5                	mov    %esp,%ebp
801022a7:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
801022aa:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801022b1:	00 
801022b2:	8b 45 0c             	mov    0xc(%ebp),%eax
801022b5:	89 44 24 04          	mov    %eax,0x4(%esp)
801022b9:	8b 45 08             	mov    0x8(%ebp),%eax
801022bc:	89 04 24             	mov    %eax,(%esp)
801022bf:	e8 1a ff ff ff       	call   801021de <dirlookup>
801022c4:	89 45 f0             	mov    %eax,-0x10(%ebp)
801022c7:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801022cb:	74 15                	je     801022e2 <dirlink+0x3e>
    iput(ip);
801022cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022d0:	89 04 24             	mov    %eax,(%esp)
801022d3:	e8 a8 f8 ff ff       	call   80101b80 <iput>
    return -1;
801022d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801022dd:	e9 b6 00 00 00       	jmp    80102398 <dirlink+0xf4>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801022e2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801022e9:	eb 45                	jmp    80102330 <dirlink+0x8c>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801022eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022ee:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801022f5:	00 
801022f6:	89 44 24 08          	mov    %eax,0x8(%esp)
801022fa:	8d 45 e0             	lea    -0x20(%ebp),%eax
801022fd:	89 44 24 04          	mov    %eax,0x4(%esp)
80102301:	8b 45 08             	mov    0x8(%ebp),%eax
80102304:	89 04 24             	mov    %eax,(%esp)
80102307:	e8 bd fb ff ff       	call   80101ec9 <readi>
8010230c:	83 f8 10             	cmp    $0x10,%eax
8010230f:	74 0c                	je     8010231d <dirlink+0x79>
      panic("dirlink read");
80102311:	c7 04 24 be 83 10 80 	movl   $0x801083be,(%esp)
80102318:	e8 37 e2 ff ff       	call   80100554 <panic>
    if(de.inum == 0)
8010231d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80102320:	66 85 c0             	test   %ax,%ax
80102323:	75 02                	jne    80102327 <dirlink+0x83>
      break;
80102325:	eb 16                	jmp    8010233d <dirlink+0x99>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
80102327:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010232a:	83 c0 10             	add    $0x10,%eax
8010232d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102330:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102333:	8b 45 08             	mov    0x8(%ebp),%eax
80102336:	8b 40 58             	mov    0x58(%eax),%eax
80102339:	39 c2                	cmp    %eax,%edx
8010233b:	72 ae                	jb     801022eb <dirlink+0x47>
      panic("dirlink read");
    if(de.inum == 0)
      break;
  }

  strncpy(de.name, name, DIRSIZ);
8010233d:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
80102344:	00 
80102345:	8b 45 0c             	mov    0xc(%ebp),%eax
80102348:	89 44 24 04          	mov    %eax,0x4(%esp)
8010234c:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010234f:	83 c0 02             	add    $0x2,%eax
80102352:	89 04 24             	mov    %eax,(%esp)
80102355:	e8 f2 2e 00 00       	call   8010524c <strncpy>
  de.inum = inum;
8010235a:	8b 45 10             	mov    0x10(%ebp),%eax
8010235d:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102361:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102364:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010236b:	00 
8010236c:	89 44 24 08          	mov    %eax,0x8(%esp)
80102370:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102373:	89 44 24 04          	mov    %eax,0x4(%esp)
80102377:	8b 45 08             	mov    0x8(%ebp),%eax
8010237a:	89 04 24             	mov    %eax,(%esp)
8010237d:	e8 ab fc ff ff       	call   8010202d <writei>
80102382:	83 f8 10             	cmp    $0x10,%eax
80102385:	74 0c                	je     80102393 <dirlink+0xef>
    panic("dirlink");
80102387:	c7 04 24 cb 83 10 80 	movl   $0x801083cb,(%esp)
8010238e:	e8 c1 e1 ff ff       	call   80100554 <panic>

  return 0;
80102393:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102398:	c9                   	leave  
80102399:	c3                   	ret    

8010239a <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
8010239a:	55                   	push   %ebp
8010239b:	89 e5                	mov    %esp,%ebp
8010239d:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
801023a0:	eb 03                	jmp    801023a5 <skipelem+0xb>
    path++;
801023a2:	ff 45 08             	incl   0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
801023a5:	8b 45 08             	mov    0x8(%ebp),%eax
801023a8:	8a 00                	mov    (%eax),%al
801023aa:	3c 2f                	cmp    $0x2f,%al
801023ac:	74 f4                	je     801023a2 <skipelem+0x8>
    path++;
  if(*path == 0)
801023ae:	8b 45 08             	mov    0x8(%ebp),%eax
801023b1:	8a 00                	mov    (%eax),%al
801023b3:	84 c0                	test   %al,%al
801023b5:	75 0a                	jne    801023c1 <skipelem+0x27>
    return 0;
801023b7:	b8 00 00 00 00       	mov    $0x0,%eax
801023bc:	e9 81 00 00 00       	jmp    80102442 <skipelem+0xa8>
  s = path;
801023c1:	8b 45 08             	mov    0x8(%ebp),%eax
801023c4:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
801023c7:	eb 03                	jmp    801023cc <skipelem+0x32>
    path++;
801023c9:	ff 45 08             	incl   0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
801023cc:	8b 45 08             	mov    0x8(%ebp),%eax
801023cf:	8a 00                	mov    (%eax),%al
801023d1:	3c 2f                	cmp    $0x2f,%al
801023d3:	74 09                	je     801023de <skipelem+0x44>
801023d5:	8b 45 08             	mov    0x8(%ebp),%eax
801023d8:	8a 00                	mov    (%eax),%al
801023da:	84 c0                	test   %al,%al
801023dc:	75 eb                	jne    801023c9 <skipelem+0x2f>
    path++;
  len = path - s;
801023de:	8b 55 08             	mov    0x8(%ebp),%edx
801023e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023e4:	29 c2                	sub    %eax,%edx
801023e6:	89 d0                	mov    %edx,%eax
801023e8:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
801023eb:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
801023ef:	7e 1c                	jle    8010240d <skipelem+0x73>
    memmove(name, s, DIRSIZ);
801023f1:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801023f8:	00 
801023f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023fc:	89 44 24 04          	mov    %eax,0x4(%esp)
80102400:	8b 45 0c             	mov    0xc(%ebp),%eax
80102403:	89 04 24             	mov    %eax,(%esp)
80102406:	e8 54 2d 00 00       	call   8010515f <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
8010240b:	eb 29                	jmp    80102436 <skipelem+0x9c>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
8010240d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102410:	89 44 24 08          	mov    %eax,0x8(%esp)
80102414:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102417:	89 44 24 04          	mov    %eax,0x4(%esp)
8010241b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010241e:	89 04 24             	mov    %eax,(%esp)
80102421:	e8 39 2d 00 00       	call   8010515f <memmove>
    name[len] = 0;
80102426:	8b 55 f0             	mov    -0x10(%ebp),%edx
80102429:	8b 45 0c             	mov    0xc(%ebp),%eax
8010242c:	01 d0                	add    %edx,%eax
8010242e:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
80102431:	eb 03                	jmp    80102436 <skipelem+0x9c>
    path++;
80102433:	ff 45 08             	incl   0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
80102436:	8b 45 08             	mov    0x8(%ebp),%eax
80102439:	8a 00                	mov    (%eax),%al
8010243b:	3c 2f                	cmp    $0x2f,%al
8010243d:	74 f4                	je     80102433 <skipelem+0x99>
    path++;
  return path;
8010243f:	8b 45 08             	mov    0x8(%ebp),%eax
}
80102442:	c9                   	leave  
80102443:	c3                   	ret    

80102444 <namex>:
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
// Must be called inside a transaction since it calls iput().
static struct inode*
namex(char *path, int nameiparent, char *name)
{
80102444:	55                   	push   %ebp
80102445:	89 e5                	mov    %esp,%ebp
80102447:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;

  if(*path == '/')
8010244a:	8b 45 08             	mov    0x8(%ebp),%eax
8010244d:	8a 00                	mov    (%eax),%al
8010244f:	3c 2f                	cmp    $0x2f,%al
80102451:	75 1c                	jne    8010246f <namex+0x2b>
    ip = iget(ROOTDEV, ROOTINO);
80102453:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010245a:	00 
8010245b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102462:	e8 c4 f4 ff ff       	call   8010192b <iget>
80102467:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(myproc()->cwd);

  while((path = skipelem(path, name)) != 0){
8010246a:	e9 ac 00 00 00       	jmp    8010251b <namex+0xd7>
  struct inode *ip, *next;

  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);
8010246f:	e8 af 1d 00 00       	call   80104223 <myproc>
80102474:	8b 40 68             	mov    0x68(%eax),%eax
80102477:	89 04 24             	mov    %eax,(%esp)
8010247a:	e8 81 f5 ff ff       	call   80101a00 <idup>
8010247f:	89 45 f4             	mov    %eax,-0xc(%ebp)

  while((path = skipelem(path, name)) != 0){
80102482:	e9 94 00 00 00       	jmp    8010251b <namex+0xd7>
    ilock(ip);
80102487:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010248a:	89 04 24             	mov    %eax,(%esp)
8010248d:	e8 a0 f5 ff ff       	call   80101a32 <ilock>
    if(ip->type != T_DIR){
80102492:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102495:	8b 40 50             	mov    0x50(%eax),%eax
80102498:	66 83 f8 01          	cmp    $0x1,%ax
8010249c:	74 15                	je     801024b3 <namex+0x6f>
      iunlockput(ip);
8010249e:	8b 45 f4             	mov    -0xc(%ebp),%eax
801024a1:	89 04 24             	mov    %eax,(%esp)
801024a4:	e8 88 f7 ff ff       	call   80101c31 <iunlockput>
      return 0;
801024a9:	b8 00 00 00 00       	mov    $0x0,%eax
801024ae:	e9 a2 00 00 00       	jmp    80102555 <namex+0x111>
    }
    if(nameiparent && *path == '\0'){
801024b3:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801024b7:	74 1c                	je     801024d5 <namex+0x91>
801024b9:	8b 45 08             	mov    0x8(%ebp),%eax
801024bc:	8a 00                	mov    (%eax),%al
801024be:	84 c0                	test   %al,%al
801024c0:	75 13                	jne    801024d5 <namex+0x91>
      // Stop one level early.
      iunlock(ip);
801024c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801024c5:	89 04 24             	mov    %eax,(%esp)
801024c8:	e8 6f f6 ff ff       	call   80101b3c <iunlock>
      return ip;
801024cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801024d0:	e9 80 00 00 00       	jmp    80102555 <namex+0x111>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
801024d5:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801024dc:	00 
801024dd:	8b 45 10             	mov    0x10(%ebp),%eax
801024e0:	89 44 24 04          	mov    %eax,0x4(%esp)
801024e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801024e7:	89 04 24             	mov    %eax,(%esp)
801024ea:	e8 ef fc ff ff       	call   801021de <dirlookup>
801024ef:	89 45 f0             	mov    %eax,-0x10(%ebp)
801024f2:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801024f6:	75 12                	jne    8010250a <namex+0xc6>
      iunlockput(ip);
801024f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801024fb:	89 04 24             	mov    %eax,(%esp)
801024fe:	e8 2e f7 ff ff       	call   80101c31 <iunlockput>
      return 0;
80102503:	b8 00 00 00 00       	mov    $0x0,%eax
80102508:	eb 4b                	jmp    80102555 <namex+0x111>
    }
    iunlockput(ip);
8010250a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010250d:	89 04 24             	mov    %eax,(%esp)
80102510:	e8 1c f7 ff ff       	call   80101c31 <iunlockput>
    ip = next;
80102515:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102518:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(myproc()->cwd);

  while((path = skipelem(path, name)) != 0){
8010251b:	8b 45 10             	mov    0x10(%ebp),%eax
8010251e:	89 44 24 04          	mov    %eax,0x4(%esp)
80102522:	8b 45 08             	mov    0x8(%ebp),%eax
80102525:	89 04 24             	mov    %eax,(%esp)
80102528:	e8 6d fe ff ff       	call   8010239a <skipelem>
8010252d:	89 45 08             	mov    %eax,0x8(%ebp)
80102530:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102534:	0f 85 4d ff ff ff    	jne    80102487 <namex+0x43>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
8010253a:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
8010253e:	74 12                	je     80102552 <namex+0x10e>
    iput(ip);
80102540:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102543:	89 04 24             	mov    %eax,(%esp)
80102546:	e8 35 f6 ff ff       	call   80101b80 <iput>
    return 0;
8010254b:	b8 00 00 00 00       	mov    $0x0,%eax
80102550:	eb 03                	jmp    80102555 <namex+0x111>
  }
  return ip;
80102552:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102555:	c9                   	leave  
80102556:	c3                   	ret    

80102557 <namei>:

struct inode*
namei(char *path)
{
80102557:	55                   	push   %ebp
80102558:	89 e5                	mov    %esp,%ebp
8010255a:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
8010255d:	8d 45 ea             	lea    -0x16(%ebp),%eax
80102560:	89 44 24 08          	mov    %eax,0x8(%esp)
80102564:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010256b:	00 
8010256c:	8b 45 08             	mov    0x8(%ebp),%eax
8010256f:	89 04 24             	mov    %eax,(%esp)
80102572:	e8 cd fe ff ff       	call   80102444 <namex>
}
80102577:	c9                   	leave  
80102578:	c3                   	ret    

80102579 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102579:	55                   	push   %ebp
8010257a:	89 e5                	mov    %esp,%ebp
8010257c:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
8010257f:	8b 45 0c             	mov    0xc(%ebp),%eax
80102582:	89 44 24 08          	mov    %eax,0x8(%esp)
80102586:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
8010258d:	00 
8010258e:	8b 45 08             	mov    0x8(%ebp),%eax
80102591:	89 04 24             	mov    %eax,(%esp)
80102594:	e8 ab fe ff ff       	call   80102444 <namex>
}
80102599:	c9                   	leave  
8010259a:	c3                   	ret    
	...

8010259c <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
8010259c:	55                   	push   %ebp
8010259d:	89 e5                	mov    %esp,%ebp
8010259f:	83 ec 14             	sub    $0x14,%esp
801025a2:	8b 45 08             	mov    0x8(%ebp),%eax
801025a5:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801025a9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801025ac:	89 c2                	mov    %eax,%edx
801025ae:	ec                   	in     (%dx),%al
801025af:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801025b2:	8a 45 ff             	mov    -0x1(%ebp),%al
}
801025b5:	c9                   	leave  
801025b6:	c3                   	ret    

801025b7 <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
801025b7:	55                   	push   %ebp
801025b8:	89 e5                	mov    %esp,%ebp
801025ba:	57                   	push   %edi
801025bb:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
801025bc:	8b 55 08             	mov    0x8(%ebp),%edx
801025bf:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801025c2:	8b 45 10             	mov    0x10(%ebp),%eax
801025c5:	89 cb                	mov    %ecx,%ebx
801025c7:	89 df                	mov    %ebx,%edi
801025c9:	89 c1                	mov    %eax,%ecx
801025cb:	fc                   	cld    
801025cc:	f3 6d                	rep insl (%dx),%es:(%edi)
801025ce:	89 c8                	mov    %ecx,%eax
801025d0:	89 fb                	mov    %edi,%ebx
801025d2:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801025d5:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
801025d8:	5b                   	pop    %ebx
801025d9:	5f                   	pop    %edi
801025da:	5d                   	pop    %ebp
801025db:	c3                   	ret    

801025dc <outb>:

static inline void
outb(ushort port, uchar data)
{
801025dc:	55                   	push   %ebp
801025dd:	89 e5                	mov    %esp,%ebp
801025df:	83 ec 08             	sub    $0x8,%esp
801025e2:	8b 45 08             	mov    0x8(%ebp),%eax
801025e5:	8b 55 0c             	mov    0xc(%ebp),%edx
801025e8:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
801025ec:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801025ef:	8a 45 f8             	mov    -0x8(%ebp),%al
801025f2:	8b 55 fc             	mov    -0x4(%ebp),%edx
801025f5:	ee                   	out    %al,(%dx)
}
801025f6:	c9                   	leave  
801025f7:	c3                   	ret    

801025f8 <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801025f8:	55                   	push   %ebp
801025f9:	89 e5                	mov    %esp,%ebp
801025fb:	56                   	push   %esi
801025fc:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801025fd:	8b 55 08             	mov    0x8(%ebp),%edx
80102600:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80102603:	8b 45 10             	mov    0x10(%ebp),%eax
80102606:	89 cb                	mov    %ecx,%ebx
80102608:	89 de                	mov    %ebx,%esi
8010260a:	89 c1                	mov    %eax,%ecx
8010260c:	fc                   	cld    
8010260d:	f3 6f                	rep outsl %ds:(%esi),(%dx)
8010260f:	89 c8                	mov    %ecx,%eax
80102611:	89 f3                	mov    %esi,%ebx
80102613:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102616:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
80102619:	5b                   	pop    %ebx
8010261a:	5e                   	pop    %esi
8010261b:	5d                   	pop    %ebp
8010261c:	c3                   	ret    

8010261d <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
8010261d:	55                   	push   %ebp
8010261e:	89 e5                	mov    %esp,%ebp
80102620:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY)
80102623:	90                   	nop
80102624:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
8010262b:	e8 6c ff ff ff       	call   8010259c <inb>
80102630:	0f b6 c0             	movzbl %al,%eax
80102633:	89 45 fc             	mov    %eax,-0x4(%ebp)
80102636:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102639:	25 c0 00 00 00       	and    $0xc0,%eax
8010263e:	83 f8 40             	cmp    $0x40,%eax
80102641:	75 e1                	jne    80102624 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
80102643:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102647:	74 11                	je     8010265a <idewait+0x3d>
80102649:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010264c:	83 e0 21             	and    $0x21,%eax
8010264f:	85 c0                	test   %eax,%eax
80102651:	74 07                	je     8010265a <idewait+0x3d>
    return -1;
80102653:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102658:	eb 05                	jmp    8010265f <idewait+0x42>
  return 0;
8010265a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010265f:	c9                   	leave  
80102660:	c3                   	ret    

80102661 <ideinit>:

void
ideinit(void)
{
80102661:	55                   	push   %ebp
80102662:	89 e5                	mov    %esp,%ebp
80102664:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102667:	c7 44 24 04 d3 83 10 	movl   $0x801083d3,0x4(%esp)
8010266e:	80 
8010266f:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80102676:	e8 97 27 00 00       	call   80104e12 <initlock>
  ioapicenable(IRQ_IDE, ncpu - 1);
8010267b:	a1 40 40 11 80       	mov    0x80114040,%eax
80102680:	48                   	dec    %eax
80102681:	89 44 24 04          	mov    %eax,0x4(%esp)
80102685:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010268c:	e8 66 04 00 00       	call   80102af7 <ioapicenable>
  idewait(0);
80102691:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102698:	e8 80 ff ff ff       	call   8010261d <idewait>

  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
8010269d:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
801026a4:	00 
801026a5:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801026ac:	e8 2b ff ff ff       	call   801025dc <outb>
  for(i=0; i<1000; i++){
801026b1:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801026b8:	eb 1f                	jmp    801026d9 <ideinit+0x78>
    if(inb(0x1f7) != 0){
801026ba:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026c1:	e8 d6 fe ff ff       	call   8010259c <inb>
801026c6:	84 c0                	test   %al,%al
801026c8:	74 0c                	je     801026d6 <ideinit+0x75>
      havedisk1 = 1;
801026ca:	c7 05 18 b6 10 80 01 	movl   $0x1,0x8010b618
801026d1:	00 00 00 
      break;
801026d4:	eb 0c                	jmp    801026e2 <ideinit+0x81>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);

  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
801026d6:	ff 45 f4             	incl   -0xc(%ebp)
801026d9:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801026e0:	7e d8                	jle    801026ba <ideinit+0x59>
      break;
    }
  }

  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801026e2:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801026e9:	00 
801026ea:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801026f1:	e8 e6 fe ff ff       	call   801025dc <outb>
}
801026f6:	c9                   	leave  
801026f7:	c3                   	ret    

801026f8 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801026f8:	55                   	push   %ebp
801026f9:	89 e5                	mov    %esp,%ebp
801026fb:	83 ec 28             	sub    $0x28,%esp
  if(b == 0)
801026fe:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102702:	75 0c                	jne    80102710 <idestart+0x18>
    panic("idestart");
80102704:	c7 04 24 d7 83 10 80 	movl   $0x801083d7,(%esp)
8010270b:	e8 44 de ff ff       	call   80100554 <panic>
  if(b->blockno >= FSSIZE)
80102710:	8b 45 08             	mov    0x8(%ebp),%eax
80102713:	8b 40 08             	mov    0x8(%eax),%eax
80102716:	3d e7 03 00 00       	cmp    $0x3e7,%eax
8010271b:	76 0c                	jbe    80102729 <idestart+0x31>
    panic("incorrect blockno");
8010271d:	c7 04 24 e0 83 10 80 	movl   $0x801083e0,(%esp)
80102724:	e8 2b de ff ff       	call   80100554 <panic>
  int sector_per_block =  BSIZE/SECTOR_SIZE;
80102729:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
  int sector = b->blockno * sector_per_block;
80102730:	8b 45 08             	mov    0x8(%ebp),%eax
80102733:	8b 50 08             	mov    0x8(%eax),%edx
80102736:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102739:	0f af c2             	imul   %edx,%eax
8010273c:	89 45 f0             	mov    %eax,-0x10(%ebp)
  int read_cmd = (sector_per_block == 1) ? IDE_CMD_READ :  IDE_CMD_RDMUL;
8010273f:	83 7d f4 01          	cmpl   $0x1,-0xc(%ebp)
80102743:	75 07                	jne    8010274c <idestart+0x54>
80102745:	b8 20 00 00 00       	mov    $0x20,%eax
8010274a:	eb 05                	jmp    80102751 <idestart+0x59>
8010274c:	b8 c4 00 00 00       	mov    $0xc4,%eax
80102751:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int write_cmd = (sector_per_block == 1) ? IDE_CMD_WRITE : IDE_CMD_WRMUL;
80102754:	83 7d f4 01          	cmpl   $0x1,-0xc(%ebp)
80102758:	75 07                	jne    80102761 <idestart+0x69>
8010275a:	b8 30 00 00 00       	mov    $0x30,%eax
8010275f:	eb 05                	jmp    80102766 <idestart+0x6e>
80102761:	b8 c5 00 00 00       	mov    $0xc5,%eax
80102766:	89 45 e8             	mov    %eax,-0x18(%ebp)

  if (sector_per_block > 7) panic("idestart");
80102769:	83 7d f4 07          	cmpl   $0x7,-0xc(%ebp)
8010276d:	7e 0c                	jle    8010277b <idestart+0x83>
8010276f:	c7 04 24 d7 83 10 80 	movl   $0x801083d7,(%esp)
80102776:	e8 d9 dd ff ff       	call   80100554 <panic>

  idewait(0);
8010277b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102782:	e8 96 fe ff ff       	call   8010261d <idewait>
  outb(0x3f6, 0);  // generate interrupt
80102787:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010278e:	00 
8010278f:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
80102796:	e8 41 fe ff ff       	call   801025dc <outb>
  outb(0x1f2, sector_per_block);  // number of sectors
8010279b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010279e:	0f b6 c0             	movzbl %al,%eax
801027a1:	89 44 24 04          	mov    %eax,0x4(%esp)
801027a5:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
801027ac:	e8 2b fe ff ff       	call   801025dc <outb>
  outb(0x1f3, sector & 0xff);
801027b1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801027b4:	0f b6 c0             	movzbl %al,%eax
801027b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801027bb:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
801027c2:	e8 15 fe ff ff       	call   801025dc <outb>
  outb(0x1f4, (sector >> 8) & 0xff);
801027c7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801027ca:	c1 f8 08             	sar    $0x8,%eax
801027cd:	0f b6 c0             	movzbl %al,%eax
801027d0:	89 44 24 04          	mov    %eax,0x4(%esp)
801027d4:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
801027db:	e8 fc fd ff ff       	call   801025dc <outb>
  outb(0x1f5, (sector >> 16) & 0xff);
801027e0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801027e3:	c1 f8 10             	sar    $0x10,%eax
801027e6:	0f b6 c0             	movzbl %al,%eax
801027e9:	89 44 24 04          	mov    %eax,0x4(%esp)
801027ed:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
801027f4:	e8 e3 fd ff ff       	call   801025dc <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((sector>>24)&0x0f));
801027f9:	8b 45 08             	mov    0x8(%ebp),%eax
801027fc:	8b 40 04             	mov    0x4(%eax),%eax
801027ff:	83 e0 01             	and    $0x1,%eax
80102802:	c1 e0 04             	shl    $0x4,%eax
80102805:	88 c2                	mov    %al,%dl
80102807:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010280a:	c1 f8 18             	sar    $0x18,%eax
8010280d:	83 e0 0f             	and    $0xf,%eax
80102810:	09 d0                	or     %edx,%eax
80102812:	83 c8 e0             	or     $0xffffffe0,%eax
80102815:	0f b6 c0             	movzbl %al,%eax
80102818:	89 44 24 04          	mov    %eax,0x4(%esp)
8010281c:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102823:	e8 b4 fd ff ff       	call   801025dc <outb>
  if(b->flags & B_DIRTY){
80102828:	8b 45 08             	mov    0x8(%ebp),%eax
8010282b:	8b 00                	mov    (%eax),%eax
8010282d:	83 e0 04             	and    $0x4,%eax
80102830:	85 c0                	test   %eax,%eax
80102832:	74 36                	je     8010286a <idestart+0x172>
    outb(0x1f7, write_cmd);
80102834:	8b 45 e8             	mov    -0x18(%ebp),%eax
80102837:	0f b6 c0             	movzbl %al,%eax
8010283a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010283e:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
80102845:	e8 92 fd ff ff       	call   801025dc <outb>
    outsl(0x1f0, b->data, BSIZE/4);
8010284a:	8b 45 08             	mov    0x8(%ebp),%eax
8010284d:	83 c0 5c             	add    $0x5c,%eax
80102850:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
80102857:	00 
80102858:	89 44 24 04          	mov    %eax,0x4(%esp)
8010285c:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
80102863:	e8 90 fd ff ff       	call   801025f8 <outsl>
80102868:	eb 16                	jmp    80102880 <idestart+0x188>
  } else {
    outb(0x1f7, read_cmd);
8010286a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010286d:	0f b6 c0             	movzbl %al,%eax
80102870:	89 44 24 04          	mov    %eax,0x4(%esp)
80102874:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
8010287b:	e8 5c fd ff ff       	call   801025dc <outb>
  }
}
80102880:	c9                   	leave  
80102881:	c3                   	ret    

80102882 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
80102882:	55                   	push   %ebp
80102883:	89 e5                	mov    %esp,%ebp
80102885:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
80102888:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
8010288f:	e8 9f 25 00 00       	call   80104e33 <acquire>

  if((b = idequeue) == 0){
80102894:	a1 14 b6 10 80       	mov    0x8010b614,%eax
80102899:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010289c:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801028a0:	75 11                	jne    801028b3 <ideintr+0x31>
    release(&idelock);
801028a2:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
801028a9:	e8 ef 25 00 00       	call   80104e9d <release>
    return;
801028ae:	e9 90 00 00 00       	jmp    80102943 <ideintr+0xc1>
  }
  idequeue = b->qnext;
801028b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801028b6:	8b 40 58             	mov    0x58(%eax),%eax
801028b9:	a3 14 b6 10 80       	mov    %eax,0x8010b614

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
801028be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801028c1:	8b 00                	mov    (%eax),%eax
801028c3:	83 e0 04             	and    $0x4,%eax
801028c6:	85 c0                	test   %eax,%eax
801028c8:	75 2e                	jne    801028f8 <ideintr+0x76>
801028ca:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028d1:	e8 47 fd ff ff       	call   8010261d <idewait>
801028d6:	85 c0                	test   %eax,%eax
801028d8:	78 1e                	js     801028f8 <ideintr+0x76>
    insl(0x1f0, b->data, BSIZE/4);
801028da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801028dd:	83 c0 5c             	add    $0x5c,%eax
801028e0:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801028e7:	00 
801028e8:	89 44 24 04          	mov    %eax,0x4(%esp)
801028ec:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801028f3:	e8 bf fc ff ff       	call   801025b7 <insl>

  // Wake process waiting for this buf.
  b->flags |= B_VALID;
801028f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801028fb:	8b 00                	mov    (%eax),%eax
801028fd:	83 c8 02             	or     $0x2,%eax
80102900:	89 c2                	mov    %eax,%edx
80102902:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102905:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
80102907:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010290a:	8b 00                	mov    (%eax),%eax
8010290c:	83 e0 fb             	and    $0xfffffffb,%eax
8010290f:	89 c2                	mov    %eax,%edx
80102911:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102914:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80102916:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102919:	89 04 24             	mov    %eax,(%esp)
8010291c:	e8 17 22 00 00       	call   80104b38 <wakeup>

  // Start disk on next buf in queue.
  if(idequeue != 0)
80102921:	a1 14 b6 10 80       	mov    0x8010b614,%eax
80102926:	85 c0                	test   %eax,%eax
80102928:	74 0d                	je     80102937 <ideintr+0xb5>
    idestart(idequeue);
8010292a:	a1 14 b6 10 80       	mov    0x8010b614,%eax
8010292f:	89 04 24             	mov    %eax,(%esp)
80102932:	e8 c1 fd ff ff       	call   801026f8 <idestart>

  release(&idelock);
80102937:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
8010293e:	e8 5a 25 00 00       	call   80104e9d <release>
}
80102943:	c9                   	leave  
80102944:	c3                   	ret    

80102945 <iderw>:
// Sync buf with disk.
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
80102945:	55                   	push   %ebp
80102946:	89 e5                	mov    %esp,%ebp
80102948:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!holdingsleep(&b->lock))
8010294b:	8b 45 08             	mov    0x8(%ebp),%eax
8010294e:	83 c0 0c             	add    $0xc,%eax
80102951:	89 04 24             	mov    %eax,(%esp)
80102954:	e8 52 24 00 00       	call   80104dab <holdingsleep>
80102959:	85 c0                	test   %eax,%eax
8010295b:	75 0c                	jne    80102969 <iderw+0x24>
    panic("iderw: buf not locked");
8010295d:	c7 04 24 f2 83 10 80 	movl   $0x801083f2,(%esp)
80102964:	e8 eb db ff ff       	call   80100554 <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
80102969:	8b 45 08             	mov    0x8(%ebp),%eax
8010296c:	8b 00                	mov    (%eax),%eax
8010296e:	83 e0 06             	and    $0x6,%eax
80102971:	83 f8 02             	cmp    $0x2,%eax
80102974:	75 0c                	jne    80102982 <iderw+0x3d>
    panic("iderw: nothing to do");
80102976:	c7 04 24 08 84 10 80 	movl   $0x80108408,(%esp)
8010297d:	e8 d2 db ff ff       	call   80100554 <panic>
  if(b->dev != 0 && !havedisk1)
80102982:	8b 45 08             	mov    0x8(%ebp),%eax
80102985:	8b 40 04             	mov    0x4(%eax),%eax
80102988:	85 c0                	test   %eax,%eax
8010298a:	74 15                	je     801029a1 <iderw+0x5c>
8010298c:	a1 18 b6 10 80       	mov    0x8010b618,%eax
80102991:	85 c0                	test   %eax,%eax
80102993:	75 0c                	jne    801029a1 <iderw+0x5c>
    panic("iderw: ide disk 1 not present");
80102995:	c7 04 24 1d 84 10 80 	movl   $0x8010841d,(%esp)
8010299c:	e8 b3 db ff ff       	call   80100554 <panic>

  acquire(&idelock);  //DOC:acquire-lock
801029a1:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
801029a8:	e8 86 24 00 00       	call   80104e33 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
801029ad:	8b 45 08             	mov    0x8(%ebp),%eax
801029b0:	c7 40 58 00 00 00 00 	movl   $0x0,0x58(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
801029b7:	c7 45 f4 14 b6 10 80 	movl   $0x8010b614,-0xc(%ebp)
801029be:	eb 0b                	jmp    801029cb <iderw+0x86>
801029c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801029c3:	8b 00                	mov    (%eax),%eax
801029c5:	83 c0 58             	add    $0x58,%eax
801029c8:	89 45 f4             	mov    %eax,-0xc(%ebp)
801029cb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801029ce:	8b 00                	mov    (%eax),%eax
801029d0:	85 c0                	test   %eax,%eax
801029d2:	75 ec                	jne    801029c0 <iderw+0x7b>
    ;
  *pp = b;
801029d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801029d7:	8b 55 08             	mov    0x8(%ebp),%edx
801029da:	89 10                	mov    %edx,(%eax)

  // Start disk if necessary.
  if(idequeue == b)
801029dc:	a1 14 b6 10 80       	mov    0x8010b614,%eax
801029e1:	3b 45 08             	cmp    0x8(%ebp),%eax
801029e4:	75 0d                	jne    801029f3 <iderw+0xae>
    idestart(b);
801029e6:	8b 45 08             	mov    0x8(%ebp),%eax
801029e9:	89 04 24             	mov    %eax,(%esp)
801029ec:	e8 07 fd ff ff       	call   801026f8 <idestart>

  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
801029f1:	eb 15                	jmp    80102a08 <iderw+0xc3>
801029f3:	eb 13                	jmp    80102a08 <iderw+0xc3>
    sleep(b, &idelock);
801029f5:	c7 44 24 04 e0 b5 10 	movl   $0x8010b5e0,0x4(%esp)
801029fc:	80 
801029fd:	8b 45 08             	mov    0x8(%ebp),%eax
80102a00:	89 04 24             	mov    %eax,(%esp)
80102a03:	e8 5c 20 00 00       	call   80104a64 <sleep>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);

  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102a08:	8b 45 08             	mov    0x8(%ebp),%eax
80102a0b:	8b 00                	mov    (%eax),%eax
80102a0d:	83 e0 06             	and    $0x6,%eax
80102a10:	83 f8 02             	cmp    $0x2,%eax
80102a13:	75 e0                	jne    801029f5 <iderw+0xb0>
    sleep(b, &idelock);
  }


  release(&idelock);
80102a15:	c7 04 24 e0 b5 10 80 	movl   $0x8010b5e0,(%esp)
80102a1c:	e8 7c 24 00 00       	call   80104e9d <release>
}
80102a21:	c9                   	leave  
80102a22:	c3                   	ret    
	...

80102a24 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102a24:	55                   	push   %ebp
80102a25:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102a27:	a1 74 39 11 80       	mov    0x80113974,%eax
80102a2c:	8b 55 08             	mov    0x8(%ebp),%edx
80102a2f:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102a31:	a1 74 39 11 80       	mov    0x80113974,%eax
80102a36:	8b 40 10             	mov    0x10(%eax),%eax
}
80102a39:	5d                   	pop    %ebp
80102a3a:	c3                   	ret    

80102a3b <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
80102a3b:	55                   	push   %ebp
80102a3c:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
80102a3e:	a1 74 39 11 80       	mov    0x80113974,%eax
80102a43:	8b 55 08             	mov    0x8(%ebp),%edx
80102a46:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
80102a48:	a1 74 39 11 80       	mov    0x80113974,%eax
80102a4d:	8b 55 0c             	mov    0xc(%ebp),%edx
80102a50:	89 50 10             	mov    %edx,0x10(%eax)
}
80102a53:	5d                   	pop    %ebp
80102a54:	c3                   	ret    

80102a55 <ioapicinit>:

void
ioapicinit(void)
{
80102a55:	55                   	push   %ebp
80102a56:	89 e5                	mov    %esp,%ebp
80102a58:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  ioapic = (volatile struct ioapic*)IOAPIC;
80102a5b:	c7 05 74 39 11 80 00 	movl   $0xfec00000,0x80113974
80102a62:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
80102a65:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102a6c:	e8 b3 ff ff ff       	call   80102a24 <ioapicread>
80102a71:	c1 e8 10             	shr    $0x10,%eax
80102a74:	25 ff 00 00 00       	and    $0xff,%eax
80102a79:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
80102a7c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102a83:	e8 9c ff ff ff       	call   80102a24 <ioapicread>
80102a88:	c1 e8 18             	shr    $0x18,%eax
80102a8b:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
80102a8e:	a0 a0 3a 11 80       	mov    0x80113aa0,%al
80102a93:	0f b6 c0             	movzbl %al,%eax
80102a96:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80102a99:	74 0c                	je     80102aa7 <ioapicinit+0x52>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
80102a9b:	c7 04 24 3c 84 10 80 	movl   $0x8010843c,(%esp)
80102aa2:	e8 1a d9 ff ff       	call   801003c1 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102aa7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102aae:	eb 3d                	jmp    80102aed <ioapicinit+0x98>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102ab0:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ab3:	83 c0 20             	add    $0x20,%eax
80102ab6:	0d 00 00 01 00       	or     $0x10000,%eax
80102abb:	89 c2                	mov    %eax,%edx
80102abd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ac0:	83 c0 08             	add    $0x8,%eax
80102ac3:	01 c0                	add    %eax,%eax
80102ac5:	89 54 24 04          	mov    %edx,0x4(%esp)
80102ac9:	89 04 24             	mov    %eax,(%esp)
80102acc:	e8 6a ff ff ff       	call   80102a3b <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102ad1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ad4:	83 c0 08             	add    $0x8,%eax
80102ad7:	01 c0                	add    %eax,%eax
80102ad9:	40                   	inc    %eax
80102ada:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ae1:	00 
80102ae2:	89 04 24             	mov    %eax,(%esp)
80102ae5:	e8 51 ff ff ff       	call   80102a3b <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
80102aea:	ff 45 f4             	incl   -0xc(%ebp)
80102aed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102af0:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102af3:	7e bb                	jle    80102ab0 <ioapicinit+0x5b>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
80102af5:	c9                   	leave  
80102af6:	c3                   	ret    

80102af7 <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
80102af7:	55                   	push   %ebp
80102af8:	89 e5                	mov    %esp,%ebp
80102afa:	83 ec 08             	sub    $0x8,%esp
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
80102afd:	8b 45 08             	mov    0x8(%ebp),%eax
80102b00:	83 c0 20             	add    $0x20,%eax
80102b03:	89 c2                	mov    %eax,%edx
80102b05:	8b 45 08             	mov    0x8(%ebp),%eax
80102b08:	83 c0 08             	add    $0x8,%eax
80102b0b:	01 c0                	add    %eax,%eax
80102b0d:	89 54 24 04          	mov    %edx,0x4(%esp)
80102b11:	89 04 24             	mov    %eax,(%esp)
80102b14:	e8 22 ff ff ff       	call   80102a3b <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102b19:	8b 45 0c             	mov    0xc(%ebp),%eax
80102b1c:	c1 e0 18             	shl    $0x18,%eax
80102b1f:	8b 55 08             	mov    0x8(%ebp),%edx
80102b22:	83 c2 08             	add    $0x8,%edx
80102b25:	01 d2                	add    %edx,%edx
80102b27:	42                   	inc    %edx
80102b28:	89 44 24 04          	mov    %eax,0x4(%esp)
80102b2c:	89 14 24             	mov    %edx,(%esp)
80102b2f:	e8 07 ff ff ff       	call   80102a3b <ioapicwrite>
}
80102b34:	c9                   	leave  
80102b35:	c3                   	ret    
	...

80102b38 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
80102b38:	55                   	push   %ebp
80102b39:	89 e5                	mov    %esp,%ebp
80102b3b:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
80102b3e:	c7 44 24 04 6e 84 10 	movl   $0x8010846e,0x4(%esp)
80102b45:	80 
80102b46:	c7 04 24 80 39 11 80 	movl   $0x80113980,(%esp)
80102b4d:	e8 c0 22 00 00       	call   80104e12 <initlock>
  kmem.use_lock = 0;
80102b52:	c7 05 b4 39 11 80 00 	movl   $0x0,0x801139b4
80102b59:	00 00 00 
  freerange(vstart, vend);
80102b5c:	8b 45 0c             	mov    0xc(%ebp),%eax
80102b5f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102b63:	8b 45 08             	mov    0x8(%ebp),%eax
80102b66:	89 04 24             	mov    %eax,(%esp)
80102b69:	e8 26 00 00 00       	call   80102b94 <freerange>
}
80102b6e:	c9                   	leave  
80102b6f:	c3                   	ret    

80102b70 <kinit2>:

void
kinit2(void *vstart, void *vend)
{
80102b70:	55                   	push   %ebp
80102b71:	89 e5                	mov    %esp,%ebp
80102b73:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
80102b76:	8b 45 0c             	mov    0xc(%ebp),%eax
80102b79:	89 44 24 04          	mov    %eax,0x4(%esp)
80102b7d:	8b 45 08             	mov    0x8(%ebp),%eax
80102b80:	89 04 24             	mov    %eax,(%esp)
80102b83:	e8 0c 00 00 00       	call   80102b94 <freerange>
  kmem.use_lock = 1;
80102b88:	c7 05 b4 39 11 80 01 	movl   $0x1,0x801139b4
80102b8f:	00 00 00 
}
80102b92:	c9                   	leave  
80102b93:	c3                   	ret    

80102b94 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102b94:	55                   	push   %ebp
80102b95:	89 e5                	mov    %esp,%ebp
80102b97:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102b9a:	8b 45 08             	mov    0x8(%ebp),%eax
80102b9d:	05 ff 0f 00 00       	add    $0xfff,%eax
80102ba2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102ba7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102baa:	eb 12                	jmp    80102bbe <freerange+0x2a>
    kfree(p);
80102bac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102baf:	89 04 24             	mov    %eax,(%esp)
80102bb2:	e8 16 00 00 00       	call   80102bcd <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102bb7:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102bbe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102bc1:	05 00 10 00 00       	add    $0x1000,%eax
80102bc6:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102bc9:	76 e1                	jbe    80102bac <freerange+0x18>
    kfree(p);
}
80102bcb:	c9                   	leave  
80102bcc:	c3                   	ret    

80102bcd <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102bcd:	55                   	push   %ebp
80102bce:	89 e5                	mov    %esp,%ebp
80102bd0:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || V2P(v) >= PHYSTOP)
80102bd3:	8b 45 08             	mov    0x8(%ebp),%eax
80102bd6:	25 ff 0f 00 00       	and    $0xfff,%eax
80102bdb:	85 c0                	test   %eax,%eax
80102bdd:	75 18                	jne    80102bf7 <kfree+0x2a>
80102bdf:	81 7d 08 e8 67 11 80 	cmpl   $0x801167e8,0x8(%ebp)
80102be6:	72 0f                	jb     80102bf7 <kfree+0x2a>
80102be8:	8b 45 08             	mov    0x8(%ebp),%eax
80102beb:	05 00 00 00 80       	add    $0x80000000,%eax
80102bf0:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102bf5:	76 0c                	jbe    80102c03 <kfree+0x36>
    panic("kfree");
80102bf7:	c7 04 24 73 84 10 80 	movl   $0x80108473,(%esp)
80102bfe:	e8 51 d9 ff ff       	call   80100554 <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102c03:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102c0a:	00 
80102c0b:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102c12:	00 
80102c13:	8b 45 08             	mov    0x8(%ebp),%eax
80102c16:	89 04 24             	mov    %eax,(%esp)
80102c19:	e8 78 24 00 00       	call   80105096 <memset>

  if(kmem.use_lock)
80102c1e:	a1 b4 39 11 80       	mov    0x801139b4,%eax
80102c23:	85 c0                	test   %eax,%eax
80102c25:	74 0c                	je     80102c33 <kfree+0x66>
    acquire(&kmem.lock);
80102c27:	c7 04 24 80 39 11 80 	movl   $0x80113980,(%esp)
80102c2e:	e8 00 22 00 00       	call   80104e33 <acquire>
  r = (struct run*)v;
80102c33:	8b 45 08             	mov    0x8(%ebp),%eax
80102c36:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102c39:	8b 15 b8 39 11 80    	mov    0x801139b8,%edx
80102c3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102c42:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102c44:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102c47:	a3 b8 39 11 80       	mov    %eax,0x801139b8
  if(kmem.use_lock)
80102c4c:	a1 b4 39 11 80       	mov    0x801139b4,%eax
80102c51:	85 c0                	test   %eax,%eax
80102c53:	74 0c                	je     80102c61 <kfree+0x94>
    release(&kmem.lock);
80102c55:	c7 04 24 80 39 11 80 	movl   $0x80113980,(%esp)
80102c5c:	e8 3c 22 00 00       	call   80104e9d <release>
}
80102c61:	c9                   	leave  
80102c62:	c3                   	ret    

80102c63 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102c63:	55                   	push   %ebp
80102c64:	89 e5                	mov    %esp,%ebp
80102c66:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102c69:	a1 b4 39 11 80       	mov    0x801139b4,%eax
80102c6e:	85 c0                	test   %eax,%eax
80102c70:	74 0c                	je     80102c7e <kalloc+0x1b>
    acquire(&kmem.lock);
80102c72:	c7 04 24 80 39 11 80 	movl   $0x80113980,(%esp)
80102c79:	e8 b5 21 00 00       	call   80104e33 <acquire>
  r = kmem.freelist;
80102c7e:	a1 b8 39 11 80       	mov    0x801139b8,%eax
80102c83:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102c86:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102c8a:	74 0a                	je     80102c96 <kalloc+0x33>
    kmem.freelist = r->next;
80102c8c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102c8f:	8b 00                	mov    (%eax),%eax
80102c91:	a3 b8 39 11 80       	mov    %eax,0x801139b8
  if(kmem.use_lock)
80102c96:	a1 b4 39 11 80       	mov    0x801139b4,%eax
80102c9b:	85 c0                	test   %eax,%eax
80102c9d:	74 0c                	je     80102cab <kalloc+0x48>
    release(&kmem.lock);
80102c9f:	c7 04 24 80 39 11 80 	movl   $0x80113980,(%esp)
80102ca6:	e8 f2 21 00 00       	call   80104e9d <release>
  return (char*)r;
80102cab:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102cae:	c9                   	leave  
80102caf:	c3                   	ret    

80102cb0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102cb0:	55                   	push   %ebp
80102cb1:	89 e5                	mov    %esp,%ebp
80102cb3:	83 ec 14             	sub    $0x14,%esp
80102cb6:	8b 45 08             	mov    0x8(%ebp),%eax
80102cb9:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102cbd:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102cc0:	89 c2                	mov    %eax,%edx
80102cc2:	ec                   	in     (%dx),%al
80102cc3:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102cc6:	8a 45 ff             	mov    -0x1(%ebp),%al
}
80102cc9:	c9                   	leave  
80102cca:	c3                   	ret    

80102ccb <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102ccb:	55                   	push   %ebp
80102ccc:	89 e5                	mov    %esp,%ebp
80102cce:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102cd1:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102cd8:	e8 d3 ff ff ff       	call   80102cb0 <inb>
80102cdd:	0f b6 c0             	movzbl %al,%eax
80102ce0:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102ce3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ce6:	83 e0 01             	and    $0x1,%eax
80102ce9:	85 c0                	test   %eax,%eax
80102ceb:	75 0a                	jne    80102cf7 <kbdgetc+0x2c>
    return -1;
80102ced:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102cf2:	e9 21 01 00 00       	jmp    80102e18 <kbdgetc+0x14d>
  data = inb(KBDATAP);
80102cf7:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102cfe:	e8 ad ff ff ff       	call   80102cb0 <inb>
80102d03:	0f b6 c0             	movzbl %al,%eax
80102d06:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102d09:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102d10:	75 17                	jne    80102d29 <kbdgetc+0x5e>
    shift |= E0ESC;
80102d12:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102d17:	83 c8 40             	or     $0x40,%eax
80102d1a:	a3 1c b6 10 80       	mov    %eax,0x8010b61c
    return 0;
80102d1f:	b8 00 00 00 00       	mov    $0x0,%eax
80102d24:	e9 ef 00 00 00       	jmp    80102e18 <kbdgetc+0x14d>
  } else if(data & 0x80){
80102d29:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102d2c:	25 80 00 00 00       	and    $0x80,%eax
80102d31:	85 c0                	test   %eax,%eax
80102d33:	74 44                	je     80102d79 <kbdgetc+0xae>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102d35:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102d3a:	83 e0 40             	and    $0x40,%eax
80102d3d:	85 c0                	test   %eax,%eax
80102d3f:	75 08                	jne    80102d49 <kbdgetc+0x7e>
80102d41:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102d44:	83 e0 7f             	and    $0x7f,%eax
80102d47:	eb 03                	jmp    80102d4c <kbdgetc+0x81>
80102d49:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102d4c:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102d4f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102d52:	05 20 90 10 80       	add    $0x80109020,%eax
80102d57:	8a 00                	mov    (%eax),%al
80102d59:	83 c8 40             	or     $0x40,%eax
80102d5c:	0f b6 c0             	movzbl %al,%eax
80102d5f:	f7 d0                	not    %eax
80102d61:	89 c2                	mov    %eax,%edx
80102d63:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102d68:	21 d0                	and    %edx,%eax
80102d6a:	a3 1c b6 10 80       	mov    %eax,0x8010b61c
    return 0;
80102d6f:	b8 00 00 00 00       	mov    $0x0,%eax
80102d74:	e9 9f 00 00 00       	jmp    80102e18 <kbdgetc+0x14d>
  } else if(shift & E0ESC){
80102d79:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102d7e:	83 e0 40             	and    $0x40,%eax
80102d81:	85 c0                	test   %eax,%eax
80102d83:	74 14                	je     80102d99 <kbdgetc+0xce>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102d85:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102d8c:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102d91:	83 e0 bf             	and    $0xffffffbf,%eax
80102d94:	a3 1c b6 10 80       	mov    %eax,0x8010b61c
  }

  shift |= shiftcode[data];
80102d99:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102d9c:	05 20 90 10 80       	add    $0x80109020,%eax
80102da1:	8a 00                	mov    (%eax),%al
80102da3:	0f b6 d0             	movzbl %al,%edx
80102da6:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102dab:	09 d0                	or     %edx,%eax
80102dad:	a3 1c b6 10 80       	mov    %eax,0x8010b61c
  shift ^= togglecode[data];
80102db2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102db5:	05 20 91 10 80       	add    $0x80109120,%eax
80102dba:	8a 00                	mov    (%eax),%al
80102dbc:	0f b6 d0             	movzbl %al,%edx
80102dbf:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102dc4:	31 d0                	xor    %edx,%eax
80102dc6:	a3 1c b6 10 80       	mov    %eax,0x8010b61c
  c = charcode[shift & (CTL | SHIFT)][data];
80102dcb:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102dd0:	83 e0 03             	and    $0x3,%eax
80102dd3:	8b 14 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%edx
80102dda:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102ddd:	01 d0                	add    %edx,%eax
80102ddf:	8a 00                	mov    (%eax),%al
80102de1:	0f b6 c0             	movzbl %al,%eax
80102de4:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102de7:	a1 1c b6 10 80       	mov    0x8010b61c,%eax
80102dec:	83 e0 08             	and    $0x8,%eax
80102def:	85 c0                	test   %eax,%eax
80102df1:	74 22                	je     80102e15 <kbdgetc+0x14a>
    if('a' <= c && c <= 'z')
80102df3:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102df7:	76 0c                	jbe    80102e05 <kbdgetc+0x13a>
80102df9:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102dfd:	77 06                	ja     80102e05 <kbdgetc+0x13a>
      c += 'A' - 'a';
80102dff:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102e03:	eb 10                	jmp    80102e15 <kbdgetc+0x14a>
    else if('A' <= c && c <= 'Z')
80102e05:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102e09:	76 0a                	jbe    80102e15 <kbdgetc+0x14a>
80102e0b:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102e0f:	77 04                	ja     80102e15 <kbdgetc+0x14a>
      c += 'a' - 'A';
80102e11:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102e15:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102e18:	c9                   	leave  
80102e19:	c3                   	ret    

80102e1a <kbdintr>:

void
kbdintr(void)
{
80102e1a:	55                   	push   %ebp
80102e1b:	89 e5                	mov    %esp,%ebp
80102e1d:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102e20:	c7 04 24 cb 2c 10 80 	movl   $0x80102ccb,(%esp)
80102e27:	e8 c9 d9 ff ff       	call   801007f5 <consoleintr>
}
80102e2c:	c9                   	leave  
80102e2d:	c3                   	ret    
	...

80102e30 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102e30:	55                   	push   %ebp
80102e31:	89 e5                	mov    %esp,%ebp
80102e33:	83 ec 14             	sub    $0x14,%esp
80102e36:	8b 45 08             	mov    0x8(%ebp),%eax
80102e39:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102e3d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102e40:	89 c2                	mov    %eax,%edx
80102e42:	ec                   	in     (%dx),%al
80102e43:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
80102e46:	8a 45 ff             	mov    -0x1(%ebp),%al
}
80102e49:	c9                   	leave  
80102e4a:	c3                   	ret    

80102e4b <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102e4b:	55                   	push   %ebp
80102e4c:	89 e5                	mov    %esp,%ebp
80102e4e:	83 ec 08             	sub    $0x8,%esp
80102e51:	8b 45 08             	mov    0x8(%ebp),%eax
80102e54:	8b 55 0c             	mov    0xc(%ebp),%edx
80102e57:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
80102e5b:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102e5e:	8a 45 f8             	mov    -0x8(%ebp),%al
80102e61:	8b 55 fc             	mov    -0x4(%ebp),%edx
80102e64:	ee                   	out    %al,(%dx)
}
80102e65:	c9                   	leave  
80102e66:	c3                   	ret    

80102e67 <lapicw>:
volatile uint *lapic;  // Initialized in mp.c

//PAGEBREAK!
static void
lapicw(int index, int value)
{
80102e67:	55                   	push   %ebp
80102e68:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102e6a:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102e6f:	8b 55 08             	mov    0x8(%ebp),%edx
80102e72:	c1 e2 02             	shl    $0x2,%edx
80102e75:	01 c2                	add    %eax,%edx
80102e77:	8b 45 0c             	mov    0xc(%ebp),%eax
80102e7a:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102e7c:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102e81:	83 c0 20             	add    $0x20,%eax
80102e84:	8b 00                	mov    (%eax),%eax
}
80102e86:	5d                   	pop    %ebp
80102e87:	c3                   	ret    

80102e88 <lapicinit>:

void
lapicinit(void)
{
80102e88:	55                   	push   %ebp
80102e89:	89 e5                	mov    %esp,%ebp
80102e8b:	83 ec 08             	sub    $0x8,%esp
  if(!lapic)
80102e8e:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102e93:	85 c0                	test   %eax,%eax
80102e95:	75 05                	jne    80102e9c <lapicinit+0x14>
    return;
80102e97:	e9 43 01 00 00       	jmp    80102fdf <lapicinit+0x157>

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102e9c:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102ea3:	00 
80102ea4:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102eab:	e8 b7 ff ff ff       	call   80102e67 <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102eb0:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102eb7:	00 
80102eb8:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102ebf:	e8 a3 ff ff ff       	call   80102e67 <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102ec4:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102ecb:	00 
80102ecc:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102ed3:	e8 8f ff ff ff       	call   80102e67 <lapicw>
  lapicw(TICR, 10000000);
80102ed8:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102edf:	00 
80102ee0:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102ee7:	e8 7b ff ff ff       	call   80102e67 <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102eec:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102ef3:	00 
80102ef4:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102efb:	e8 67 ff ff ff       	call   80102e67 <lapicw>
  lapicw(LINT1, MASKED);
80102f00:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102f07:	00 
80102f08:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102f0f:	e8 53 ff ff ff       	call   80102e67 <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102f14:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102f19:	83 c0 30             	add    $0x30,%eax
80102f1c:	8b 00                	mov    (%eax),%eax
80102f1e:	c1 e8 10             	shr    $0x10,%eax
80102f21:	0f b6 c0             	movzbl %al,%eax
80102f24:	83 f8 03             	cmp    $0x3,%eax
80102f27:	76 14                	jbe    80102f3d <lapicinit+0xb5>
    lapicw(PCINT, MASKED);
80102f29:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102f30:	00 
80102f31:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102f38:	e8 2a ff ff ff       	call   80102e67 <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102f3d:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102f44:	00 
80102f45:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102f4c:	e8 16 ff ff ff       	call   80102e67 <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102f51:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102f58:	00 
80102f59:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102f60:	e8 02 ff ff ff       	call   80102e67 <lapicw>
  lapicw(ESR, 0);
80102f65:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102f6c:	00 
80102f6d:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102f74:	e8 ee fe ff ff       	call   80102e67 <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102f79:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102f80:	00 
80102f81:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102f88:	e8 da fe ff ff       	call   80102e67 <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102f8d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102f94:	00 
80102f95:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f9c:	e8 c6 fe ff ff       	call   80102e67 <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102fa1:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102fa8:	00 
80102fa9:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fb0:	e8 b2 fe ff ff       	call   80102e67 <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102fb5:	90                   	nop
80102fb6:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102fbb:	05 00 03 00 00       	add    $0x300,%eax
80102fc0:	8b 00                	mov    (%eax),%eax
80102fc2:	25 00 10 00 00       	and    $0x1000,%eax
80102fc7:	85 c0                	test   %eax,%eax
80102fc9:	75 eb                	jne    80102fb6 <lapicinit+0x12e>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102fcb:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102fd2:	00 
80102fd3:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102fda:	e8 88 fe ff ff       	call   80102e67 <lapicw>
}
80102fdf:	c9                   	leave  
80102fe0:	c3                   	ret    

80102fe1 <lapicid>:

int
lapicid(void)
{
80102fe1:	55                   	push   %ebp
80102fe2:	89 e5                	mov    %esp,%ebp
  if (!lapic)
80102fe4:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102fe9:	85 c0                	test   %eax,%eax
80102feb:	75 07                	jne    80102ff4 <lapicid+0x13>
    return 0;
80102fed:	b8 00 00 00 00       	mov    $0x0,%eax
80102ff2:	eb 0d                	jmp    80103001 <lapicid+0x20>
  return lapic[ID] >> 24;
80102ff4:	a1 bc 39 11 80       	mov    0x801139bc,%eax
80102ff9:	83 c0 20             	add    $0x20,%eax
80102ffc:	8b 00                	mov    (%eax),%eax
80102ffe:	c1 e8 18             	shr    $0x18,%eax
}
80103001:	5d                   	pop    %ebp
80103002:	c3                   	ret    

80103003 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80103003:	55                   	push   %ebp
80103004:	89 e5                	mov    %esp,%ebp
80103006:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80103009:	a1 bc 39 11 80       	mov    0x801139bc,%eax
8010300e:	85 c0                	test   %eax,%eax
80103010:	74 14                	je     80103026 <lapiceoi+0x23>
    lapicw(EOI, 0);
80103012:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80103019:	00 
8010301a:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80103021:	e8 41 fe ff ff       	call   80102e67 <lapicw>
}
80103026:	c9                   	leave  
80103027:	c3                   	ret    

80103028 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80103028:	55                   	push   %ebp
80103029:	89 e5                	mov    %esp,%ebp
}
8010302b:	5d                   	pop    %ebp
8010302c:	c3                   	ret    

8010302d <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
8010302d:	55                   	push   %ebp
8010302e:	89 e5                	mov    %esp,%ebp
80103030:	83 ec 1c             	sub    $0x1c,%esp
80103033:	8b 45 08             	mov    0x8(%ebp),%eax
80103036:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;

  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(CMOS_PORT, 0xF);  // offset 0xF is shutdown code
80103039:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80103040:	00 
80103041:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80103048:	e8 fe fd ff ff       	call   80102e4b <outb>
  outb(CMOS_PORT+1, 0x0A);
8010304d:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103054:	00 
80103055:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
8010305c:	e8 ea fd ff ff       	call   80102e4b <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80103061:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80103068:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010306b:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80103070:	8b 45 f8             	mov    -0x8(%ebp),%eax
80103073:	8d 50 02             	lea    0x2(%eax),%edx
80103076:	8b 45 0c             	mov    0xc(%ebp),%eax
80103079:	c1 e8 04             	shr    $0x4,%eax
8010307c:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
8010307f:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80103083:	c1 e0 18             	shl    $0x18,%eax
80103086:	89 44 24 04          	mov    %eax,0x4(%esp)
8010308a:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80103091:	e8 d1 fd ff ff       	call   80102e67 <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80103096:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
8010309d:	00 
8010309e:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
801030a5:	e8 bd fd ff ff       	call   80102e67 <lapicw>
  microdelay(200);
801030aa:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
801030b1:	e8 72 ff ff ff       	call   80103028 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
801030b6:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
801030bd:	00 
801030be:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
801030c5:	e8 9d fd ff ff       	call   80102e67 <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
801030ca:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
801030d1:	e8 52 ff ff ff       	call   80103028 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
801030d6:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801030dd:	eb 3f                	jmp    8010311e <lapicstartap+0xf1>
    lapicw(ICRHI, apicid<<24);
801030df:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
801030e3:	c1 e0 18             	shl    $0x18,%eax
801030e6:	89 44 24 04          	mov    %eax,0x4(%esp)
801030ea:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
801030f1:	e8 71 fd ff ff       	call   80102e67 <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
801030f6:	8b 45 0c             	mov    0xc(%ebp),%eax
801030f9:	c1 e8 0c             	shr    $0xc,%eax
801030fc:	80 cc 06             	or     $0x6,%ah
801030ff:	89 44 24 04          	mov    %eax,0x4(%esp)
80103103:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
8010310a:	e8 58 fd ff ff       	call   80102e67 <lapicw>
    microdelay(200);
8010310f:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80103116:	e8 0d ff ff ff       	call   80103028 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
8010311b:	ff 45 fc             	incl   -0x4(%ebp)
8010311e:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80103122:	7e bb                	jle    801030df <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80103124:	c9                   	leave  
80103125:	c3                   	ret    

80103126 <cmos_read>:
#define DAY     0x07
#define MONTH   0x08
#define YEAR    0x09

static uint cmos_read(uint reg)
{
80103126:	55                   	push   %ebp
80103127:	89 e5                	mov    %esp,%ebp
80103129:	83 ec 08             	sub    $0x8,%esp
  outb(CMOS_PORT,  reg);
8010312c:	8b 45 08             	mov    0x8(%ebp),%eax
8010312f:	0f b6 c0             	movzbl %al,%eax
80103132:	89 44 24 04          	mov    %eax,0x4(%esp)
80103136:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
8010313d:	e8 09 fd ff ff       	call   80102e4b <outb>
  microdelay(200);
80103142:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80103149:	e8 da fe ff ff       	call   80103028 <microdelay>

  return inb(CMOS_RETURN);
8010314e:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80103155:	e8 d6 fc ff ff       	call   80102e30 <inb>
8010315a:	0f b6 c0             	movzbl %al,%eax
}
8010315d:	c9                   	leave  
8010315e:	c3                   	ret    

8010315f <fill_rtcdate>:

static void fill_rtcdate(struct rtcdate *r)
{
8010315f:	55                   	push   %ebp
80103160:	89 e5                	mov    %esp,%ebp
80103162:	83 ec 04             	sub    $0x4,%esp
  r->second = cmos_read(SECS);
80103165:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010316c:	e8 b5 ff ff ff       	call   80103126 <cmos_read>
80103171:	8b 55 08             	mov    0x8(%ebp),%edx
80103174:	89 02                	mov    %eax,(%edx)
  r->minute = cmos_read(MINS);
80103176:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010317d:	e8 a4 ff ff ff       	call   80103126 <cmos_read>
80103182:	8b 55 08             	mov    0x8(%ebp),%edx
80103185:	89 42 04             	mov    %eax,0x4(%edx)
  r->hour   = cmos_read(HOURS);
80103188:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
8010318f:	e8 92 ff ff ff       	call   80103126 <cmos_read>
80103194:	8b 55 08             	mov    0x8(%ebp),%edx
80103197:	89 42 08             	mov    %eax,0x8(%edx)
  r->day    = cmos_read(DAY);
8010319a:	c7 04 24 07 00 00 00 	movl   $0x7,(%esp)
801031a1:	e8 80 ff ff ff       	call   80103126 <cmos_read>
801031a6:	8b 55 08             	mov    0x8(%ebp),%edx
801031a9:	89 42 0c             	mov    %eax,0xc(%edx)
  r->month  = cmos_read(MONTH);
801031ac:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
801031b3:	e8 6e ff ff ff       	call   80103126 <cmos_read>
801031b8:	8b 55 08             	mov    0x8(%ebp),%edx
801031bb:	89 42 10             	mov    %eax,0x10(%edx)
  r->year   = cmos_read(YEAR);
801031be:	c7 04 24 09 00 00 00 	movl   $0x9,(%esp)
801031c5:	e8 5c ff ff ff       	call   80103126 <cmos_read>
801031ca:	8b 55 08             	mov    0x8(%ebp),%edx
801031cd:	89 42 14             	mov    %eax,0x14(%edx)
}
801031d0:	c9                   	leave  
801031d1:	c3                   	ret    

801031d2 <cmostime>:

// qemu seems to use 24-hour GWT and the values are BCD encoded
void cmostime(struct rtcdate *r)
{
801031d2:	55                   	push   %ebp
801031d3:	89 e5                	mov    %esp,%ebp
801031d5:	57                   	push   %edi
801031d6:	56                   	push   %esi
801031d7:	53                   	push   %ebx
801031d8:	83 ec 5c             	sub    $0x5c,%esp
  struct rtcdate t1, t2;
  int sb, bcd;

  sb = cmos_read(CMOS_STATB);
801031db:	c7 04 24 0b 00 00 00 	movl   $0xb,(%esp)
801031e2:	e8 3f ff ff ff       	call   80103126 <cmos_read>
801031e7:	89 45 e4             	mov    %eax,-0x1c(%ebp)

  bcd = (sb & (1 << 2)) == 0;
801031ea:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801031ed:	83 e0 04             	and    $0x4,%eax
801031f0:	85 c0                	test   %eax,%eax
801031f2:	0f 94 c0             	sete   %al
801031f5:	0f b6 c0             	movzbl %al,%eax
801031f8:	89 45 e0             	mov    %eax,-0x20(%ebp)

  // make sure CMOS doesn't modify time while we read it
  for(;;) {
    fill_rtcdate(&t1);
801031fb:	8d 45 c8             	lea    -0x38(%ebp),%eax
801031fe:	89 04 24             	mov    %eax,(%esp)
80103201:	e8 59 ff ff ff       	call   8010315f <fill_rtcdate>
    if(cmos_read(CMOS_STATA) & CMOS_UIP)
80103206:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
8010320d:	e8 14 ff ff ff       	call   80103126 <cmos_read>
80103212:	25 80 00 00 00       	and    $0x80,%eax
80103217:	85 c0                	test   %eax,%eax
80103219:	74 02                	je     8010321d <cmostime+0x4b>
        continue;
8010321b:	eb 36                	jmp    80103253 <cmostime+0x81>
    fill_rtcdate(&t2);
8010321d:	8d 45 b0             	lea    -0x50(%ebp),%eax
80103220:	89 04 24             	mov    %eax,(%esp)
80103223:	e8 37 ff ff ff       	call   8010315f <fill_rtcdate>
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
80103228:	c7 44 24 08 18 00 00 	movl   $0x18,0x8(%esp)
8010322f:	00 
80103230:	8d 45 b0             	lea    -0x50(%ebp),%eax
80103233:	89 44 24 04          	mov    %eax,0x4(%esp)
80103237:	8d 45 c8             	lea    -0x38(%ebp),%eax
8010323a:	89 04 24             	mov    %eax,(%esp)
8010323d:	e8 cb 1e 00 00       	call   8010510d <memcmp>
80103242:	85 c0                	test   %eax,%eax
80103244:	75 0d                	jne    80103253 <cmostime+0x81>
      break;
80103246:	90                   	nop
  }

  // convert
  if(bcd) {
80103247:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
8010324b:	0f 84 ac 00 00 00    	je     801032fd <cmostime+0x12b>
80103251:	eb 02                	jmp    80103255 <cmostime+0x83>
    if(cmos_read(CMOS_STATA) & CMOS_UIP)
        continue;
    fill_rtcdate(&t2);
    if(memcmp(&t1, &t2, sizeof(t1)) == 0)
      break;
  }
80103253:	eb a6                	jmp    801031fb <cmostime+0x29>

  // convert
  if(bcd) {
#define    CONV(x)     (t1.x = ((t1.x >> 4) * 10) + (t1.x & 0xf))
    CONV(second);
80103255:	8b 45 c8             	mov    -0x38(%ebp),%eax
80103258:	c1 e8 04             	shr    $0x4,%eax
8010325b:	89 c2                	mov    %eax,%edx
8010325d:	89 d0                	mov    %edx,%eax
8010325f:	c1 e0 02             	shl    $0x2,%eax
80103262:	01 d0                	add    %edx,%eax
80103264:	01 c0                	add    %eax,%eax
80103266:	8b 55 c8             	mov    -0x38(%ebp),%edx
80103269:	83 e2 0f             	and    $0xf,%edx
8010326c:	01 d0                	add    %edx,%eax
8010326e:	89 45 c8             	mov    %eax,-0x38(%ebp)
    CONV(minute);
80103271:	8b 45 cc             	mov    -0x34(%ebp),%eax
80103274:	c1 e8 04             	shr    $0x4,%eax
80103277:	89 c2                	mov    %eax,%edx
80103279:	89 d0                	mov    %edx,%eax
8010327b:	c1 e0 02             	shl    $0x2,%eax
8010327e:	01 d0                	add    %edx,%eax
80103280:	01 c0                	add    %eax,%eax
80103282:	8b 55 cc             	mov    -0x34(%ebp),%edx
80103285:	83 e2 0f             	and    $0xf,%edx
80103288:	01 d0                	add    %edx,%eax
8010328a:	89 45 cc             	mov    %eax,-0x34(%ebp)
    CONV(hour  );
8010328d:	8b 45 d0             	mov    -0x30(%ebp),%eax
80103290:	c1 e8 04             	shr    $0x4,%eax
80103293:	89 c2                	mov    %eax,%edx
80103295:	89 d0                	mov    %edx,%eax
80103297:	c1 e0 02             	shl    $0x2,%eax
8010329a:	01 d0                	add    %edx,%eax
8010329c:	01 c0                	add    %eax,%eax
8010329e:	8b 55 d0             	mov    -0x30(%ebp),%edx
801032a1:	83 e2 0f             	and    $0xf,%edx
801032a4:	01 d0                	add    %edx,%eax
801032a6:	89 45 d0             	mov    %eax,-0x30(%ebp)
    CONV(day   );
801032a9:	8b 45 d4             	mov    -0x2c(%ebp),%eax
801032ac:	c1 e8 04             	shr    $0x4,%eax
801032af:	89 c2                	mov    %eax,%edx
801032b1:	89 d0                	mov    %edx,%eax
801032b3:	c1 e0 02             	shl    $0x2,%eax
801032b6:	01 d0                	add    %edx,%eax
801032b8:	01 c0                	add    %eax,%eax
801032ba:	8b 55 d4             	mov    -0x2c(%ebp),%edx
801032bd:	83 e2 0f             	and    $0xf,%edx
801032c0:	01 d0                	add    %edx,%eax
801032c2:	89 45 d4             	mov    %eax,-0x2c(%ebp)
    CONV(month );
801032c5:	8b 45 d8             	mov    -0x28(%ebp),%eax
801032c8:	c1 e8 04             	shr    $0x4,%eax
801032cb:	89 c2                	mov    %eax,%edx
801032cd:	89 d0                	mov    %edx,%eax
801032cf:	c1 e0 02             	shl    $0x2,%eax
801032d2:	01 d0                	add    %edx,%eax
801032d4:	01 c0                	add    %eax,%eax
801032d6:	8b 55 d8             	mov    -0x28(%ebp),%edx
801032d9:	83 e2 0f             	and    $0xf,%edx
801032dc:	01 d0                	add    %edx,%eax
801032de:	89 45 d8             	mov    %eax,-0x28(%ebp)
    CONV(year  );
801032e1:	8b 45 dc             	mov    -0x24(%ebp),%eax
801032e4:	c1 e8 04             	shr    $0x4,%eax
801032e7:	89 c2                	mov    %eax,%edx
801032e9:	89 d0                	mov    %edx,%eax
801032eb:	c1 e0 02             	shl    $0x2,%eax
801032ee:	01 d0                	add    %edx,%eax
801032f0:	01 c0                	add    %eax,%eax
801032f2:	8b 55 dc             	mov    -0x24(%ebp),%edx
801032f5:	83 e2 0f             	and    $0xf,%edx
801032f8:	01 d0                	add    %edx,%eax
801032fa:	89 45 dc             	mov    %eax,-0x24(%ebp)
#undef     CONV
  }

  *r = t1;
801032fd:	8b 45 08             	mov    0x8(%ebp),%eax
80103300:	89 c2                	mov    %eax,%edx
80103302:	8d 5d c8             	lea    -0x38(%ebp),%ebx
80103305:	b8 06 00 00 00       	mov    $0x6,%eax
8010330a:	89 d7                	mov    %edx,%edi
8010330c:	89 de                	mov    %ebx,%esi
8010330e:	89 c1                	mov    %eax,%ecx
80103310:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)
  r->year += 2000;
80103312:	8b 45 08             	mov    0x8(%ebp),%eax
80103315:	8b 40 14             	mov    0x14(%eax),%eax
80103318:	8d 90 d0 07 00 00    	lea    0x7d0(%eax),%edx
8010331e:	8b 45 08             	mov    0x8(%ebp),%eax
80103321:	89 50 14             	mov    %edx,0x14(%eax)
}
80103324:	83 c4 5c             	add    $0x5c,%esp
80103327:	5b                   	pop    %ebx
80103328:	5e                   	pop    %esi
80103329:	5f                   	pop    %edi
8010332a:	5d                   	pop    %ebp
8010332b:	c3                   	ret    

8010332c <initlog>:
static void recover_from_log(void);
static void commit();

void
initlog(int dev)
{
8010332c:	55                   	push   %ebp
8010332d:	89 e5                	mov    %esp,%ebp
8010332f:	83 ec 38             	sub    $0x38,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
80103332:	c7 44 24 04 79 84 10 	movl   $0x80108479,0x4(%esp)
80103339:	80 
8010333a:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103341:	e8 cc 1a 00 00       	call   80104e12 <initlock>
  readsb(dev, &sb);
80103346:	8d 45 dc             	lea    -0x24(%ebp),%eax
80103349:	89 44 24 04          	mov    %eax,0x4(%esp)
8010334d:	8b 45 08             	mov    0x8(%ebp),%eax
80103350:	89 04 24             	mov    %eax,(%esp)
80103353:	e8 d8 e0 ff ff       	call   80101430 <readsb>
  log.start = sb.logstart;
80103358:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010335b:	a3 f4 39 11 80       	mov    %eax,0x801139f4
  log.size = sb.nlog;
80103360:	8b 45 e8             	mov    -0x18(%ebp),%eax
80103363:	a3 f8 39 11 80       	mov    %eax,0x801139f8
  log.dev = dev;
80103368:	8b 45 08             	mov    0x8(%ebp),%eax
8010336b:	a3 04 3a 11 80       	mov    %eax,0x80113a04
  recover_from_log();
80103370:	e8 95 01 00 00       	call   8010350a <recover_from_log>
}
80103375:	c9                   	leave  
80103376:	c3                   	ret    

80103377 <install_trans>:

// Copy committed blocks from log to their home location
static void
install_trans(void)
{
80103377:	55                   	push   %ebp
80103378:	89 e5                	mov    %esp,%ebp
8010337a:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
8010337d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103384:	e9 89 00 00 00       	jmp    80103412 <install_trans+0x9b>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103389:	8b 15 f4 39 11 80    	mov    0x801139f4,%edx
8010338f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103392:	01 d0                	add    %edx,%eax
80103394:	40                   	inc    %eax
80103395:	89 c2                	mov    %eax,%edx
80103397:	a1 04 3a 11 80       	mov    0x80113a04,%eax
8010339c:	89 54 24 04          	mov    %edx,0x4(%esp)
801033a0:	89 04 24             	mov    %eax,(%esp)
801033a3:	e8 0d ce ff ff       	call   801001b5 <bread>
801033a8:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.block[tail]); // read dst
801033ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801033ae:	83 c0 10             	add    $0x10,%eax
801033b1:	8b 04 85 cc 39 11 80 	mov    -0x7feec634(,%eax,4),%eax
801033b8:	89 c2                	mov    %eax,%edx
801033ba:	a1 04 3a 11 80       	mov    0x80113a04,%eax
801033bf:	89 54 24 04          	mov    %edx,0x4(%esp)
801033c3:	89 04 24             	mov    %eax,(%esp)
801033c6:	e8 ea cd ff ff       	call   801001b5 <bread>
801033cb:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801033ce:	8b 45 f0             	mov    -0x10(%ebp),%eax
801033d1:	8d 50 5c             	lea    0x5c(%eax),%edx
801033d4:	8b 45 ec             	mov    -0x14(%ebp),%eax
801033d7:	83 c0 5c             	add    $0x5c,%eax
801033da:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801033e1:	00 
801033e2:	89 54 24 04          	mov    %edx,0x4(%esp)
801033e6:	89 04 24             	mov    %eax,(%esp)
801033e9:	e8 71 1d 00 00       	call   8010515f <memmove>
    bwrite(dbuf);  // write dst to disk
801033ee:	8b 45 ec             	mov    -0x14(%ebp),%eax
801033f1:	89 04 24             	mov    %eax,(%esp)
801033f4:	e8 f3 cd ff ff       	call   801001ec <bwrite>
    brelse(lbuf);
801033f9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801033fc:	89 04 24             	mov    %eax,(%esp)
801033ff:	e8 28 ce ff ff       	call   8010022c <brelse>
    brelse(dbuf);
80103404:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103407:	89 04 24             	mov    %eax,(%esp)
8010340a:	e8 1d ce ff ff       	call   8010022c <brelse>
static void
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
8010340f:	ff 45 f4             	incl   -0xc(%ebp)
80103412:	a1 08 3a 11 80       	mov    0x80113a08,%eax
80103417:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010341a:	0f 8f 69 ff ff ff    	jg     80103389 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf);
    brelse(dbuf);
  }
}
80103420:	c9                   	leave  
80103421:	c3                   	ret    

80103422 <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
80103422:	55                   	push   %ebp
80103423:	89 e5                	mov    %esp,%ebp
80103425:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103428:	a1 f4 39 11 80       	mov    0x801139f4,%eax
8010342d:	89 c2                	mov    %eax,%edx
8010342f:	a1 04 3a 11 80       	mov    0x80113a04,%eax
80103434:	89 54 24 04          	mov    %edx,0x4(%esp)
80103438:	89 04 24             	mov    %eax,(%esp)
8010343b:	e8 75 cd ff ff       	call   801001b5 <bread>
80103440:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
80103443:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103446:	83 c0 5c             	add    $0x5c,%eax
80103449:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
8010344c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010344f:	8b 00                	mov    (%eax),%eax
80103451:	a3 08 3a 11 80       	mov    %eax,0x80113a08
  for (i = 0; i < log.lh.n; i++) {
80103456:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010345d:	eb 1a                	jmp    80103479 <read_head+0x57>
    log.lh.block[i] = lh->block[i];
8010345f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103462:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103465:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103469:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010346c:	83 c2 10             	add    $0x10,%edx
8010346f:	89 04 95 cc 39 11 80 	mov    %eax,-0x7feec634(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
80103476:	ff 45 f4             	incl   -0xc(%ebp)
80103479:	a1 08 3a 11 80       	mov    0x80113a08,%eax
8010347e:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103481:	7f dc                	jg     8010345f <read_head+0x3d>
    log.lh.block[i] = lh->block[i];
  }
  brelse(buf);
80103483:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103486:	89 04 24             	mov    %eax,(%esp)
80103489:	e8 9e cd ff ff       	call   8010022c <brelse>
}
8010348e:	c9                   	leave  
8010348f:	c3                   	ret    

80103490 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103490:	55                   	push   %ebp
80103491:	89 e5                	mov    %esp,%ebp
80103493:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103496:	a1 f4 39 11 80       	mov    0x801139f4,%eax
8010349b:	89 c2                	mov    %eax,%edx
8010349d:	a1 04 3a 11 80       	mov    0x80113a04,%eax
801034a2:	89 54 24 04          	mov    %edx,0x4(%esp)
801034a6:	89 04 24             	mov    %eax,(%esp)
801034a9:	e8 07 cd ff ff       	call   801001b5 <bread>
801034ae:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
801034b1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801034b4:	83 c0 5c             	add    $0x5c,%eax
801034b7:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
801034ba:	8b 15 08 3a 11 80    	mov    0x80113a08,%edx
801034c0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801034c3:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801034c5:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801034cc:	eb 1a                	jmp    801034e8 <write_head+0x58>
    hb->block[i] = log.lh.block[i];
801034ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801034d1:	83 c0 10             	add    $0x10,%eax
801034d4:	8b 0c 85 cc 39 11 80 	mov    -0x7feec634(,%eax,4),%ecx
801034db:	8b 45 ec             	mov    -0x14(%ebp),%eax
801034de:	8b 55 f4             	mov    -0xc(%ebp),%edx
801034e1:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801034e5:	ff 45 f4             	incl   -0xc(%ebp)
801034e8:	a1 08 3a 11 80       	mov    0x80113a08,%eax
801034ed:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801034f0:	7f dc                	jg     801034ce <write_head+0x3e>
    hb->block[i] = log.lh.block[i];
  }
  bwrite(buf);
801034f2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801034f5:	89 04 24             	mov    %eax,(%esp)
801034f8:	e8 ef cc ff ff       	call   801001ec <bwrite>
  brelse(buf);
801034fd:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103500:	89 04 24             	mov    %eax,(%esp)
80103503:	e8 24 cd ff ff       	call   8010022c <brelse>
}
80103508:	c9                   	leave  
80103509:	c3                   	ret    

8010350a <recover_from_log>:

static void
recover_from_log(void)
{
8010350a:	55                   	push   %ebp
8010350b:	89 e5                	mov    %esp,%ebp
8010350d:	83 ec 08             	sub    $0x8,%esp
  read_head();
80103510:	e8 0d ff ff ff       	call   80103422 <read_head>
  install_trans(); // if committed, copy from log to disk
80103515:	e8 5d fe ff ff       	call   80103377 <install_trans>
  log.lh.n = 0;
8010351a:	c7 05 08 3a 11 80 00 	movl   $0x0,0x80113a08
80103521:	00 00 00 
  write_head(); // clear the log
80103524:	e8 67 ff ff ff       	call   80103490 <write_head>
}
80103529:	c9                   	leave  
8010352a:	c3                   	ret    

8010352b <begin_op>:

// called at the start of each FS system call.
void
begin_op(void)
{
8010352b:	55                   	push   %ebp
8010352c:	89 e5                	mov    %esp,%ebp
8010352e:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
80103531:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103538:	e8 f6 18 00 00       	call   80104e33 <acquire>
  while(1){
    if(log.committing){
8010353d:	a1 00 3a 11 80       	mov    0x80113a00,%eax
80103542:	85 c0                	test   %eax,%eax
80103544:	74 16                	je     8010355c <begin_op+0x31>
      sleep(&log, &log.lock);
80103546:	c7 44 24 04 c0 39 11 	movl   $0x801139c0,0x4(%esp)
8010354d:	80 
8010354e:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103555:	e8 0a 15 00 00       	call   80104a64 <sleep>
8010355a:	eb 4d                	jmp    801035a9 <begin_op+0x7e>
    } else if(log.lh.n + (log.outstanding+1)*MAXOPBLOCKS > LOGSIZE){
8010355c:	8b 15 08 3a 11 80    	mov    0x80113a08,%edx
80103562:	a1 fc 39 11 80       	mov    0x801139fc,%eax
80103567:	8d 48 01             	lea    0x1(%eax),%ecx
8010356a:	89 c8                	mov    %ecx,%eax
8010356c:	c1 e0 02             	shl    $0x2,%eax
8010356f:	01 c8                	add    %ecx,%eax
80103571:	01 c0                	add    %eax,%eax
80103573:	01 d0                	add    %edx,%eax
80103575:	83 f8 1e             	cmp    $0x1e,%eax
80103578:	7e 16                	jle    80103590 <begin_op+0x65>
      // this op might exhaust log space; wait for commit.
      sleep(&log, &log.lock);
8010357a:	c7 44 24 04 c0 39 11 	movl   $0x801139c0,0x4(%esp)
80103581:	80 
80103582:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103589:	e8 d6 14 00 00       	call   80104a64 <sleep>
8010358e:	eb 19                	jmp    801035a9 <begin_op+0x7e>
    } else {
      log.outstanding += 1;
80103590:	a1 fc 39 11 80       	mov    0x801139fc,%eax
80103595:	40                   	inc    %eax
80103596:	a3 fc 39 11 80       	mov    %eax,0x801139fc
      release(&log.lock);
8010359b:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
801035a2:	e8 f6 18 00 00       	call   80104e9d <release>
      break;
801035a7:	eb 02                	jmp    801035ab <begin_op+0x80>
    }
  }
801035a9:	eb 92                	jmp    8010353d <begin_op+0x12>
}
801035ab:	c9                   	leave  
801035ac:	c3                   	ret    

801035ad <end_op>:

// called at the end of each FS system call.
// commits if this was the last outstanding operation.
void
end_op(void)
{
801035ad:	55                   	push   %ebp
801035ae:	89 e5                	mov    %esp,%ebp
801035b0:	83 ec 28             	sub    $0x28,%esp
  int do_commit = 0;
801035b3:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)

  acquire(&log.lock);
801035ba:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
801035c1:	e8 6d 18 00 00       	call   80104e33 <acquire>
  log.outstanding -= 1;
801035c6:	a1 fc 39 11 80       	mov    0x801139fc,%eax
801035cb:	48                   	dec    %eax
801035cc:	a3 fc 39 11 80       	mov    %eax,0x801139fc
  if(log.committing)
801035d1:	a1 00 3a 11 80       	mov    0x80113a00,%eax
801035d6:	85 c0                	test   %eax,%eax
801035d8:	74 0c                	je     801035e6 <end_op+0x39>
    panic("log.committing");
801035da:	c7 04 24 7d 84 10 80 	movl   $0x8010847d,(%esp)
801035e1:	e8 6e cf ff ff       	call   80100554 <panic>
  if(log.outstanding == 0){
801035e6:	a1 fc 39 11 80       	mov    0x801139fc,%eax
801035eb:	85 c0                	test   %eax,%eax
801035ed:	75 13                	jne    80103602 <end_op+0x55>
    do_commit = 1;
801035ef:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
    log.committing = 1;
801035f6:	c7 05 00 3a 11 80 01 	movl   $0x1,0x80113a00
801035fd:	00 00 00 
80103600:	eb 0c                	jmp    8010360e <end_op+0x61>
  } else {
    // begin_op() may be waiting for log space,
    // and decrementing log.outstanding has decreased
    // the amount of reserved space.
    wakeup(&log);
80103602:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103609:	e8 2a 15 00 00       	call   80104b38 <wakeup>
  }
  release(&log.lock);
8010360e:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103615:	e8 83 18 00 00       	call   80104e9d <release>

  if(do_commit){
8010361a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010361e:	74 33                	je     80103653 <end_op+0xa6>
    // call commit w/o holding locks, since not allowed
    // to sleep with locks.
    commit();
80103620:	e8 db 00 00 00       	call   80103700 <commit>
    acquire(&log.lock);
80103625:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
8010362c:	e8 02 18 00 00       	call   80104e33 <acquire>
    log.committing = 0;
80103631:	c7 05 00 3a 11 80 00 	movl   $0x0,0x80113a00
80103638:	00 00 00 
    wakeup(&log);
8010363b:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103642:	e8 f1 14 00 00       	call   80104b38 <wakeup>
    release(&log.lock);
80103647:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
8010364e:	e8 4a 18 00 00       	call   80104e9d <release>
  }
}
80103653:	c9                   	leave  
80103654:	c3                   	ret    

80103655 <write_log>:

// Copy modified blocks from cache to log.
static void
write_log(void)
{
80103655:	55                   	push   %ebp
80103656:	89 e5                	mov    %esp,%ebp
80103658:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
8010365b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103662:	e9 89 00 00 00       	jmp    801036f0 <write_log+0x9b>
    struct buf *to = bread(log.dev, log.start+tail+1); // log block
80103667:	8b 15 f4 39 11 80    	mov    0x801139f4,%edx
8010366d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103670:	01 d0                	add    %edx,%eax
80103672:	40                   	inc    %eax
80103673:	89 c2                	mov    %eax,%edx
80103675:	a1 04 3a 11 80       	mov    0x80113a04,%eax
8010367a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010367e:	89 04 24             	mov    %eax,(%esp)
80103681:	e8 2f cb ff ff       	call   801001b5 <bread>
80103686:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *from = bread(log.dev, log.lh.block[tail]); // cache block
80103689:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010368c:	83 c0 10             	add    $0x10,%eax
8010368f:	8b 04 85 cc 39 11 80 	mov    -0x7feec634(,%eax,4),%eax
80103696:	89 c2                	mov    %eax,%edx
80103698:	a1 04 3a 11 80       	mov    0x80113a04,%eax
8010369d:	89 54 24 04          	mov    %edx,0x4(%esp)
801036a1:	89 04 24             	mov    %eax,(%esp)
801036a4:	e8 0c cb ff ff       	call   801001b5 <bread>
801036a9:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(to->data, from->data, BSIZE);
801036ac:	8b 45 ec             	mov    -0x14(%ebp),%eax
801036af:	8d 50 5c             	lea    0x5c(%eax),%edx
801036b2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036b5:	83 c0 5c             	add    $0x5c,%eax
801036b8:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801036bf:	00 
801036c0:	89 54 24 04          	mov    %edx,0x4(%esp)
801036c4:	89 04 24             	mov    %eax,(%esp)
801036c7:	e8 93 1a 00 00       	call   8010515f <memmove>
    bwrite(to);  // write the log
801036cc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036cf:	89 04 24             	mov    %eax,(%esp)
801036d2:	e8 15 cb ff ff       	call   801001ec <bwrite>
    brelse(from);
801036d7:	8b 45 ec             	mov    -0x14(%ebp),%eax
801036da:	89 04 24             	mov    %eax,(%esp)
801036dd:	e8 4a cb ff ff       	call   8010022c <brelse>
    brelse(to);
801036e2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036e5:	89 04 24             	mov    %eax,(%esp)
801036e8:	e8 3f cb ff ff       	call   8010022c <brelse>
static void
write_log(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801036ed:	ff 45 f4             	incl   -0xc(%ebp)
801036f0:	a1 08 3a 11 80       	mov    0x80113a08,%eax
801036f5:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801036f8:	0f 8f 69 ff ff ff    	jg     80103667 <write_log+0x12>
    memmove(to->data, from->data, BSIZE);
    bwrite(to);  // write the log
    brelse(from);
    brelse(to);
  }
}
801036fe:	c9                   	leave  
801036ff:	c3                   	ret    

80103700 <commit>:

static void
commit()
{
80103700:	55                   	push   %ebp
80103701:	89 e5                	mov    %esp,%ebp
80103703:	83 ec 08             	sub    $0x8,%esp
  if (log.lh.n > 0) {
80103706:	a1 08 3a 11 80       	mov    0x80113a08,%eax
8010370b:	85 c0                	test   %eax,%eax
8010370d:	7e 1e                	jle    8010372d <commit+0x2d>
    write_log();     // Write modified blocks from cache to log
8010370f:	e8 41 ff ff ff       	call   80103655 <write_log>
    write_head();    // Write header to disk -- the real commit
80103714:	e8 77 fd ff ff       	call   80103490 <write_head>
    install_trans(); // Now install writes to home locations
80103719:	e8 59 fc ff ff       	call   80103377 <install_trans>
    log.lh.n = 0;
8010371e:	c7 05 08 3a 11 80 00 	movl   $0x0,0x80113a08
80103725:	00 00 00 
    write_head();    // Erase the transaction from the log
80103728:	e8 63 fd ff ff       	call   80103490 <write_head>
  }
}
8010372d:	c9                   	leave  
8010372e:	c3                   	ret    

8010372f <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
8010372f:	55                   	push   %ebp
80103730:	89 e5                	mov    %esp,%ebp
80103732:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
80103735:	a1 08 3a 11 80       	mov    0x80113a08,%eax
8010373a:	83 f8 1d             	cmp    $0x1d,%eax
8010373d:	7f 10                	jg     8010374f <log_write+0x20>
8010373f:	a1 08 3a 11 80       	mov    0x80113a08,%eax
80103744:	8b 15 f8 39 11 80    	mov    0x801139f8,%edx
8010374a:	4a                   	dec    %edx
8010374b:	39 d0                	cmp    %edx,%eax
8010374d:	7c 0c                	jl     8010375b <log_write+0x2c>
    panic("too big a transaction");
8010374f:	c7 04 24 8c 84 10 80 	movl   $0x8010848c,(%esp)
80103756:	e8 f9 cd ff ff       	call   80100554 <panic>
  if (log.outstanding < 1)
8010375b:	a1 fc 39 11 80       	mov    0x801139fc,%eax
80103760:	85 c0                	test   %eax,%eax
80103762:	7f 0c                	jg     80103770 <log_write+0x41>
    panic("log_write outside of trans");
80103764:	c7 04 24 a2 84 10 80 	movl   $0x801084a2,(%esp)
8010376b:	e8 e4 cd ff ff       	call   80100554 <panic>

  acquire(&log.lock);
80103770:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
80103777:	e8 b7 16 00 00       	call   80104e33 <acquire>
  for (i = 0; i < log.lh.n; i++) {
8010377c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103783:	eb 1e                	jmp    801037a3 <log_write+0x74>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
80103785:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103788:	83 c0 10             	add    $0x10,%eax
8010378b:	8b 04 85 cc 39 11 80 	mov    -0x7feec634(,%eax,4),%eax
80103792:	89 c2                	mov    %eax,%edx
80103794:	8b 45 08             	mov    0x8(%ebp),%eax
80103797:	8b 40 08             	mov    0x8(%eax),%eax
8010379a:	39 c2                	cmp    %eax,%edx
8010379c:	75 02                	jne    801037a0 <log_write+0x71>
      break;
8010379e:	eb 0d                	jmp    801037ad <log_write+0x7e>
    panic("too big a transaction");
  if (log.outstanding < 1)
    panic("log_write outside of trans");

  acquire(&log.lock);
  for (i = 0; i < log.lh.n; i++) {
801037a0:	ff 45 f4             	incl   -0xc(%ebp)
801037a3:	a1 08 3a 11 80       	mov    0x80113a08,%eax
801037a8:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801037ab:	7f d8                	jg     80103785 <log_write+0x56>
    if (log.lh.block[i] == b->blockno)   // log absorbtion
      break;
  }
  log.lh.block[i] = b->blockno;
801037ad:	8b 45 08             	mov    0x8(%ebp),%eax
801037b0:	8b 40 08             	mov    0x8(%eax),%eax
801037b3:	8b 55 f4             	mov    -0xc(%ebp),%edx
801037b6:	83 c2 10             	add    $0x10,%edx
801037b9:	89 04 95 cc 39 11 80 	mov    %eax,-0x7feec634(,%edx,4)
  if (i == log.lh.n)
801037c0:	a1 08 3a 11 80       	mov    0x80113a08,%eax
801037c5:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801037c8:	75 0b                	jne    801037d5 <log_write+0xa6>
    log.lh.n++;
801037ca:	a1 08 3a 11 80       	mov    0x80113a08,%eax
801037cf:	40                   	inc    %eax
801037d0:	a3 08 3a 11 80       	mov    %eax,0x80113a08
  b->flags |= B_DIRTY; // prevent eviction
801037d5:	8b 45 08             	mov    0x8(%ebp),%eax
801037d8:	8b 00                	mov    (%eax),%eax
801037da:	83 c8 04             	or     $0x4,%eax
801037dd:	89 c2                	mov    %eax,%edx
801037df:	8b 45 08             	mov    0x8(%ebp),%eax
801037e2:	89 10                	mov    %edx,(%eax)
  release(&log.lock);
801037e4:	c7 04 24 c0 39 11 80 	movl   $0x801139c0,(%esp)
801037eb:	e8 ad 16 00 00       	call   80104e9d <release>
}
801037f0:	c9                   	leave  
801037f1:	c3                   	ret    
	...

801037f4 <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801037f4:	55                   	push   %ebp
801037f5:	89 e5                	mov    %esp,%ebp
801037f7:	83 ec 10             	sub    $0x10,%esp
  uint result;

  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801037fa:	8b 55 08             	mov    0x8(%ebp),%edx
801037fd:	8b 45 0c             	mov    0xc(%ebp),%eax
80103800:	8b 4d 08             	mov    0x8(%ebp),%ecx
80103803:	f0 87 02             	lock xchg %eax,(%edx)
80103806:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80103809:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010380c:	c9                   	leave  
8010380d:	c3                   	ret    

8010380e <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
8010380e:	55                   	push   %ebp
8010380f:	89 e5                	mov    %esp,%ebp
80103811:	83 e4 f0             	and    $0xfffffff0,%esp
80103814:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103817:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
8010381e:	80 
8010381f:	c7 04 24 e8 67 11 80 	movl   $0x801167e8,(%esp)
80103826:	e8 0d f3 ff ff       	call   80102b38 <kinit1>
  kvmalloc();      // kernel page table
8010382b:	e8 c7 41 00 00       	call   801079f7 <kvmalloc>
  mpinit();        // detect other processors
80103830:	e8 c4 03 00 00       	call   80103bf9 <mpinit>
  lapicinit();     // interrupt controller
80103835:	e8 4e f6 ff ff       	call   80102e88 <lapicinit>
  seginit();       // segment descriptors
8010383a:	e8 a0 3c 00 00       	call   801074df <seginit>
  picinit();       // disable pic
8010383f:	e8 04 05 00 00       	call   80103d48 <picinit>
  ioapicinit();    // another interrupt controller
80103844:	e8 0c f2 ff ff       	call   80102a55 <ioapicinit>
  consoleinit();   // console hardware
80103849:	e8 76 d3 ff ff       	call   80100bc4 <consoleinit>
  uartinit();      // serial port
8010384e:	e8 18 30 00 00       	call   8010686b <uartinit>
  pinit();         // process table
80103853:	e8 e6 08 00 00       	call   8010413e <pinit>
  tvinit();        // trap vectors
80103858:	e8 f7 2b 00 00       	call   80106454 <tvinit>
  binit();         // buffer cache
8010385d:	e8 d2 c7 ff ff       	call   80100034 <binit>
  fileinit();      // file table
80103862:	e8 ed d7 ff ff       	call   80101054 <fileinit>
  ideinit();       // disk 
80103867:	e8 f5 ed ff ff       	call   80102661 <ideinit>
  startothers();   // start other processors
8010386c:	e8 83 00 00 00       	call   801038f4 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103871:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
80103878:	8e 
80103879:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
80103880:	e8 eb f2 ff ff       	call   80102b70 <kinit2>
  userinit();      // first user process
80103885:	e8 c1 0a 00 00       	call   8010434b <userinit>
  mpmain();        // finish this processor's setup
8010388a:	e8 1a 00 00 00       	call   801038a9 <mpmain>

8010388f <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
8010388f:	55                   	push   %ebp
80103890:	89 e5                	mov    %esp,%ebp
80103892:	83 ec 08             	sub    $0x8,%esp
  switchkvm();
80103895:	e8 74 41 00 00       	call   80107a0e <switchkvm>
  seginit();
8010389a:	e8 40 3c 00 00       	call   801074df <seginit>
  lapicinit();
8010389f:	e8 e4 f5 ff ff       	call   80102e88 <lapicinit>
  mpmain();
801038a4:	e8 00 00 00 00       	call   801038a9 <mpmain>

801038a9 <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801038a9:	55                   	push   %ebp
801038aa:	89 e5                	mov    %esp,%ebp
801038ac:	53                   	push   %ebx
801038ad:	83 ec 14             	sub    $0x14,%esp
  cprintf("cpu%d: starting %d\n", cpuid(), cpuid());
801038b0:	e8 a5 08 00 00       	call   8010415a <cpuid>
801038b5:	89 c3                	mov    %eax,%ebx
801038b7:	e8 9e 08 00 00       	call   8010415a <cpuid>
801038bc:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801038c0:	89 44 24 04          	mov    %eax,0x4(%esp)
801038c4:	c7 04 24 bd 84 10 80 	movl   $0x801084bd,(%esp)
801038cb:	e8 f1 ca ff ff       	call   801003c1 <cprintf>
  idtinit();       // load idt register
801038d0:	e8 dc 2c 00 00       	call   801065b1 <idtinit>
  xchg(&(mycpu()->started), 1); // tell startothers() we're up
801038d5:	e8 c5 08 00 00       	call   8010419f <mycpu>
801038da:	05 a0 00 00 00       	add    $0xa0,%eax
801038df:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801038e6:	00 
801038e7:	89 04 24             	mov    %eax,(%esp)
801038ea:	e8 05 ff ff ff       	call   801037f4 <xchg>
  scheduler();     // start running processes
801038ef:	e8 a6 0f 00 00       	call   8010489a <scheduler>

801038f4 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
801038f4:	55                   	push   %ebp
801038f5:	89 e5                	mov    %esp,%ebp
801038f7:	83 ec 28             	sub    $0x28,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = P2V(0x7000);
801038fa:	c7 45 f0 00 70 00 80 	movl   $0x80007000,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
80103901:	b8 8a 00 00 00       	mov    $0x8a,%eax
80103906:	89 44 24 08          	mov    %eax,0x8(%esp)
8010390a:	c7 44 24 04 ec b4 10 	movl   $0x8010b4ec,0x4(%esp)
80103911:	80 
80103912:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103915:	89 04 24             	mov    %eax,(%esp)
80103918:	e8 42 18 00 00       	call   8010515f <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
8010391d:	c7 45 f4 c0 3a 11 80 	movl   $0x80113ac0,-0xc(%ebp)
80103924:	eb 75                	jmp    8010399b <startothers+0xa7>
    if(c == mycpu())  // We've started already.
80103926:	e8 74 08 00 00       	call   8010419f <mycpu>
8010392b:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010392e:	75 02                	jne    80103932 <startothers+0x3e>
      continue;
80103930:	eb 62                	jmp    80103994 <startothers+0xa0>

    // Tell entryother.S what stack to use, where to enter, and what
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103932:	e8 2c f3 ff ff       	call   80102c63 <kalloc>
80103937:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
8010393a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010393d:	83 e8 04             	sub    $0x4,%eax
80103940:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103943:	81 c2 00 10 00 00    	add    $0x1000,%edx
80103949:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
8010394b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010394e:	83 e8 08             	sub    $0x8,%eax
80103951:	c7 00 8f 38 10 80    	movl   $0x8010388f,(%eax)
    *(int**)(code-12) = (void *) V2P(entrypgdir);
80103957:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010395a:	8d 50 f4             	lea    -0xc(%eax),%edx
8010395d:	b8 00 a0 10 80       	mov    $0x8010a000,%eax
80103962:	05 00 00 00 80       	add    $0x80000000,%eax
80103967:	89 02                	mov    %eax,(%edx)

    lapicstartap(c->apicid, V2P(code));
80103969:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010396c:	8d 90 00 00 00 80    	lea    -0x80000000(%eax),%edx
80103972:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103975:	8a 00                	mov    (%eax),%al
80103977:	0f b6 c0             	movzbl %al,%eax
8010397a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010397e:	89 04 24             	mov    %eax,(%esp)
80103981:	e8 a7 f6 ff ff       	call   8010302d <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
80103986:	90                   	nop
80103987:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010398a:	8b 80 a0 00 00 00    	mov    0xa0(%eax),%eax
80103990:	85 c0                	test   %eax,%eax
80103992:	74 f3                	je     80103987 <startothers+0x93>
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = P2V(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
80103994:	81 45 f4 b0 00 00 00 	addl   $0xb0,-0xc(%ebp)
8010399b:	a1 40 40 11 80       	mov    0x80114040,%eax
801039a0:	89 c2                	mov    %eax,%edx
801039a2:	89 d0                	mov    %edx,%eax
801039a4:	c1 e0 02             	shl    $0x2,%eax
801039a7:	01 d0                	add    %edx,%eax
801039a9:	01 c0                	add    %eax,%eax
801039ab:	01 d0                	add    %edx,%eax
801039ad:	c1 e0 04             	shl    $0x4,%eax
801039b0:	05 c0 3a 11 80       	add    $0x80113ac0,%eax
801039b5:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801039b8:	0f 87 68 ff ff ff    	ja     80103926 <startothers+0x32>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801039be:	c9                   	leave  
801039bf:	c3                   	ret    

801039c0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801039c0:	55                   	push   %ebp
801039c1:	89 e5                	mov    %esp,%ebp
801039c3:	83 ec 14             	sub    $0x14,%esp
801039c6:	8b 45 08             	mov    0x8(%ebp),%eax
801039c9:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801039cd:	8b 45 ec             	mov    -0x14(%ebp),%eax
801039d0:	89 c2                	mov    %eax,%edx
801039d2:	ec                   	in     (%dx),%al
801039d3:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
801039d6:	8a 45 ff             	mov    -0x1(%ebp),%al
}
801039d9:	c9                   	leave  
801039da:	c3                   	ret    

801039db <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801039db:	55                   	push   %ebp
801039dc:	89 e5                	mov    %esp,%ebp
801039de:	83 ec 08             	sub    $0x8,%esp
801039e1:	8b 45 08             	mov    0x8(%ebp),%eax
801039e4:	8b 55 0c             	mov    0xc(%ebp),%edx
801039e7:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
801039eb:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801039ee:	8a 45 f8             	mov    -0x8(%ebp),%al
801039f1:	8b 55 fc             	mov    -0x4(%ebp),%edx
801039f4:	ee                   	out    %al,(%dx)
}
801039f5:	c9                   	leave  
801039f6:	c3                   	ret    

801039f7 <sum>:
int ncpu;
uchar ioapicid;

static uchar
sum(uchar *addr, int len)
{
801039f7:	55                   	push   %ebp
801039f8:	89 e5                	mov    %esp,%ebp
801039fa:	83 ec 10             	sub    $0x10,%esp
  int i, sum;

  sum = 0;
801039fd:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
80103a04:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103a0b:	eb 13                	jmp    80103a20 <sum+0x29>
    sum += addr[i];
80103a0d:	8b 55 fc             	mov    -0x4(%ebp),%edx
80103a10:	8b 45 08             	mov    0x8(%ebp),%eax
80103a13:	01 d0                	add    %edx,%eax
80103a15:	8a 00                	mov    (%eax),%al
80103a17:	0f b6 c0             	movzbl %al,%eax
80103a1a:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;

  sum = 0;
  for(i=0; i<len; i++)
80103a1d:	ff 45 fc             	incl   -0x4(%ebp)
80103a20:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103a23:	3b 45 0c             	cmp    0xc(%ebp),%eax
80103a26:	7c e5                	jl     80103a0d <sum+0x16>
    sum += addr[i];
  return sum;
80103a28:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103a2b:	c9                   	leave  
80103a2c:	c3                   	ret    

80103a2d <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
80103a2d:	55                   	push   %ebp
80103a2e:	89 e5                	mov    %esp,%ebp
80103a30:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = P2V(a);
80103a33:	8b 45 08             	mov    0x8(%ebp),%eax
80103a36:	05 00 00 00 80       	add    $0x80000000,%eax
80103a3b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
80103a3e:	8b 55 0c             	mov    0xc(%ebp),%edx
80103a41:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103a44:	01 d0                	add    %edx,%eax
80103a46:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
80103a49:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103a4c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103a4f:	eb 3f                	jmp    80103a90 <mpsearch1+0x63>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
80103a51:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103a58:	00 
80103a59:	c7 44 24 04 d4 84 10 	movl   $0x801084d4,0x4(%esp)
80103a60:	80 
80103a61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a64:	89 04 24             	mov    %eax,(%esp)
80103a67:	e8 a1 16 00 00       	call   8010510d <memcmp>
80103a6c:	85 c0                	test   %eax,%eax
80103a6e:	75 1c                	jne    80103a8c <mpsearch1+0x5f>
80103a70:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
80103a77:	00 
80103a78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a7b:	89 04 24             	mov    %eax,(%esp)
80103a7e:	e8 74 ff ff ff       	call   801039f7 <sum>
80103a83:	84 c0                	test   %al,%al
80103a85:	75 05                	jne    80103a8c <mpsearch1+0x5f>
      return (struct mp*)p;
80103a87:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a8a:	eb 11                	jmp    80103a9d <mpsearch1+0x70>
{
  uchar *e, *p, *addr;

  addr = P2V(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
80103a8c:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80103a90:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103a93:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80103a96:	72 b9                	jb     80103a51 <mpsearch1+0x24>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
80103a98:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103a9d:	c9                   	leave  
80103a9e:	c3                   	ret    

80103a9f <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
80103a9f:	55                   	push   %ebp
80103aa0:	89 e5                	mov    %esp,%ebp
80103aa2:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
80103aa5:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103aac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103aaf:	83 c0 0f             	add    $0xf,%eax
80103ab2:	8a 00                	mov    (%eax),%al
80103ab4:	0f b6 c0             	movzbl %al,%eax
80103ab7:	c1 e0 08             	shl    $0x8,%eax
80103aba:	89 c2                	mov    %eax,%edx
80103abc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103abf:	83 c0 0e             	add    $0xe,%eax
80103ac2:	8a 00                	mov    (%eax),%al
80103ac4:	0f b6 c0             	movzbl %al,%eax
80103ac7:	09 d0                	or     %edx,%eax
80103ac9:	c1 e0 04             	shl    $0x4,%eax
80103acc:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103acf:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103ad3:	74 21                	je     80103af6 <mpsearch+0x57>
    if((mp = mpsearch1(p, 1024)))
80103ad5:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103adc:	00 
80103add:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103ae0:	89 04 24             	mov    %eax,(%esp)
80103ae3:	e8 45 ff ff ff       	call   80103a2d <mpsearch1>
80103ae8:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103aeb:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103aef:	74 4e                	je     80103b3f <mpsearch+0xa0>
      return mp;
80103af1:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103af4:	eb 5d                	jmp    80103b53 <mpsearch+0xb4>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
80103af6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103af9:	83 c0 14             	add    $0x14,%eax
80103afc:	8a 00                	mov    (%eax),%al
80103afe:	0f b6 c0             	movzbl %al,%eax
80103b01:	c1 e0 08             	shl    $0x8,%eax
80103b04:	89 c2                	mov    %eax,%edx
80103b06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103b09:	83 c0 13             	add    $0x13,%eax
80103b0c:	8a 00                	mov    (%eax),%al
80103b0e:	0f b6 c0             	movzbl %al,%eax
80103b11:	09 d0                	or     %edx,%eax
80103b13:	c1 e0 0a             	shl    $0xa,%eax
80103b16:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103b19:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b1c:	2d 00 04 00 00       	sub    $0x400,%eax
80103b21:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103b28:	00 
80103b29:	89 04 24             	mov    %eax,(%esp)
80103b2c:	e8 fc fe ff ff       	call   80103a2d <mpsearch1>
80103b31:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103b34:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103b38:	74 05                	je     80103b3f <mpsearch+0xa0>
      return mp;
80103b3a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103b3d:	eb 14                	jmp    80103b53 <mpsearch+0xb4>
  }
  return mpsearch1(0xF0000, 0x10000);
80103b3f:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80103b46:	00 
80103b47:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
80103b4e:	e8 da fe ff ff       	call   80103a2d <mpsearch1>
}
80103b53:	c9                   	leave  
80103b54:	c3                   	ret    

80103b55 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
80103b55:	55                   	push   %ebp
80103b56:	89 e5                	mov    %esp,%ebp
80103b58:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
80103b5b:	e8 3f ff ff ff       	call   80103a9f <mpsearch>
80103b60:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103b63:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103b67:	74 0a                	je     80103b73 <mpconfig+0x1e>
80103b69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103b6c:	8b 40 04             	mov    0x4(%eax),%eax
80103b6f:	85 c0                	test   %eax,%eax
80103b71:	75 07                	jne    80103b7a <mpconfig+0x25>
    return 0;
80103b73:	b8 00 00 00 00       	mov    $0x0,%eax
80103b78:	eb 7d                	jmp    80103bf7 <mpconfig+0xa2>
  conf = (struct mpconf*) P2V((uint) mp->physaddr);
80103b7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103b7d:	8b 40 04             	mov    0x4(%eax),%eax
80103b80:	05 00 00 00 80       	add    $0x80000000,%eax
80103b85:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80103b88:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103b8f:	00 
80103b90:	c7 44 24 04 d9 84 10 	movl   $0x801084d9,0x4(%esp)
80103b97:	80 
80103b98:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103b9b:	89 04 24             	mov    %eax,(%esp)
80103b9e:	e8 6a 15 00 00       	call   8010510d <memcmp>
80103ba3:	85 c0                	test   %eax,%eax
80103ba5:	74 07                	je     80103bae <mpconfig+0x59>
    return 0;
80103ba7:	b8 00 00 00 00       	mov    $0x0,%eax
80103bac:	eb 49                	jmp    80103bf7 <mpconfig+0xa2>
  if(conf->version != 1 && conf->version != 4)
80103bae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bb1:	8a 40 06             	mov    0x6(%eax),%al
80103bb4:	3c 01                	cmp    $0x1,%al
80103bb6:	74 11                	je     80103bc9 <mpconfig+0x74>
80103bb8:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bbb:	8a 40 06             	mov    0x6(%eax),%al
80103bbe:	3c 04                	cmp    $0x4,%al
80103bc0:	74 07                	je     80103bc9 <mpconfig+0x74>
    return 0;
80103bc2:	b8 00 00 00 00       	mov    $0x0,%eax
80103bc7:	eb 2e                	jmp    80103bf7 <mpconfig+0xa2>
  if(sum((uchar*)conf, conf->length) != 0)
80103bc9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bcc:	8b 40 04             	mov    0x4(%eax),%eax
80103bcf:	0f b7 c0             	movzwl %ax,%eax
80103bd2:	89 44 24 04          	mov    %eax,0x4(%esp)
80103bd6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103bd9:	89 04 24             	mov    %eax,(%esp)
80103bdc:	e8 16 fe ff ff       	call   801039f7 <sum>
80103be1:	84 c0                	test   %al,%al
80103be3:	74 07                	je     80103bec <mpconfig+0x97>
    return 0;
80103be5:	b8 00 00 00 00       	mov    $0x0,%eax
80103bea:	eb 0b                	jmp    80103bf7 <mpconfig+0xa2>
  *pmp = mp;
80103bec:	8b 45 08             	mov    0x8(%ebp),%eax
80103bef:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103bf2:	89 10                	mov    %edx,(%eax)
  return conf;
80103bf4:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80103bf7:	c9                   	leave  
80103bf8:	c3                   	ret    

80103bf9 <mpinit>:

void
mpinit(void)
{
80103bf9:	55                   	push   %ebp
80103bfa:	89 e5                	mov    %esp,%ebp
80103bfc:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  if((conf = mpconfig(&mp)) == 0)
80103bff:	8d 45 dc             	lea    -0x24(%ebp),%eax
80103c02:	89 04 24             	mov    %eax,(%esp)
80103c05:	e8 4b ff ff ff       	call   80103b55 <mpconfig>
80103c0a:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103c0d:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103c11:	75 0c                	jne    80103c1f <mpinit+0x26>
    panic("Expect to run on an SMP");
80103c13:	c7 04 24 de 84 10 80 	movl   $0x801084de,(%esp)
80103c1a:	e8 35 c9 ff ff       	call   80100554 <panic>
  ismp = 1;
80103c1f:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
  lapic = (uint*)conf->lapicaddr;
80103c26:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103c29:	8b 40 24             	mov    0x24(%eax),%eax
80103c2c:	a3 bc 39 11 80       	mov    %eax,0x801139bc
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103c31:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103c34:	83 c0 2c             	add    $0x2c,%eax
80103c37:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103c3a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103c3d:	8b 40 04             	mov    0x4(%eax),%eax
80103c40:	0f b7 d0             	movzwl %ax,%edx
80103c43:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103c46:	01 d0                	add    %edx,%eax
80103c48:	89 45 e8             	mov    %eax,-0x18(%ebp)
80103c4b:	eb 7d                	jmp    80103cca <mpinit+0xd1>
    switch(*p){
80103c4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c50:	8a 00                	mov    (%eax),%al
80103c52:	0f b6 c0             	movzbl %al,%eax
80103c55:	83 f8 04             	cmp    $0x4,%eax
80103c58:	77 68                	ja     80103cc2 <mpinit+0xc9>
80103c5a:	8b 04 85 18 85 10 80 	mov    -0x7fef7ae8(,%eax,4),%eax
80103c61:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
80103c63:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c66:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      if(ncpu < NCPU) {
80103c69:	a1 40 40 11 80       	mov    0x80114040,%eax
80103c6e:	83 f8 07             	cmp    $0x7,%eax
80103c71:	7f 2c                	jg     80103c9f <mpinit+0xa6>
        cpus[ncpu].apicid = proc->apicid;  // apicid may differ from ncpu
80103c73:	8b 15 40 40 11 80    	mov    0x80114040,%edx
80103c79:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103c7c:	8a 48 01             	mov    0x1(%eax),%cl
80103c7f:	89 d0                	mov    %edx,%eax
80103c81:	c1 e0 02             	shl    $0x2,%eax
80103c84:	01 d0                	add    %edx,%eax
80103c86:	01 c0                	add    %eax,%eax
80103c88:	01 d0                	add    %edx,%eax
80103c8a:	c1 e0 04             	shl    $0x4,%eax
80103c8d:	05 c0 3a 11 80       	add    $0x80113ac0,%eax
80103c92:	88 08                	mov    %cl,(%eax)
        ncpu++;
80103c94:	a1 40 40 11 80       	mov    0x80114040,%eax
80103c99:	40                   	inc    %eax
80103c9a:	a3 40 40 11 80       	mov    %eax,0x80114040
      }
      p += sizeof(struct mpproc);
80103c9f:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103ca3:	eb 25                	jmp    80103cca <mpinit+0xd1>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
80103ca5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ca8:	89 45 e0             	mov    %eax,-0x20(%ebp)
      ioapicid = ioapic->apicno;
80103cab:	8b 45 e0             	mov    -0x20(%ebp),%eax
80103cae:	8a 40 01             	mov    0x1(%eax),%al
80103cb1:	a2 a0 3a 11 80       	mov    %al,0x80113aa0
      p += sizeof(struct mpioapic);
80103cb6:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103cba:	eb 0e                	jmp    80103cca <mpinit+0xd1>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
80103cbc:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
80103cc0:	eb 08                	jmp    80103cca <mpinit+0xd1>
    default:
      ismp = 0;
80103cc2:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
      break;
80103cc9:	90                   	nop

  if((conf = mpconfig(&mp)) == 0)
    panic("Expect to run on an SMP");
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
80103cca:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ccd:	3b 45 e8             	cmp    -0x18(%ebp),%eax
80103cd0:	0f 82 77 ff ff ff    	jb     80103c4d <mpinit+0x54>
    default:
      ismp = 0;
      break;
    }
  }
  if(!ismp)
80103cd6:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80103cda:	75 0c                	jne    80103ce8 <mpinit+0xef>
    panic("Didn't find a suitable machine");
80103cdc:	c7 04 24 f8 84 10 80 	movl   $0x801084f8,(%esp)
80103ce3:	e8 6c c8 ff ff       	call   80100554 <panic>

  if(mp->imcrp){
80103ce8:	8b 45 dc             	mov    -0x24(%ebp),%eax
80103ceb:	8a 40 0c             	mov    0xc(%eax),%al
80103cee:	84 c0                	test   %al,%al
80103cf0:	74 36                	je     80103d28 <mpinit+0x12f>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80103cf2:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103cf9:	00 
80103cfa:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103d01:	e8 d5 fc ff ff       	call   801039db <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103d06:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103d0d:	e8 ae fc ff ff       	call   801039c0 <inb>
80103d12:	83 c8 01             	or     $0x1,%eax
80103d15:	0f b6 c0             	movzbl %al,%eax
80103d18:	89 44 24 04          	mov    %eax,0x4(%esp)
80103d1c:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103d23:	e8 b3 fc ff ff       	call   801039db <outb>
  }
}
80103d28:	c9                   	leave  
80103d29:	c3                   	ret    
	...

80103d2c <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80103d2c:	55                   	push   %ebp
80103d2d:	89 e5                	mov    %esp,%ebp
80103d2f:	83 ec 08             	sub    $0x8,%esp
80103d32:	8b 45 08             	mov    0x8(%ebp),%eax
80103d35:	8b 55 0c             	mov    0xc(%ebp),%edx
80103d38:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
80103d3c:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103d3f:	8a 45 f8             	mov    -0x8(%ebp),%al
80103d42:	8b 55 fc             	mov    -0x4(%ebp),%edx
80103d45:	ee                   	out    %al,(%dx)
}
80103d46:	c9                   	leave  
80103d47:	c3                   	ret    

80103d48 <picinit>:
#define IO_PIC2         0xA0    // Slave (IRQs 8-15)

// Don't use the 8259A interrupt controllers.  Xv6 assumes SMP hardware.
void
picinit(void)
{
80103d48:	55                   	push   %ebp
80103d49:	89 e5                	mov    %esp,%ebp
80103d4b:	83 ec 08             	sub    $0x8,%esp
  // mask all interrupts
  outb(IO_PIC1+1, 0xFF);
80103d4e:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103d55:	00 
80103d56:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103d5d:	e8 ca ff ff ff       	call   80103d2c <outb>
  outb(IO_PIC2+1, 0xFF);
80103d62:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103d69:	00 
80103d6a:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103d71:	e8 b6 ff ff ff       	call   80103d2c <outb>
}
80103d76:	c9                   	leave  
80103d77:	c3                   	ret    

80103d78 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103d78:	55                   	push   %ebp
80103d79:	89 e5                	mov    %esp,%ebp
80103d7b:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103d7e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103d85:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d88:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103d8e:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d91:	8b 10                	mov    (%eax),%edx
80103d93:	8b 45 08             	mov    0x8(%ebp),%eax
80103d96:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103d98:	e8 d3 d2 ff ff       	call   80101070 <filealloc>
80103d9d:	8b 55 08             	mov    0x8(%ebp),%edx
80103da0:	89 02                	mov    %eax,(%edx)
80103da2:	8b 45 08             	mov    0x8(%ebp),%eax
80103da5:	8b 00                	mov    (%eax),%eax
80103da7:	85 c0                	test   %eax,%eax
80103da9:	0f 84 c8 00 00 00    	je     80103e77 <pipealloc+0xff>
80103daf:	e8 bc d2 ff ff       	call   80101070 <filealloc>
80103db4:	8b 55 0c             	mov    0xc(%ebp),%edx
80103db7:	89 02                	mov    %eax,(%edx)
80103db9:	8b 45 0c             	mov    0xc(%ebp),%eax
80103dbc:	8b 00                	mov    (%eax),%eax
80103dbe:	85 c0                	test   %eax,%eax
80103dc0:	0f 84 b1 00 00 00    	je     80103e77 <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103dc6:	e8 98 ee ff ff       	call   80102c63 <kalloc>
80103dcb:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103dce:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103dd2:	75 05                	jne    80103dd9 <pipealloc+0x61>
    goto bad;
80103dd4:	e9 9e 00 00 00       	jmp    80103e77 <pipealloc+0xff>
  p->readopen = 1;
80103dd9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ddc:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103de3:	00 00 00 
  p->writeopen = 1;
80103de6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103de9:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103df0:	00 00 00 
  p->nwrite = 0;
80103df3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103df6:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103dfd:	00 00 00 
  p->nread = 0;
80103e00:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103e03:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103e0a:	00 00 00 
  initlock(&p->lock, "pipe");
80103e0d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103e10:	c7 44 24 04 2c 85 10 	movl   $0x8010852c,0x4(%esp)
80103e17:	80 
80103e18:	89 04 24             	mov    %eax,(%esp)
80103e1b:	e8 f2 0f 00 00       	call   80104e12 <initlock>
  (*f0)->type = FD_PIPE;
80103e20:	8b 45 08             	mov    0x8(%ebp),%eax
80103e23:	8b 00                	mov    (%eax),%eax
80103e25:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103e2b:	8b 45 08             	mov    0x8(%ebp),%eax
80103e2e:	8b 00                	mov    (%eax),%eax
80103e30:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103e34:	8b 45 08             	mov    0x8(%ebp),%eax
80103e37:	8b 00                	mov    (%eax),%eax
80103e39:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103e3d:	8b 45 08             	mov    0x8(%ebp),%eax
80103e40:	8b 00                	mov    (%eax),%eax
80103e42:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103e45:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103e48:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e4b:	8b 00                	mov    (%eax),%eax
80103e4d:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103e53:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e56:	8b 00                	mov    (%eax),%eax
80103e58:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103e5c:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e5f:	8b 00                	mov    (%eax),%eax
80103e61:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103e65:	8b 45 0c             	mov    0xc(%ebp),%eax
80103e68:	8b 00                	mov    (%eax),%eax
80103e6a:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103e6d:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103e70:	b8 00 00 00 00       	mov    $0x0,%eax
80103e75:	eb 42                	jmp    80103eb9 <pipealloc+0x141>

//PAGEBREAK: 20
 bad:
  if(p)
80103e77:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103e7b:	74 0b                	je     80103e88 <pipealloc+0x110>
    kfree((char*)p);
80103e7d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103e80:	89 04 24             	mov    %eax,(%esp)
80103e83:	e8 45 ed ff ff       	call   80102bcd <kfree>
  if(*f0)
80103e88:	8b 45 08             	mov    0x8(%ebp),%eax
80103e8b:	8b 00                	mov    (%eax),%eax
80103e8d:	85 c0                	test   %eax,%eax
80103e8f:	74 0d                	je     80103e9e <pipealloc+0x126>
    fileclose(*f0);
80103e91:	8b 45 08             	mov    0x8(%ebp),%eax
80103e94:	8b 00                	mov    (%eax),%eax
80103e96:	89 04 24             	mov    %eax,(%esp)
80103e99:	e8 7a d2 ff ff       	call   80101118 <fileclose>
  if(*f1)
80103e9e:	8b 45 0c             	mov    0xc(%ebp),%eax
80103ea1:	8b 00                	mov    (%eax),%eax
80103ea3:	85 c0                	test   %eax,%eax
80103ea5:	74 0d                	je     80103eb4 <pipealloc+0x13c>
    fileclose(*f1);
80103ea7:	8b 45 0c             	mov    0xc(%ebp),%eax
80103eaa:	8b 00                	mov    (%eax),%eax
80103eac:	89 04 24             	mov    %eax,(%esp)
80103eaf:	e8 64 d2 ff ff       	call   80101118 <fileclose>
  return -1;
80103eb4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103eb9:	c9                   	leave  
80103eba:	c3                   	ret    

80103ebb <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103ebb:	55                   	push   %ebp
80103ebc:	89 e5                	mov    %esp,%ebp
80103ebe:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103ec1:	8b 45 08             	mov    0x8(%ebp),%eax
80103ec4:	89 04 24             	mov    %eax,(%esp)
80103ec7:	e8 67 0f 00 00       	call   80104e33 <acquire>
  if(writable){
80103ecc:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103ed0:	74 1f                	je     80103ef1 <pipeclose+0x36>
    p->writeopen = 0;
80103ed2:	8b 45 08             	mov    0x8(%ebp),%eax
80103ed5:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103edc:	00 00 00 
    wakeup(&p->nread);
80103edf:	8b 45 08             	mov    0x8(%ebp),%eax
80103ee2:	05 34 02 00 00       	add    $0x234,%eax
80103ee7:	89 04 24             	mov    %eax,(%esp)
80103eea:	e8 49 0c 00 00       	call   80104b38 <wakeup>
80103eef:	eb 1d                	jmp    80103f0e <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103ef1:	8b 45 08             	mov    0x8(%ebp),%eax
80103ef4:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103efb:	00 00 00 
    wakeup(&p->nwrite);
80103efe:	8b 45 08             	mov    0x8(%ebp),%eax
80103f01:	05 38 02 00 00       	add    $0x238,%eax
80103f06:	89 04 24             	mov    %eax,(%esp)
80103f09:	e8 2a 0c 00 00       	call   80104b38 <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103f0e:	8b 45 08             	mov    0x8(%ebp),%eax
80103f11:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103f17:	85 c0                	test   %eax,%eax
80103f19:	75 25                	jne    80103f40 <pipeclose+0x85>
80103f1b:	8b 45 08             	mov    0x8(%ebp),%eax
80103f1e:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103f24:	85 c0                	test   %eax,%eax
80103f26:	75 18                	jne    80103f40 <pipeclose+0x85>
    release(&p->lock);
80103f28:	8b 45 08             	mov    0x8(%ebp),%eax
80103f2b:	89 04 24             	mov    %eax,(%esp)
80103f2e:	e8 6a 0f 00 00       	call   80104e9d <release>
    kfree((char*)p);
80103f33:	8b 45 08             	mov    0x8(%ebp),%eax
80103f36:	89 04 24             	mov    %eax,(%esp)
80103f39:	e8 8f ec ff ff       	call   80102bcd <kfree>
80103f3e:	eb 0b                	jmp    80103f4b <pipeclose+0x90>
  } else
    release(&p->lock);
80103f40:	8b 45 08             	mov    0x8(%ebp),%eax
80103f43:	89 04 24             	mov    %eax,(%esp)
80103f46:	e8 52 0f 00 00       	call   80104e9d <release>
}
80103f4b:	c9                   	leave  
80103f4c:	c3                   	ret    

80103f4d <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103f4d:	55                   	push   %ebp
80103f4e:	89 e5                	mov    %esp,%ebp
80103f50:	83 ec 28             	sub    $0x28,%esp
  int i;

  acquire(&p->lock);
80103f53:	8b 45 08             	mov    0x8(%ebp),%eax
80103f56:	89 04 24             	mov    %eax,(%esp)
80103f59:	e8 d5 0e 00 00       	call   80104e33 <acquire>
  for(i = 0; i < n; i++){
80103f5e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103f65:	e9 a3 00 00 00       	jmp    8010400d <pipewrite+0xc0>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103f6a:	eb 56                	jmp    80103fc2 <pipewrite+0x75>
      if(p->readopen == 0 || myproc()->killed){
80103f6c:	8b 45 08             	mov    0x8(%ebp),%eax
80103f6f:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103f75:	85 c0                	test   %eax,%eax
80103f77:	74 0c                	je     80103f85 <pipewrite+0x38>
80103f79:	e8 a5 02 00 00       	call   80104223 <myproc>
80103f7e:	8b 40 24             	mov    0x24(%eax),%eax
80103f81:	85 c0                	test   %eax,%eax
80103f83:	74 15                	je     80103f9a <pipewrite+0x4d>
        release(&p->lock);
80103f85:	8b 45 08             	mov    0x8(%ebp),%eax
80103f88:	89 04 24             	mov    %eax,(%esp)
80103f8b:	e8 0d 0f 00 00       	call   80104e9d <release>
        return -1;
80103f90:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103f95:	e9 9d 00 00 00       	jmp    80104037 <pipewrite+0xea>
      }
      wakeup(&p->nread);
80103f9a:	8b 45 08             	mov    0x8(%ebp),%eax
80103f9d:	05 34 02 00 00       	add    $0x234,%eax
80103fa2:	89 04 24             	mov    %eax,(%esp)
80103fa5:	e8 8e 0b 00 00       	call   80104b38 <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103faa:	8b 45 08             	mov    0x8(%ebp),%eax
80103fad:	8b 55 08             	mov    0x8(%ebp),%edx
80103fb0:	81 c2 38 02 00 00    	add    $0x238,%edx
80103fb6:	89 44 24 04          	mov    %eax,0x4(%esp)
80103fba:	89 14 24             	mov    %edx,(%esp)
80103fbd:	e8 a2 0a 00 00       	call   80104a64 <sleep>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103fc2:	8b 45 08             	mov    0x8(%ebp),%eax
80103fc5:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103fcb:	8b 45 08             	mov    0x8(%ebp),%eax
80103fce:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103fd4:	05 00 02 00 00       	add    $0x200,%eax
80103fd9:	39 c2                	cmp    %eax,%edx
80103fdb:	74 8f                	je     80103f6c <pipewrite+0x1f>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103fdd:	8b 45 08             	mov    0x8(%ebp),%eax
80103fe0:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103fe6:	8d 48 01             	lea    0x1(%eax),%ecx
80103fe9:	8b 55 08             	mov    0x8(%ebp),%edx
80103fec:	89 8a 38 02 00 00    	mov    %ecx,0x238(%edx)
80103ff2:	25 ff 01 00 00       	and    $0x1ff,%eax
80103ff7:	89 c1                	mov    %eax,%ecx
80103ff9:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103ffc:	8b 45 0c             	mov    0xc(%ebp),%eax
80103fff:	01 d0                	add    %edx,%eax
80104001:	8a 10                	mov    (%eax),%dl
80104003:	8b 45 08             	mov    0x8(%ebp),%eax
80104006:	88 54 08 34          	mov    %dl,0x34(%eax,%ecx,1)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
8010400a:	ff 45 f4             	incl   -0xc(%ebp)
8010400d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104010:	3b 45 10             	cmp    0x10(%ebp),%eax
80104013:	0f 8c 51 ff ff ff    	jl     80103f6a <pipewrite+0x1d>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80104019:	8b 45 08             	mov    0x8(%ebp),%eax
8010401c:	05 34 02 00 00       	add    $0x234,%eax
80104021:	89 04 24             	mov    %eax,(%esp)
80104024:	e8 0f 0b 00 00       	call   80104b38 <wakeup>
  release(&p->lock);
80104029:	8b 45 08             	mov    0x8(%ebp),%eax
8010402c:	89 04 24             	mov    %eax,(%esp)
8010402f:	e8 69 0e 00 00       	call   80104e9d <release>
  return n;
80104034:	8b 45 10             	mov    0x10(%ebp),%eax
}
80104037:	c9                   	leave  
80104038:	c3                   	ret    

80104039 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80104039:	55                   	push   %ebp
8010403a:	89 e5                	mov    %esp,%ebp
8010403c:	53                   	push   %ebx
8010403d:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80104040:	8b 45 08             	mov    0x8(%ebp),%eax
80104043:	89 04 24             	mov    %eax,(%esp)
80104046:	e8 e8 0d 00 00       	call   80104e33 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
8010404b:	eb 39                	jmp    80104086 <piperead+0x4d>
    if(myproc()->killed){
8010404d:	e8 d1 01 00 00       	call   80104223 <myproc>
80104052:	8b 40 24             	mov    0x24(%eax),%eax
80104055:	85 c0                	test   %eax,%eax
80104057:	74 15                	je     8010406e <piperead+0x35>
      release(&p->lock);
80104059:	8b 45 08             	mov    0x8(%ebp),%eax
8010405c:	89 04 24             	mov    %eax,(%esp)
8010405f:	e8 39 0e 00 00       	call   80104e9d <release>
      return -1;
80104064:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104069:	e9 b3 00 00 00       	jmp    80104121 <piperead+0xe8>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
8010406e:	8b 45 08             	mov    0x8(%ebp),%eax
80104071:	8b 55 08             	mov    0x8(%ebp),%edx
80104074:	81 c2 34 02 00 00    	add    $0x234,%edx
8010407a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010407e:	89 14 24             	mov    %edx,(%esp)
80104081:	e8 de 09 00 00       	call   80104a64 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80104086:	8b 45 08             	mov    0x8(%ebp),%eax
80104089:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
8010408f:	8b 45 08             	mov    0x8(%ebp),%eax
80104092:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80104098:	39 c2                	cmp    %eax,%edx
8010409a:	75 0d                	jne    801040a9 <piperead+0x70>
8010409c:	8b 45 08             	mov    0x8(%ebp),%eax
8010409f:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
801040a5:	85 c0                	test   %eax,%eax
801040a7:	75 a4                	jne    8010404d <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
801040a9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801040b0:	eb 49                	jmp    801040fb <piperead+0xc2>
    if(p->nread == p->nwrite)
801040b2:	8b 45 08             	mov    0x8(%ebp),%eax
801040b5:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
801040bb:	8b 45 08             	mov    0x8(%ebp),%eax
801040be:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
801040c4:	39 c2                	cmp    %eax,%edx
801040c6:	75 02                	jne    801040ca <piperead+0x91>
      break;
801040c8:	eb 39                	jmp    80104103 <piperead+0xca>
    addr[i] = p->data[p->nread++ % PIPESIZE];
801040ca:	8b 55 f4             	mov    -0xc(%ebp),%edx
801040cd:	8b 45 0c             	mov    0xc(%ebp),%eax
801040d0:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
801040d3:	8b 45 08             	mov    0x8(%ebp),%eax
801040d6:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
801040dc:	8d 48 01             	lea    0x1(%eax),%ecx
801040df:	8b 55 08             	mov    0x8(%ebp),%edx
801040e2:	89 8a 34 02 00 00    	mov    %ecx,0x234(%edx)
801040e8:	25 ff 01 00 00       	and    $0x1ff,%eax
801040ed:	89 c2                	mov    %eax,%edx
801040ef:	8b 45 08             	mov    0x8(%ebp),%eax
801040f2:	8a 44 10 34          	mov    0x34(%eax,%edx,1),%al
801040f6:	88 03                	mov    %al,(%ebx)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
801040f8:	ff 45 f4             	incl   -0xc(%ebp)
801040fb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040fe:	3b 45 10             	cmp    0x10(%ebp),%eax
80104101:	7c af                	jl     801040b2 <piperead+0x79>
    if(p->nread == p->nwrite)
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80104103:	8b 45 08             	mov    0x8(%ebp),%eax
80104106:	05 38 02 00 00       	add    $0x238,%eax
8010410b:	89 04 24             	mov    %eax,(%esp)
8010410e:	e8 25 0a 00 00       	call   80104b38 <wakeup>
  release(&p->lock);
80104113:	8b 45 08             	mov    0x8(%ebp),%eax
80104116:	89 04 24             	mov    %eax,(%esp)
80104119:	e8 7f 0d 00 00       	call   80104e9d <release>
  return i;
8010411e:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104121:	83 c4 24             	add    $0x24,%esp
80104124:	5b                   	pop    %ebx
80104125:	5d                   	pop    %ebp
80104126:	c3                   	ret    
	...

80104128 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104128:	55                   	push   %ebp
80104129:	89 e5                	mov    %esp,%ebp
8010412b:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
8010412e:	9c                   	pushf  
8010412f:	58                   	pop    %eax
80104130:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80104133:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104136:	c9                   	leave  
80104137:	c3                   	ret    

80104138 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
80104138:	55                   	push   %ebp
80104139:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
8010413b:	fb                   	sti    
}
8010413c:	5d                   	pop    %ebp
8010413d:	c3                   	ret    

8010413e <pinit>:

static void wakeup1(void *chan);

void
pinit(void)
{
8010413e:	55                   	push   %ebp
8010413f:	89 e5                	mov    %esp,%ebp
80104141:	83 ec 18             	sub    $0x18,%esp
  initlock(&ptable.lock, "ptable");
80104144:	c7 44 24 04 34 85 10 	movl   $0x80108534,0x4(%esp)
8010414b:	80 
8010414c:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104153:	e8 ba 0c 00 00       	call   80104e12 <initlock>
}
80104158:	c9                   	leave  
80104159:	c3                   	ret    

8010415a <cpuid>:

// Must be called with interrupts disabled
int
cpuid() {
8010415a:	55                   	push   %ebp
8010415b:	89 e5                	mov    %esp,%ebp
8010415d:	83 ec 08             	sub    $0x8,%esp
  return mycpu()-cpus;
80104160:	e8 3a 00 00 00       	call   8010419f <mycpu>
80104165:	89 c2                	mov    %eax,%edx
80104167:	b8 c0 3a 11 80       	mov    $0x80113ac0,%eax
8010416c:	29 c2                	sub    %eax,%edx
8010416e:	89 d0                	mov    %edx,%eax
80104170:	c1 f8 04             	sar    $0x4,%eax
80104173:	89 c1                	mov    %eax,%ecx
80104175:	89 ca                	mov    %ecx,%edx
80104177:	c1 e2 03             	shl    $0x3,%edx
8010417a:	01 ca                	add    %ecx,%edx
8010417c:	89 d0                	mov    %edx,%eax
8010417e:	c1 e0 05             	shl    $0x5,%eax
80104181:	29 d0                	sub    %edx,%eax
80104183:	c1 e0 02             	shl    $0x2,%eax
80104186:	01 c8                	add    %ecx,%eax
80104188:	c1 e0 03             	shl    $0x3,%eax
8010418b:	01 c8                	add    %ecx,%eax
8010418d:	89 c2                	mov    %eax,%edx
8010418f:	c1 e2 0f             	shl    $0xf,%edx
80104192:	29 c2                	sub    %eax,%edx
80104194:	c1 e2 02             	shl    $0x2,%edx
80104197:	01 ca                	add    %ecx,%edx
80104199:	89 d0                	mov    %edx,%eax
8010419b:	f7 d8                	neg    %eax
}
8010419d:	c9                   	leave  
8010419e:	c3                   	ret    

8010419f <mycpu>:

// Must be called with interrupts disabled to avoid the caller being
// rescheduled between reading lapicid and running through the loop.
struct cpu*
mycpu(void)
{
8010419f:	55                   	push   %ebp
801041a0:	89 e5                	mov    %esp,%ebp
801041a2:	83 ec 28             	sub    $0x28,%esp
  int apicid, i;
  
  if(readeflags()&FL_IF)
801041a5:	e8 7e ff ff ff       	call   80104128 <readeflags>
801041aa:	25 00 02 00 00       	and    $0x200,%eax
801041af:	85 c0                	test   %eax,%eax
801041b1:	74 0c                	je     801041bf <mycpu+0x20>
    panic("mycpu called with interrupts enabled\n");
801041b3:	c7 04 24 3c 85 10 80 	movl   $0x8010853c,(%esp)
801041ba:	e8 95 c3 ff ff       	call   80100554 <panic>
  
  apicid = lapicid();
801041bf:	e8 1d ee ff ff       	call   80102fe1 <lapicid>
801041c4:	89 45 f0             	mov    %eax,-0x10(%ebp)
  // APIC IDs are not guaranteed to be contiguous. Maybe we should have
  // a reverse map, or reserve a register to store &cpus[i].
  for (i = 0; i < ncpu; ++i) {
801041c7:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801041ce:	eb 3b                	jmp    8010420b <mycpu+0x6c>
    if (cpus[i].apicid == apicid)
801041d0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041d3:	89 d0                	mov    %edx,%eax
801041d5:	c1 e0 02             	shl    $0x2,%eax
801041d8:	01 d0                	add    %edx,%eax
801041da:	01 c0                	add    %eax,%eax
801041dc:	01 d0                	add    %edx,%eax
801041de:	c1 e0 04             	shl    $0x4,%eax
801041e1:	05 c0 3a 11 80       	add    $0x80113ac0,%eax
801041e6:	8a 00                	mov    (%eax),%al
801041e8:	0f b6 c0             	movzbl %al,%eax
801041eb:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801041ee:	75 18                	jne    80104208 <mycpu+0x69>
      return &cpus[i];
801041f0:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041f3:	89 d0                	mov    %edx,%eax
801041f5:	c1 e0 02             	shl    $0x2,%eax
801041f8:	01 d0                	add    %edx,%eax
801041fa:	01 c0                	add    %eax,%eax
801041fc:	01 d0                	add    %edx,%eax
801041fe:	c1 e0 04             	shl    $0x4,%eax
80104201:	05 c0 3a 11 80       	add    $0x80113ac0,%eax
80104206:	eb 19                	jmp    80104221 <mycpu+0x82>
    panic("mycpu called with interrupts enabled\n");
  
  apicid = lapicid();
  // APIC IDs are not guaranteed to be contiguous. Maybe we should have
  // a reverse map, or reserve a register to store &cpus[i].
  for (i = 0; i < ncpu; ++i) {
80104208:	ff 45 f4             	incl   -0xc(%ebp)
8010420b:	a1 40 40 11 80       	mov    0x80114040,%eax
80104210:	39 45 f4             	cmp    %eax,-0xc(%ebp)
80104213:	7c bb                	jl     801041d0 <mycpu+0x31>
    if (cpus[i].apicid == apicid)
      return &cpus[i];
  }
  panic("unknown apicid\n");
80104215:	c7 04 24 62 85 10 80 	movl   $0x80108562,(%esp)
8010421c:	e8 33 c3 ff ff       	call   80100554 <panic>
}
80104221:	c9                   	leave  
80104222:	c3                   	ret    

80104223 <myproc>:

// Disable interrupts so that we are not rescheduled
// while reading proc from the cpu structure
struct proc*
myproc(void) {
80104223:	55                   	push   %ebp
80104224:	89 e5                	mov    %esp,%ebp
80104226:	83 ec 18             	sub    $0x18,%esp
  struct cpu *c;
  struct proc *p;
  pushcli();
80104229:	e8 64 0d 00 00       	call   80104f92 <pushcli>
  c = mycpu();
8010422e:	e8 6c ff ff ff       	call   8010419f <mycpu>
80104233:	89 45 f4             	mov    %eax,-0xc(%ebp)
  p = c->proc;
80104236:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104239:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
8010423f:	89 45 f0             	mov    %eax,-0x10(%ebp)
  popcli();
80104242:	e8 95 0d 00 00       	call   80104fdc <popcli>
  return p;
80104247:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010424a:	c9                   	leave  
8010424b:	c3                   	ret    

8010424c <allocproc>:
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void)
{
8010424c:	55                   	push   %ebp
8010424d:	89 e5                	mov    %esp,%ebp
8010424f:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);
80104252:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104259:	e8 d5 0b 00 00       	call   80104e33 <acquire>

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
8010425e:	c7 45 f4 94 40 11 80 	movl   $0x80114094,-0xc(%ebp)
80104265:	eb 50                	jmp    801042b7 <allocproc+0x6b>
    if(p->state == UNUSED)
80104267:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010426a:	8b 40 0c             	mov    0xc(%eax),%eax
8010426d:	85 c0                	test   %eax,%eax
8010426f:	75 42                	jne    801042b3 <allocproc+0x67>
      goto found;
80104271:	90                   	nop

  release(&ptable.lock);
  return 0;

found:
  p->state = EMBRYO;
80104272:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104275:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
  p->pid = nextpid++;
8010427c:	a1 00 b0 10 80       	mov    0x8010b000,%eax
80104281:	8d 50 01             	lea    0x1(%eax),%edx
80104284:	89 15 00 b0 10 80    	mov    %edx,0x8010b000
8010428a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010428d:	89 42 10             	mov    %eax,0x10(%edx)

  release(&ptable.lock);
80104290:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104297:	e8 01 0c 00 00       	call   80104e9d <release>

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
8010429c:	e8 c2 e9 ff ff       	call   80102c63 <kalloc>
801042a1:	8b 55 f4             	mov    -0xc(%ebp),%edx
801042a4:	89 42 08             	mov    %eax,0x8(%edx)
801042a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042aa:	8b 40 08             	mov    0x8(%eax),%eax
801042ad:	85 c0                	test   %eax,%eax
801042af:	75 33                	jne    801042e4 <allocproc+0x98>
801042b1:	eb 20                	jmp    801042d3 <allocproc+0x87>
  struct proc *p;
  char *sp;

  acquire(&ptable.lock);

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801042b3:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
801042b7:	81 7d f4 94 5f 11 80 	cmpl   $0x80115f94,-0xc(%ebp)
801042be:	72 a7                	jb     80104267 <allocproc+0x1b>
    if(p->state == UNUSED)
      goto found;

  release(&ptable.lock);
801042c0:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
801042c7:	e8 d1 0b 00 00       	call   80104e9d <release>
  return 0;
801042cc:	b8 00 00 00 00       	mov    $0x0,%eax
801042d1:	eb 76                	jmp    80104349 <allocproc+0xfd>

  release(&ptable.lock);

  // Allocate kernel stack.
  if((p->kstack = kalloc()) == 0){
    p->state = UNUSED;
801042d3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042d6:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return 0;
801042dd:	b8 00 00 00 00       	mov    $0x0,%eax
801042e2:	eb 65                	jmp    80104349 <allocproc+0xfd>
  }
  sp = p->kstack + KSTACKSIZE;
801042e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042e7:	8b 40 08             	mov    0x8(%eax),%eax
801042ea:	05 00 10 00 00       	add    $0x1000,%eax
801042ef:	89 45 f0             	mov    %eax,-0x10(%ebp)

  // Leave room for trap frame.
  sp -= sizeof *p->tf;
801042f2:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
  p->tf = (struct trapframe*)sp;
801042f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801042f9:	8b 55 f0             	mov    -0x10(%ebp),%edx
801042fc:	89 50 18             	mov    %edx,0x18(%eax)

  // Set up new context to start executing at forkret,
  // which returns to trapret.
  sp -= 4;
801042ff:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
  *(uint*)sp = (uint)trapret;
80104303:	ba 10 64 10 80       	mov    $0x80106410,%edx
80104308:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010430b:	89 10                	mov    %edx,(%eax)

  sp -= sizeof *p->context;
8010430d:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
  p->context = (struct context*)sp;
80104311:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104314:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104317:	89 50 1c             	mov    %edx,0x1c(%eax)
  memset(p->context, 0, sizeof *p->context);
8010431a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010431d:	8b 40 1c             	mov    0x1c(%eax),%eax
80104320:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
80104327:	00 
80104328:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010432f:	00 
80104330:	89 04 24             	mov    %eax,(%esp)
80104333:	e8 5e 0d 00 00       	call   80105096 <memset>
  p->context->eip = (uint)forkret;
80104338:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010433b:	8b 40 1c             	mov    0x1c(%eax),%eax
8010433e:	ba 25 4a 10 80       	mov    $0x80104a25,%edx
80104343:	89 50 10             	mov    %edx,0x10(%eax)

  return p;
80104346:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104349:	c9                   	leave  
8010434a:	c3                   	ret    

8010434b <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void
userinit(void)
{
8010434b:	55                   	push   %ebp
8010434c:	89 e5                	mov    %esp,%ebp
8010434e:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  extern char _binary_initcode_start[], _binary_initcode_size[];

  p = allocproc();
80104351:	e8 f6 fe ff ff       	call   8010424c <allocproc>
80104356:	89 45 f4             	mov    %eax,-0xc(%ebp)
  
  initproc = p;
80104359:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010435c:	a3 20 b6 10 80       	mov    %eax,0x8010b620
  if((p->pgdir = setupkvm()) == 0)
80104361:	e8 e8 35 00 00       	call   8010794e <setupkvm>
80104366:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104369:	89 42 04             	mov    %eax,0x4(%edx)
8010436c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010436f:	8b 40 04             	mov    0x4(%eax),%eax
80104372:	85 c0                	test   %eax,%eax
80104374:	75 0c                	jne    80104382 <userinit+0x37>
    panic("userinit: out of memory?");
80104376:	c7 04 24 72 85 10 80 	movl   $0x80108572,(%esp)
8010437d:	e8 d2 c1 ff ff       	call   80100554 <panic>
  inituvm(p->pgdir, _binary_initcode_start, (int)_binary_initcode_size);
80104382:	ba 2c 00 00 00       	mov    $0x2c,%edx
80104387:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010438a:	8b 40 04             	mov    0x4(%eax),%eax
8010438d:	89 54 24 08          	mov    %edx,0x8(%esp)
80104391:	c7 44 24 04 c0 b4 10 	movl   $0x8010b4c0,0x4(%esp)
80104398:	80 
80104399:	89 04 24             	mov    %eax,(%esp)
8010439c:	e8 0e 38 00 00       	call   80107baf <inituvm>
  p->sz = PGSIZE;
801043a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043a4:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
  memset(p->tf, 0, sizeof(*p->tf));
801043aa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043ad:	8b 40 18             	mov    0x18(%eax),%eax
801043b0:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
801043b7:	00 
801043b8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801043bf:	00 
801043c0:	89 04 24             	mov    %eax,(%esp)
801043c3:	e8 ce 0c 00 00       	call   80105096 <memset>
  p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
801043c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043cb:	8b 40 18             	mov    0x18(%eax),%eax
801043ce:	66 c7 40 3c 1b 00    	movw   $0x1b,0x3c(%eax)
  p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
801043d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043d7:	8b 40 18             	mov    0x18(%eax),%eax
801043da:	66 c7 40 2c 23 00    	movw   $0x23,0x2c(%eax)
  p->tf->es = p->tf->ds;
801043e0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043e3:	8b 50 18             	mov    0x18(%eax),%edx
801043e6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043e9:	8b 40 18             	mov    0x18(%eax),%eax
801043ec:	8b 40 2c             	mov    0x2c(%eax),%eax
801043ef:	66 89 42 28          	mov    %ax,0x28(%edx)
  p->tf->ss = p->tf->ds;
801043f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043f6:	8b 50 18             	mov    0x18(%eax),%edx
801043f9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801043fc:	8b 40 18             	mov    0x18(%eax),%eax
801043ff:	8b 40 2c             	mov    0x2c(%eax),%eax
80104402:	66 89 42 48          	mov    %ax,0x48(%edx)
  p->tf->eflags = FL_IF;
80104406:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104409:	8b 40 18             	mov    0x18(%eax),%eax
8010440c:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
  p->tf->esp = PGSIZE;
80104413:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104416:	8b 40 18             	mov    0x18(%eax),%eax
80104419:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
  p->tf->eip = 0;  // beginning of initcode.S
80104420:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104423:	8b 40 18             	mov    0x18(%eax),%eax
80104426:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

  safestrcpy(p->name, "initcode", sizeof(p->name));
8010442d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104430:	83 c0 6c             	add    $0x6c,%eax
80104433:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010443a:	00 
8010443b:	c7 44 24 04 8b 85 10 	movl   $0x8010858b,0x4(%esp)
80104442:	80 
80104443:	89 04 24             	mov    %eax,(%esp)
80104446:	e8 57 0e 00 00       	call   801052a2 <safestrcpy>
  p->cwd = namei("/");
8010444b:	c7 04 24 94 85 10 80 	movl   $0x80108594,(%esp)
80104452:	e8 00 e1 ff ff       	call   80102557 <namei>
80104457:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010445a:	89 42 68             	mov    %eax,0x68(%edx)

  // this assignment to p->state lets other cores
  // run this process. the acquire forces the above
  // writes to be visible, and the lock is also needed
  // because the assignment might not be atomic.
  acquire(&ptable.lock);
8010445d:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104464:	e8 ca 09 00 00       	call   80104e33 <acquire>

  p->state = RUNNABLE;
80104469:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010446c:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)

  release(&ptable.lock);
80104473:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
8010447a:	e8 1e 0a 00 00       	call   80104e9d <release>
}
8010447f:	c9                   	leave  
80104480:	c3                   	ret    

80104481 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int
growproc(int n)
{
80104481:	55                   	push   %ebp
80104482:	89 e5                	mov    %esp,%ebp
80104484:	83 ec 28             	sub    $0x28,%esp
  uint sz;
  struct proc *curproc = myproc();
80104487:	e8 97 fd ff ff       	call   80104223 <myproc>
8010448c:	89 45 f0             	mov    %eax,-0x10(%ebp)

  sz = curproc->sz;
8010448f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104492:	8b 00                	mov    (%eax),%eax
80104494:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(n > 0){
80104497:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010449b:	7e 31                	jle    801044ce <growproc+0x4d>
    if((sz = allocuvm(curproc->pgdir, sz, sz + n)) == 0)
8010449d:	8b 55 08             	mov    0x8(%ebp),%edx
801044a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044a3:	01 c2                	add    %eax,%edx
801044a5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801044a8:	8b 40 04             	mov    0x4(%eax),%eax
801044ab:	89 54 24 08          	mov    %edx,0x8(%esp)
801044af:	8b 55 f4             	mov    -0xc(%ebp),%edx
801044b2:	89 54 24 04          	mov    %edx,0x4(%esp)
801044b6:	89 04 24             	mov    %eax,(%esp)
801044b9:	e8 5c 38 00 00       	call   80107d1a <allocuvm>
801044be:	89 45 f4             	mov    %eax,-0xc(%ebp)
801044c1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801044c5:	75 3e                	jne    80104505 <growproc+0x84>
      return -1;
801044c7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801044cc:	eb 4f                	jmp    8010451d <growproc+0x9c>
  } else if(n < 0){
801044ce:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801044d2:	79 31                	jns    80104505 <growproc+0x84>
    if((sz = deallocuvm(curproc->pgdir, sz, sz + n)) == 0)
801044d4:	8b 55 08             	mov    0x8(%ebp),%edx
801044d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044da:	01 c2                	add    %eax,%edx
801044dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801044df:	8b 40 04             	mov    0x4(%eax),%eax
801044e2:	89 54 24 08          	mov    %edx,0x8(%esp)
801044e6:	8b 55 f4             	mov    -0xc(%ebp),%edx
801044e9:	89 54 24 04          	mov    %edx,0x4(%esp)
801044ed:	89 04 24             	mov    %eax,(%esp)
801044f0:	e8 3b 39 00 00       	call   80107e30 <deallocuvm>
801044f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
801044f8:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801044fc:	75 07                	jne    80104505 <growproc+0x84>
      return -1;
801044fe:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104503:	eb 18                	jmp    8010451d <growproc+0x9c>
  }
  curproc->sz = sz;
80104505:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104508:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010450b:	89 10                	mov    %edx,(%eax)
  switchuvm(curproc);
8010450d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104510:	89 04 24             	mov    %eax,(%esp)
80104513:	e8 10 35 00 00       	call   80107a28 <switchuvm>
  return 0;
80104518:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010451d:	c9                   	leave  
8010451e:	c3                   	ret    

8010451f <fork>:
// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int
fork(void)
{
8010451f:	55                   	push   %ebp
80104520:	89 e5                	mov    %esp,%ebp
80104522:	57                   	push   %edi
80104523:	56                   	push   %esi
80104524:	53                   	push   %ebx
80104525:	83 ec 2c             	sub    $0x2c,%esp
  int i, pid;
  struct proc *np;
  struct proc *curproc = myproc();
80104528:	e8 f6 fc ff ff       	call   80104223 <myproc>
8010452d:	89 45 e0             	mov    %eax,-0x20(%ebp)

  // Allocate process.
  if((np = allocproc()) == 0){
80104530:	e8 17 fd ff ff       	call   8010424c <allocproc>
80104535:	89 45 dc             	mov    %eax,-0x24(%ebp)
80104538:	83 7d dc 00          	cmpl   $0x0,-0x24(%ebp)
8010453c:	75 0a                	jne    80104548 <fork+0x29>
    return -1;
8010453e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104543:	e9 35 01 00 00       	jmp    8010467d <fork+0x15e>
  }

  // Copy process state from proc.
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0){
80104548:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010454b:	8b 10                	mov    (%eax),%edx
8010454d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104550:	8b 40 04             	mov    0x4(%eax),%eax
80104553:	89 54 24 04          	mov    %edx,0x4(%esp)
80104557:	89 04 24             	mov    %eax,(%esp)
8010455a:	e8 71 3a 00 00       	call   80107fd0 <copyuvm>
8010455f:	8b 55 dc             	mov    -0x24(%ebp),%edx
80104562:	89 42 04             	mov    %eax,0x4(%edx)
80104565:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104568:	8b 40 04             	mov    0x4(%eax),%eax
8010456b:	85 c0                	test   %eax,%eax
8010456d:	75 2c                	jne    8010459b <fork+0x7c>
    kfree(np->kstack);
8010456f:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104572:	8b 40 08             	mov    0x8(%eax),%eax
80104575:	89 04 24             	mov    %eax,(%esp)
80104578:	e8 50 e6 ff ff       	call   80102bcd <kfree>
    np->kstack = 0;
8010457d:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104580:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
    np->state = UNUSED;
80104587:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010458a:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    return -1;
80104591:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104596:	e9 e2 00 00 00       	jmp    8010467d <fork+0x15e>
  }
  np->sz = curproc->sz;
8010459b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010459e:	8b 10                	mov    (%eax),%edx
801045a0:	8b 45 dc             	mov    -0x24(%ebp),%eax
801045a3:	89 10                	mov    %edx,(%eax)
  np->parent = curproc;
801045a5:	8b 45 dc             	mov    -0x24(%ebp),%eax
801045a8:	8b 55 e0             	mov    -0x20(%ebp),%edx
801045ab:	89 50 14             	mov    %edx,0x14(%eax)
  *np->tf = *curproc->tf;
801045ae:	8b 45 dc             	mov    -0x24(%ebp),%eax
801045b1:	8b 50 18             	mov    0x18(%eax),%edx
801045b4:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045b7:	8b 40 18             	mov    0x18(%eax),%eax
801045ba:	89 c3                	mov    %eax,%ebx
801045bc:	b8 13 00 00 00       	mov    $0x13,%eax
801045c1:	89 d7                	mov    %edx,%edi
801045c3:	89 de                	mov    %ebx,%esi
801045c5:	89 c1                	mov    %eax,%ecx
801045c7:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;
801045c9:	8b 45 dc             	mov    -0x24(%ebp),%eax
801045cc:	8b 40 18             	mov    0x18(%eax),%eax
801045cf:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

  for(i = 0; i < NOFILE; i++)
801045d6:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
801045dd:	eb 36                	jmp    80104615 <fork+0xf6>
    if(curproc->ofile[i])
801045df:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045e2:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801045e5:	83 c2 08             	add    $0x8,%edx
801045e8:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801045ec:	85 c0                	test   %eax,%eax
801045ee:	74 22                	je     80104612 <fork+0xf3>
      np->ofile[i] = filedup(curproc->ofile[i]);
801045f0:	8b 45 e0             	mov    -0x20(%ebp),%eax
801045f3:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801045f6:	83 c2 08             	add    $0x8,%edx
801045f9:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801045fd:	89 04 24             	mov    %eax,(%esp)
80104600:	e8 cb ca ff ff       	call   801010d0 <filedup>
80104605:	8b 55 dc             	mov    -0x24(%ebp),%edx
80104608:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
8010460b:	83 c1 08             	add    $0x8,%ecx
8010460e:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
  *np->tf = *curproc->tf;

  // Clear %eax so that fork returns 0 in the child.
  np->tf->eax = 0;

  for(i = 0; i < NOFILE; i++)
80104612:	ff 45 e4             	incl   -0x1c(%ebp)
80104615:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
80104619:	7e c4                	jle    801045df <fork+0xc0>
    if(curproc->ofile[i])
      np->ofile[i] = filedup(curproc->ofile[i]);
  np->cwd = idup(curproc->cwd);
8010461b:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010461e:	8b 40 68             	mov    0x68(%eax),%eax
80104621:	89 04 24             	mov    %eax,(%esp)
80104624:	e8 d7 d3 ff ff       	call   80101a00 <idup>
80104629:	8b 55 dc             	mov    -0x24(%ebp),%edx
8010462c:	89 42 68             	mov    %eax,0x68(%edx)

  safestrcpy(np->name, curproc->name, sizeof(curproc->name));
8010462f:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104632:	8d 50 6c             	lea    0x6c(%eax),%edx
80104635:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104638:	83 c0 6c             	add    $0x6c,%eax
8010463b:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104642:	00 
80104643:	89 54 24 04          	mov    %edx,0x4(%esp)
80104647:	89 04 24             	mov    %eax,(%esp)
8010464a:	e8 53 0c 00 00       	call   801052a2 <safestrcpy>

  pid = np->pid;
8010464f:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104652:	8b 40 10             	mov    0x10(%eax),%eax
80104655:	89 45 d8             	mov    %eax,-0x28(%ebp)

  acquire(&ptable.lock);
80104658:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
8010465f:	e8 cf 07 00 00       	call   80104e33 <acquire>

  np->state = RUNNABLE;
80104664:	8b 45 dc             	mov    -0x24(%ebp),%eax
80104667:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)

  release(&ptable.lock);
8010466e:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104675:	e8 23 08 00 00       	call   80104e9d <release>

  return pid;
8010467a:	8b 45 d8             	mov    -0x28(%ebp),%eax
}
8010467d:	83 c4 2c             	add    $0x2c,%esp
80104680:	5b                   	pop    %ebx
80104681:	5e                   	pop    %esi
80104682:	5f                   	pop    %edi
80104683:	5d                   	pop    %ebp
80104684:	c3                   	ret    

80104685 <exit>:
// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void
exit(void)
{
80104685:	55                   	push   %ebp
80104686:	89 e5                	mov    %esp,%ebp
80104688:	83 ec 28             	sub    $0x28,%esp
  struct proc *curproc = myproc();
8010468b:	e8 93 fb ff ff       	call   80104223 <myproc>
80104690:	89 45 ec             	mov    %eax,-0x14(%ebp)
  struct proc *p;
  int fd;

  if(curproc == initproc)
80104693:	a1 20 b6 10 80       	mov    0x8010b620,%eax
80104698:	39 45 ec             	cmp    %eax,-0x14(%ebp)
8010469b:	75 0c                	jne    801046a9 <exit+0x24>
    panic("init exiting");
8010469d:	c7 04 24 96 85 10 80 	movl   $0x80108596,(%esp)
801046a4:	e8 ab be ff ff       	call   80100554 <panic>

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801046a9:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801046b0:	eb 3a                	jmp    801046ec <exit+0x67>
    if(curproc->ofile[fd]){
801046b2:	8b 45 ec             	mov    -0x14(%ebp),%eax
801046b5:	8b 55 f0             	mov    -0x10(%ebp),%edx
801046b8:	83 c2 08             	add    $0x8,%edx
801046bb:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046bf:	85 c0                	test   %eax,%eax
801046c1:	74 26                	je     801046e9 <exit+0x64>
      fileclose(curproc->ofile[fd]);
801046c3:	8b 45 ec             	mov    -0x14(%ebp),%eax
801046c6:	8b 55 f0             	mov    -0x10(%ebp),%edx
801046c9:	83 c2 08             	add    $0x8,%edx
801046cc:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801046d0:	89 04 24             	mov    %eax,(%esp)
801046d3:	e8 40 ca ff ff       	call   80101118 <fileclose>
      curproc->ofile[fd] = 0;
801046d8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801046db:	8b 55 f0             	mov    -0x10(%ebp),%edx
801046de:	83 c2 08             	add    $0x8,%edx
801046e1:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801046e8:	00 

  if(curproc == initproc)
    panic("init exiting");

  // Close all open files.
  for(fd = 0; fd < NOFILE; fd++){
801046e9:	ff 45 f0             	incl   -0x10(%ebp)
801046ec:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801046f0:	7e c0                	jle    801046b2 <exit+0x2d>
      fileclose(curproc->ofile[fd]);
      curproc->ofile[fd] = 0;
    }
  }

  begin_op();
801046f2:	e8 34 ee ff ff       	call   8010352b <begin_op>
  iput(curproc->cwd);
801046f7:	8b 45 ec             	mov    -0x14(%ebp),%eax
801046fa:	8b 40 68             	mov    0x68(%eax),%eax
801046fd:	89 04 24             	mov    %eax,(%esp)
80104700:	e8 7b d4 ff ff       	call   80101b80 <iput>
  end_op();
80104705:	e8 a3 ee ff ff       	call   801035ad <end_op>
  curproc->cwd = 0;
8010470a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010470d:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

  acquire(&ptable.lock);
80104714:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
8010471b:	e8 13 07 00 00       	call   80104e33 <acquire>

  // Parent might be sleeping in wait().
  wakeup1(curproc->parent);
80104720:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104723:	8b 40 14             	mov    0x14(%eax),%eax
80104726:	89 04 24             	mov    %eax,(%esp)
80104729:	e8 cc 03 00 00       	call   80104afa <wakeup1>

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010472e:	c7 45 f4 94 40 11 80 	movl   $0x80114094,-0xc(%ebp)
80104735:	eb 33                	jmp    8010476a <exit+0xe5>
    if(p->parent == curproc){
80104737:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010473a:	8b 40 14             	mov    0x14(%eax),%eax
8010473d:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80104740:	75 24                	jne    80104766 <exit+0xe1>
      p->parent = initproc;
80104742:	8b 15 20 b6 10 80    	mov    0x8010b620,%edx
80104748:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010474b:	89 50 14             	mov    %edx,0x14(%eax)
      if(p->state == ZOMBIE)
8010474e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104751:	8b 40 0c             	mov    0xc(%eax),%eax
80104754:	83 f8 05             	cmp    $0x5,%eax
80104757:	75 0d                	jne    80104766 <exit+0xe1>
        wakeup1(initproc);
80104759:	a1 20 b6 10 80       	mov    0x8010b620,%eax
8010475e:	89 04 24             	mov    %eax,(%esp)
80104761:	e8 94 03 00 00       	call   80104afa <wakeup1>

  // Parent might be sleeping in wait().
  wakeup1(curproc->parent);

  // Pass abandoned children to init.
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104766:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
8010476a:	81 7d f4 94 5f 11 80 	cmpl   $0x80115f94,-0xc(%ebp)
80104771:	72 c4                	jb     80104737 <exit+0xb2>
        wakeup1(initproc);
    }
  }

  // Jump into the scheduler, never to return.
  curproc->state = ZOMBIE;
80104773:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104776:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
  sched();
8010477d:	e8 c3 01 00 00       	call   80104945 <sched>
  panic("zombie exit");
80104782:	c7 04 24 a3 85 10 80 	movl   $0x801085a3,(%esp)
80104789:	e8 c6 bd ff ff       	call   80100554 <panic>

8010478e <wait>:

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int
wait(void)
{
8010478e:	55                   	push   %ebp
8010478f:	89 e5                	mov    %esp,%ebp
80104791:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  int havekids, pid;
  struct proc *curproc = myproc();
80104794:	e8 8a fa ff ff       	call   80104223 <myproc>
80104799:	89 45 ec             	mov    %eax,-0x14(%ebp)
  
  acquire(&ptable.lock);
8010479c:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
801047a3:	e8 8b 06 00 00       	call   80104e33 <acquire>
  for(;;){
    // Scan through table looking for exited children.
    havekids = 0;
801047a8:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801047af:	c7 45 f4 94 40 11 80 	movl   $0x80114094,-0xc(%ebp)
801047b6:	e9 95 00 00 00       	jmp    80104850 <wait+0xc2>
      if(p->parent != curproc)
801047bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047be:	8b 40 14             	mov    0x14(%eax),%eax
801047c1:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801047c4:	74 05                	je     801047cb <wait+0x3d>
        continue;
801047c6:	e9 81 00 00 00       	jmp    8010484c <wait+0xbe>
      havekids = 1;
801047cb:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
      if(p->state == ZOMBIE){
801047d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047d5:	8b 40 0c             	mov    0xc(%eax),%eax
801047d8:	83 f8 05             	cmp    $0x5,%eax
801047db:	75 6f                	jne    8010484c <wait+0xbe>
        // Found one.
        pid = p->pid;
801047dd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047e0:	8b 40 10             	mov    0x10(%eax),%eax
801047e3:	89 45 e8             	mov    %eax,-0x18(%ebp)
        kfree(p->kstack);
801047e6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047e9:	8b 40 08             	mov    0x8(%eax),%eax
801047ec:	89 04 24             	mov    %eax,(%esp)
801047ef:	e8 d9 e3 ff ff       	call   80102bcd <kfree>
        p->kstack = 0;
801047f4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801047f7:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
        freevm(p->pgdir);
801047fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104801:	8b 40 04             	mov    0x4(%eax),%eax
80104804:	89 04 24             	mov    %eax,(%esp)
80104807:	e8 e8 36 00 00       	call   80107ef4 <freevm>
        p->pid = 0;
8010480c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010480f:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
        p->parent = 0;
80104816:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104819:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
        p->name[0] = 0;
80104820:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104823:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
        p->killed = 0;
80104827:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010482a:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
        p->state = UNUSED;
80104831:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104834:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
        release(&ptable.lock);
8010483b:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104842:	e8 56 06 00 00       	call   80104e9d <release>
        return pid;
80104847:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010484a:	eb 4c                	jmp    80104898 <wait+0x10a>
  
  acquire(&ptable.lock);
  for(;;){
    // Scan through table looking for exited children.
    havekids = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
8010484c:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
80104850:	81 7d f4 94 5f 11 80 	cmpl   $0x80115f94,-0xc(%ebp)
80104857:	0f 82 5e ff ff ff    	jb     801047bb <wait+0x2d>
        return pid;
      }
    }

    // No point waiting if we don't have any children.
    if(!havekids || curproc->killed){
8010485d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104861:	74 0a                	je     8010486d <wait+0xdf>
80104863:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104866:	8b 40 24             	mov    0x24(%eax),%eax
80104869:	85 c0                	test   %eax,%eax
8010486b:	74 13                	je     80104880 <wait+0xf2>
      release(&ptable.lock);
8010486d:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104874:	e8 24 06 00 00       	call   80104e9d <release>
      return -1;
80104879:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010487e:	eb 18                	jmp    80104898 <wait+0x10a>
    }

    // Wait for children to exit.  (See wakeup1 call in proc_exit.)
    sleep(curproc, &ptable.lock);  //DOC: wait-sleep
80104880:	c7 44 24 04 60 40 11 	movl   $0x80114060,0x4(%esp)
80104887:	80 
80104888:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010488b:	89 04 24             	mov    %eax,(%esp)
8010488e:	e8 d1 01 00 00       	call   80104a64 <sleep>
  }
80104893:	e9 10 ff ff ff       	jmp    801047a8 <wait+0x1a>
}
80104898:	c9                   	leave  
80104899:	c3                   	ret    

8010489a <scheduler>:
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void
scheduler(void)
{
8010489a:	55                   	push   %ebp
8010489b:	89 e5                	mov    %esp,%ebp
8010489d:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;
  struct cpu *c = mycpu();
801048a0:	e8 fa f8 ff ff       	call   8010419f <mycpu>
801048a5:	89 45 f0             	mov    %eax,-0x10(%ebp)
  c->proc = 0;
801048a8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801048ab:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
801048b2:	00 00 00 
  
  for(;;){
    // Enable interrupts on this processor.
    sti();
801048b5:	e8 7e f8 ff ff       	call   80104138 <sti>

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
801048ba:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
801048c1:	e8 6d 05 00 00       	call   80104e33 <acquire>
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
801048c6:	c7 45 f4 94 40 11 80 	movl   $0x80114094,-0xc(%ebp)
801048cd:	eb 5c                	jmp    8010492b <scheduler+0x91>
      if(p->state != RUNNABLE)
801048cf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048d2:	8b 40 0c             	mov    0xc(%eax),%eax
801048d5:	83 f8 03             	cmp    $0x3,%eax
801048d8:	74 02                	je     801048dc <scheduler+0x42>
        continue;
801048da:	eb 4b                	jmp    80104927 <scheduler+0x8d>

      // Switch to chosen process.  It is the process's job
      // to release ptable.lock and then reacquire it
      // before jumping back to us.
      c->proc = p;
801048dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801048df:	8b 55 f4             	mov    -0xc(%ebp),%edx
801048e2:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
      switchuvm(p);
801048e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048eb:	89 04 24             	mov    %eax,(%esp)
801048ee:	e8 35 31 00 00       	call   80107a28 <switchuvm>
      p->state = RUNNING;
801048f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048f6:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)

      swtch(&(c->scheduler), p->context);
801048fd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104900:	8b 40 1c             	mov    0x1c(%eax),%eax
80104903:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104906:	83 c2 04             	add    $0x4,%edx
80104909:	89 44 24 04          	mov    %eax,0x4(%esp)
8010490d:	89 14 24             	mov    %edx,(%esp)
80104910:	e8 fb 09 00 00       	call   80105310 <swtch>
      switchkvm();
80104915:	e8 f4 30 00 00       	call   80107a0e <switchkvm>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      c->proc = 0;
8010491a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010491d:	c7 80 ac 00 00 00 00 	movl   $0x0,0xac(%eax)
80104924:	00 00 00 
    // Enable interrupts on this processor.
    sti();

    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104927:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
8010492b:	81 7d f4 94 5f 11 80 	cmpl   $0x80115f94,-0xc(%ebp)
80104932:	72 9b                	jb     801048cf <scheduler+0x35>

      // Process is done running for now.
      // It should have changed its p->state before coming back.
      c->proc = 0;
    }
    release(&ptable.lock);
80104934:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
8010493b:	e8 5d 05 00 00       	call   80104e9d <release>

  }
80104940:	e9 70 ff ff ff       	jmp    801048b5 <scheduler+0x1b>

80104945 <sched>:
// be proc->intena and proc->ncli, but that would
// break in the few places where a lock is held but
// there's no process.
void
sched(void)
{
80104945:	55                   	push   %ebp
80104946:	89 e5                	mov    %esp,%ebp
80104948:	83 ec 28             	sub    $0x28,%esp
  int intena;
  struct proc *p = myproc();
8010494b:	e8 d3 f8 ff ff       	call   80104223 <myproc>
80104950:	89 45 f4             	mov    %eax,-0xc(%ebp)

  if(!holding(&ptable.lock))
80104953:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
8010495a:	e8 02 06 00 00       	call   80104f61 <holding>
8010495f:	85 c0                	test   %eax,%eax
80104961:	75 0c                	jne    8010496f <sched+0x2a>
    panic("sched ptable.lock");
80104963:	c7 04 24 af 85 10 80 	movl   $0x801085af,(%esp)
8010496a:	e8 e5 bb ff ff       	call   80100554 <panic>
  if(mycpu()->ncli != 1)
8010496f:	e8 2b f8 ff ff       	call   8010419f <mycpu>
80104974:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
8010497a:	83 f8 01             	cmp    $0x1,%eax
8010497d:	74 0c                	je     8010498b <sched+0x46>
    panic("sched locks");
8010497f:	c7 04 24 c1 85 10 80 	movl   $0x801085c1,(%esp)
80104986:	e8 c9 bb ff ff       	call   80100554 <panic>
  if(p->state == RUNNING)
8010498b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010498e:	8b 40 0c             	mov    0xc(%eax),%eax
80104991:	83 f8 04             	cmp    $0x4,%eax
80104994:	75 0c                	jne    801049a2 <sched+0x5d>
    panic("sched running");
80104996:	c7 04 24 cd 85 10 80 	movl   $0x801085cd,(%esp)
8010499d:	e8 b2 bb ff ff       	call   80100554 <panic>
  if(readeflags()&FL_IF)
801049a2:	e8 81 f7 ff ff       	call   80104128 <readeflags>
801049a7:	25 00 02 00 00       	and    $0x200,%eax
801049ac:	85 c0                	test   %eax,%eax
801049ae:	74 0c                	je     801049bc <sched+0x77>
    panic("sched interruptible");
801049b0:	c7 04 24 db 85 10 80 	movl   $0x801085db,(%esp)
801049b7:	e8 98 bb ff ff       	call   80100554 <panic>
  intena = mycpu()->intena;
801049bc:	e8 de f7 ff ff       	call   8010419f <mycpu>
801049c1:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
801049c7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  swtch(&p->context, mycpu()->scheduler);
801049ca:	e8 d0 f7 ff ff       	call   8010419f <mycpu>
801049cf:	8b 40 04             	mov    0x4(%eax),%eax
801049d2:	8b 55 f4             	mov    -0xc(%ebp),%edx
801049d5:	83 c2 1c             	add    $0x1c,%edx
801049d8:	89 44 24 04          	mov    %eax,0x4(%esp)
801049dc:	89 14 24             	mov    %edx,(%esp)
801049df:	e8 2c 09 00 00       	call   80105310 <swtch>
  mycpu()->intena = intena;
801049e4:	e8 b6 f7 ff ff       	call   8010419f <mycpu>
801049e9:	8b 55 f0             	mov    -0x10(%ebp),%edx
801049ec:	89 90 a8 00 00 00    	mov    %edx,0xa8(%eax)
}
801049f2:	c9                   	leave  
801049f3:	c3                   	ret    

801049f4 <yield>:

// Give up the CPU for one scheduling round.
void
yield(void)
{
801049f4:	55                   	push   %ebp
801049f5:	89 e5                	mov    %esp,%ebp
801049f7:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);  //DOC: yieldlock
801049fa:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104a01:	e8 2d 04 00 00       	call   80104e33 <acquire>
  myproc()->state = RUNNABLE;
80104a06:	e8 18 f8 ff ff       	call   80104223 <myproc>
80104a0b:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
  sched();
80104a12:	e8 2e ff ff ff       	call   80104945 <sched>
  release(&ptable.lock);
80104a17:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104a1e:	e8 7a 04 00 00       	call   80104e9d <release>
}
80104a23:	c9                   	leave  
80104a24:	c3                   	ret    

80104a25 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void
forkret(void)
{
80104a25:	55                   	push   %ebp
80104a26:	89 e5                	mov    %esp,%ebp
80104a28:	83 ec 18             	sub    $0x18,%esp
  static int first = 1;
  // Still holding ptable.lock from scheduler.
  release(&ptable.lock);
80104a2b:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104a32:	e8 66 04 00 00       	call   80104e9d <release>

  if (first) {
80104a37:	a1 04 b0 10 80       	mov    0x8010b004,%eax
80104a3c:	85 c0                	test   %eax,%eax
80104a3e:	74 22                	je     80104a62 <forkret+0x3d>
    // Some initialization functions must be run in the context
    // of a regular process (e.g., they call sleep), and thus cannot
    // be run from main().
    first = 0;
80104a40:	c7 05 04 b0 10 80 00 	movl   $0x0,0x8010b004
80104a47:	00 00 00 
    iinit(ROOTDEV);
80104a4a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80104a51:	e8 75 cc ff ff       	call   801016cb <iinit>
    initlog(ROOTDEV);
80104a56:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80104a5d:	e8 ca e8 ff ff       	call   8010332c <initlog>
  }

  // Return to "caller", actually trapret (see allocproc).
}
80104a62:	c9                   	leave  
80104a63:	c3                   	ret    

80104a64 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void
sleep(void *chan, struct spinlock *lk)
{
80104a64:	55                   	push   %ebp
80104a65:	89 e5                	mov    %esp,%ebp
80104a67:	83 ec 28             	sub    $0x28,%esp
  struct proc *p = myproc();
80104a6a:	e8 b4 f7 ff ff       	call   80104223 <myproc>
80104a6f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  
  if(p == 0)
80104a72:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104a76:	75 0c                	jne    80104a84 <sleep+0x20>
    panic("sleep");
80104a78:	c7 04 24 ef 85 10 80 	movl   $0x801085ef,(%esp)
80104a7f:	e8 d0 ba ff ff       	call   80100554 <panic>

  if(lk == 0)
80104a84:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104a88:	75 0c                	jne    80104a96 <sleep+0x32>
    panic("sleep without lk");
80104a8a:	c7 04 24 f5 85 10 80 	movl   $0x801085f5,(%esp)
80104a91:	e8 be ba ff ff       	call   80100554 <panic>
  // change p->state and then call sched.
  // Once we hold ptable.lock, we can be
  // guaranteed that we won't miss any wakeup
  // (wakeup runs with ptable.lock locked),
  // so it's okay to release lk.
  if(lk != &ptable.lock){  //DOC: sleeplock0
80104a96:	81 7d 0c 60 40 11 80 	cmpl   $0x80114060,0xc(%ebp)
80104a9d:	74 17                	je     80104ab6 <sleep+0x52>
    acquire(&ptable.lock);  //DOC: sleeplock1
80104a9f:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104aa6:	e8 88 03 00 00       	call   80104e33 <acquire>
    release(lk);
80104aab:	8b 45 0c             	mov    0xc(%ebp),%eax
80104aae:	89 04 24             	mov    %eax,(%esp)
80104ab1:	e8 e7 03 00 00       	call   80104e9d <release>
  }
  // Go to sleep.
  p->chan = chan;
80104ab6:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ab9:	8b 55 08             	mov    0x8(%ebp),%edx
80104abc:	89 50 20             	mov    %edx,0x20(%eax)
  p->state = SLEEPING;
80104abf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ac2:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)

  sched();
80104ac9:	e8 77 fe ff ff       	call   80104945 <sched>

  // Tidy up.
  p->chan = 0;
80104ace:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ad1:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

  // Reacquire original lock.
  if(lk != &ptable.lock){  //DOC: sleeplock2
80104ad8:	81 7d 0c 60 40 11 80 	cmpl   $0x80114060,0xc(%ebp)
80104adf:	74 17                	je     80104af8 <sleep+0x94>
    release(&ptable.lock);
80104ae1:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104ae8:	e8 b0 03 00 00       	call   80104e9d <release>
    acquire(lk);
80104aed:	8b 45 0c             	mov    0xc(%ebp),%eax
80104af0:	89 04 24             	mov    %eax,(%esp)
80104af3:	e8 3b 03 00 00       	call   80104e33 <acquire>
  }
}
80104af8:	c9                   	leave  
80104af9:	c3                   	ret    

80104afa <wakeup1>:
//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void
wakeup1(void *chan)
{
80104afa:	55                   	push   %ebp
80104afb:	89 e5                	mov    %esp,%ebp
80104afd:	83 ec 10             	sub    $0x10,%esp
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104b00:	c7 45 fc 94 40 11 80 	movl   $0x80114094,-0x4(%ebp)
80104b07:	eb 24                	jmp    80104b2d <wakeup1+0x33>
    if(p->state == SLEEPING && p->chan == chan)
80104b09:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b0c:	8b 40 0c             	mov    0xc(%eax),%eax
80104b0f:	83 f8 02             	cmp    $0x2,%eax
80104b12:	75 15                	jne    80104b29 <wakeup1+0x2f>
80104b14:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b17:	8b 40 20             	mov    0x20(%eax),%eax
80104b1a:	3b 45 08             	cmp    0x8(%ebp),%eax
80104b1d:	75 0a                	jne    80104b29 <wakeup1+0x2f>
      p->state = RUNNABLE;
80104b1f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104b22:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
static void
wakeup1(void *chan)
{
  struct proc *p;

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104b29:	83 45 fc 7c          	addl   $0x7c,-0x4(%ebp)
80104b2d:	81 7d fc 94 5f 11 80 	cmpl   $0x80115f94,-0x4(%ebp)
80104b34:	72 d3                	jb     80104b09 <wakeup1+0xf>
    if(p->state == SLEEPING && p->chan == chan)
      p->state = RUNNABLE;
}
80104b36:	c9                   	leave  
80104b37:	c3                   	ret    

80104b38 <wakeup>:

// Wake up all processes sleeping on chan.
void
wakeup(void *chan)
{
80104b38:	55                   	push   %ebp
80104b39:	89 e5                	mov    %esp,%ebp
80104b3b:	83 ec 18             	sub    $0x18,%esp
  acquire(&ptable.lock);
80104b3e:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104b45:	e8 e9 02 00 00       	call   80104e33 <acquire>
  wakeup1(chan);
80104b4a:	8b 45 08             	mov    0x8(%ebp),%eax
80104b4d:	89 04 24             	mov    %eax,(%esp)
80104b50:	e8 a5 ff ff ff       	call   80104afa <wakeup1>
  release(&ptable.lock);
80104b55:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104b5c:	e8 3c 03 00 00       	call   80104e9d <release>
}
80104b61:	c9                   	leave  
80104b62:	c3                   	ret    

80104b63 <kill>:
// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int
kill(int pid)
{
80104b63:	55                   	push   %ebp
80104b64:	89 e5                	mov    %esp,%ebp
80104b66:	83 ec 28             	sub    $0x28,%esp
  struct proc *p;

  acquire(&ptable.lock);
80104b69:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104b70:	e8 be 02 00 00       	call   80104e33 <acquire>
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104b75:	c7 45 f4 94 40 11 80 	movl   $0x80114094,-0xc(%ebp)
80104b7c:	eb 41                	jmp    80104bbf <kill+0x5c>
    if(p->pid == pid){
80104b7e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b81:	8b 40 10             	mov    0x10(%eax),%eax
80104b84:	3b 45 08             	cmp    0x8(%ebp),%eax
80104b87:	75 32                	jne    80104bbb <kill+0x58>
      p->killed = 1;
80104b89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b8c:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
      // Wake process from sleep if necessary.
      if(p->state == SLEEPING)
80104b93:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b96:	8b 40 0c             	mov    0xc(%eax),%eax
80104b99:	83 f8 02             	cmp    $0x2,%eax
80104b9c:	75 0a                	jne    80104ba8 <kill+0x45>
        p->state = RUNNABLE;
80104b9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104ba1:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
      release(&ptable.lock);
80104ba8:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104baf:	e8 e9 02 00 00       	call   80104e9d <release>
      return 0;
80104bb4:	b8 00 00 00 00       	mov    $0x0,%eax
80104bb9:	eb 1e                	jmp    80104bd9 <kill+0x76>
kill(int pid)
{
  struct proc *p;

  acquire(&ptable.lock);
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104bbb:	83 45 f4 7c          	addl   $0x7c,-0xc(%ebp)
80104bbf:	81 7d f4 94 5f 11 80 	cmpl   $0x80115f94,-0xc(%ebp)
80104bc6:	72 b6                	jb     80104b7e <kill+0x1b>
        p->state = RUNNABLE;
      release(&ptable.lock);
      return 0;
    }
  }
  release(&ptable.lock);
80104bc8:	c7 04 24 60 40 11 80 	movl   $0x80114060,(%esp)
80104bcf:	e8 c9 02 00 00       	call   80104e9d <release>
  return -1;
80104bd4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104bd9:	c9                   	leave  
80104bda:	c3                   	ret    

80104bdb <procdump>:
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void
procdump(void)
{
80104bdb:	55                   	push   %ebp
80104bdc:	89 e5                	mov    %esp,%ebp
80104bde:	83 ec 58             	sub    $0x58,%esp
  int i;
  struct proc *p;
  char *state;
  uint pc[10];

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104be1:	c7 45 f0 94 40 11 80 	movl   $0x80114094,-0x10(%ebp)
80104be8:	e9 d5 00 00 00       	jmp    80104cc2 <procdump+0xe7>
    if(p->state == UNUSED)
80104bed:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104bf0:	8b 40 0c             	mov    0xc(%eax),%eax
80104bf3:	85 c0                	test   %eax,%eax
80104bf5:	75 05                	jne    80104bfc <procdump+0x21>
      continue;
80104bf7:	e9 c2 00 00 00       	jmp    80104cbe <procdump+0xe3>
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
80104bfc:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104bff:	8b 40 0c             	mov    0xc(%eax),%eax
80104c02:	83 f8 05             	cmp    $0x5,%eax
80104c05:	77 23                	ja     80104c2a <procdump+0x4f>
80104c07:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c0a:	8b 40 0c             	mov    0xc(%eax),%eax
80104c0d:	8b 04 85 08 b0 10 80 	mov    -0x7fef4ff8(,%eax,4),%eax
80104c14:	85 c0                	test   %eax,%eax
80104c16:	74 12                	je     80104c2a <procdump+0x4f>
      state = states[p->state];
80104c18:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c1b:	8b 40 0c             	mov    0xc(%eax),%eax
80104c1e:	8b 04 85 08 b0 10 80 	mov    -0x7fef4ff8(,%eax,4),%eax
80104c25:	89 45 ec             	mov    %eax,-0x14(%ebp)
80104c28:	eb 07                	jmp    80104c31 <procdump+0x56>
    else
      state = "???";
80104c2a:	c7 45 ec 06 86 10 80 	movl   $0x80108606,-0x14(%ebp)
    cprintf("%d %s %s", p->pid, state, p->name);
80104c31:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c34:	8d 50 6c             	lea    0x6c(%eax),%edx
80104c37:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c3a:	8b 40 10             	mov    0x10(%eax),%eax
80104c3d:	89 54 24 0c          	mov    %edx,0xc(%esp)
80104c41:	8b 55 ec             	mov    -0x14(%ebp),%edx
80104c44:	89 54 24 08          	mov    %edx,0x8(%esp)
80104c48:	89 44 24 04          	mov    %eax,0x4(%esp)
80104c4c:	c7 04 24 0a 86 10 80 	movl   $0x8010860a,(%esp)
80104c53:	e8 69 b7 ff ff       	call   801003c1 <cprintf>
    if(p->state == SLEEPING){
80104c58:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c5b:	8b 40 0c             	mov    0xc(%eax),%eax
80104c5e:	83 f8 02             	cmp    $0x2,%eax
80104c61:	75 4f                	jne    80104cb2 <procdump+0xd7>
      getcallerpcs((uint*)p->context->ebp+2, pc);
80104c63:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104c66:	8b 40 1c             	mov    0x1c(%eax),%eax
80104c69:	8b 40 0c             	mov    0xc(%eax),%eax
80104c6c:	83 c0 08             	add    $0x8,%eax
80104c6f:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80104c72:	89 54 24 04          	mov    %edx,0x4(%esp)
80104c76:	89 04 24             	mov    %eax,(%esp)
80104c79:	e8 6c 02 00 00       	call   80104eea <getcallerpcs>
      for(i=0; i<10 && pc[i] != 0; i++)
80104c7e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104c85:	eb 1a                	jmp    80104ca1 <procdump+0xc6>
        cprintf(" %p", pc[i]);
80104c87:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104c8a:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104c8e:	89 44 24 04          	mov    %eax,0x4(%esp)
80104c92:	c7 04 24 13 86 10 80 	movl   $0x80108613,(%esp)
80104c99:	e8 23 b7 ff ff       	call   801003c1 <cprintf>
    else
      state = "???";
    cprintf("%d %s %s", p->pid, state, p->name);
    if(p->state == SLEEPING){
      getcallerpcs((uint*)p->context->ebp+2, pc);
      for(i=0; i<10 && pc[i] != 0; i++)
80104c9e:	ff 45 f4             	incl   -0xc(%ebp)
80104ca1:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104ca5:	7f 0b                	jg     80104cb2 <procdump+0xd7>
80104ca7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104caa:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104cae:	85 c0                	test   %eax,%eax
80104cb0:	75 d5                	jne    80104c87 <procdump+0xac>
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
80104cb2:	c7 04 24 17 86 10 80 	movl   $0x80108617,(%esp)
80104cb9:	e8 03 b7 ff ff       	call   801003c1 <cprintf>
  int i;
  struct proc *p;
  char *state;
  uint pc[10];

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80104cbe:	83 45 f0 7c          	addl   $0x7c,-0x10(%ebp)
80104cc2:	81 7d f0 94 5f 11 80 	cmpl   $0x80115f94,-0x10(%ebp)
80104cc9:	0f 82 1e ff ff ff    	jb     80104bed <procdump+0x12>
      for(i=0; i<10 && pc[i] != 0; i++)
        cprintf(" %p", pc[i]);
    }
    cprintf("\n");
  }
}
80104ccf:	c9                   	leave  
80104cd0:	c3                   	ret    
80104cd1:	00 00                	add    %al,(%eax)
	...

80104cd4 <initsleeplock>:
#include "spinlock.h"
#include "sleeplock.h"

void
initsleeplock(struct sleeplock *lk, char *name)
{
80104cd4:	55                   	push   %ebp
80104cd5:	89 e5                	mov    %esp,%ebp
80104cd7:	83 ec 18             	sub    $0x18,%esp
  initlock(&lk->lk, "sleep lock");
80104cda:	8b 45 08             	mov    0x8(%ebp),%eax
80104cdd:	83 c0 04             	add    $0x4,%eax
80104ce0:	c7 44 24 04 43 86 10 	movl   $0x80108643,0x4(%esp)
80104ce7:	80 
80104ce8:	89 04 24             	mov    %eax,(%esp)
80104ceb:	e8 22 01 00 00       	call   80104e12 <initlock>
  lk->name = name;
80104cf0:	8b 45 08             	mov    0x8(%ebp),%eax
80104cf3:	8b 55 0c             	mov    0xc(%ebp),%edx
80104cf6:	89 50 38             	mov    %edx,0x38(%eax)
  lk->locked = 0;
80104cf9:	8b 45 08             	mov    0x8(%ebp),%eax
80104cfc:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->pid = 0;
80104d02:	8b 45 08             	mov    0x8(%ebp),%eax
80104d05:	c7 40 3c 00 00 00 00 	movl   $0x0,0x3c(%eax)
}
80104d0c:	c9                   	leave  
80104d0d:	c3                   	ret    

80104d0e <acquiresleep>:

void
acquiresleep(struct sleeplock *lk)
{
80104d0e:	55                   	push   %ebp
80104d0f:	89 e5                	mov    %esp,%ebp
80104d11:	83 ec 18             	sub    $0x18,%esp
  acquire(&lk->lk);
80104d14:	8b 45 08             	mov    0x8(%ebp),%eax
80104d17:	83 c0 04             	add    $0x4,%eax
80104d1a:	89 04 24             	mov    %eax,(%esp)
80104d1d:	e8 11 01 00 00       	call   80104e33 <acquire>
  while (lk->locked) {
80104d22:	eb 15                	jmp    80104d39 <acquiresleep+0x2b>
    sleep(lk, &lk->lk);
80104d24:	8b 45 08             	mov    0x8(%ebp),%eax
80104d27:	83 c0 04             	add    $0x4,%eax
80104d2a:	89 44 24 04          	mov    %eax,0x4(%esp)
80104d2e:	8b 45 08             	mov    0x8(%ebp),%eax
80104d31:	89 04 24             	mov    %eax,(%esp)
80104d34:	e8 2b fd ff ff       	call   80104a64 <sleep>

void
acquiresleep(struct sleeplock *lk)
{
  acquire(&lk->lk);
  while (lk->locked) {
80104d39:	8b 45 08             	mov    0x8(%ebp),%eax
80104d3c:	8b 00                	mov    (%eax),%eax
80104d3e:	85 c0                	test   %eax,%eax
80104d40:	75 e2                	jne    80104d24 <acquiresleep+0x16>
    sleep(lk, &lk->lk);
  }
  lk->locked = 1;
80104d42:	8b 45 08             	mov    0x8(%ebp),%eax
80104d45:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  lk->pid = myproc()->pid;
80104d4b:	e8 d3 f4 ff ff       	call   80104223 <myproc>
80104d50:	8b 50 10             	mov    0x10(%eax),%edx
80104d53:	8b 45 08             	mov    0x8(%ebp),%eax
80104d56:	89 50 3c             	mov    %edx,0x3c(%eax)
  release(&lk->lk);
80104d59:	8b 45 08             	mov    0x8(%ebp),%eax
80104d5c:	83 c0 04             	add    $0x4,%eax
80104d5f:	89 04 24             	mov    %eax,(%esp)
80104d62:	e8 36 01 00 00       	call   80104e9d <release>
}
80104d67:	c9                   	leave  
80104d68:	c3                   	ret    

80104d69 <releasesleep>:

void
releasesleep(struct sleeplock *lk)
{
80104d69:	55                   	push   %ebp
80104d6a:	89 e5                	mov    %esp,%ebp
80104d6c:	83 ec 18             	sub    $0x18,%esp
  acquire(&lk->lk);
80104d6f:	8b 45 08             	mov    0x8(%ebp),%eax
80104d72:	83 c0 04             	add    $0x4,%eax
80104d75:	89 04 24             	mov    %eax,(%esp)
80104d78:	e8 b6 00 00 00       	call   80104e33 <acquire>
  lk->locked = 0;
80104d7d:	8b 45 08             	mov    0x8(%ebp),%eax
80104d80:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->pid = 0;
80104d86:	8b 45 08             	mov    0x8(%ebp),%eax
80104d89:	c7 40 3c 00 00 00 00 	movl   $0x0,0x3c(%eax)
  wakeup(lk);
80104d90:	8b 45 08             	mov    0x8(%ebp),%eax
80104d93:	89 04 24             	mov    %eax,(%esp)
80104d96:	e8 9d fd ff ff       	call   80104b38 <wakeup>
  release(&lk->lk);
80104d9b:	8b 45 08             	mov    0x8(%ebp),%eax
80104d9e:	83 c0 04             	add    $0x4,%eax
80104da1:	89 04 24             	mov    %eax,(%esp)
80104da4:	e8 f4 00 00 00       	call   80104e9d <release>
}
80104da9:	c9                   	leave  
80104daa:	c3                   	ret    

80104dab <holdingsleep>:

int
holdingsleep(struct sleeplock *lk)
{
80104dab:	55                   	push   %ebp
80104dac:	89 e5                	mov    %esp,%ebp
80104dae:	83 ec 28             	sub    $0x28,%esp
  int r;
  
  acquire(&lk->lk);
80104db1:	8b 45 08             	mov    0x8(%ebp),%eax
80104db4:	83 c0 04             	add    $0x4,%eax
80104db7:	89 04 24             	mov    %eax,(%esp)
80104dba:	e8 74 00 00 00       	call   80104e33 <acquire>
  r = lk->locked;
80104dbf:	8b 45 08             	mov    0x8(%ebp),%eax
80104dc2:	8b 00                	mov    (%eax),%eax
80104dc4:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&lk->lk);
80104dc7:	8b 45 08             	mov    0x8(%ebp),%eax
80104dca:	83 c0 04             	add    $0x4,%eax
80104dcd:	89 04 24             	mov    %eax,(%esp)
80104dd0:	e8 c8 00 00 00       	call   80104e9d <release>
  return r;
80104dd5:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104dd8:	c9                   	leave  
80104dd9:	c3                   	ret    
	...

80104ddc <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104ddc:	55                   	push   %ebp
80104ddd:	89 e5                	mov    %esp,%ebp
80104ddf:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104de2:	9c                   	pushf  
80104de3:	58                   	pop    %eax
80104de4:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return eflags;
80104de7:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104dea:	c9                   	leave  
80104deb:	c3                   	ret    

80104dec <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80104dec:	55                   	push   %ebp
80104ded:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80104def:	fa                   	cli    
}
80104df0:	5d                   	pop    %ebp
80104df1:	c3                   	ret    

80104df2 <sti>:

static inline void
sti(void)
{
80104df2:	55                   	push   %ebp
80104df3:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80104df5:	fb                   	sti    
}
80104df6:	5d                   	pop    %ebp
80104df7:	c3                   	ret    

80104df8 <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
80104df8:	55                   	push   %ebp
80104df9:	89 e5                	mov    %esp,%ebp
80104dfb:	83 ec 10             	sub    $0x10,%esp
  uint result;

  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80104dfe:	8b 55 08             	mov    0x8(%ebp),%edx
80104e01:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e04:	8b 4d 08             	mov    0x8(%ebp),%ecx
80104e07:	f0 87 02             	lock xchg %eax,(%edx)
80104e0a:	89 45 fc             	mov    %eax,-0x4(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80104e0d:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80104e10:	c9                   	leave  
80104e11:	c3                   	ret    

80104e12 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80104e12:	55                   	push   %ebp
80104e13:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80104e15:	8b 45 08             	mov    0x8(%ebp),%eax
80104e18:	8b 55 0c             	mov    0xc(%ebp),%edx
80104e1b:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
80104e1e:	8b 45 08             	mov    0x8(%ebp),%eax
80104e21:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80104e27:	8b 45 08             	mov    0x8(%ebp),%eax
80104e2a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80104e31:	5d                   	pop    %ebp
80104e32:	c3                   	ret    

80104e33 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80104e33:	55                   	push   %ebp
80104e34:	89 e5                	mov    %esp,%ebp
80104e36:	53                   	push   %ebx
80104e37:	83 ec 14             	sub    $0x14,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80104e3a:	e8 53 01 00 00       	call   80104f92 <pushcli>
  if(holding(lk))
80104e3f:	8b 45 08             	mov    0x8(%ebp),%eax
80104e42:	89 04 24             	mov    %eax,(%esp)
80104e45:	e8 17 01 00 00       	call   80104f61 <holding>
80104e4a:	85 c0                	test   %eax,%eax
80104e4c:	74 0c                	je     80104e5a <acquire+0x27>
    panic("acquire");
80104e4e:	c7 04 24 4e 86 10 80 	movl   $0x8010864e,(%esp)
80104e55:	e8 fa b6 ff ff       	call   80100554 <panic>

  // The xchg is atomic.
  while(xchg(&lk->locked, 1) != 0)
80104e5a:	90                   	nop
80104e5b:	8b 45 08             	mov    0x8(%ebp),%eax
80104e5e:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80104e65:	00 
80104e66:	89 04 24             	mov    %eax,(%esp)
80104e69:	e8 8a ff ff ff       	call   80104df8 <xchg>
80104e6e:	85 c0                	test   %eax,%eax
80104e70:	75 e9                	jne    80104e5b <acquire+0x28>
    ;

  // Tell the C compiler and the processor to not move loads or stores
  // past this point, to ensure that the critical section's memory
  // references happen after the lock is acquired.
  __sync_synchronize();
80104e72:	f0 83 0c 24 00       	lock orl $0x0,(%esp)

  // Record info about lock acquisition for debugging.
  lk->cpu = mycpu();
80104e77:	8b 5d 08             	mov    0x8(%ebp),%ebx
80104e7a:	e8 20 f3 ff ff       	call   8010419f <mycpu>
80104e7f:	89 43 08             	mov    %eax,0x8(%ebx)
  getcallerpcs(&lk, lk->pcs);
80104e82:	8b 45 08             	mov    0x8(%ebp),%eax
80104e85:	83 c0 0c             	add    $0xc,%eax
80104e88:	89 44 24 04          	mov    %eax,0x4(%esp)
80104e8c:	8d 45 08             	lea    0x8(%ebp),%eax
80104e8f:	89 04 24             	mov    %eax,(%esp)
80104e92:	e8 53 00 00 00       	call   80104eea <getcallerpcs>
}
80104e97:	83 c4 14             	add    $0x14,%esp
80104e9a:	5b                   	pop    %ebx
80104e9b:	5d                   	pop    %ebp
80104e9c:	c3                   	ret    

80104e9d <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
80104e9d:	55                   	push   %ebp
80104e9e:	89 e5                	mov    %esp,%ebp
80104ea0:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
80104ea3:	8b 45 08             	mov    0x8(%ebp),%eax
80104ea6:	89 04 24             	mov    %eax,(%esp)
80104ea9:	e8 b3 00 00 00       	call   80104f61 <holding>
80104eae:	85 c0                	test   %eax,%eax
80104eb0:	75 0c                	jne    80104ebe <release+0x21>
    panic("release");
80104eb2:	c7 04 24 56 86 10 80 	movl   $0x80108656,(%esp)
80104eb9:	e8 96 b6 ff ff       	call   80100554 <panic>

  lk->pcs[0] = 0;
80104ebe:	8b 45 08             	mov    0x8(%ebp),%eax
80104ec1:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
80104ec8:	8b 45 08             	mov    0x8(%ebp),%eax
80104ecb:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // Tell the C compiler and the processor to not move loads or stores
  // past this point, to ensure that all the stores in the critical
  // section are visible to other cores before the lock is released.
  // Both the C compiler and the hardware may re-order loads and
  // stores; __sync_synchronize() tells them both not to.
  __sync_synchronize();
80104ed2:	f0 83 0c 24 00       	lock orl $0x0,(%esp)

  // Release the lock, equivalent to lk->locked = 0.
  // This code can't use a C assignment, since it might
  // not be atomic. A real OS would use C atomics here.
  asm volatile("movl $0, %0" : "+m" (lk->locked) : );
80104ed7:	8b 45 08             	mov    0x8(%ebp),%eax
80104eda:	8b 55 08             	mov    0x8(%ebp),%edx
80104edd:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  popcli();
80104ee3:	e8 f4 00 00 00       	call   80104fdc <popcli>
}
80104ee8:	c9                   	leave  
80104ee9:	c3                   	ret    

80104eea <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80104eea:	55                   	push   %ebp
80104eeb:	89 e5                	mov    %esp,%ebp
80104eed:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;

  ebp = (uint*)v - 2;
80104ef0:	8b 45 08             	mov    0x8(%ebp),%eax
80104ef3:	83 e8 08             	sub    $0x8,%eax
80104ef6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80104ef9:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80104f00:	eb 37                	jmp    80104f39 <getcallerpcs+0x4f>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
80104f02:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80104f06:	74 37                	je     80104f3f <getcallerpcs+0x55>
80104f08:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80104f0f:	76 2e                	jbe    80104f3f <getcallerpcs+0x55>
80104f11:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
80104f15:	74 28                	je     80104f3f <getcallerpcs+0x55>
      break;
    pcs[i] = ebp[1];     // saved %eip
80104f17:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104f1a:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104f21:	8b 45 0c             	mov    0xc(%ebp),%eax
80104f24:	01 c2                	add    %eax,%edx
80104f26:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f29:	8b 40 04             	mov    0x4(%eax),%eax
80104f2c:	89 02                	mov    %eax,(%edx)
    ebp = (uint*)ebp[0]; // saved %ebp
80104f2e:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f31:	8b 00                	mov    (%eax),%eax
80104f33:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;

  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
80104f36:	ff 45 f8             	incl   -0x8(%ebp)
80104f39:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104f3d:	7e c3                	jle    80104f02 <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104f3f:	eb 18                	jmp    80104f59 <getcallerpcs+0x6f>
    pcs[i] = 0;
80104f41:	8b 45 f8             	mov    -0x8(%ebp),%eax
80104f44:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80104f4b:	8b 45 0c             	mov    0xc(%ebp),%eax
80104f4e:	01 d0                	add    %edx,%eax
80104f50:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80104f56:	ff 45 f8             	incl   -0x8(%ebp)
80104f59:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80104f5d:	7e e2                	jle    80104f41 <getcallerpcs+0x57>
    pcs[i] = 0;
}
80104f5f:	c9                   	leave  
80104f60:	c3                   	ret    

80104f61 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80104f61:	55                   	push   %ebp
80104f62:	89 e5                	mov    %esp,%ebp
80104f64:	53                   	push   %ebx
80104f65:	83 ec 04             	sub    $0x4,%esp
  return lock->locked && lock->cpu == mycpu();
80104f68:	8b 45 08             	mov    0x8(%ebp),%eax
80104f6b:	8b 00                	mov    (%eax),%eax
80104f6d:	85 c0                	test   %eax,%eax
80104f6f:	74 16                	je     80104f87 <holding+0x26>
80104f71:	8b 45 08             	mov    0x8(%ebp),%eax
80104f74:	8b 58 08             	mov    0x8(%eax),%ebx
80104f77:	e8 23 f2 ff ff       	call   8010419f <mycpu>
80104f7c:	39 c3                	cmp    %eax,%ebx
80104f7e:	75 07                	jne    80104f87 <holding+0x26>
80104f80:	b8 01 00 00 00       	mov    $0x1,%eax
80104f85:	eb 05                	jmp    80104f8c <holding+0x2b>
80104f87:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104f8c:	83 c4 04             	add    $0x4,%esp
80104f8f:	5b                   	pop    %ebx
80104f90:	5d                   	pop    %ebp
80104f91:	c3                   	ret    

80104f92 <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
80104f92:	55                   	push   %ebp
80104f93:	89 e5                	mov    %esp,%ebp
80104f95:	83 ec 18             	sub    $0x18,%esp
  int eflags;

  eflags = readeflags();
80104f98:	e8 3f fe ff ff       	call   80104ddc <readeflags>
80104f9d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  cli();
80104fa0:	e8 47 fe ff ff       	call   80104dec <cli>
  if(mycpu()->ncli == 0)
80104fa5:	e8 f5 f1 ff ff       	call   8010419f <mycpu>
80104faa:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
80104fb0:	85 c0                	test   %eax,%eax
80104fb2:	75 14                	jne    80104fc8 <pushcli+0x36>
    mycpu()->intena = eflags & FL_IF;
80104fb4:	e8 e6 f1 ff ff       	call   8010419f <mycpu>
80104fb9:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104fbc:	81 e2 00 02 00 00    	and    $0x200,%edx
80104fc2:	89 90 a8 00 00 00    	mov    %edx,0xa8(%eax)
  mycpu()->ncli += 1;
80104fc8:	e8 d2 f1 ff ff       	call   8010419f <mycpu>
80104fcd:	8b 90 a4 00 00 00    	mov    0xa4(%eax),%edx
80104fd3:	42                   	inc    %edx
80104fd4:	89 90 a4 00 00 00    	mov    %edx,0xa4(%eax)
}
80104fda:	c9                   	leave  
80104fdb:	c3                   	ret    

80104fdc <popcli>:

void
popcli(void)
{
80104fdc:	55                   	push   %ebp
80104fdd:	89 e5                	mov    %esp,%ebp
80104fdf:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
80104fe2:	e8 f5 fd ff ff       	call   80104ddc <readeflags>
80104fe7:	25 00 02 00 00       	and    $0x200,%eax
80104fec:	85 c0                	test   %eax,%eax
80104fee:	74 0c                	je     80104ffc <popcli+0x20>
    panic("popcli - interruptible");
80104ff0:	c7 04 24 5e 86 10 80 	movl   $0x8010865e,(%esp)
80104ff7:	e8 58 b5 ff ff       	call   80100554 <panic>
  if(--mycpu()->ncli < 0)
80104ffc:	e8 9e f1 ff ff       	call   8010419f <mycpu>
80105001:	8b 90 a4 00 00 00    	mov    0xa4(%eax),%edx
80105007:	4a                   	dec    %edx
80105008:	89 90 a4 00 00 00    	mov    %edx,0xa4(%eax)
8010500e:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
80105014:	85 c0                	test   %eax,%eax
80105016:	79 0c                	jns    80105024 <popcli+0x48>
    panic("popcli");
80105018:	c7 04 24 75 86 10 80 	movl   $0x80108675,(%esp)
8010501f:	e8 30 b5 ff ff       	call   80100554 <panic>
  if(mycpu()->ncli == 0 && mycpu()->intena)
80105024:	e8 76 f1 ff ff       	call   8010419f <mycpu>
80105029:	8b 80 a4 00 00 00    	mov    0xa4(%eax),%eax
8010502f:	85 c0                	test   %eax,%eax
80105031:	75 14                	jne    80105047 <popcli+0x6b>
80105033:	e8 67 f1 ff ff       	call   8010419f <mycpu>
80105038:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
8010503e:	85 c0                	test   %eax,%eax
80105040:	74 05                	je     80105047 <popcli+0x6b>
    sti();
80105042:	e8 ab fd ff ff       	call   80104df2 <sti>
}
80105047:	c9                   	leave  
80105048:	c3                   	ret    
80105049:	00 00                	add    %al,(%eax)
	...

8010504c <stosb>:
               "cc");
}

static inline void
stosb(void *addr, int data, int cnt)
{
8010504c:	55                   	push   %ebp
8010504d:	89 e5                	mov    %esp,%ebp
8010504f:	57                   	push   %edi
80105050:	53                   	push   %ebx
  asm volatile("cld; rep stosb" :
80105051:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105054:	8b 55 10             	mov    0x10(%ebp),%edx
80105057:	8b 45 0c             	mov    0xc(%ebp),%eax
8010505a:	89 cb                	mov    %ecx,%ebx
8010505c:	89 df                	mov    %ebx,%edi
8010505e:	89 d1                	mov    %edx,%ecx
80105060:	fc                   	cld    
80105061:	f3 aa                	rep stos %al,%es:(%edi)
80105063:	89 ca                	mov    %ecx,%edx
80105065:	89 fb                	mov    %edi,%ebx
80105067:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010506a:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
8010506d:	5b                   	pop    %ebx
8010506e:	5f                   	pop    %edi
8010506f:	5d                   	pop    %ebp
80105070:	c3                   	ret    

80105071 <stosl>:

static inline void
stosl(void *addr, int data, int cnt)
{
80105071:	55                   	push   %ebp
80105072:	89 e5                	mov    %esp,%ebp
80105074:	57                   	push   %edi
80105075:	53                   	push   %ebx
  asm volatile("cld; rep stosl" :
80105076:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105079:	8b 55 10             	mov    0x10(%ebp),%edx
8010507c:	8b 45 0c             	mov    0xc(%ebp),%eax
8010507f:	89 cb                	mov    %ecx,%ebx
80105081:	89 df                	mov    %ebx,%edi
80105083:	89 d1                	mov    %edx,%ecx
80105085:	fc                   	cld    
80105086:	f3 ab                	rep stos %eax,%es:(%edi)
80105088:	89 ca                	mov    %ecx,%edx
8010508a:	89 fb                	mov    %edi,%ebx
8010508c:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010508f:	89 55 10             	mov    %edx,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "0" (addr), "1" (cnt), "a" (data) :
               "memory", "cc");
}
80105092:	5b                   	pop    %ebx
80105093:	5f                   	pop    %edi
80105094:	5d                   	pop    %ebp
80105095:	c3                   	ret    

80105096 <memset>:
#include "types.h"
#include "x86.h"

void*
memset(void *dst, int c, uint n)
{
80105096:	55                   	push   %ebp
80105097:	89 e5                	mov    %esp,%ebp
80105099:	83 ec 0c             	sub    $0xc,%esp
  if ((int)dst%4 == 0 && n%4 == 0){
8010509c:	8b 45 08             	mov    0x8(%ebp),%eax
8010509f:	83 e0 03             	and    $0x3,%eax
801050a2:	85 c0                	test   %eax,%eax
801050a4:	75 49                	jne    801050ef <memset+0x59>
801050a6:	8b 45 10             	mov    0x10(%ebp),%eax
801050a9:	83 e0 03             	and    $0x3,%eax
801050ac:	85 c0                	test   %eax,%eax
801050ae:	75 3f                	jne    801050ef <memset+0x59>
    c &= 0xFF;
801050b0:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
    stosl(dst, (c<<24)|(c<<16)|(c<<8)|c, n/4);
801050b7:	8b 45 10             	mov    0x10(%ebp),%eax
801050ba:	c1 e8 02             	shr    $0x2,%eax
801050bd:	89 c2                	mov    %eax,%edx
801050bf:	8b 45 0c             	mov    0xc(%ebp),%eax
801050c2:	c1 e0 18             	shl    $0x18,%eax
801050c5:	89 c1                	mov    %eax,%ecx
801050c7:	8b 45 0c             	mov    0xc(%ebp),%eax
801050ca:	c1 e0 10             	shl    $0x10,%eax
801050cd:	09 c1                	or     %eax,%ecx
801050cf:	8b 45 0c             	mov    0xc(%ebp),%eax
801050d2:	c1 e0 08             	shl    $0x8,%eax
801050d5:	09 c8                	or     %ecx,%eax
801050d7:	0b 45 0c             	or     0xc(%ebp),%eax
801050da:	89 54 24 08          	mov    %edx,0x8(%esp)
801050de:	89 44 24 04          	mov    %eax,0x4(%esp)
801050e2:	8b 45 08             	mov    0x8(%ebp),%eax
801050e5:	89 04 24             	mov    %eax,(%esp)
801050e8:	e8 84 ff ff ff       	call   80105071 <stosl>
801050ed:	eb 19                	jmp    80105108 <memset+0x72>
  } else
    stosb(dst, c, n);
801050ef:	8b 45 10             	mov    0x10(%ebp),%eax
801050f2:	89 44 24 08          	mov    %eax,0x8(%esp)
801050f6:	8b 45 0c             	mov    0xc(%ebp),%eax
801050f9:	89 44 24 04          	mov    %eax,0x4(%esp)
801050fd:	8b 45 08             	mov    0x8(%ebp),%eax
80105100:	89 04 24             	mov    %eax,(%esp)
80105103:	e8 44 ff ff ff       	call   8010504c <stosb>
  return dst;
80105108:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010510b:	c9                   	leave  
8010510c:	c3                   	ret    

8010510d <memcmp>:

int
memcmp(const void *v1, const void *v2, uint n)
{
8010510d:	55                   	push   %ebp
8010510e:	89 e5                	mov    %esp,%ebp
80105110:	83 ec 10             	sub    $0x10,%esp
  const uchar *s1, *s2;

  s1 = v1;
80105113:	8b 45 08             	mov    0x8(%ebp),%eax
80105116:	89 45 fc             	mov    %eax,-0x4(%ebp)
  s2 = v2;
80105119:	8b 45 0c             	mov    0xc(%ebp),%eax
8010511c:	89 45 f8             	mov    %eax,-0x8(%ebp)
  while(n-- > 0){
8010511f:	eb 2a                	jmp    8010514b <memcmp+0x3e>
    if(*s1 != *s2)
80105121:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105124:	8a 10                	mov    (%eax),%dl
80105126:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105129:	8a 00                	mov    (%eax),%al
8010512b:	38 c2                	cmp    %al,%dl
8010512d:	74 16                	je     80105145 <memcmp+0x38>
      return *s1 - *s2;
8010512f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105132:	8a 00                	mov    (%eax),%al
80105134:	0f b6 d0             	movzbl %al,%edx
80105137:	8b 45 f8             	mov    -0x8(%ebp),%eax
8010513a:	8a 00                	mov    (%eax),%al
8010513c:	0f b6 c0             	movzbl %al,%eax
8010513f:	29 c2                	sub    %eax,%edx
80105141:	89 d0                	mov    %edx,%eax
80105143:	eb 18                	jmp    8010515d <memcmp+0x50>
    s1++, s2++;
80105145:	ff 45 fc             	incl   -0x4(%ebp)
80105148:	ff 45 f8             	incl   -0x8(%ebp)
{
  const uchar *s1, *s2;

  s1 = v1;
  s2 = v2;
  while(n-- > 0){
8010514b:	8b 45 10             	mov    0x10(%ebp),%eax
8010514e:	8d 50 ff             	lea    -0x1(%eax),%edx
80105151:	89 55 10             	mov    %edx,0x10(%ebp)
80105154:	85 c0                	test   %eax,%eax
80105156:	75 c9                	jne    80105121 <memcmp+0x14>
    if(*s1 != *s2)
      return *s1 - *s2;
    s1++, s2++;
  }

  return 0;
80105158:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010515d:	c9                   	leave  
8010515e:	c3                   	ret    

8010515f <memmove>:

void*
memmove(void *dst, const void *src, uint n)
{
8010515f:	55                   	push   %ebp
80105160:	89 e5                	mov    %esp,%ebp
80105162:	83 ec 10             	sub    $0x10,%esp
  const char *s;
  char *d;

  s = src;
80105165:	8b 45 0c             	mov    0xc(%ebp),%eax
80105168:	89 45 fc             	mov    %eax,-0x4(%ebp)
  d = dst;
8010516b:	8b 45 08             	mov    0x8(%ebp),%eax
8010516e:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(s < d && s + n > d){
80105171:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105174:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105177:	73 3a                	jae    801051b3 <memmove+0x54>
80105179:	8b 45 10             	mov    0x10(%ebp),%eax
8010517c:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010517f:	01 d0                	add    %edx,%eax
80105181:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80105184:	76 2d                	jbe    801051b3 <memmove+0x54>
    s += n;
80105186:	8b 45 10             	mov    0x10(%ebp),%eax
80105189:	01 45 fc             	add    %eax,-0x4(%ebp)
    d += n;
8010518c:	8b 45 10             	mov    0x10(%ebp),%eax
8010518f:	01 45 f8             	add    %eax,-0x8(%ebp)
    while(n-- > 0)
80105192:	eb 10                	jmp    801051a4 <memmove+0x45>
      *--d = *--s;
80105194:	ff 4d f8             	decl   -0x8(%ebp)
80105197:	ff 4d fc             	decl   -0x4(%ebp)
8010519a:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010519d:	8a 10                	mov    (%eax),%dl
8010519f:	8b 45 f8             	mov    -0x8(%ebp),%eax
801051a2:	88 10                	mov    %dl,(%eax)
  s = src;
  d = dst;
  if(s < d && s + n > d){
    s += n;
    d += n;
    while(n-- > 0)
801051a4:	8b 45 10             	mov    0x10(%ebp),%eax
801051a7:	8d 50 ff             	lea    -0x1(%eax),%edx
801051aa:	89 55 10             	mov    %edx,0x10(%ebp)
801051ad:	85 c0                	test   %eax,%eax
801051af:	75 e3                	jne    80105194 <memmove+0x35>
  const char *s;
  char *d;

  s = src;
  d = dst;
  if(s < d && s + n > d){
801051b1:	eb 25                	jmp    801051d8 <memmove+0x79>
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
801051b3:	eb 16                	jmp    801051cb <memmove+0x6c>
      *d++ = *s++;
801051b5:	8b 45 f8             	mov    -0x8(%ebp),%eax
801051b8:	8d 50 01             	lea    0x1(%eax),%edx
801051bb:	89 55 f8             	mov    %edx,-0x8(%ebp)
801051be:	8b 55 fc             	mov    -0x4(%ebp),%edx
801051c1:	8d 4a 01             	lea    0x1(%edx),%ecx
801051c4:	89 4d fc             	mov    %ecx,-0x4(%ebp)
801051c7:	8a 12                	mov    (%edx),%dl
801051c9:	88 10                	mov    %dl,(%eax)
    s += n;
    d += n;
    while(n-- > 0)
      *--d = *--s;
  } else
    while(n-- > 0)
801051cb:	8b 45 10             	mov    0x10(%ebp),%eax
801051ce:	8d 50 ff             	lea    -0x1(%eax),%edx
801051d1:	89 55 10             	mov    %edx,0x10(%ebp)
801051d4:	85 c0                	test   %eax,%eax
801051d6:	75 dd                	jne    801051b5 <memmove+0x56>
      *d++ = *s++;

  return dst;
801051d8:	8b 45 08             	mov    0x8(%ebp),%eax
}
801051db:	c9                   	leave  
801051dc:	c3                   	ret    

801051dd <memcpy>:

// memcpy exists to placate GCC.  Use memmove.
void*
memcpy(void *dst, const void *src, uint n)
{
801051dd:	55                   	push   %ebp
801051de:	89 e5                	mov    %esp,%ebp
801051e0:	83 ec 0c             	sub    $0xc,%esp
  return memmove(dst, src, n);
801051e3:	8b 45 10             	mov    0x10(%ebp),%eax
801051e6:	89 44 24 08          	mov    %eax,0x8(%esp)
801051ea:	8b 45 0c             	mov    0xc(%ebp),%eax
801051ed:	89 44 24 04          	mov    %eax,0x4(%esp)
801051f1:	8b 45 08             	mov    0x8(%ebp),%eax
801051f4:	89 04 24             	mov    %eax,(%esp)
801051f7:	e8 63 ff ff ff       	call   8010515f <memmove>
}
801051fc:	c9                   	leave  
801051fd:	c3                   	ret    

801051fe <strncmp>:

int
strncmp(const char *p, const char *q, uint n)
{
801051fe:	55                   	push   %ebp
801051ff:	89 e5                	mov    %esp,%ebp
  while(n > 0 && *p && *p == *q)
80105201:	eb 09                	jmp    8010520c <strncmp+0xe>
    n--, p++, q++;
80105203:	ff 4d 10             	decl   0x10(%ebp)
80105206:	ff 45 08             	incl   0x8(%ebp)
80105209:	ff 45 0c             	incl   0xc(%ebp)
}

int
strncmp(const char *p, const char *q, uint n)
{
  while(n > 0 && *p && *p == *q)
8010520c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105210:	74 17                	je     80105229 <strncmp+0x2b>
80105212:	8b 45 08             	mov    0x8(%ebp),%eax
80105215:	8a 00                	mov    (%eax),%al
80105217:	84 c0                	test   %al,%al
80105219:	74 0e                	je     80105229 <strncmp+0x2b>
8010521b:	8b 45 08             	mov    0x8(%ebp),%eax
8010521e:	8a 10                	mov    (%eax),%dl
80105220:	8b 45 0c             	mov    0xc(%ebp),%eax
80105223:	8a 00                	mov    (%eax),%al
80105225:	38 c2                	cmp    %al,%dl
80105227:	74 da                	je     80105203 <strncmp+0x5>
    n--, p++, q++;
  if(n == 0)
80105229:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010522d:	75 07                	jne    80105236 <strncmp+0x38>
    return 0;
8010522f:	b8 00 00 00 00       	mov    $0x0,%eax
80105234:	eb 14                	jmp    8010524a <strncmp+0x4c>
  return (uchar)*p - (uchar)*q;
80105236:	8b 45 08             	mov    0x8(%ebp),%eax
80105239:	8a 00                	mov    (%eax),%al
8010523b:	0f b6 d0             	movzbl %al,%edx
8010523e:	8b 45 0c             	mov    0xc(%ebp),%eax
80105241:	8a 00                	mov    (%eax),%al
80105243:	0f b6 c0             	movzbl %al,%eax
80105246:	29 c2                	sub    %eax,%edx
80105248:	89 d0                	mov    %edx,%eax
}
8010524a:	5d                   	pop    %ebp
8010524b:	c3                   	ret    

8010524c <strncpy>:

char*
strncpy(char *s, const char *t, int n)
{
8010524c:	55                   	push   %ebp
8010524d:	89 e5                	mov    %esp,%ebp
8010524f:	83 ec 10             	sub    $0x10,%esp
  char *os;

  os = s;
80105252:	8b 45 08             	mov    0x8(%ebp),%eax
80105255:	89 45 fc             	mov    %eax,-0x4(%ebp)
  while(n-- > 0 && (*s++ = *t++) != 0)
80105258:	90                   	nop
80105259:	8b 45 10             	mov    0x10(%ebp),%eax
8010525c:	8d 50 ff             	lea    -0x1(%eax),%edx
8010525f:	89 55 10             	mov    %edx,0x10(%ebp)
80105262:	85 c0                	test   %eax,%eax
80105264:	7e 1c                	jle    80105282 <strncpy+0x36>
80105266:	8b 45 08             	mov    0x8(%ebp),%eax
80105269:	8d 50 01             	lea    0x1(%eax),%edx
8010526c:	89 55 08             	mov    %edx,0x8(%ebp)
8010526f:	8b 55 0c             	mov    0xc(%ebp),%edx
80105272:	8d 4a 01             	lea    0x1(%edx),%ecx
80105275:	89 4d 0c             	mov    %ecx,0xc(%ebp)
80105278:	8a 12                	mov    (%edx),%dl
8010527a:	88 10                	mov    %dl,(%eax)
8010527c:	8a 00                	mov    (%eax),%al
8010527e:	84 c0                	test   %al,%al
80105280:	75 d7                	jne    80105259 <strncpy+0xd>
    ;
  while(n-- > 0)
80105282:	eb 0c                	jmp    80105290 <strncpy+0x44>
    *s++ = 0;
80105284:	8b 45 08             	mov    0x8(%ebp),%eax
80105287:	8d 50 01             	lea    0x1(%eax),%edx
8010528a:	89 55 08             	mov    %edx,0x8(%ebp)
8010528d:	c6 00 00             	movb   $0x0,(%eax)
  char *os;

  os = s;
  while(n-- > 0 && (*s++ = *t++) != 0)
    ;
  while(n-- > 0)
80105290:	8b 45 10             	mov    0x10(%ebp),%eax
80105293:	8d 50 ff             	lea    -0x1(%eax),%edx
80105296:	89 55 10             	mov    %edx,0x10(%ebp)
80105299:	85 c0                	test   %eax,%eax
8010529b:	7f e7                	jg     80105284 <strncpy+0x38>
    *s++ = 0;
  return os;
8010529d:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801052a0:	c9                   	leave  
801052a1:	c3                   	ret    

801052a2 <safestrcpy>:

// Like strncpy but guaranteed to NUL-terminate.
char*
safestrcpy(char *s, const char *t, int n)
{
801052a2:	55                   	push   %ebp
801052a3:	89 e5                	mov    %esp,%ebp
801052a5:	83 ec 10             	sub    $0x10,%esp
  char *os;

  os = s;
801052a8:	8b 45 08             	mov    0x8(%ebp),%eax
801052ab:	89 45 fc             	mov    %eax,-0x4(%ebp)
  if(n <= 0)
801052ae:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801052b2:	7f 05                	jg     801052b9 <safestrcpy+0x17>
    return os;
801052b4:	8b 45 fc             	mov    -0x4(%ebp),%eax
801052b7:	eb 2e                	jmp    801052e7 <safestrcpy+0x45>
  while(--n > 0 && (*s++ = *t++) != 0)
801052b9:	ff 4d 10             	decl   0x10(%ebp)
801052bc:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801052c0:	7e 1c                	jle    801052de <safestrcpy+0x3c>
801052c2:	8b 45 08             	mov    0x8(%ebp),%eax
801052c5:	8d 50 01             	lea    0x1(%eax),%edx
801052c8:	89 55 08             	mov    %edx,0x8(%ebp)
801052cb:	8b 55 0c             	mov    0xc(%ebp),%edx
801052ce:	8d 4a 01             	lea    0x1(%edx),%ecx
801052d1:	89 4d 0c             	mov    %ecx,0xc(%ebp)
801052d4:	8a 12                	mov    (%edx),%dl
801052d6:	88 10                	mov    %dl,(%eax)
801052d8:	8a 00                	mov    (%eax),%al
801052da:	84 c0                	test   %al,%al
801052dc:	75 db                	jne    801052b9 <safestrcpy+0x17>
    ;
  *s = 0;
801052de:	8b 45 08             	mov    0x8(%ebp),%eax
801052e1:	c6 00 00             	movb   $0x0,(%eax)
  return os;
801052e4:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
801052e7:	c9                   	leave  
801052e8:	c3                   	ret    

801052e9 <strlen>:

int
strlen(const char *s)
{
801052e9:	55                   	push   %ebp
801052ea:	89 e5                	mov    %esp,%ebp
801052ec:	83 ec 10             	sub    $0x10,%esp
  int n;

  for(n = 0; s[n]; n++)
801052ef:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801052f6:	eb 03                	jmp    801052fb <strlen+0x12>
801052f8:	ff 45 fc             	incl   -0x4(%ebp)
801052fb:	8b 55 fc             	mov    -0x4(%ebp),%edx
801052fe:	8b 45 08             	mov    0x8(%ebp),%eax
80105301:	01 d0                	add    %edx,%eax
80105303:	8a 00                	mov    (%eax),%al
80105305:	84 c0                	test   %al,%al
80105307:	75 ef                	jne    801052f8 <strlen+0xf>
    ;
  return n;
80105309:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
8010530c:	c9                   	leave  
8010530d:	c3                   	ret    
	...

80105310 <swtch>:
# Save current register context in old
# and then load register context from new.

.globl swtch
swtch:
  movl 4(%esp), %eax
80105310:	8b 44 24 04          	mov    0x4(%esp),%eax
  movl 8(%esp), %edx
80105314:	8b 54 24 08          	mov    0x8(%esp),%edx

  # Save old callee-save registers
  pushl %ebp
80105318:	55                   	push   %ebp
  pushl %ebx
80105319:	53                   	push   %ebx
  pushl %esi
8010531a:	56                   	push   %esi
  pushl %edi
8010531b:	57                   	push   %edi

  # Switch stacks
  movl %esp, (%eax)
8010531c:	89 20                	mov    %esp,(%eax)
  movl %edx, %esp
8010531e:	89 d4                	mov    %edx,%esp

  # Load new callee-save registers
  popl %edi
80105320:	5f                   	pop    %edi
  popl %esi
80105321:	5e                   	pop    %esi
  popl %ebx
80105322:	5b                   	pop    %ebx
  popl %ebp
80105323:	5d                   	pop    %ebp
  ret
80105324:	c3                   	ret    
80105325:	00 00                	add    %al,(%eax)
	...

80105328 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80105328:	55                   	push   %ebp
80105329:	89 e5                	mov    %esp,%ebp
8010532b:	83 ec 18             	sub    $0x18,%esp
  struct proc *curproc = myproc();
8010532e:	e8 f0 ee ff ff       	call   80104223 <myproc>
80105333:	89 45 f4             	mov    %eax,-0xc(%ebp)

  if(addr >= curproc->sz || addr+4 > curproc->sz)
80105336:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105339:	8b 00                	mov    (%eax),%eax
8010533b:	3b 45 08             	cmp    0x8(%ebp),%eax
8010533e:	76 0f                	jbe    8010534f <fetchint+0x27>
80105340:	8b 45 08             	mov    0x8(%ebp),%eax
80105343:	8d 50 04             	lea    0x4(%eax),%edx
80105346:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105349:	8b 00                	mov    (%eax),%eax
8010534b:	39 c2                	cmp    %eax,%edx
8010534d:	76 07                	jbe    80105356 <fetchint+0x2e>
    return -1;
8010534f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105354:	eb 0f                	jmp    80105365 <fetchint+0x3d>
  *ip = *(int*)(addr);
80105356:	8b 45 08             	mov    0x8(%ebp),%eax
80105359:	8b 10                	mov    (%eax),%edx
8010535b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010535e:	89 10                	mov    %edx,(%eax)
  return 0;
80105360:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105365:	c9                   	leave  
80105366:	c3                   	ret    

80105367 <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
80105367:	55                   	push   %ebp
80105368:	89 e5                	mov    %esp,%ebp
8010536a:	83 ec 18             	sub    $0x18,%esp
  char *s, *ep;
  struct proc *curproc = myproc();
8010536d:	e8 b1 ee ff ff       	call   80104223 <myproc>
80105372:	89 45 f0             	mov    %eax,-0x10(%ebp)

  if(addr >= curproc->sz)
80105375:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105378:	8b 00                	mov    (%eax),%eax
8010537a:	3b 45 08             	cmp    0x8(%ebp),%eax
8010537d:	77 07                	ja     80105386 <fetchstr+0x1f>
    return -1;
8010537f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105384:	eb 41                	jmp    801053c7 <fetchstr+0x60>
  *pp = (char*)addr;
80105386:	8b 55 08             	mov    0x8(%ebp),%edx
80105389:	8b 45 0c             	mov    0xc(%ebp),%eax
8010538c:	89 10                	mov    %edx,(%eax)
  ep = (char*)curproc->sz;
8010538e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105391:	8b 00                	mov    (%eax),%eax
80105393:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(s = *pp; s < ep; s++){
80105396:	8b 45 0c             	mov    0xc(%ebp),%eax
80105399:	8b 00                	mov    (%eax),%eax
8010539b:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010539e:	eb 1a                	jmp    801053ba <fetchstr+0x53>
    if(*s == 0)
801053a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053a3:	8a 00                	mov    (%eax),%al
801053a5:	84 c0                	test   %al,%al
801053a7:	75 0e                	jne    801053b7 <fetchstr+0x50>
      return s - *pp;
801053a9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801053ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801053af:	8b 00                	mov    (%eax),%eax
801053b1:	29 c2                	sub    %eax,%edx
801053b3:	89 d0                	mov    %edx,%eax
801053b5:	eb 10                	jmp    801053c7 <fetchstr+0x60>

  if(addr >= curproc->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)curproc->sz;
  for(s = *pp; s < ep; s++){
801053b7:	ff 45 f4             	incl   -0xc(%ebp)
801053ba:	8b 45 f4             	mov    -0xc(%ebp),%eax
801053bd:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801053c0:	72 de                	jb     801053a0 <fetchstr+0x39>
    if(*s == 0)
      return s - *pp;
  }
  return -1;
801053c2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801053c7:	c9                   	leave  
801053c8:	c3                   	ret    

801053c9 <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
801053c9:	55                   	push   %ebp
801053ca:	89 e5                	mov    %esp,%ebp
801053cc:	83 ec 18             	sub    $0x18,%esp
  return fetchint((myproc()->tf->esp) + 4 + 4*n, ip);
801053cf:	e8 4f ee ff ff       	call   80104223 <myproc>
801053d4:	8b 40 18             	mov    0x18(%eax),%eax
801053d7:	8b 50 44             	mov    0x44(%eax),%edx
801053da:	8b 45 08             	mov    0x8(%ebp),%eax
801053dd:	c1 e0 02             	shl    $0x2,%eax
801053e0:	01 d0                	add    %edx,%eax
801053e2:	8d 50 04             	lea    0x4(%eax),%edx
801053e5:	8b 45 0c             	mov    0xc(%ebp),%eax
801053e8:	89 44 24 04          	mov    %eax,0x4(%esp)
801053ec:	89 14 24             	mov    %edx,(%esp)
801053ef:	e8 34 ff ff ff       	call   80105328 <fetchint>
}
801053f4:	c9                   	leave  
801053f5:	c3                   	ret    

801053f6 <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
801053f6:	55                   	push   %ebp
801053f7:	89 e5                	mov    %esp,%ebp
801053f9:	83 ec 28             	sub    $0x28,%esp
  int i;
  struct proc *curproc = myproc();
801053fc:	e8 22 ee ff ff       	call   80104223 <myproc>
80105401:	89 45 f4             	mov    %eax,-0xc(%ebp)
 
  if(argint(n, &i) < 0)
80105404:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105407:	89 44 24 04          	mov    %eax,0x4(%esp)
8010540b:	8b 45 08             	mov    0x8(%ebp),%eax
8010540e:	89 04 24             	mov    %eax,(%esp)
80105411:	e8 b3 ff ff ff       	call   801053c9 <argint>
80105416:	85 c0                	test   %eax,%eax
80105418:	79 07                	jns    80105421 <argptr+0x2b>
    return -1;
8010541a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010541f:	eb 3d                	jmp    8010545e <argptr+0x68>
  if(size < 0 || (uint)i >= curproc->sz || (uint)i+size > curproc->sz)
80105421:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105425:	78 21                	js     80105448 <argptr+0x52>
80105427:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010542a:	89 c2                	mov    %eax,%edx
8010542c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010542f:	8b 00                	mov    (%eax),%eax
80105431:	39 c2                	cmp    %eax,%edx
80105433:	73 13                	jae    80105448 <argptr+0x52>
80105435:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105438:	89 c2                	mov    %eax,%edx
8010543a:	8b 45 10             	mov    0x10(%ebp),%eax
8010543d:	01 c2                	add    %eax,%edx
8010543f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105442:	8b 00                	mov    (%eax),%eax
80105444:	39 c2                	cmp    %eax,%edx
80105446:	76 07                	jbe    8010544f <argptr+0x59>
    return -1;
80105448:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010544d:	eb 0f                	jmp    8010545e <argptr+0x68>
  *pp = (char*)i;
8010544f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105452:	89 c2                	mov    %eax,%edx
80105454:	8b 45 0c             	mov    0xc(%ebp),%eax
80105457:	89 10                	mov    %edx,(%eax)
  return 0;
80105459:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010545e:	c9                   	leave  
8010545f:	c3                   	ret    

80105460 <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
80105460:	55                   	push   %ebp
80105461:	89 e5                	mov    %esp,%ebp
80105463:	83 ec 28             	sub    $0x28,%esp
  int addr;
  if(argint(n, &addr) < 0)
80105466:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105469:	89 44 24 04          	mov    %eax,0x4(%esp)
8010546d:	8b 45 08             	mov    0x8(%ebp),%eax
80105470:	89 04 24             	mov    %eax,(%esp)
80105473:	e8 51 ff ff ff       	call   801053c9 <argint>
80105478:	85 c0                	test   %eax,%eax
8010547a:	79 07                	jns    80105483 <argstr+0x23>
    return -1;
8010547c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105481:	eb 12                	jmp    80105495 <argstr+0x35>
  return fetchstr(addr, pp);
80105483:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105486:	8b 55 0c             	mov    0xc(%ebp),%edx
80105489:	89 54 24 04          	mov    %edx,0x4(%esp)
8010548d:	89 04 24             	mov    %eax,(%esp)
80105490:	e8 d2 fe ff ff       	call   80105367 <fetchstr>
}
80105495:	c9                   	leave  
80105496:	c3                   	ret    

80105497 <syscall>:
[SYS_close]   sys_close,
};

void
syscall(void)
{
80105497:	55                   	push   %ebp
80105498:	89 e5                	mov    %esp,%ebp
8010549a:	53                   	push   %ebx
8010549b:	83 ec 24             	sub    $0x24,%esp
  int num;
  struct proc *curproc = myproc();
8010549e:	e8 80 ed ff ff       	call   80104223 <myproc>
801054a3:	89 45 f4             	mov    %eax,-0xc(%ebp)

  num = curproc->tf->eax;
801054a6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054a9:	8b 40 18             	mov    0x18(%eax),%eax
801054ac:	8b 40 1c             	mov    0x1c(%eax),%eax
801054af:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
801054b2:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801054b6:	7e 2d                	jle    801054e5 <syscall+0x4e>
801054b8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801054bb:	83 f8 15             	cmp    $0x15,%eax
801054be:	77 25                	ja     801054e5 <syscall+0x4e>
801054c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801054c3:	8b 04 85 20 b0 10 80 	mov    -0x7fef4fe0(,%eax,4),%eax
801054ca:	85 c0                	test   %eax,%eax
801054cc:	74 17                	je     801054e5 <syscall+0x4e>
    curproc->tf->eax = syscalls[num]();
801054ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054d1:	8b 58 18             	mov    0x18(%eax),%ebx
801054d4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801054d7:	8b 04 85 20 b0 10 80 	mov    -0x7fef4fe0(,%eax,4),%eax
801054de:	ff d0                	call   *%eax
801054e0:	89 43 1c             	mov    %eax,0x1c(%ebx)
801054e3:	eb 34                	jmp    80105519 <syscall+0x82>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            curproc->pid, curproc->name, num);
801054e5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054e8:	8d 48 6c             	lea    0x6c(%eax),%ecx

  num = curproc->tf->eax;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    curproc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
801054eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801054ee:	8b 40 10             	mov    0x10(%eax),%eax
801054f1:	8b 55 f0             	mov    -0x10(%ebp),%edx
801054f4:	89 54 24 0c          	mov    %edx,0xc(%esp)
801054f8:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801054fc:	89 44 24 04          	mov    %eax,0x4(%esp)
80105500:	c7 04 24 7c 86 10 80 	movl   $0x8010867c,(%esp)
80105507:	e8 b5 ae ff ff       	call   801003c1 <cprintf>
            curproc->pid, curproc->name, num);
    curproc->tf->eax = -1;
8010550c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010550f:	8b 40 18             	mov    0x18(%eax),%eax
80105512:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
80105519:	83 c4 24             	add    $0x24,%esp
8010551c:	5b                   	pop    %ebx
8010551d:	5d                   	pop    %ebp
8010551e:	c3                   	ret    
	...

80105520 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
80105520:	55                   	push   %ebp
80105521:	89 e5                	mov    %esp,%ebp
80105523:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
80105526:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105529:	89 44 24 04          	mov    %eax,0x4(%esp)
8010552d:	8b 45 08             	mov    0x8(%ebp),%eax
80105530:	89 04 24             	mov    %eax,(%esp)
80105533:	e8 91 fe ff ff       	call   801053c9 <argint>
80105538:	85 c0                	test   %eax,%eax
8010553a:	79 07                	jns    80105543 <argfd+0x23>
    return -1;
8010553c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105541:	eb 4f                	jmp    80105592 <argfd+0x72>
  if(fd < 0 || fd >= NOFILE || (f=myproc()->ofile[fd]) == 0)
80105543:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105546:	85 c0                	test   %eax,%eax
80105548:	78 20                	js     8010556a <argfd+0x4a>
8010554a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010554d:	83 f8 0f             	cmp    $0xf,%eax
80105550:	7f 18                	jg     8010556a <argfd+0x4a>
80105552:	e8 cc ec ff ff       	call   80104223 <myproc>
80105557:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010555a:	83 c2 08             	add    $0x8,%edx
8010555d:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105561:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105564:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105568:	75 07                	jne    80105571 <argfd+0x51>
    return -1;
8010556a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010556f:	eb 21                	jmp    80105592 <argfd+0x72>
  if(pfd)
80105571:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80105575:	74 08                	je     8010557f <argfd+0x5f>
    *pfd = fd;
80105577:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010557a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010557d:	89 10                	mov    %edx,(%eax)
  if(pf)
8010557f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105583:	74 08                	je     8010558d <argfd+0x6d>
    *pf = f;
80105585:	8b 45 10             	mov    0x10(%ebp),%eax
80105588:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010558b:	89 10                	mov    %edx,(%eax)
  return 0;
8010558d:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105592:	c9                   	leave  
80105593:	c3                   	ret    

80105594 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
80105594:	55                   	push   %ebp
80105595:	89 e5                	mov    %esp,%ebp
80105597:	83 ec 18             	sub    $0x18,%esp
  int fd;
  struct proc *curproc = myproc();
8010559a:	e8 84 ec ff ff       	call   80104223 <myproc>
8010559f:	89 45 f0             	mov    %eax,-0x10(%ebp)

  for(fd = 0; fd < NOFILE; fd++){
801055a2:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801055a9:	eb 29                	jmp    801055d4 <fdalloc+0x40>
    if(curproc->ofile[fd] == 0){
801055ab:	8b 45 f0             	mov    -0x10(%ebp),%eax
801055ae:	8b 55 f4             	mov    -0xc(%ebp),%edx
801055b1:	83 c2 08             	add    $0x8,%edx
801055b4:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801055b8:	85 c0                	test   %eax,%eax
801055ba:	75 15                	jne    801055d1 <fdalloc+0x3d>
      curproc->ofile[fd] = f;
801055bc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801055bf:	8b 55 f4             	mov    -0xc(%ebp),%edx
801055c2:	8d 4a 08             	lea    0x8(%edx),%ecx
801055c5:	8b 55 08             	mov    0x8(%ebp),%edx
801055c8:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
801055cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801055cf:	eb 0e                	jmp    801055df <fdalloc+0x4b>
fdalloc(struct file *f)
{
  int fd;
  struct proc *curproc = myproc();

  for(fd = 0; fd < NOFILE; fd++){
801055d1:	ff 45 f4             	incl   -0xc(%ebp)
801055d4:	83 7d f4 0f          	cmpl   $0xf,-0xc(%ebp)
801055d8:	7e d1                	jle    801055ab <fdalloc+0x17>
    if(curproc->ofile[fd] == 0){
      curproc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
801055da:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801055df:	c9                   	leave  
801055e0:	c3                   	ret    

801055e1 <sys_dup>:

int
sys_dup(void)
{
801055e1:	55                   	push   %ebp
801055e2:	89 e5                	mov    %esp,%ebp
801055e4:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;

  if(argfd(0, 0, &f) < 0)
801055e7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801055ea:	89 44 24 08          	mov    %eax,0x8(%esp)
801055ee:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801055f5:	00 
801055f6:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801055fd:	e8 1e ff ff ff       	call   80105520 <argfd>
80105602:	85 c0                	test   %eax,%eax
80105604:	79 07                	jns    8010560d <sys_dup+0x2c>
    return -1;
80105606:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010560b:	eb 29                	jmp    80105636 <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
8010560d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105610:	89 04 24             	mov    %eax,(%esp)
80105613:	e8 7c ff ff ff       	call   80105594 <fdalloc>
80105618:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010561b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010561f:	79 07                	jns    80105628 <sys_dup+0x47>
    return -1;
80105621:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105626:	eb 0e                	jmp    80105636 <sys_dup+0x55>
  filedup(f);
80105628:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010562b:	89 04 24             	mov    %eax,(%esp)
8010562e:	e8 9d ba ff ff       	call   801010d0 <filedup>
  return fd;
80105633:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80105636:	c9                   	leave  
80105637:	c3                   	ret    

80105638 <sys_read>:

int
sys_read(void)
{
80105638:	55                   	push   %ebp
80105639:	89 e5                	mov    %esp,%ebp
8010563b:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
8010563e:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105641:	89 44 24 08          	mov    %eax,0x8(%esp)
80105645:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010564c:	00 
8010564d:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105654:	e8 c7 fe ff ff       	call   80105520 <argfd>
80105659:	85 c0                	test   %eax,%eax
8010565b:	78 35                	js     80105692 <sys_read+0x5a>
8010565d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105660:	89 44 24 04          	mov    %eax,0x4(%esp)
80105664:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010566b:	e8 59 fd ff ff       	call   801053c9 <argint>
80105670:	85 c0                	test   %eax,%eax
80105672:	78 1e                	js     80105692 <sys_read+0x5a>
80105674:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105677:	89 44 24 08          	mov    %eax,0x8(%esp)
8010567b:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010567e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105682:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105689:	e8 68 fd ff ff       	call   801053f6 <argptr>
8010568e:	85 c0                	test   %eax,%eax
80105690:	79 07                	jns    80105699 <sys_read+0x61>
    return -1;
80105692:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105697:	eb 19                	jmp    801056b2 <sys_read+0x7a>
  return fileread(f, p, n);
80105699:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010569c:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010569f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801056a2:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801056a6:	89 54 24 04          	mov    %edx,0x4(%esp)
801056aa:	89 04 24             	mov    %eax,(%esp)
801056ad:	e8 7f bb ff ff       	call   80101231 <fileread>
}
801056b2:	c9                   	leave  
801056b3:	c3                   	ret    

801056b4 <sys_write>:

int
sys_write(void)
{
801056b4:	55                   	push   %ebp
801056b5:	89 e5                	mov    %esp,%ebp
801056b7:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801056ba:	8d 45 f4             	lea    -0xc(%ebp),%eax
801056bd:	89 44 24 08          	mov    %eax,0x8(%esp)
801056c1:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801056c8:	00 
801056c9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801056d0:	e8 4b fe ff ff       	call   80105520 <argfd>
801056d5:	85 c0                	test   %eax,%eax
801056d7:	78 35                	js     8010570e <sys_write+0x5a>
801056d9:	8d 45 f0             	lea    -0x10(%ebp),%eax
801056dc:	89 44 24 04          	mov    %eax,0x4(%esp)
801056e0:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801056e7:	e8 dd fc ff ff       	call   801053c9 <argint>
801056ec:	85 c0                	test   %eax,%eax
801056ee:	78 1e                	js     8010570e <sys_write+0x5a>
801056f0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801056f3:	89 44 24 08          	mov    %eax,0x8(%esp)
801056f7:	8d 45 ec             	lea    -0x14(%ebp),%eax
801056fa:	89 44 24 04          	mov    %eax,0x4(%esp)
801056fe:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105705:	e8 ec fc ff ff       	call   801053f6 <argptr>
8010570a:	85 c0                	test   %eax,%eax
8010570c:	79 07                	jns    80105715 <sys_write+0x61>
    return -1;
8010570e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105713:	eb 19                	jmp    8010572e <sys_write+0x7a>
  return filewrite(f, p, n);
80105715:	8b 4d f0             	mov    -0x10(%ebp),%ecx
80105718:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010571b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010571e:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105722:	89 54 24 04          	mov    %edx,0x4(%esp)
80105726:	89 04 24             	mov    %eax,(%esp)
80105729:	e8 be bb ff ff       	call   801012ec <filewrite>
}
8010572e:	c9                   	leave  
8010572f:	c3                   	ret    

80105730 <sys_close>:

int
sys_close(void)
{
80105730:	55                   	push   %ebp
80105731:	89 e5                	mov    %esp,%ebp
80105733:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argfd(0, &fd, &f) < 0)
80105736:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105739:	89 44 24 08          	mov    %eax,0x8(%esp)
8010573d:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105740:	89 44 24 04          	mov    %eax,0x4(%esp)
80105744:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010574b:	e8 d0 fd ff ff       	call   80105520 <argfd>
80105750:	85 c0                	test   %eax,%eax
80105752:	79 07                	jns    8010575b <sys_close+0x2b>
    return -1;
80105754:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105759:	eb 23                	jmp    8010577e <sys_close+0x4e>
  myproc()->ofile[fd] = 0;
8010575b:	e8 c3 ea ff ff       	call   80104223 <myproc>
80105760:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105763:	83 c2 08             	add    $0x8,%edx
80105766:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010576d:	00 
  fileclose(f);
8010576e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105771:	89 04 24             	mov    %eax,(%esp)
80105774:	e8 9f b9 ff ff       	call   80101118 <fileclose>
  return 0;
80105779:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010577e:	c9                   	leave  
8010577f:	c3                   	ret    

80105780 <sys_fstat>:

int
sys_fstat(void)
{
80105780:	55                   	push   %ebp
80105781:	89 e5                	mov    %esp,%ebp
80105783:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;

  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
80105786:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105789:	89 44 24 08          	mov    %eax,0x8(%esp)
8010578d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105794:	00 
80105795:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010579c:	e8 7f fd ff ff       	call   80105520 <argfd>
801057a1:	85 c0                	test   %eax,%eax
801057a3:	78 1f                	js     801057c4 <sys_fstat+0x44>
801057a5:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801057ac:	00 
801057ad:	8d 45 f0             	lea    -0x10(%ebp),%eax
801057b0:	89 44 24 04          	mov    %eax,0x4(%esp)
801057b4:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801057bb:	e8 36 fc ff ff       	call   801053f6 <argptr>
801057c0:	85 c0                	test   %eax,%eax
801057c2:	79 07                	jns    801057cb <sys_fstat+0x4b>
    return -1;
801057c4:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057c9:	eb 12                	jmp    801057dd <sys_fstat+0x5d>
  return filestat(f, st);
801057cb:	8b 55 f0             	mov    -0x10(%ebp),%edx
801057ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801057d1:	89 54 24 04          	mov    %edx,0x4(%esp)
801057d5:	89 04 24             	mov    %eax,(%esp)
801057d8:	e8 05 ba ff ff       	call   801011e2 <filestat>
}
801057dd:	c9                   	leave  
801057de:	c3                   	ret    

801057df <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
801057df:	55                   	push   %ebp
801057e0:	89 e5                	mov    %esp,%ebp
801057e2:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
801057e5:	8d 45 d8             	lea    -0x28(%ebp),%eax
801057e8:	89 44 24 04          	mov    %eax,0x4(%esp)
801057ec:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801057f3:	e8 68 fc ff ff       	call   80105460 <argstr>
801057f8:	85 c0                	test   %eax,%eax
801057fa:	78 17                	js     80105813 <sys_link+0x34>
801057fc:	8d 45 dc             	lea    -0x24(%ebp),%eax
801057ff:	89 44 24 04          	mov    %eax,0x4(%esp)
80105803:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010580a:	e8 51 fc ff ff       	call   80105460 <argstr>
8010580f:	85 c0                	test   %eax,%eax
80105811:	79 0a                	jns    8010581d <sys_link+0x3e>
    return -1;
80105813:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105818:	e9 3d 01 00 00       	jmp    8010595a <sys_link+0x17b>

  begin_op();
8010581d:	e8 09 dd ff ff       	call   8010352b <begin_op>
  if((ip = namei(old)) == 0){
80105822:	8b 45 d8             	mov    -0x28(%ebp),%eax
80105825:	89 04 24             	mov    %eax,(%esp)
80105828:	e8 2a cd ff ff       	call   80102557 <namei>
8010582d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105830:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105834:	75 0f                	jne    80105845 <sys_link+0x66>
    end_op();
80105836:	e8 72 dd ff ff       	call   801035ad <end_op>
    return -1;
8010583b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105840:	e9 15 01 00 00       	jmp    8010595a <sys_link+0x17b>
  }

  ilock(ip);
80105845:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105848:	89 04 24             	mov    %eax,(%esp)
8010584b:	e8 e2 c1 ff ff       	call   80101a32 <ilock>
  if(ip->type == T_DIR){
80105850:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105853:	8b 40 50             	mov    0x50(%eax),%eax
80105856:	66 83 f8 01          	cmp    $0x1,%ax
8010585a:	75 1a                	jne    80105876 <sys_link+0x97>
    iunlockput(ip);
8010585c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010585f:	89 04 24             	mov    %eax,(%esp)
80105862:	e8 ca c3 ff ff       	call   80101c31 <iunlockput>
    end_op();
80105867:	e8 41 dd ff ff       	call   801035ad <end_op>
    return -1;
8010586c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105871:	e9 e4 00 00 00       	jmp    8010595a <sys_link+0x17b>
  }

  ip->nlink++;
80105876:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105879:	66 8b 40 56          	mov    0x56(%eax),%ax
8010587d:	40                   	inc    %eax
8010587e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105881:	66 89 42 56          	mov    %ax,0x56(%edx)
  iupdate(ip);
80105885:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105888:	89 04 24             	mov    %eax,(%esp)
8010588b:	e8 df bf ff ff       	call   8010186f <iupdate>
  iunlock(ip);
80105890:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105893:	89 04 24             	mov    %eax,(%esp)
80105896:	e8 a1 c2 ff ff       	call   80101b3c <iunlock>

  if((dp = nameiparent(new, name)) == 0)
8010589b:	8b 45 dc             	mov    -0x24(%ebp),%eax
8010589e:	8d 55 e2             	lea    -0x1e(%ebp),%edx
801058a1:	89 54 24 04          	mov    %edx,0x4(%esp)
801058a5:	89 04 24             	mov    %eax,(%esp)
801058a8:	e8 cc cc ff ff       	call   80102579 <nameiparent>
801058ad:	89 45 f0             	mov    %eax,-0x10(%ebp)
801058b0:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801058b4:	75 02                	jne    801058b8 <sys_link+0xd9>
    goto bad;
801058b6:	eb 68                	jmp    80105920 <sys_link+0x141>
  ilock(dp);
801058b8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058bb:	89 04 24             	mov    %eax,(%esp)
801058be:	e8 6f c1 ff ff       	call   80101a32 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
801058c3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058c6:	8b 10                	mov    (%eax),%edx
801058c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058cb:	8b 00                	mov    (%eax),%eax
801058cd:	39 c2                	cmp    %eax,%edx
801058cf:	75 20                	jne    801058f1 <sys_link+0x112>
801058d1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058d4:	8b 40 04             	mov    0x4(%eax),%eax
801058d7:	89 44 24 08          	mov    %eax,0x8(%esp)
801058db:	8d 45 e2             	lea    -0x1e(%ebp),%eax
801058de:	89 44 24 04          	mov    %eax,0x4(%esp)
801058e2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058e5:	89 04 24             	mov    %eax,(%esp)
801058e8:	e8 b7 c9 ff ff       	call   801022a4 <dirlink>
801058ed:	85 c0                	test   %eax,%eax
801058ef:	79 0d                	jns    801058fe <sys_link+0x11f>
    iunlockput(dp);
801058f1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058f4:	89 04 24             	mov    %eax,(%esp)
801058f7:	e8 35 c3 ff ff       	call   80101c31 <iunlockput>
    goto bad;
801058fc:	eb 22                	jmp    80105920 <sys_link+0x141>
  }
  iunlockput(dp);
801058fe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105901:	89 04 24             	mov    %eax,(%esp)
80105904:	e8 28 c3 ff ff       	call   80101c31 <iunlockput>
  iput(ip);
80105909:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010590c:	89 04 24             	mov    %eax,(%esp)
8010590f:	e8 6c c2 ff ff       	call   80101b80 <iput>

  end_op();
80105914:	e8 94 dc ff ff       	call   801035ad <end_op>

  return 0;
80105919:	b8 00 00 00 00       	mov    $0x0,%eax
8010591e:	eb 3a                	jmp    8010595a <sys_link+0x17b>

bad:
  ilock(ip);
80105920:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105923:	89 04 24             	mov    %eax,(%esp)
80105926:	e8 07 c1 ff ff       	call   80101a32 <ilock>
  ip->nlink--;
8010592b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010592e:	66 8b 40 56          	mov    0x56(%eax),%ax
80105932:	48                   	dec    %eax
80105933:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105936:	66 89 42 56          	mov    %ax,0x56(%edx)
  iupdate(ip);
8010593a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010593d:	89 04 24             	mov    %eax,(%esp)
80105940:	e8 2a bf ff ff       	call   8010186f <iupdate>
  iunlockput(ip);
80105945:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105948:	89 04 24             	mov    %eax,(%esp)
8010594b:	e8 e1 c2 ff ff       	call   80101c31 <iunlockput>
  end_op();
80105950:	e8 58 dc ff ff       	call   801035ad <end_op>
  return -1;
80105955:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010595a:	c9                   	leave  
8010595b:	c3                   	ret    

8010595c <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
8010595c:	55                   	push   %ebp
8010595d:	89 e5                	mov    %esp,%ebp
8010595f:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105962:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80105969:	eb 4a                	jmp    801059b5 <isdirempty+0x59>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
8010596b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010596e:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105975:	00 
80105976:	89 44 24 08          	mov    %eax,0x8(%esp)
8010597a:	8d 45 e4             	lea    -0x1c(%ebp),%eax
8010597d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105981:	8b 45 08             	mov    0x8(%ebp),%eax
80105984:	89 04 24             	mov    %eax,(%esp)
80105987:	e8 3d c5 ff ff       	call   80101ec9 <readi>
8010598c:	83 f8 10             	cmp    $0x10,%eax
8010598f:	74 0c                	je     8010599d <isdirempty+0x41>
      panic("isdirempty: readi");
80105991:	c7 04 24 98 86 10 80 	movl   $0x80108698,(%esp)
80105998:	e8 b7 ab ff ff       	call   80100554 <panic>
    if(de.inum != 0)
8010599d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801059a0:	66 85 c0             	test   %ax,%ax
801059a3:	74 07                	je     801059ac <isdirempty+0x50>
      return 0;
801059a5:	b8 00 00 00 00       	mov    $0x0,%eax
801059aa:	eb 1b                	jmp    801059c7 <isdirempty+0x6b>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
801059ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801059af:	83 c0 10             	add    $0x10,%eax
801059b2:	89 45 f4             	mov    %eax,-0xc(%ebp)
801059b5:	8b 55 f4             	mov    -0xc(%ebp),%edx
801059b8:	8b 45 08             	mov    0x8(%ebp),%eax
801059bb:	8b 40 58             	mov    0x58(%eax),%eax
801059be:	39 c2                	cmp    %eax,%edx
801059c0:	72 a9                	jb     8010596b <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
801059c2:	b8 01 00 00 00       	mov    $0x1,%eax
}
801059c7:	c9                   	leave  
801059c8:	c3                   	ret    

801059c9 <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
801059c9:	55                   	push   %ebp
801059ca:	89 e5                	mov    %esp,%ebp
801059cc:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
801059cf:	8d 45 cc             	lea    -0x34(%ebp),%eax
801059d2:	89 44 24 04          	mov    %eax,0x4(%esp)
801059d6:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801059dd:	e8 7e fa ff ff       	call   80105460 <argstr>
801059e2:	85 c0                	test   %eax,%eax
801059e4:	79 0a                	jns    801059f0 <sys_unlink+0x27>
    return -1;
801059e6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059eb:	e9 a9 01 00 00       	jmp    80105b99 <sys_unlink+0x1d0>

  begin_op();
801059f0:	e8 36 db ff ff       	call   8010352b <begin_op>
  if((dp = nameiparent(path, name)) == 0){
801059f5:	8b 45 cc             	mov    -0x34(%ebp),%eax
801059f8:	8d 55 d2             	lea    -0x2e(%ebp),%edx
801059fb:	89 54 24 04          	mov    %edx,0x4(%esp)
801059ff:	89 04 24             	mov    %eax,(%esp)
80105a02:	e8 72 cb ff ff       	call   80102579 <nameiparent>
80105a07:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105a0a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105a0e:	75 0f                	jne    80105a1f <sys_unlink+0x56>
    end_op();
80105a10:	e8 98 db ff ff       	call   801035ad <end_op>
    return -1;
80105a15:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a1a:	e9 7a 01 00 00       	jmp    80105b99 <sys_unlink+0x1d0>
  }

  ilock(dp);
80105a1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a22:	89 04 24             	mov    %eax,(%esp)
80105a25:	e8 08 c0 ff ff       	call   80101a32 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105a2a:	c7 44 24 04 aa 86 10 	movl   $0x801086aa,0x4(%esp)
80105a31:	80 
80105a32:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105a35:	89 04 24             	mov    %eax,(%esp)
80105a38:	e8 7f c7 ff ff       	call   801021bc <namecmp>
80105a3d:	85 c0                	test   %eax,%eax
80105a3f:	0f 84 3f 01 00 00    	je     80105b84 <sys_unlink+0x1bb>
80105a45:	c7 44 24 04 ac 86 10 	movl   $0x801086ac,0x4(%esp)
80105a4c:	80 
80105a4d:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105a50:	89 04 24             	mov    %eax,(%esp)
80105a53:	e8 64 c7 ff ff       	call   801021bc <namecmp>
80105a58:	85 c0                	test   %eax,%eax
80105a5a:	0f 84 24 01 00 00    	je     80105b84 <sys_unlink+0x1bb>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
80105a60:	8d 45 c8             	lea    -0x38(%ebp),%eax
80105a63:	89 44 24 08          	mov    %eax,0x8(%esp)
80105a67:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105a6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a71:	89 04 24             	mov    %eax,(%esp)
80105a74:	e8 65 c7 ff ff       	call   801021de <dirlookup>
80105a79:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105a7c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105a80:	75 05                	jne    80105a87 <sys_unlink+0xbe>
    goto bad;
80105a82:	e9 fd 00 00 00       	jmp    80105b84 <sys_unlink+0x1bb>
  ilock(ip);
80105a87:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a8a:	89 04 24             	mov    %eax,(%esp)
80105a8d:	e8 a0 bf ff ff       	call   80101a32 <ilock>

  if(ip->nlink < 1)
80105a92:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105a95:	66 8b 40 56          	mov    0x56(%eax),%ax
80105a99:	66 85 c0             	test   %ax,%ax
80105a9c:	7f 0c                	jg     80105aaa <sys_unlink+0xe1>
    panic("unlink: nlink < 1");
80105a9e:	c7 04 24 af 86 10 80 	movl   $0x801086af,(%esp)
80105aa5:	e8 aa aa ff ff       	call   80100554 <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105aaa:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105aad:	8b 40 50             	mov    0x50(%eax),%eax
80105ab0:	66 83 f8 01          	cmp    $0x1,%ax
80105ab4:	75 1f                	jne    80105ad5 <sys_unlink+0x10c>
80105ab6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ab9:	89 04 24             	mov    %eax,(%esp)
80105abc:	e8 9b fe ff ff       	call   8010595c <isdirempty>
80105ac1:	85 c0                	test   %eax,%eax
80105ac3:	75 10                	jne    80105ad5 <sys_unlink+0x10c>
    iunlockput(ip);
80105ac5:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ac8:	89 04 24             	mov    %eax,(%esp)
80105acb:	e8 61 c1 ff ff       	call   80101c31 <iunlockput>
    goto bad;
80105ad0:	e9 af 00 00 00       	jmp    80105b84 <sys_unlink+0x1bb>
  }

  memset(&de, 0, sizeof(de));
80105ad5:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105adc:	00 
80105add:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105ae4:	00 
80105ae5:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105ae8:	89 04 24             	mov    %eax,(%esp)
80105aeb:	e8 a6 f5 ff ff       	call   80105096 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105af0:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105af3:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105afa:	00 
80105afb:	89 44 24 08          	mov    %eax,0x8(%esp)
80105aff:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105b02:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b06:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b09:	89 04 24             	mov    %eax,(%esp)
80105b0c:	e8 1c c5 ff ff       	call   8010202d <writei>
80105b11:	83 f8 10             	cmp    $0x10,%eax
80105b14:	74 0c                	je     80105b22 <sys_unlink+0x159>
    panic("unlink: writei");
80105b16:	c7 04 24 c1 86 10 80 	movl   $0x801086c1,(%esp)
80105b1d:	e8 32 aa ff ff       	call   80100554 <panic>
  if(ip->type == T_DIR){
80105b22:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b25:	8b 40 50             	mov    0x50(%eax),%eax
80105b28:	66 83 f8 01          	cmp    $0x1,%ax
80105b2c:	75 1a                	jne    80105b48 <sys_unlink+0x17f>
    dp->nlink--;
80105b2e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b31:	66 8b 40 56          	mov    0x56(%eax),%ax
80105b35:	48                   	dec    %eax
80105b36:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105b39:	66 89 42 56          	mov    %ax,0x56(%edx)
    iupdate(dp);
80105b3d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b40:	89 04 24             	mov    %eax,(%esp)
80105b43:	e8 27 bd ff ff       	call   8010186f <iupdate>
  }
  iunlockput(dp);
80105b48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b4b:	89 04 24             	mov    %eax,(%esp)
80105b4e:	e8 de c0 ff ff       	call   80101c31 <iunlockput>

  ip->nlink--;
80105b53:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b56:	66 8b 40 56          	mov    0x56(%eax),%ax
80105b5a:	48                   	dec    %eax
80105b5b:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105b5e:	66 89 42 56          	mov    %ax,0x56(%edx)
  iupdate(ip);
80105b62:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b65:	89 04 24             	mov    %eax,(%esp)
80105b68:	e8 02 bd ff ff       	call   8010186f <iupdate>
  iunlockput(ip);
80105b6d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b70:	89 04 24             	mov    %eax,(%esp)
80105b73:	e8 b9 c0 ff ff       	call   80101c31 <iunlockput>

  end_op();
80105b78:	e8 30 da ff ff       	call   801035ad <end_op>

  return 0;
80105b7d:	b8 00 00 00 00       	mov    $0x0,%eax
80105b82:	eb 15                	jmp    80105b99 <sys_unlink+0x1d0>

bad:
  iunlockput(dp);
80105b84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b87:	89 04 24             	mov    %eax,(%esp)
80105b8a:	e8 a2 c0 ff ff       	call   80101c31 <iunlockput>
  end_op();
80105b8f:	e8 19 da ff ff       	call   801035ad <end_op>
  return -1;
80105b94:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105b99:	c9                   	leave  
80105b9a:	c3                   	ret    

80105b9b <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105b9b:	55                   	push   %ebp
80105b9c:	89 e5                	mov    %esp,%ebp
80105b9e:	83 ec 48             	sub    $0x48,%esp
80105ba1:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105ba4:	8b 55 10             	mov    0x10(%ebp),%edx
80105ba7:	8b 45 14             	mov    0x14(%ebp),%eax
80105baa:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105bae:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105bb2:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105bb6:	8d 45 de             	lea    -0x22(%ebp),%eax
80105bb9:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bbd:	8b 45 08             	mov    0x8(%ebp),%eax
80105bc0:	89 04 24             	mov    %eax,(%esp)
80105bc3:	e8 b1 c9 ff ff       	call   80102579 <nameiparent>
80105bc8:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bcb:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105bcf:	75 0a                	jne    80105bdb <create+0x40>
    return 0;
80105bd1:	b8 00 00 00 00       	mov    $0x0,%eax
80105bd6:	e9 79 01 00 00       	jmp    80105d54 <create+0x1b9>
  ilock(dp);
80105bdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bde:	89 04 24             	mov    %eax,(%esp)
80105be1:	e8 4c be ff ff       	call   80101a32 <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
80105be6:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105be9:	89 44 24 08          	mov    %eax,0x8(%esp)
80105bed:	8d 45 de             	lea    -0x22(%ebp),%eax
80105bf0:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bf4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105bf7:	89 04 24             	mov    %eax,(%esp)
80105bfa:	e8 df c5 ff ff       	call   801021de <dirlookup>
80105bff:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105c02:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105c06:	74 46                	je     80105c4e <create+0xb3>
    iunlockput(dp);
80105c08:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c0b:	89 04 24             	mov    %eax,(%esp)
80105c0e:	e8 1e c0 ff ff       	call   80101c31 <iunlockput>
    ilock(ip);
80105c13:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c16:	89 04 24             	mov    %eax,(%esp)
80105c19:	e8 14 be ff ff       	call   80101a32 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105c1e:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105c23:	75 14                	jne    80105c39 <create+0x9e>
80105c25:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c28:	8b 40 50             	mov    0x50(%eax),%eax
80105c2b:	66 83 f8 02          	cmp    $0x2,%ax
80105c2f:	75 08                	jne    80105c39 <create+0x9e>
      return ip;
80105c31:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c34:	e9 1b 01 00 00       	jmp    80105d54 <create+0x1b9>
    iunlockput(ip);
80105c39:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c3c:	89 04 24             	mov    %eax,(%esp)
80105c3f:	e8 ed bf ff ff       	call   80101c31 <iunlockput>
    return 0;
80105c44:	b8 00 00 00 00       	mov    $0x0,%eax
80105c49:	e9 06 01 00 00       	jmp    80105d54 <create+0x1b9>
  }

  if((ip = ialloc(dp->dev, type)) == 0)
80105c4e:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80105c52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c55:	8b 00                	mov    (%eax),%eax
80105c57:	89 54 24 04          	mov    %edx,0x4(%esp)
80105c5b:	89 04 24             	mov    %eax,(%esp)
80105c5e:	e8 3a bb ff ff       	call   8010179d <ialloc>
80105c63:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105c66:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105c6a:	75 0c                	jne    80105c78 <create+0xdd>
    panic("create: ialloc");
80105c6c:	c7 04 24 d0 86 10 80 	movl   $0x801086d0,(%esp)
80105c73:	e8 dc a8 ff ff       	call   80100554 <panic>

  ilock(ip);
80105c78:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c7b:	89 04 24             	mov    %eax,(%esp)
80105c7e:	e8 af bd ff ff       	call   80101a32 <ilock>
  ip->major = major;
80105c83:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105c86:	8b 45 d0             	mov    -0x30(%ebp),%eax
80105c89:	66 89 42 52          	mov    %ax,0x52(%edx)
  ip->minor = minor;
80105c8d:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105c90:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105c93:	66 89 42 54          	mov    %ax,0x54(%edx)
  ip->nlink = 1;
80105c97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105c9a:	66 c7 40 56 01 00    	movw   $0x1,0x56(%eax)
  iupdate(ip);
80105ca0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ca3:	89 04 24             	mov    %eax,(%esp)
80105ca6:	e8 c4 bb ff ff       	call   8010186f <iupdate>

  if(type == T_DIR){  // Create . and .. entries.
80105cab:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105cb0:	75 68                	jne    80105d1a <create+0x17f>
    dp->nlink++;  // for ".."
80105cb2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cb5:	66 8b 40 56          	mov    0x56(%eax),%ax
80105cb9:	40                   	inc    %eax
80105cba:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105cbd:	66 89 42 56          	mov    %ax,0x56(%edx)
    iupdate(dp);
80105cc1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cc4:	89 04 24             	mov    %eax,(%esp)
80105cc7:	e8 a3 bb ff ff       	call   8010186f <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105ccc:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ccf:	8b 40 04             	mov    0x4(%eax),%eax
80105cd2:	89 44 24 08          	mov    %eax,0x8(%esp)
80105cd6:	c7 44 24 04 aa 86 10 	movl   $0x801086aa,0x4(%esp)
80105cdd:	80 
80105cde:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ce1:	89 04 24             	mov    %eax,(%esp)
80105ce4:	e8 bb c5 ff ff       	call   801022a4 <dirlink>
80105ce9:	85 c0                	test   %eax,%eax
80105ceb:	78 21                	js     80105d0e <create+0x173>
80105ced:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105cf0:	8b 40 04             	mov    0x4(%eax),%eax
80105cf3:	89 44 24 08          	mov    %eax,0x8(%esp)
80105cf7:	c7 44 24 04 ac 86 10 	movl   $0x801086ac,0x4(%esp)
80105cfe:	80 
80105cff:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d02:	89 04 24             	mov    %eax,(%esp)
80105d05:	e8 9a c5 ff ff       	call   801022a4 <dirlink>
80105d0a:	85 c0                	test   %eax,%eax
80105d0c:	79 0c                	jns    80105d1a <create+0x17f>
      panic("create dots");
80105d0e:	c7 04 24 df 86 10 80 	movl   $0x801086df,(%esp)
80105d15:	e8 3a a8 ff ff       	call   80100554 <panic>
  }

  if(dirlink(dp, name, ip->inum) < 0)
80105d1a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d1d:	8b 40 04             	mov    0x4(%eax),%eax
80105d20:	89 44 24 08          	mov    %eax,0x8(%esp)
80105d24:	8d 45 de             	lea    -0x22(%ebp),%eax
80105d27:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d2b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d2e:	89 04 24             	mov    %eax,(%esp)
80105d31:	e8 6e c5 ff ff       	call   801022a4 <dirlink>
80105d36:	85 c0                	test   %eax,%eax
80105d38:	79 0c                	jns    80105d46 <create+0x1ab>
    panic("create: dirlink");
80105d3a:	c7 04 24 eb 86 10 80 	movl   $0x801086eb,(%esp)
80105d41:	e8 0e a8 ff ff       	call   80100554 <panic>

  iunlockput(dp);
80105d46:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d49:	89 04 24             	mov    %eax,(%esp)
80105d4c:	e8 e0 be ff ff       	call   80101c31 <iunlockput>

  return ip;
80105d51:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105d54:	c9                   	leave  
80105d55:	c3                   	ret    

80105d56 <sys_open>:

int
sys_open(void)
{
80105d56:	55                   	push   %ebp
80105d57:	89 e5                	mov    %esp,%ebp
80105d59:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105d5c:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105d5f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d63:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105d6a:	e8 f1 f6 ff ff       	call   80105460 <argstr>
80105d6f:	85 c0                	test   %eax,%eax
80105d71:	78 17                	js     80105d8a <sys_open+0x34>
80105d73:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105d76:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d7a:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105d81:	e8 43 f6 ff ff       	call   801053c9 <argint>
80105d86:	85 c0                	test   %eax,%eax
80105d88:	79 0a                	jns    80105d94 <sys_open+0x3e>
    return -1;
80105d8a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105d8f:	e9 5b 01 00 00       	jmp    80105eef <sys_open+0x199>

  begin_op();
80105d94:	e8 92 d7 ff ff       	call   8010352b <begin_op>

  if(omode & O_CREATE){
80105d99:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105d9c:	25 00 02 00 00       	and    $0x200,%eax
80105da1:	85 c0                	test   %eax,%eax
80105da3:	74 3b                	je     80105de0 <sys_open+0x8a>
    ip = create(path, T_FILE, 0, 0);
80105da5:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105da8:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105daf:	00 
80105db0:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105db7:	00 
80105db8:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105dbf:	00 
80105dc0:	89 04 24             	mov    %eax,(%esp)
80105dc3:	e8 d3 fd ff ff       	call   80105b9b <create>
80105dc8:	89 45 f4             	mov    %eax,-0xc(%ebp)
    if(ip == 0){
80105dcb:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105dcf:	75 6a                	jne    80105e3b <sys_open+0xe5>
      end_op();
80105dd1:	e8 d7 d7 ff ff       	call   801035ad <end_op>
      return -1;
80105dd6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105ddb:	e9 0f 01 00 00       	jmp    80105eef <sys_open+0x199>
    }
  } else {
    if((ip = namei(path)) == 0){
80105de0:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105de3:	89 04 24             	mov    %eax,(%esp)
80105de6:	e8 6c c7 ff ff       	call   80102557 <namei>
80105deb:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105dee:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105df2:	75 0f                	jne    80105e03 <sys_open+0xad>
      end_op();
80105df4:	e8 b4 d7 ff ff       	call   801035ad <end_op>
      return -1;
80105df9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105dfe:	e9 ec 00 00 00       	jmp    80105eef <sys_open+0x199>
    }
    ilock(ip);
80105e03:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e06:	89 04 24             	mov    %eax,(%esp)
80105e09:	e8 24 bc ff ff       	call   80101a32 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80105e0e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e11:	8b 40 50             	mov    0x50(%eax),%eax
80105e14:	66 83 f8 01          	cmp    $0x1,%ax
80105e18:	75 21                	jne    80105e3b <sys_open+0xe5>
80105e1a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105e1d:	85 c0                	test   %eax,%eax
80105e1f:	74 1a                	je     80105e3b <sys_open+0xe5>
      iunlockput(ip);
80105e21:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e24:	89 04 24             	mov    %eax,(%esp)
80105e27:	e8 05 be ff ff       	call   80101c31 <iunlockput>
      end_op();
80105e2c:	e8 7c d7 ff ff       	call   801035ad <end_op>
      return -1;
80105e31:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e36:	e9 b4 00 00 00       	jmp    80105eef <sys_open+0x199>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
80105e3b:	e8 30 b2 ff ff       	call   80101070 <filealloc>
80105e40:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105e43:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105e47:	74 14                	je     80105e5d <sys_open+0x107>
80105e49:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e4c:	89 04 24             	mov    %eax,(%esp)
80105e4f:	e8 40 f7 ff ff       	call   80105594 <fdalloc>
80105e54:	89 45 ec             	mov    %eax,-0x14(%ebp)
80105e57:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80105e5b:	79 28                	jns    80105e85 <sys_open+0x12f>
    if(f)
80105e5d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105e61:	74 0b                	je     80105e6e <sys_open+0x118>
      fileclose(f);
80105e63:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e66:	89 04 24             	mov    %eax,(%esp)
80105e69:	e8 aa b2 ff ff       	call   80101118 <fileclose>
    iunlockput(ip);
80105e6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e71:	89 04 24             	mov    %eax,(%esp)
80105e74:	e8 b8 bd ff ff       	call   80101c31 <iunlockput>
    end_op();
80105e79:	e8 2f d7 ff ff       	call   801035ad <end_op>
    return -1;
80105e7e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105e83:	eb 6a                	jmp    80105eef <sys_open+0x199>
  }
  iunlock(ip);
80105e85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e88:	89 04 24             	mov    %eax,(%esp)
80105e8b:	e8 ac bc ff ff       	call   80101b3c <iunlock>
  end_op();
80105e90:	e8 18 d7 ff ff       	call   801035ad <end_op>

  f->type = FD_INODE;
80105e95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e98:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
80105e9e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ea1:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105ea4:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
80105ea7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105eaa:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
80105eb1:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105eb4:	83 e0 01             	and    $0x1,%eax
80105eb7:	85 c0                	test   %eax,%eax
80105eb9:	0f 94 c0             	sete   %al
80105ebc:	88 c2                	mov    %al,%dl
80105ebe:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ec1:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
80105ec4:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105ec7:	83 e0 01             	and    $0x1,%eax
80105eca:	85 c0                	test   %eax,%eax
80105ecc:	75 0a                	jne    80105ed8 <sys_open+0x182>
80105ece:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105ed1:	83 e0 02             	and    $0x2,%eax
80105ed4:	85 c0                	test   %eax,%eax
80105ed6:	74 07                	je     80105edf <sys_open+0x189>
80105ed8:	b8 01 00 00 00       	mov    $0x1,%eax
80105edd:	eb 05                	jmp    80105ee4 <sys_open+0x18e>
80105edf:	b8 00 00 00 00       	mov    $0x0,%eax
80105ee4:	88 c2                	mov    %al,%dl
80105ee6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ee9:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80105eec:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80105eef:	c9                   	leave  
80105ef0:	c3                   	ret    

80105ef1 <sys_mkdir>:

int
sys_mkdir(void)
{
80105ef1:	55                   	push   %ebp
80105ef2:	89 e5                	mov    %esp,%ebp
80105ef4:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_op();
80105ef7:	e8 2f d6 ff ff       	call   8010352b <begin_op>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80105efc:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105eff:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f03:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f0a:	e8 51 f5 ff ff       	call   80105460 <argstr>
80105f0f:	85 c0                	test   %eax,%eax
80105f11:	78 2c                	js     80105f3f <sys_mkdir+0x4e>
80105f13:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f16:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105f1d:	00 
80105f1e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105f25:	00 
80105f26:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80105f2d:	00 
80105f2e:	89 04 24             	mov    %eax,(%esp)
80105f31:	e8 65 fc ff ff       	call   80105b9b <create>
80105f36:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105f39:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105f3d:	75 0c                	jne    80105f4b <sys_mkdir+0x5a>
    end_op();
80105f3f:	e8 69 d6 ff ff       	call   801035ad <end_op>
    return -1;
80105f44:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105f49:	eb 15                	jmp    80105f60 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80105f4b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f4e:	89 04 24             	mov    %eax,(%esp)
80105f51:	e8 db bc ff ff       	call   80101c31 <iunlockput>
  end_op();
80105f56:	e8 52 d6 ff ff       	call   801035ad <end_op>
  return 0;
80105f5b:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105f60:	c9                   	leave  
80105f61:	c3                   	ret    

80105f62 <sys_mknod>:

int
sys_mknod(void)
{
80105f62:	55                   	push   %ebp
80105f63:	89 e5                	mov    %esp,%ebp
80105f65:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip;
  char *path;
  int major, minor;

  begin_op();
80105f68:	e8 be d5 ff ff       	call   8010352b <begin_op>
  if((argstr(0, &path)) < 0 ||
80105f6d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105f70:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f74:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105f7b:	e8 e0 f4 ff ff       	call   80105460 <argstr>
80105f80:	85 c0                	test   %eax,%eax
80105f82:	78 5e                	js     80105fe2 <sys_mknod+0x80>
     argint(1, &major) < 0 ||
80105f84:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105f87:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f8b:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105f92:	e8 32 f4 ff ff       	call   801053c9 <argint>
  struct inode *ip;
  char *path;
  int major, minor;

  begin_op();
  if((argstr(0, &path)) < 0 ||
80105f97:	85 c0                	test   %eax,%eax
80105f99:	78 47                	js     80105fe2 <sys_mknod+0x80>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105f9b:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105f9e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105fa2:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
80105fa9:	e8 1b f4 ff ff       	call   801053c9 <argint>
  char *path;
  int major, minor;

  begin_op();
  if((argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
80105fae:	85 c0                	test   %eax,%eax
80105fb0:	78 30                	js     80105fe2 <sys_mknod+0x80>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
80105fb2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105fb5:	0f bf c8             	movswl %ax,%ecx
80105fb8:	8b 45 ec             	mov    -0x14(%ebp),%eax
80105fbb:	0f bf d0             	movswl %ax,%edx
80105fbe:	8b 45 f0             	mov    -0x10(%ebp),%eax
  int major, minor;

  begin_op();
  if((argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
80105fc1:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80105fc5:	89 54 24 08          	mov    %edx,0x8(%esp)
80105fc9:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80105fd0:	00 
80105fd1:	89 04 24             	mov    %eax,(%esp)
80105fd4:	e8 c2 fb ff ff       	call   80105b9b <create>
80105fd9:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105fdc:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105fe0:	75 0c                	jne    80105fee <sys_mknod+0x8c>
     (ip = create(path, T_DEV, major, minor)) == 0){
    end_op();
80105fe2:	e8 c6 d5 ff ff       	call   801035ad <end_op>
    return -1;
80105fe7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fec:	eb 15                	jmp    80106003 <sys_mknod+0xa1>
  }
  iunlockput(ip);
80105fee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ff1:	89 04 24             	mov    %eax,(%esp)
80105ff4:	e8 38 bc ff ff       	call   80101c31 <iunlockput>
  end_op();
80105ff9:	e8 af d5 ff ff       	call   801035ad <end_op>
  return 0;
80105ffe:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106003:	c9                   	leave  
80106004:	c3                   	ret    

80106005 <sys_chdir>:

int
sys_chdir(void)
{
80106005:	55                   	push   %ebp
80106006:	89 e5                	mov    %esp,%ebp
80106008:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;
  struct proc *curproc = myproc();
8010600b:	e8 13 e2 ff ff       	call   80104223 <myproc>
80106010:	89 45 f4             	mov    %eax,-0xc(%ebp)
  
  begin_op();
80106013:	e8 13 d5 ff ff       	call   8010352b <begin_op>
  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0){
80106018:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010601b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010601f:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106026:	e8 35 f4 ff ff       	call   80105460 <argstr>
8010602b:	85 c0                	test   %eax,%eax
8010602d:	78 14                	js     80106043 <sys_chdir+0x3e>
8010602f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106032:	89 04 24             	mov    %eax,(%esp)
80106035:	e8 1d c5 ff ff       	call   80102557 <namei>
8010603a:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010603d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106041:	75 0c                	jne    8010604f <sys_chdir+0x4a>
    end_op();
80106043:	e8 65 d5 ff ff       	call   801035ad <end_op>
    return -1;
80106048:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010604d:	eb 5a                	jmp    801060a9 <sys_chdir+0xa4>
  }
  ilock(ip);
8010604f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106052:	89 04 24             	mov    %eax,(%esp)
80106055:	e8 d8 b9 ff ff       	call   80101a32 <ilock>
  if(ip->type != T_DIR){
8010605a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010605d:	8b 40 50             	mov    0x50(%eax),%eax
80106060:	66 83 f8 01          	cmp    $0x1,%ax
80106064:	74 17                	je     8010607d <sys_chdir+0x78>
    iunlockput(ip);
80106066:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106069:	89 04 24             	mov    %eax,(%esp)
8010606c:	e8 c0 bb ff ff       	call   80101c31 <iunlockput>
    end_op();
80106071:	e8 37 d5 ff ff       	call   801035ad <end_op>
    return -1;
80106076:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010607b:	eb 2c                	jmp    801060a9 <sys_chdir+0xa4>
  }
  iunlock(ip);
8010607d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106080:	89 04 24             	mov    %eax,(%esp)
80106083:	e8 b4 ba ff ff       	call   80101b3c <iunlock>
  iput(curproc->cwd);
80106088:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010608b:	8b 40 68             	mov    0x68(%eax),%eax
8010608e:	89 04 24             	mov    %eax,(%esp)
80106091:	e8 ea ba ff ff       	call   80101b80 <iput>
  end_op();
80106096:	e8 12 d5 ff ff       	call   801035ad <end_op>
  curproc->cwd = ip;
8010609b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010609e:	8b 55 f0             	mov    -0x10(%ebp),%edx
801060a1:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
801060a4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801060a9:	c9                   	leave  
801060aa:	c3                   	ret    

801060ab <sys_exec>:

int
sys_exec(void)
{
801060ab:	55                   	push   %ebp
801060ac:	89 e5                	mov    %esp,%ebp
801060ae:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
801060b4:	8d 45 f0             	lea    -0x10(%ebp),%eax
801060b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801060bb:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801060c2:	e8 99 f3 ff ff       	call   80105460 <argstr>
801060c7:	85 c0                	test   %eax,%eax
801060c9:	78 1a                	js     801060e5 <sys_exec+0x3a>
801060cb:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
801060d1:	89 44 24 04          	mov    %eax,0x4(%esp)
801060d5:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801060dc:	e8 e8 f2 ff ff       	call   801053c9 <argint>
801060e1:	85 c0                	test   %eax,%eax
801060e3:	79 0a                	jns    801060ef <sys_exec+0x44>
    return -1;
801060e5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060ea:	e9 c7 00 00 00       	jmp    801061b6 <sys_exec+0x10b>
  }
  memset(argv, 0, sizeof(argv));
801060ef:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801060f6:	00 
801060f7:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801060fe:	00 
801060ff:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
80106105:	89 04 24             	mov    %eax,(%esp)
80106108:	e8 89 ef ff ff       	call   80105096 <memset>
  for(i=0;; i++){
8010610d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
80106114:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106117:	83 f8 1f             	cmp    $0x1f,%eax
8010611a:	76 0a                	jbe    80106126 <sys_exec+0x7b>
      return -1;
8010611c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106121:	e9 90 00 00 00       	jmp    801061b6 <sys_exec+0x10b>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
80106126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106129:	c1 e0 02             	shl    $0x2,%eax
8010612c:	89 c2                	mov    %eax,%edx
8010612e:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
80106134:	01 c2                	add    %eax,%edx
80106136:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
8010613c:	89 44 24 04          	mov    %eax,0x4(%esp)
80106140:	89 14 24             	mov    %edx,(%esp)
80106143:	e8 e0 f1 ff ff       	call   80105328 <fetchint>
80106148:	85 c0                	test   %eax,%eax
8010614a:	79 07                	jns    80106153 <sys_exec+0xa8>
      return -1;
8010614c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106151:	eb 63                	jmp    801061b6 <sys_exec+0x10b>
    if(uarg == 0){
80106153:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
80106159:	85 c0                	test   %eax,%eax
8010615b:	75 26                	jne    80106183 <sys_exec+0xd8>
      argv[i] = 0;
8010615d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106160:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
80106167:	00 00 00 00 
      break;
8010616b:	90                   	nop
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
8010616c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010616f:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
80106175:	89 54 24 04          	mov    %edx,0x4(%esp)
80106179:	89 04 24             	mov    %eax,(%esp)
8010617c:	e8 93 aa ff ff       	call   80100c14 <exec>
80106181:	eb 33                	jmp    801061b6 <sys_exec+0x10b>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
80106183:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
80106189:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010618c:	c1 e2 02             	shl    $0x2,%edx
8010618f:	01 c2                	add    %eax,%edx
80106191:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
80106197:	89 54 24 04          	mov    %edx,0x4(%esp)
8010619b:	89 04 24             	mov    %eax,(%esp)
8010619e:	e8 c4 f1 ff ff       	call   80105367 <fetchstr>
801061a3:	85 c0                	test   %eax,%eax
801061a5:	79 07                	jns    801061ae <sys_exec+0x103>
      return -1;
801061a7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061ac:	eb 08                	jmp    801061b6 <sys_exec+0x10b>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
801061ae:	ff 45 f4             	incl   -0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
801061b1:	e9 5e ff ff ff       	jmp    80106114 <sys_exec+0x69>
  return exec(path, argv);
}
801061b6:	c9                   	leave  
801061b7:	c3                   	ret    

801061b8 <sys_pipe>:

int
sys_pipe(void)
{
801061b8:	55                   	push   %ebp
801061b9:	89 e5                	mov    %esp,%ebp
801061bb:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
801061be:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801061c5:	00 
801061c6:	8d 45 ec             	lea    -0x14(%ebp),%eax
801061c9:	89 44 24 04          	mov    %eax,0x4(%esp)
801061cd:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801061d4:	e8 1d f2 ff ff       	call   801053f6 <argptr>
801061d9:	85 c0                	test   %eax,%eax
801061db:	79 0a                	jns    801061e7 <sys_pipe+0x2f>
    return -1;
801061dd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801061e2:	e9 9a 00 00 00       	jmp    80106281 <sys_pipe+0xc9>
  if(pipealloc(&rf, &wf) < 0)
801061e7:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801061ea:	89 44 24 04          	mov    %eax,0x4(%esp)
801061ee:	8d 45 e8             	lea    -0x18(%ebp),%eax
801061f1:	89 04 24             	mov    %eax,(%esp)
801061f4:	e8 7f db ff ff       	call   80103d78 <pipealloc>
801061f9:	85 c0                	test   %eax,%eax
801061fb:	79 07                	jns    80106204 <sys_pipe+0x4c>
    return -1;
801061fd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106202:	eb 7d                	jmp    80106281 <sys_pipe+0xc9>
  fd0 = -1;
80106204:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
8010620b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010620e:	89 04 24             	mov    %eax,(%esp)
80106211:	e8 7e f3 ff ff       	call   80105594 <fdalloc>
80106216:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106219:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010621d:	78 14                	js     80106233 <sys_pipe+0x7b>
8010621f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106222:	89 04 24             	mov    %eax,(%esp)
80106225:	e8 6a f3 ff ff       	call   80105594 <fdalloc>
8010622a:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010622d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106231:	79 36                	jns    80106269 <sys_pipe+0xb1>
    if(fd0 >= 0)
80106233:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106237:	78 13                	js     8010624c <sys_pipe+0x94>
      myproc()->ofile[fd0] = 0;
80106239:	e8 e5 df ff ff       	call   80104223 <myproc>
8010623e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106241:	83 c2 08             	add    $0x8,%edx
80106244:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
8010624b:	00 
    fileclose(rf);
8010624c:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010624f:	89 04 24             	mov    %eax,(%esp)
80106252:	e8 c1 ae ff ff       	call   80101118 <fileclose>
    fileclose(wf);
80106257:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010625a:	89 04 24             	mov    %eax,(%esp)
8010625d:	e8 b6 ae ff ff       	call   80101118 <fileclose>
    return -1;
80106262:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106267:	eb 18                	jmp    80106281 <sys_pipe+0xc9>
  }
  fd[0] = fd0;
80106269:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010626c:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010626f:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
80106271:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106274:	8d 50 04             	lea    0x4(%eax),%edx
80106277:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010627a:	89 02                	mov    %eax,(%edx)
  return 0;
8010627c:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106281:	c9                   	leave  
80106282:	c3                   	ret    
	...

80106284 <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
80106284:	55                   	push   %ebp
80106285:	89 e5                	mov    %esp,%ebp
80106287:	83 ec 08             	sub    $0x8,%esp
  return fork();
8010628a:	e8 90 e2 ff ff       	call   8010451f <fork>
}
8010628f:	c9                   	leave  
80106290:	c3                   	ret    

80106291 <sys_exit>:

int
sys_exit(void)
{
80106291:	55                   	push   %ebp
80106292:	89 e5                	mov    %esp,%ebp
80106294:	83 ec 08             	sub    $0x8,%esp
  exit();
80106297:	e8 e9 e3 ff ff       	call   80104685 <exit>
  return 0;  // not reached
8010629c:	b8 00 00 00 00       	mov    $0x0,%eax
}
801062a1:	c9                   	leave  
801062a2:	c3                   	ret    

801062a3 <sys_wait>:

int
sys_wait(void)
{
801062a3:	55                   	push   %ebp
801062a4:	89 e5                	mov    %esp,%ebp
801062a6:	83 ec 08             	sub    $0x8,%esp
  return wait();
801062a9:	e8 e0 e4 ff ff       	call   8010478e <wait>
}
801062ae:	c9                   	leave  
801062af:	c3                   	ret    

801062b0 <sys_kill>:

int
sys_kill(void)
{
801062b0:	55                   	push   %ebp
801062b1:	89 e5                	mov    %esp,%ebp
801062b3:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
801062b6:	8d 45 f4             	lea    -0xc(%ebp),%eax
801062b9:	89 44 24 04          	mov    %eax,0x4(%esp)
801062bd:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801062c4:	e8 00 f1 ff ff       	call   801053c9 <argint>
801062c9:	85 c0                	test   %eax,%eax
801062cb:	79 07                	jns    801062d4 <sys_kill+0x24>
    return -1;
801062cd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801062d2:	eb 0b                	jmp    801062df <sys_kill+0x2f>
  return kill(pid);
801062d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801062d7:	89 04 24             	mov    %eax,(%esp)
801062da:	e8 84 e8 ff ff       	call   80104b63 <kill>
}
801062df:	c9                   	leave  
801062e0:	c3                   	ret    

801062e1 <sys_getpid>:

int
sys_getpid(void)
{
801062e1:	55                   	push   %ebp
801062e2:	89 e5                	mov    %esp,%ebp
801062e4:	83 ec 08             	sub    $0x8,%esp
  return myproc()->pid;
801062e7:	e8 37 df ff ff       	call   80104223 <myproc>
801062ec:	8b 40 10             	mov    0x10(%eax),%eax
}
801062ef:	c9                   	leave  
801062f0:	c3                   	ret    

801062f1 <sys_sbrk>:

int
sys_sbrk(void)
{
801062f1:	55                   	push   %ebp
801062f2:	89 e5                	mov    %esp,%ebp
801062f4:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
801062f7:	8d 45 f0             	lea    -0x10(%ebp),%eax
801062fa:	89 44 24 04          	mov    %eax,0x4(%esp)
801062fe:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106305:	e8 bf f0 ff ff       	call   801053c9 <argint>
8010630a:	85 c0                	test   %eax,%eax
8010630c:	79 07                	jns    80106315 <sys_sbrk+0x24>
    return -1;
8010630e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106313:	eb 23                	jmp    80106338 <sys_sbrk+0x47>
  addr = myproc()->sz;
80106315:	e8 09 df ff ff       	call   80104223 <myproc>
8010631a:	8b 00                	mov    (%eax),%eax
8010631c:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
8010631f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106322:	89 04 24             	mov    %eax,(%esp)
80106325:	e8 57 e1 ff ff       	call   80104481 <growproc>
8010632a:	85 c0                	test   %eax,%eax
8010632c:	79 07                	jns    80106335 <sys_sbrk+0x44>
    return -1;
8010632e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106333:	eb 03                	jmp    80106338 <sys_sbrk+0x47>
  return addr;
80106335:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106338:	c9                   	leave  
80106339:	c3                   	ret    

8010633a <sys_sleep>:

int
sys_sleep(void)
{
8010633a:	55                   	push   %ebp
8010633b:	89 e5                	mov    %esp,%ebp
8010633d:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;

  if(argint(0, &n) < 0)
80106340:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106343:	89 44 24 04          	mov    %eax,0x4(%esp)
80106347:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010634e:	e8 76 f0 ff ff       	call   801053c9 <argint>
80106353:	85 c0                	test   %eax,%eax
80106355:	79 07                	jns    8010635e <sys_sleep+0x24>
    return -1;
80106357:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010635c:	eb 6b                	jmp    801063c9 <sys_sleep+0x8f>
  acquire(&tickslock);
8010635e:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
80106365:	e8 c9 ea ff ff       	call   80104e33 <acquire>
  ticks0 = ticks;
8010636a:	a1 e0 67 11 80       	mov    0x801167e0,%eax
8010636f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
80106372:	eb 33                	jmp    801063a7 <sys_sleep+0x6d>
    if(myproc()->killed){
80106374:	e8 aa de ff ff       	call   80104223 <myproc>
80106379:	8b 40 24             	mov    0x24(%eax),%eax
8010637c:	85 c0                	test   %eax,%eax
8010637e:	74 13                	je     80106393 <sys_sleep+0x59>
      release(&tickslock);
80106380:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
80106387:	e8 11 eb ff ff       	call   80104e9d <release>
      return -1;
8010638c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106391:	eb 36                	jmp    801063c9 <sys_sleep+0x8f>
    }
    sleep(&ticks, &tickslock);
80106393:	c7 44 24 04 a0 5f 11 	movl   $0x80115fa0,0x4(%esp)
8010639a:	80 
8010639b:	c7 04 24 e0 67 11 80 	movl   $0x801167e0,(%esp)
801063a2:	e8 bd e6 ff ff       	call   80104a64 <sleep>

  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
801063a7:	a1 e0 67 11 80       	mov    0x801167e0,%eax
801063ac:	2b 45 f4             	sub    -0xc(%ebp),%eax
801063af:	89 c2                	mov    %eax,%edx
801063b1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801063b4:	39 c2                	cmp    %eax,%edx
801063b6:	72 bc                	jb     80106374 <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
801063b8:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
801063bf:	e8 d9 ea ff ff       	call   80104e9d <release>
  return 0;
801063c4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801063c9:	c9                   	leave  
801063ca:	c3                   	ret    

801063cb <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
801063cb:	55                   	push   %ebp
801063cc:	89 e5                	mov    %esp,%ebp
801063ce:	83 ec 28             	sub    $0x28,%esp
  uint xticks;

  acquire(&tickslock);
801063d1:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
801063d8:	e8 56 ea ff ff       	call   80104e33 <acquire>
  xticks = ticks;
801063dd:	a1 e0 67 11 80       	mov    0x801167e0,%eax
801063e2:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
801063e5:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
801063ec:	e8 ac ea ff ff       	call   80104e9d <release>
  return xticks;
801063f1:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801063f4:	c9                   	leave  
801063f5:	c3                   	ret    
	...

801063f8 <alltraps>:

  # vectors.S sends all traps here.
.globl alltraps
alltraps:
  # Build trap frame.
  pushl %ds
801063f8:	1e                   	push   %ds
  pushl %es
801063f9:	06                   	push   %es
  pushl %fs
801063fa:	0f a0                	push   %fs
  pushl %gs
801063fc:	0f a8                	push   %gs
  pushal
801063fe:	60                   	pusha  
  
  # Set up data segments.
  movw $(SEG_KDATA<<3), %ax
801063ff:	66 b8 10 00          	mov    $0x10,%ax
  movw %ax, %ds
80106403:	8e d8                	mov    %eax,%ds
  movw %ax, %es
80106405:	8e c0                	mov    %eax,%es

  # Call trap(tf), where tf=%esp
  pushl %esp
80106407:	54                   	push   %esp
  call trap
80106408:	e8 c0 01 00 00       	call   801065cd <trap>
  addl $4, %esp
8010640d:	83 c4 04             	add    $0x4,%esp

80106410 <trapret>:

  # Return falls through to trapret...
.globl trapret
trapret:
  popal
80106410:	61                   	popa   
  popl %gs
80106411:	0f a9                	pop    %gs
  popl %fs
80106413:	0f a1                	pop    %fs
  popl %es
80106415:	07                   	pop    %es
  popl %ds
80106416:	1f                   	pop    %ds
  addl $0x8, %esp  # trapno and errcode
80106417:	83 c4 08             	add    $0x8,%esp
  iret
8010641a:	cf                   	iret   
	...

8010641c <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
8010641c:	55                   	push   %ebp
8010641d:	89 e5                	mov    %esp,%ebp
8010641f:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80106422:	8b 45 0c             	mov    0xc(%ebp),%eax
80106425:	48                   	dec    %eax
80106426:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
8010642a:	8b 45 08             	mov    0x8(%ebp),%eax
8010642d:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80106431:	8b 45 08             	mov    0x8(%ebp),%eax
80106434:	c1 e8 10             	shr    $0x10,%eax
80106437:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
8010643b:	8d 45 fa             	lea    -0x6(%ebp),%eax
8010643e:	0f 01 18             	lidtl  (%eax)
}
80106441:	c9                   	leave  
80106442:	c3                   	ret    

80106443 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
80106443:	55                   	push   %ebp
80106444:	89 e5                	mov    %esp,%ebp
80106446:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80106449:	0f 20 d0             	mov    %cr2,%eax
8010644c:	89 45 fc             	mov    %eax,-0x4(%ebp)
  return val;
8010644f:	8b 45 fc             	mov    -0x4(%ebp),%eax
}
80106452:	c9                   	leave  
80106453:	c3                   	ret    

80106454 <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
80106454:	55                   	push   %ebp
80106455:	89 e5                	mov    %esp,%ebp
80106457:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
8010645a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106461:	e9 b8 00 00 00       	jmp    8010651e <tvinit+0xca>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
80106466:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106469:	8b 04 85 78 b0 10 80 	mov    -0x7fef4f88(,%eax,4),%eax
80106470:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106473:	66 89 04 d5 e0 5f 11 	mov    %ax,-0x7feea020(,%edx,8)
8010647a:	80 
8010647b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010647e:	66 c7 04 c5 e2 5f 11 	movw   $0x8,-0x7feea01e(,%eax,8)
80106485:	80 08 00 
80106488:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010648b:	8a 14 c5 e4 5f 11 80 	mov    -0x7feea01c(,%eax,8),%dl
80106492:	83 e2 e0             	and    $0xffffffe0,%edx
80106495:	88 14 c5 e4 5f 11 80 	mov    %dl,-0x7feea01c(,%eax,8)
8010649c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010649f:	8a 14 c5 e4 5f 11 80 	mov    -0x7feea01c(,%eax,8),%dl
801064a6:	83 e2 1f             	and    $0x1f,%edx
801064a9:	88 14 c5 e4 5f 11 80 	mov    %dl,-0x7feea01c(,%eax,8)
801064b0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064b3:	8a 14 c5 e5 5f 11 80 	mov    -0x7feea01b(,%eax,8),%dl
801064ba:	83 e2 f0             	and    $0xfffffff0,%edx
801064bd:	83 ca 0e             	or     $0xe,%edx
801064c0:	88 14 c5 e5 5f 11 80 	mov    %dl,-0x7feea01b(,%eax,8)
801064c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064ca:	8a 14 c5 e5 5f 11 80 	mov    -0x7feea01b(,%eax,8),%dl
801064d1:	83 e2 ef             	and    $0xffffffef,%edx
801064d4:	88 14 c5 e5 5f 11 80 	mov    %dl,-0x7feea01b(,%eax,8)
801064db:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064de:	8a 14 c5 e5 5f 11 80 	mov    -0x7feea01b(,%eax,8),%dl
801064e5:	83 e2 9f             	and    $0xffffff9f,%edx
801064e8:	88 14 c5 e5 5f 11 80 	mov    %dl,-0x7feea01b(,%eax,8)
801064ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064f2:	8a 14 c5 e5 5f 11 80 	mov    -0x7feea01b(,%eax,8),%dl
801064f9:	83 ca 80             	or     $0xffffff80,%edx
801064fc:	88 14 c5 e5 5f 11 80 	mov    %dl,-0x7feea01b(,%eax,8)
80106503:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106506:	8b 04 85 78 b0 10 80 	mov    -0x7fef4f88(,%eax,4),%eax
8010650d:	c1 e8 10             	shr    $0x10,%eax
80106510:	8b 55 f4             	mov    -0xc(%ebp),%edx
80106513:	66 89 04 d5 e6 5f 11 	mov    %ax,-0x7feea01a(,%edx,8)
8010651a:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
8010651b:	ff 45 f4             	incl   -0xc(%ebp)
8010651e:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
80106525:	0f 8e 3b ff ff ff    	jle    80106466 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
8010652b:	a1 78 b1 10 80       	mov    0x8010b178,%eax
80106530:	66 a3 e0 61 11 80    	mov    %ax,0x801161e0
80106536:	66 c7 05 e2 61 11 80 	movw   $0x8,0x801161e2
8010653d:	08 00 
8010653f:	a0 e4 61 11 80       	mov    0x801161e4,%al
80106544:	83 e0 e0             	and    $0xffffffe0,%eax
80106547:	a2 e4 61 11 80       	mov    %al,0x801161e4
8010654c:	a0 e4 61 11 80       	mov    0x801161e4,%al
80106551:	83 e0 1f             	and    $0x1f,%eax
80106554:	a2 e4 61 11 80       	mov    %al,0x801161e4
80106559:	a0 e5 61 11 80       	mov    0x801161e5,%al
8010655e:	83 c8 0f             	or     $0xf,%eax
80106561:	a2 e5 61 11 80       	mov    %al,0x801161e5
80106566:	a0 e5 61 11 80       	mov    0x801161e5,%al
8010656b:	83 e0 ef             	and    $0xffffffef,%eax
8010656e:	a2 e5 61 11 80       	mov    %al,0x801161e5
80106573:	a0 e5 61 11 80       	mov    0x801161e5,%al
80106578:	83 c8 60             	or     $0x60,%eax
8010657b:	a2 e5 61 11 80       	mov    %al,0x801161e5
80106580:	a0 e5 61 11 80       	mov    0x801161e5,%al
80106585:	83 c8 80             	or     $0xffffff80,%eax
80106588:	a2 e5 61 11 80       	mov    %al,0x801161e5
8010658d:	a1 78 b1 10 80       	mov    0x8010b178,%eax
80106592:	c1 e8 10             	shr    $0x10,%eax
80106595:	66 a3 e6 61 11 80    	mov    %ax,0x801161e6

  initlock(&tickslock, "time");
8010659b:	c7 44 24 04 fc 86 10 	movl   $0x801086fc,0x4(%esp)
801065a2:	80 
801065a3:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
801065aa:	e8 63 e8 ff ff       	call   80104e12 <initlock>
}
801065af:	c9                   	leave  
801065b0:	c3                   	ret    

801065b1 <idtinit>:

void
idtinit(void)
{
801065b1:	55                   	push   %ebp
801065b2:	89 e5                	mov    %esp,%ebp
801065b4:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
801065b7:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
801065be:	00 
801065bf:	c7 04 24 e0 5f 11 80 	movl   $0x80115fe0,(%esp)
801065c6:	e8 51 fe ff ff       	call   8010641c <lidt>
}
801065cb:	c9                   	leave  
801065cc:	c3                   	ret    

801065cd <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
801065cd:	55                   	push   %ebp
801065ce:	89 e5                	mov    %esp,%ebp
801065d0:	57                   	push   %edi
801065d1:	56                   	push   %esi
801065d2:	53                   	push   %ebx
801065d3:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
801065d6:	8b 45 08             	mov    0x8(%ebp),%eax
801065d9:	8b 40 30             	mov    0x30(%eax),%eax
801065dc:	83 f8 40             	cmp    $0x40,%eax
801065df:	75 3c                	jne    8010661d <trap+0x50>
    if(myproc()->killed)
801065e1:	e8 3d dc ff ff       	call   80104223 <myproc>
801065e6:	8b 40 24             	mov    0x24(%eax),%eax
801065e9:	85 c0                	test   %eax,%eax
801065eb:	74 05                	je     801065f2 <trap+0x25>
      exit();
801065ed:	e8 93 e0 ff ff       	call   80104685 <exit>
    myproc()->tf = tf;
801065f2:	e8 2c dc ff ff       	call   80104223 <myproc>
801065f7:	8b 55 08             	mov    0x8(%ebp),%edx
801065fa:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
801065fd:	e8 95 ee ff ff       	call   80105497 <syscall>
    if(myproc()->killed)
80106602:	e8 1c dc ff ff       	call   80104223 <myproc>
80106607:	8b 40 24             	mov    0x24(%eax),%eax
8010660a:	85 c0                	test   %eax,%eax
8010660c:	74 0a                	je     80106618 <trap+0x4b>
      exit();
8010660e:	e8 72 e0 ff ff       	call   80104685 <exit>
    return;
80106613:	e9 13 02 00 00       	jmp    8010682b <trap+0x25e>
80106618:	e9 0e 02 00 00       	jmp    8010682b <trap+0x25e>
  }

  switch(tf->trapno){
8010661d:	8b 45 08             	mov    0x8(%ebp),%eax
80106620:	8b 40 30             	mov    0x30(%eax),%eax
80106623:	83 e8 20             	sub    $0x20,%eax
80106626:	83 f8 1f             	cmp    $0x1f,%eax
80106629:	0f 87 ae 00 00 00    	ja     801066dd <trap+0x110>
8010662f:	8b 04 85 a4 87 10 80 	mov    -0x7fef785c(,%eax,4),%eax
80106636:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpuid() == 0){
80106638:	e8 1d db ff ff       	call   8010415a <cpuid>
8010663d:	85 c0                	test   %eax,%eax
8010663f:	75 2f                	jne    80106670 <trap+0xa3>
      acquire(&tickslock);
80106641:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
80106648:	e8 e6 e7 ff ff       	call   80104e33 <acquire>
      ticks++;
8010664d:	a1 e0 67 11 80       	mov    0x801167e0,%eax
80106652:	40                   	inc    %eax
80106653:	a3 e0 67 11 80       	mov    %eax,0x801167e0
      wakeup(&ticks);
80106658:	c7 04 24 e0 67 11 80 	movl   $0x801167e0,(%esp)
8010665f:	e8 d4 e4 ff ff       	call   80104b38 <wakeup>
      release(&tickslock);
80106664:	c7 04 24 a0 5f 11 80 	movl   $0x80115fa0,(%esp)
8010666b:	e8 2d e8 ff ff       	call   80104e9d <release>
    }
    lapiceoi();
80106670:	e8 8e c9 ff ff       	call   80103003 <lapiceoi>
    break;
80106675:	e9 35 01 00 00       	jmp    801067af <trap+0x1e2>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
8010667a:	e8 03 c2 ff ff       	call   80102882 <ideintr>
    lapiceoi();
8010667f:	e8 7f c9 ff ff       	call   80103003 <lapiceoi>
    break;
80106684:	e9 26 01 00 00       	jmp    801067af <trap+0x1e2>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
80106689:	e8 8c c7 ff ff       	call   80102e1a <kbdintr>
    lapiceoi();
8010668e:	e8 70 c9 ff ff       	call   80103003 <lapiceoi>
    break;
80106693:	e9 17 01 00 00       	jmp    801067af <trap+0x1e2>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
80106698:	e8 70 03 00 00       	call   80106a0d <uartintr>
    lapiceoi();
8010669d:	e8 61 c9 ff ff       	call   80103003 <lapiceoi>
    break;
801066a2:	e9 08 01 00 00       	jmp    801067af <trap+0x1e2>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801066a7:	8b 45 08             	mov    0x8(%ebp),%eax
801066aa:	8b 70 38             	mov    0x38(%eax),%esi
            cpuid(), tf->cs, tf->eip);
801066ad:	8b 45 08             	mov    0x8(%ebp),%eax
801066b0:	8b 40 3c             	mov    0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
801066b3:	0f b7 d8             	movzwl %ax,%ebx
801066b6:	e8 9f da ff ff       	call   8010415a <cpuid>
801066bb:	89 74 24 0c          	mov    %esi,0xc(%esp)
801066bf:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801066c3:	89 44 24 04          	mov    %eax,0x4(%esp)
801066c7:	c7 04 24 04 87 10 80 	movl   $0x80108704,(%esp)
801066ce:	e8 ee 9c ff ff       	call   801003c1 <cprintf>
            cpuid(), tf->cs, tf->eip);
    lapiceoi();
801066d3:	e8 2b c9 ff ff       	call   80103003 <lapiceoi>
    break;
801066d8:	e9 d2 00 00 00       	jmp    801067af <trap+0x1e2>

  //PAGEBREAK: 13
  default:
    if(myproc() == 0 || (tf->cs&3) == 0){
801066dd:	e8 41 db ff ff       	call   80104223 <myproc>
801066e2:	85 c0                	test   %eax,%eax
801066e4:	74 10                	je     801066f6 <trap+0x129>
801066e6:	8b 45 08             	mov    0x8(%ebp),%eax
801066e9:	8b 40 3c             	mov    0x3c(%eax),%eax
801066ec:	0f b7 c0             	movzwl %ax,%eax
801066ef:	83 e0 03             	and    $0x3,%eax
801066f2:	85 c0                	test   %eax,%eax
801066f4:	75 40                	jne    80106736 <trap+0x169>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
801066f6:	e8 48 fd ff ff       	call   80106443 <rcr2>
801066fb:	89 c3                	mov    %eax,%ebx
801066fd:	8b 45 08             	mov    0x8(%ebp),%eax
80106700:	8b 70 38             	mov    0x38(%eax),%esi
80106703:	e8 52 da ff ff       	call   8010415a <cpuid>
80106708:	8b 55 08             	mov    0x8(%ebp),%edx
8010670b:	8b 52 30             	mov    0x30(%edx),%edx
8010670e:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106712:	89 74 24 0c          	mov    %esi,0xc(%esp)
80106716:	89 44 24 08          	mov    %eax,0x8(%esp)
8010671a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010671e:	c7 04 24 28 87 10 80 	movl   $0x80108728,(%esp)
80106725:	e8 97 9c ff ff       	call   801003c1 <cprintf>
              tf->trapno, cpuid(), tf->eip, rcr2());
      panic("trap");
8010672a:	c7 04 24 5a 87 10 80 	movl   $0x8010875a,(%esp)
80106731:	e8 1e 9e ff ff       	call   80100554 <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106736:	e8 08 fd ff ff       	call   80106443 <rcr2>
8010673b:	89 c6                	mov    %eax,%esi
8010673d:	8b 45 08             	mov    0x8(%ebp),%eax
80106740:	8b 40 38             	mov    0x38(%eax),%eax
80106743:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106746:	e8 0f da ff ff       	call   8010415a <cpuid>
8010674b:	89 c3                	mov    %eax,%ebx
8010674d:	8b 45 08             	mov    0x8(%ebp),%eax
80106750:	8b 78 34             	mov    0x34(%eax),%edi
80106753:	89 7d e0             	mov    %edi,-0x20(%ebp)
80106756:	8b 45 08             	mov    0x8(%ebp),%eax
80106759:	8b 78 30             	mov    0x30(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            myproc()->pid, myproc()->name, tf->trapno,
8010675c:	e8 c2 da ff ff       	call   80104223 <myproc>
80106761:	8d 50 6c             	lea    0x6c(%eax),%edx
80106764:	89 55 dc             	mov    %edx,-0x24(%ebp)
80106767:	e8 b7 da ff ff       	call   80104223 <myproc>
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpuid(), tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
8010676c:	8b 40 10             	mov    0x10(%eax),%eax
8010676f:	89 74 24 1c          	mov    %esi,0x1c(%esp)
80106773:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
80106776:	89 4c 24 18          	mov    %ecx,0x18(%esp)
8010677a:	89 5c 24 14          	mov    %ebx,0x14(%esp)
8010677e:	8b 4d e0             	mov    -0x20(%ebp),%ecx
80106781:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80106785:	89 7c 24 0c          	mov    %edi,0xc(%esp)
80106789:	8b 55 dc             	mov    -0x24(%ebp),%edx
8010678c:	89 54 24 08          	mov    %edx,0x8(%esp)
80106790:	89 44 24 04          	mov    %eax,0x4(%esp)
80106794:	c7 04 24 60 87 10 80 	movl   $0x80108760,(%esp)
8010679b:	e8 21 9c ff ff       	call   801003c1 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            myproc()->pid, myproc()->name, tf->trapno,
            tf->err, cpuid(), tf->eip, rcr2());
    myproc()->killed = 1;
801067a0:	e8 7e da ff ff       	call   80104223 <myproc>
801067a5:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
801067ac:	eb 01                	jmp    801067af <trap+0x1e2>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
801067ae:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running
  // until it gets to the regular system call return.)
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
801067af:	e8 6f da ff ff       	call   80104223 <myproc>
801067b4:	85 c0                	test   %eax,%eax
801067b6:	74 22                	je     801067da <trap+0x20d>
801067b8:	e8 66 da ff ff       	call   80104223 <myproc>
801067bd:	8b 40 24             	mov    0x24(%eax),%eax
801067c0:	85 c0                	test   %eax,%eax
801067c2:	74 16                	je     801067da <trap+0x20d>
801067c4:	8b 45 08             	mov    0x8(%ebp),%eax
801067c7:	8b 40 3c             	mov    0x3c(%eax),%eax
801067ca:	0f b7 c0             	movzwl %ax,%eax
801067cd:	83 e0 03             	and    $0x3,%eax
801067d0:	83 f8 03             	cmp    $0x3,%eax
801067d3:	75 05                	jne    801067da <trap+0x20d>
    exit();
801067d5:	e8 ab de ff ff       	call   80104685 <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(myproc() && myproc()->state == RUNNING &&
801067da:	e8 44 da ff ff       	call   80104223 <myproc>
801067df:	85 c0                	test   %eax,%eax
801067e1:	74 1d                	je     80106800 <trap+0x233>
801067e3:	e8 3b da ff ff       	call   80104223 <myproc>
801067e8:	8b 40 0c             	mov    0xc(%eax),%eax
801067eb:	83 f8 04             	cmp    $0x4,%eax
801067ee:	75 10                	jne    80106800 <trap+0x233>
     tf->trapno == T_IRQ0+IRQ_TIMER)
801067f0:	8b 45 08             	mov    0x8(%ebp),%eax
801067f3:	8b 40 30             	mov    0x30(%eax),%eax
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
    exit();

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(myproc() && myproc()->state == RUNNING &&
801067f6:	83 f8 20             	cmp    $0x20,%eax
801067f9:	75 05                	jne    80106800 <trap+0x233>
     tf->trapno == T_IRQ0+IRQ_TIMER)
    yield();
801067fb:	e8 f4 e1 ff ff       	call   801049f4 <yield>

  // Check if the process has been killed since we yielded
  if(myproc() && myproc()->killed && (tf->cs&3) == DPL_USER)
80106800:	e8 1e da ff ff       	call   80104223 <myproc>
80106805:	85 c0                	test   %eax,%eax
80106807:	74 22                	je     8010682b <trap+0x25e>
80106809:	e8 15 da ff ff       	call   80104223 <myproc>
8010680e:	8b 40 24             	mov    0x24(%eax),%eax
80106811:	85 c0                	test   %eax,%eax
80106813:	74 16                	je     8010682b <trap+0x25e>
80106815:	8b 45 08             	mov    0x8(%ebp),%eax
80106818:	8b 40 3c             	mov    0x3c(%eax),%eax
8010681b:	0f b7 c0             	movzwl %ax,%eax
8010681e:	83 e0 03             	and    $0x3,%eax
80106821:	83 f8 03             	cmp    $0x3,%eax
80106824:	75 05                	jne    8010682b <trap+0x25e>
    exit();
80106826:	e8 5a de ff ff       	call   80104685 <exit>
}
8010682b:	83 c4 3c             	add    $0x3c,%esp
8010682e:	5b                   	pop    %ebx
8010682f:	5e                   	pop    %esi
80106830:	5f                   	pop    %edi
80106831:	5d                   	pop    %ebp
80106832:	c3                   	ret    
	...

80106834 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106834:	55                   	push   %ebp
80106835:	89 e5                	mov    %esp,%ebp
80106837:	83 ec 14             	sub    $0x14,%esp
8010683a:	8b 45 08             	mov    0x8(%ebp),%eax
8010683d:	66 89 45 ec          	mov    %ax,-0x14(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106841:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106844:	89 c2                	mov    %eax,%edx
80106846:	ec                   	in     (%dx),%al
80106847:	88 45 ff             	mov    %al,-0x1(%ebp)
  return data;
8010684a:	8a 45 ff             	mov    -0x1(%ebp),%al
}
8010684d:	c9                   	leave  
8010684e:	c3                   	ret    

8010684f <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
8010684f:	55                   	push   %ebp
80106850:	89 e5                	mov    %esp,%ebp
80106852:	83 ec 08             	sub    $0x8,%esp
80106855:	8b 45 08             	mov    0x8(%ebp),%eax
80106858:	8b 55 0c             	mov    0xc(%ebp),%edx
8010685b:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
8010685f:	88 55 f8             	mov    %dl,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106862:	8a 45 f8             	mov    -0x8(%ebp),%al
80106865:	8b 55 fc             	mov    -0x4(%ebp),%edx
80106868:	ee                   	out    %al,(%dx)
}
80106869:	c9                   	leave  
8010686a:	c3                   	ret    

8010686b <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
8010686b:	55                   	push   %ebp
8010686c:	89 e5                	mov    %esp,%ebp
8010686e:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106871:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106878:	00 
80106879:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106880:	e8 ca ff ff ff       	call   8010684f <outb>

  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106885:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
8010688c:	00 
8010688d:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106894:	e8 b6 ff ff ff       	call   8010684f <outb>
  outb(COM1+0, 115200/9600);
80106899:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
801068a0:	00 
801068a1:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
801068a8:	e8 a2 ff ff ff       	call   8010684f <outb>
  outb(COM1+1, 0);
801068ad:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801068b4:	00 
801068b5:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
801068bc:	e8 8e ff ff ff       	call   8010684f <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
801068c1:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801068c8:	00 
801068c9:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
801068d0:	e8 7a ff ff ff       	call   8010684f <outb>
  outb(COM1+4, 0);
801068d5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801068dc:	00 
801068dd:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
801068e4:	e8 66 ff ff ff       	call   8010684f <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
801068e9:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801068f0:	00 
801068f1:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
801068f8:	e8 52 ff ff ff       	call   8010684f <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
801068fd:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106904:	e8 2b ff ff ff       	call   80106834 <inb>
80106909:	3c ff                	cmp    $0xff,%al
8010690b:	75 02                	jne    8010690f <uartinit+0xa4>
    return;
8010690d:	eb 5b                	jmp    8010696a <uartinit+0xff>
  uart = 1;
8010690f:	c7 05 24 b6 10 80 01 	movl   $0x1,0x8010b624
80106916:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106919:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106920:	e8 0f ff ff ff       	call   80106834 <inb>
  inb(COM1+0);
80106925:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
8010692c:	e8 03 ff ff ff       	call   80106834 <inb>
  ioapicenable(IRQ_COM1, 0);
80106931:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106938:	00 
80106939:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106940:	e8 b2 c1 ff ff       	call   80102af7 <ioapicenable>

  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106945:	c7 45 f4 24 88 10 80 	movl   $0x80108824,-0xc(%ebp)
8010694c:	eb 13                	jmp    80106961 <uartinit+0xf6>
    uartputc(*p);
8010694e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106951:	8a 00                	mov    (%eax),%al
80106953:	0f be c0             	movsbl %al,%eax
80106956:	89 04 24             	mov    %eax,(%esp)
80106959:	e8 0e 00 00 00       	call   8010696c <uartputc>
  inb(COM1+2);
  inb(COM1+0);
  ioapicenable(IRQ_COM1, 0);

  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
8010695e:	ff 45 f4             	incl   -0xc(%ebp)
80106961:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106964:	8a 00                	mov    (%eax),%al
80106966:	84 c0                	test   %al,%al
80106968:	75 e4                	jne    8010694e <uartinit+0xe3>
    uartputc(*p);
}
8010696a:	c9                   	leave  
8010696b:	c3                   	ret    

8010696c <uartputc>:

void
uartputc(int c)
{
8010696c:	55                   	push   %ebp
8010696d:	89 e5                	mov    %esp,%ebp
8010696f:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106972:	a1 24 b6 10 80       	mov    0x8010b624,%eax
80106977:	85 c0                	test   %eax,%eax
80106979:	75 02                	jne    8010697d <uartputc+0x11>
    return;
8010697b:	eb 4a                	jmp    801069c7 <uartputc+0x5b>
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
8010697d:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106984:	eb 0f                	jmp    80106995 <uartputc+0x29>
    microdelay(10);
80106986:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
8010698d:	e8 96 c6 ff ff       	call   80103028 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106992:	ff 45 f4             	incl   -0xc(%ebp)
80106995:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106999:	7f 16                	jg     801069b1 <uartputc+0x45>
8010699b:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
801069a2:	e8 8d fe ff ff       	call   80106834 <inb>
801069a7:	0f b6 c0             	movzbl %al,%eax
801069aa:	83 e0 20             	and    $0x20,%eax
801069ad:	85 c0                	test   %eax,%eax
801069af:	74 d5                	je     80106986 <uartputc+0x1a>
    microdelay(10);
  outb(COM1+0, c);
801069b1:	8b 45 08             	mov    0x8(%ebp),%eax
801069b4:	0f b6 c0             	movzbl %al,%eax
801069b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801069bb:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
801069c2:	e8 88 fe ff ff       	call   8010684f <outb>
}
801069c7:	c9                   	leave  
801069c8:	c3                   	ret    

801069c9 <uartgetc>:

static int
uartgetc(void)
{
801069c9:	55                   	push   %ebp
801069ca:	89 e5                	mov    %esp,%ebp
801069cc:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
801069cf:	a1 24 b6 10 80       	mov    0x8010b624,%eax
801069d4:	85 c0                	test   %eax,%eax
801069d6:	75 07                	jne    801069df <uartgetc+0x16>
    return -1;
801069d8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801069dd:	eb 2c                	jmp    80106a0b <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
801069df:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
801069e6:	e8 49 fe ff ff       	call   80106834 <inb>
801069eb:	0f b6 c0             	movzbl %al,%eax
801069ee:	83 e0 01             	and    $0x1,%eax
801069f1:	85 c0                	test   %eax,%eax
801069f3:	75 07                	jne    801069fc <uartgetc+0x33>
    return -1;
801069f5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801069fa:	eb 0f                	jmp    80106a0b <uartgetc+0x42>
  return inb(COM1+0);
801069fc:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106a03:	e8 2c fe ff ff       	call   80106834 <inb>
80106a08:	0f b6 c0             	movzbl %al,%eax
}
80106a0b:	c9                   	leave  
80106a0c:	c3                   	ret    

80106a0d <uartintr>:

void
uartintr(void)
{
80106a0d:	55                   	push   %ebp
80106a0e:	89 e5                	mov    %esp,%ebp
80106a10:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80106a13:	c7 04 24 c9 69 10 80 	movl   $0x801069c9,(%esp)
80106a1a:	e8 d6 9d ff ff       	call   801007f5 <consoleintr>
}
80106a1f:	c9                   	leave  
80106a20:	c3                   	ret    
80106a21:	00 00                	add    %al,(%eax)
	...

80106a24 <vector0>:
# generated by vectors.pl - do not edit
# handlers
.globl alltraps
.globl vector0
vector0:
  pushl $0
80106a24:	6a 00                	push   $0x0
  pushl $0
80106a26:	6a 00                	push   $0x0
  jmp alltraps
80106a28:	e9 cb f9 ff ff       	jmp    801063f8 <alltraps>

80106a2d <vector1>:
.globl vector1
vector1:
  pushl $0
80106a2d:	6a 00                	push   $0x0
  pushl $1
80106a2f:	6a 01                	push   $0x1
  jmp alltraps
80106a31:	e9 c2 f9 ff ff       	jmp    801063f8 <alltraps>

80106a36 <vector2>:
.globl vector2
vector2:
  pushl $0
80106a36:	6a 00                	push   $0x0
  pushl $2
80106a38:	6a 02                	push   $0x2
  jmp alltraps
80106a3a:	e9 b9 f9 ff ff       	jmp    801063f8 <alltraps>

80106a3f <vector3>:
.globl vector3
vector3:
  pushl $0
80106a3f:	6a 00                	push   $0x0
  pushl $3
80106a41:	6a 03                	push   $0x3
  jmp alltraps
80106a43:	e9 b0 f9 ff ff       	jmp    801063f8 <alltraps>

80106a48 <vector4>:
.globl vector4
vector4:
  pushl $0
80106a48:	6a 00                	push   $0x0
  pushl $4
80106a4a:	6a 04                	push   $0x4
  jmp alltraps
80106a4c:	e9 a7 f9 ff ff       	jmp    801063f8 <alltraps>

80106a51 <vector5>:
.globl vector5
vector5:
  pushl $0
80106a51:	6a 00                	push   $0x0
  pushl $5
80106a53:	6a 05                	push   $0x5
  jmp alltraps
80106a55:	e9 9e f9 ff ff       	jmp    801063f8 <alltraps>

80106a5a <vector6>:
.globl vector6
vector6:
  pushl $0
80106a5a:	6a 00                	push   $0x0
  pushl $6
80106a5c:	6a 06                	push   $0x6
  jmp alltraps
80106a5e:	e9 95 f9 ff ff       	jmp    801063f8 <alltraps>

80106a63 <vector7>:
.globl vector7
vector7:
  pushl $0
80106a63:	6a 00                	push   $0x0
  pushl $7
80106a65:	6a 07                	push   $0x7
  jmp alltraps
80106a67:	e9 8c f9 ff ff       	jmp    801063f8 <alltraps>

80106a6c <vector8>:
.globl vector8
vector8:
  pushl $8
80106a6c:	6a 08                	push   $0x8
  jmp alltraps
80106a6e:	e9 85 f9 ff ff       	jmp    801063f8 <alltraps>

80106a73 <vector9>:
.globl vector9
vector9:
  pushl $0
80106a73:	6a 00                	push   $0x0
  pushl $9
80106a75:	6a 09                	push   $0x9
  jmp alltraps
80106a77:	e9 7c f9 ff ff       	jmp    801063f8 <alltraps>

80106a7c <vector10>:
.globl vector10
vector10:
  pushl $10
80106a7c:	6a 0a                	push   $0xa
  jmp alltraps
80106a7e:	e9 75 f9 ff ff       	jmp    801063f8 <alltraps>

80106a83 <vector11>:
.globl vector11
vector11:
  pushl $11
80106a83:	6a 0b                	push   $0xb
  jmp alltraps
80106a85:	e9 6e f9 ff ff       	jmp    801063f8 <alltraps>

80106a8a <vector12>:
.globl vector12
vector12:
  pushl $12
80106a8a:	6a 0c                	push   $0xc
  jmp alltraps
80106a8c:	e9 67 f9 ff ff       	jmp    801063f8 <alltraps>

80106a91 <vector13>:
.globl vector13
vector13:
  pushl $13
80106a91:	6a 0d                	push   $0xd
  jmp alltraps
80106a93:	e9 60 f9 ff ff       	jmp    801063f8 <alltraps>

80106a98 <vector14>:
.globl vector14
vector14:
  pushl $14
80106a98:	6a 0e                	push   $0xe
  jmp alltraps
80106a9a:	e9 59 f9 ff ff       	jmp    801063f8 <alltraps>

80106a9f <vector15>:
.globl vector15
vector15:
  pushl $0
80106a9f:	6a 00                	push   $0x0
  pushl $15
80106aa1:	6a 0f                	push   $0xf
  jmp alltraps
80106aa3:	e9 50 f9 ff ff       	jmp    801063f8 <alltraps>

80106aa8 <vector16>:
.globl vector16
vector16:
  pushl $0
80106aa8:	6a 00                	push   $0x0
  pushl $16
80106aaa:	6a 10                	push   $0x10
  jmp alltraps
80106aac:	e9 47 f9 ff ff       	jmp    801063f8 <alltraps>

80106ab1 <vector17>:
.globl vector17
vector17:
  pushl $17
80106ab1:	6a 11                	push   $0x11
  jmp alltraps
80106ab3:	e9 40 f9 ff ff       	jmp    801063f8 <alltraps>

80106ab8 <vector18>:
.globl vector18
vector18:
  pushl $0
80106ab8:	6a 00                	push   $0x0
  pushl $18
80106aba:	6a 12                	push   $0x12
  jmp alltraps
80106abc:	e9 37 f9 ff ff       	jmp    801063f8 <alltraps>

80106ac1 <vector19>:
.globl vector19
vector19:
  pushl $0
80106ac1:	6a 00                	push   $0x0
  pushl $19
80106ac3:	6a 13                	push   $0x13
  jmp alltraps
80106ac5:	e9 2e f9 ff ff       	jmp    801063f8 <alltraps>

80106aca <vector20>:
.globl vector20
vector20:
  pushl $0
80106aca:	6a 00                	push   $0x0
  pushl $20
80106acc:	6a 14                	push   $0x14
  jmp alltraps
80106ace:	e9 25 f9 ff ff       	jmp    801063f8 <alltraps>

80106ad3 <vector21>:
.globl vector21
vector21:
  pushl $0
80106ad3:	6a 00                	push   $0x0
  pushl $21
80106ad5:	6a 15                	push   $0x15
  jmp alltraps
80106ad7:	e9 1c f9 ff ff       	jmp    801063f8 <alltraps>

80106adc <vector22>:
.globl vector22
vector22:
  pushl $0
80106adc:	6a 00                	push   $0x0
  pushl $22
80106ade:	6a 16                	push   $0x16
  jmp alltraps
80106ae0:	e9 13 f9 ff ff       	jmp    801063f8 <alltraps>

80106ae5 <vector23>:
.globl vector23
vector23:
  pushl $0
80106ae5:	6a 00                	push   $0x0
  pushl $23
80106ae7:	6a 17                	push   $0x17
  jmp alltraps
80106ae9:	e9 0a f9 ff ff       	jmp    801063f8 <alltraps>

80106aee <vector24>:
.globl vector24
vector24:
  pushl $0
80106aee:	6a 00                	push   $0x0
  pushl $24
80106af0:	6a 18                	push   $0x18
  jmp alltraps
80106af2:	e9 01 f9 ff ff       	jmp    801063f8 <alltraps>

80106af7 <vector25>:
.globl vector25
vector25:
  pushl $0
80106af7:	6a 00                	push   $0x0
  pushl $25
80106af9:	6a 19                	push   $0x19
  jmp alltraps
80106afb:	e9 f8 f8 ff ff       	jmp    801063f8 <alltraps>

80106b00 <vector26>:
.globl vector26
vector26:
  pushl $0
80106b00:	6a 00                	push   $0x0
  pushl $26
80106b02:	6a 1a                	push   $0x1a
  jmp alltraps
80106b04:	e9 ef f8 ff ff       	jmp    801063f8 <alltraps>

80106b09 <vector27>:
.globl vector27
vector27:
  pushl $0
80106b09:	6a 00                	push   $0x0
  pushl $27
80106b0b:	6a 1b                	push   $0x1b
  jmp alltraps
80106b0d:	e9 e6 f8 ff ff       	jmp    801063f8 <alltraps>

80106b12 <vector28>:
.globl vector28
vector28:
  pushl $0
80106b12:	6a 00                	push   $0x0
  pushl $28
80106b14:	6a 1c                	push   $0x1c
  jmp alltraps
80106b16:	e9 dd f8 ff ff       	jmp    801063f8 <alltraps>

80106b1b <vector29>:
.globl vector29
vector29:
  pushl $0
80106b1b:	6a 00                	push   $0x0
  pushl $29
80106b1d:	6a 1d                	push   $0x1d
  jmp alltraps
80106b1f:	e9 d4 f8 ff ff       	jmp    801063f8 <alltraps>

80106b24 <vector30>:
.globl vector30
vector30:
  pushl $0
80106b24:	6a 00                	push   $0x0
  pushl $30
80106b26:	6a 1e                	push   $0x1e
  jmp alltraps
80106b28:	e9 cb f8 ff ff       	jmp    801063f8 <alltraps>

80106b2d <vector31>:
.globl vector31
vector31:
  pushl $0
80106b2d:	6a 00                	push   $0x0
  pushl $31
80106b2f:	6a 1f                	push   $0x1f
  jmp alltraps
80106b31:	e9 c2 f8 ff ff       	jmp    801063f8 <alltraps>

80106b36 <vector32>:
.globl vector32
vector32:
  pushl $0
80106b36:	6a 00                	push   $0x0
  pushl $32
80106b38:	6a 20                	push   $0x20
  jmp alltraps
80106b3a:	e9 b9 f8 ff ff       	jmp    801063f8 <alltraps>

80106b3f <vector33>:
.globl vector33
vector33:
  pushl $0
80106b3f:	6a 00                	push   $0x0
  pushl $33
80106b41:	6a 21                	push   $0x21
  jmp alltraps
80106b43:	e9 b0 f8 ff ff       	jmp    801063f8 <alltraps>

80106b48 <vector34>:
.globl vector34
vector34:
  pushl $0
80106b48:	6a 00                	push   $0x0
  pushl $34
80106b4a:	6a 22                	push   $0x22
  jmp alltraps
80106b4c:	e9 a7 f8 ff ff       	jmp    801063f8 <alltraps>

80106b51 <vector35>:
.globl vector35
vector35:
  pushl $0
80106b51:	6a 00                	push   $0x0
  pushl $35
80106b53:	6a 23                	push   $0x23
  jmp alltraps
80106b55:	e9 9e f8 ff ff       	jmp    801063f8 <alltraps>

80106b5a <vector36>:
.globl vector36
vector36:
  pushl $0
80106b5a:	6a 00                	push   $0x0
  pushl $36
80106b5c:	6a 24                	push   $0x24
  jmp alltraps
80106b5e:	e9 95 f8 ff ff       	jmp    801063f8 <alltraps>

80106b63 <vector37>:
.globl vector37
vector37:
  pushl $0
80106b63:	6a 00                	push   $0x0
  pushl $37
80106b65:	6a 25                	push   $0x25
  jmp alltraps
80106b67:	e9 8c f8 ff ff       	jmp    801063f8 <alltraps>

80106b6c <vector38>:
.globl vector38
vector38:
  pushl $0
80106b6c:	6a 00                	push   $0x0
  pushl $38
80106b6e:	6a 26                	push   $0x26
  jmp alltraps
80106b70:	e9 83 f8 ff ff       	jmp    801063f8 <alltraps>

80106b75 <vector39>:
.globl vector39
vector39:
  pushl $0
80106b75:	6a 00                	push   $0x0
  pushl $39
80106b77:	6a 27                	push   $0x27
  jmp alltraps
80106b79:	e9 7a f8 ff ff       	jmp    801063f8 <alltraps>

80106b7e <vector40>:
.globl vector40
vector40:
  pushl $0
80106b7e:	6a 00                	push   $0x0
  pushl $40
80106b80:	6a 28                	push   $0x28
  jmp alltraps
80106b82:	e9 71 f8 ff ff       	jmp    801063f8 <alltraps>

80106b87 <vector41>:
.globl vector41
vector41:
  pushl $0
80106b87:	6a 00                	push   $0x0
  pushl $41
80106b89:	6a 29                	push   $0x29
  jmp alltraps
80106b8b:	e9 68 f8 ff ff       	jmp    801063f8 <alltraps>

80106b90 <vector42>:
.globl vector42
vector42:
  pushl $0
80106b90:	6a 00                	push   $0x0
  pushl $42
80106b92:	6a 2a                	push   $0x2a
  jmp alltraps
80106b94:	e9 5f f8 ff ff       	jmp    801063f8 <alltraps>

80106b99 <vector43>:
.globl vector43
vector43:
  pushl $0
80106b99:	6a 00                	push   $0x0
  pushl $43
80106b9b:	6a 2b                	push   $0x2b
  jmp alltraps
80106b9d:	e9 56 f8 ff ff       	jmp    801063f8 <alltraps>

80106ba2 <vector44>:
.globl vector44
vector44:
  pushl $0
80106ba2:	6a 00                	push   $0x0
  pushl $44
80106ba4:	6a 2c                	push   $0x2c
  jmp alltraps
80106ba6:	e9 4d f8 ff ff       	jmp    801063f8 <alltraps>

80106bab <vector45>:
.globl vector45
vector45:
  pushl $0
80106bab:	6a 00                	push   $0x0
  pushl $45
80106bad:	6a 2d                	push   $0x2d
  jmp alltraps
80106baf:	e9 44 f8 ff ff       	jmp    801063f8 <alltraps>

80106bb4 <vector46>:
.globl vector46
vector46:
  pushl $0
80106bb4:	6a 00                	push   $0x0
  pushl $46
80106bb6:	6a 2e                	push   $0x2e
  jmp alltraps
80106bb8:	e9 3b f8 ff ff       	jmp    801063f8 <alltraps>

80106bbd <vector47>:
.globl vector47
vector47:
  pushl $0
80106bbd:	6a 00                	push   $0x0
  pushl $47
80106bbf:	6a 2f                	push   $0x2f
  jmp alltraps
80106bc1:	e9 32 f8 ff ff       	jmp    801063f8 <alltraps>

80106bc6 <vector48>:
.globl vector48
vector48:
  pushl $0
80106bc6:	6a 00                	push   $0x0
  pushl $48
80106bc8:	6a 30                	push   $0x30
  jmp alltraps
80106bca:	e9 29 f8 ff ff       	jmp    801063f8 <alltraps>

80106bcf <vector49>:
.globl vector49
vector49:
  pushl $0
80106bcf:	6a 00                	push   $0x0
  pushl $49
80106bd1:	6a 31                	push   $0x31
  jmp alltraps
80106bd3:	e9 20 f8 ff ff       	jmp    801063f8 <alltraps>

80106bd8 <vector50>:
.globl vector50
vector50:
  pushl $0
80106bd8:	6a 00                	push   $0x0
  pushl $50
80106bda:	6a 32                	push   $0x32
  jmp alltraps
80106bdc:	e9 17 f8 ff ff       	jmp    801063f8 <alltraps>

80106be1 <vector51>:
.globl vector51
vector51:
  pushl $0
80106be1:	6a 00                	push   $0x0
  pushl $51
80106be3:	6a 33                	push   $0x33
  jmp alltraps
80106be5:	e9 0e f8 ff ff       	jmp    801063f8 <alltraps>

80106bea <vector52>:
.globl vector52
vector52:
  pushl $0
80106bea:	6a 00                	push   $0x0
  pushl $52
80106bec:	6a 34                	push   $0x34
  jmp alltraps
80106bee:	e9 05 f8 ff ff       	jmp    801063f8 <alltraps>

80106bf3 <vector53>:
.globl vector53
vector53:
  pushl $0
80106bf3:	6a 00                	push   $0x0
  pushl $53
80106bf5:	6a 35                	push   $0x35
  jmp alltraps
80106bf7:	e9 fc f7 ff ff       	jmp    801063f8 <alltraps>

80106bfc <vector54>:
.globl vector54
vector54:
  pushl $0
80106bfc:	6a 00                	push   $0x0
  pushl $54
80106bfe:	6a 36                	push   $0x36
  jmp alltraps
80106c00:	e9 f3 f7 ff ff       	jmp    801063f8 <alltraps>

80106c05 <vector55>:
.globl vector55
vector55:
  pushl $0
80106c05:	6a 00                	push   $0x0
  pushl $55
80106c07:	6a 37                	push   $0x37
  jmp alltraps
80106c09:	e9 ea f7 ff ff       	jmp    801063f8 <alltraps>

80106c0e <vector56>:
.globl vector56
vector56:
  pushl $0
80106c0e:	6a 00                	push   $0x0
  pushl $56
80106c10:	6a 38                	push   $0x38
  jmp alltraps
80106c12:	e9 e1 f7 ff ff       	jmp    801063f8 <alltraps>

80106c17 <vector57>:
.globl vector57
vector57:
  pushl $0
80106c17:	6a 00                	push   $0x0
  pushl $57
80106c19:	6a 39                	push   $0x39
  jmp alltraps
80106c1b:	e9 d8 f7 ff ff       	jmp    801063f8 <alltraps>

80106c20 <vector58>:
.globl vector58
vector58:
  pushl $0
80106c20:	6a 00                	push   $0x0
  pushl $58
80106c22:	6a 3a                	push   $0x3a
  jmp alltraps
80106c24:	e9 cf f7 ff ff       	jmp    801063f8 <alltraps>

80106c29 <vector59>:
.globl vector59
vector59:
  pushl $0
80106c29:	6a 00                	push   $0x0
  pushl $59
80106c2b:	6a 3b                	push   $0x3b
  jmp alltraps
80106c2d:	e9 c6 f7 ff ff       	jmp    801063f8 <alltraps>

80106c32 <vector60>:
.globl vector60
vector60:
  pushl $0
80106c32:	6a 00                	push   $0x0
  pushl $60
80106c34:	6a 3c                	push   $0x3c
  jmp alltraps
80106c36:	e9 bd f7 ff ff       	jmp    801063f8 <alltraps>

80106c3b <vector61>:
.globl vector61
vector61:
  pushl $0
80106c3b:	6a 00                	push   $0x0
  pushl $61
80106c3d:	6a 3d                	push   $0x3d
  jmp alltraps
80106c3f:	e9 b4 f7 ff ff       	jmp    801063f8 <alltraps>

80106c44 <vector62>:
.globl vector62
vector62:
  pushl $0
80106c44:	6a 00                	push   $0x0
  pushl $62
80106c46:	6a 3e                	push   $0x3e
  jmp alltraps
80106c48:	e9 ab f7 ff ff       	jmp    801063f8 <alltraps>

80106c4d <vector63>:
.globl vector63
vector63:
  pushl $0
80106c4d:	6a 00                	push   $0x0
  pushl $63
80106c4f:	6a 3f                	push   $0x3f
  jmp alltraps
80106c51:	e9 a2 f7 ff ff       	jmp    801063f8 <alltraps>

80106c56 <vector64>:
.globl vector64
vector64:
  pushl $0
80106c56:	6a 00                	push   $0x0
  pushl $64
80106c58:	6a 40                	push   $0x40
  jmp alltraps
80106c5a:	e9 99 f7 ff ff       	jmp    801063f8 <alltraps>

80106c5f <vector65>:
.globl vector65
vector65:
  pushl $0
80106c5f:	6a 00                	push   $0x0
  pushl $65
80106c61:	6a 41                	push   $0x41
  jmp alltraps
80106c63:	e9 90 f7 ff ff       	jmp    801063f8 <alltraps>

80106c68 <vector66>:
.globl vector66
vector66:
  pushl $0
80106c68:	6a 00                	push   $0x0
  pushl $66
80106c6a:	6a 42                	push   $0x42
  jmp alltraps
80106c6c:	e9 87 f7 ff ff       	jmp    801063f8 <alltraps>

80106c71 <vector67>:
.globl vector67
vector67:
  pushl $0
80106c71:	6a 00                	push   $0x0
  pushl $67
80106c73:	6a 43                	push   $0x43
  jmp alltraps
80106c75:	e9 7e f7 ff ff       	jmp    801063f8 <alltraps>

80106c7a <vector68>:
.globl vector68
vector68:
  pushl $0
80106c7a:	6a 00                	push   $0x0
  pushl $68
80106c7c:	6a 44                	push   $0x44
  jmp alltraps
80106c7e:	e9 75 f7 ff ff       	jmp    801063f8 <alltraps>

80106c83 <vector69>:
.globl vector69
vector69:
  pushl $0
80106c83:	6a 00                	push   $0x0
  pushl $69
80106c85:	6a 45                	push   $0x45
  jmp alltraps
80106c87:	e9 6c f7 ff ff       	jmp    801063f8 <alltraps>

80106c8c <vector70>:
.globl vector70
vector70:
  pushl $0
80106c8c:	6a 00                	push   $0x0
  pushl $70
80106c8e:	6a 46                	push   $0x46
  jmp alltraps
80106c90:	e9 63 f7 ff ff       	jmp    801063f8 <alltraps>

80106c95 <vector71>:
.globl vector71
vector71:
  pushl $0
80106c95:	6a 00                	push   $0x0
  pushl $71
80106c97:	6a 47                	push   $0x47
  jmp alltraps
80106c99:	e9 5a f7 ff ff       	jmp    801063f8 <alltraps>

80106c9e <vector72>:
.globl vector72
vector72:
  pushl $0
80106c9e:	6a 00                	push   $0x0
  pushl $72
80106ca0:	6a 48                	push   $0x48
  jmp alltraps
80106ca2:	e9 51 f7 ff ff       	jmp    801063f8 <alltraps>

80106ca7 <vector73>:
.globl vector73
vector73:
  pushl $0
80106ca7:	6a 00                	push   $0x0
  pushl $73
80106ca9:	6a 49                	push   $0x49
  jmp alltraps
80106cab:	e9 48 f7 ff ff       	jmp    801063f8 <alltraps>

80106cb0 <vector74>:
.globl vector74
vector74:
  pushl $0
80106cb0:	6a 00                	push   $0x0
  pushl $74
80106cb2:	6a 4a                	push   $0x4a
  jmp alltraps
80106cb4:	e9 3f f7 ff ff       	jmp    801063f8 <alltraps>

80106cb9 <vector75>:
.globl vector75
vector75:
  pushl $0
80106cb9:	6a 00                	push   $0x0
  pushl $75
80106cbb:	6a 4b                	push   $0x4b
  jmp alltraps
80106cbd:	e9 36 f7 ff ff       	jmp    801063f8 <alltraps>

80106cc2 <vector76>:
.globl vector76
vector76:
  pushl $0
80106cc2:	6a 00                	push   $0x0
  pushl $76
80106cc4:	6a 4c                	push   $0x4c
  jmp alltraps
80106cc6:	e9 2d f7 ff ff       	jmp    801063f8 <alltraps>

80106ccb <vector77>:
.globl vector77
vector77:
  pushl $0
80106ccb:	6a 00                	push   $0x0
  pushl $77
80106ccd:	6a 4d                	push   $0x4d
  jmp alltraps
80106ccf:	e9 24 f7 ff ff       	jmp    801063f8 <alltraps>

80106cd4 <vector78>:
.globl vector78
vector78:
  pushl $0
80106cd4:	6a 00                	push   $0x0
  pushl $78
80106cd6:	6a 4e                	push   $0x4e
  jmp alltraps
80106cd8:	e9 1b f7 ff ff       	jmp    801063f8 <alltraps>

80106cdd <vector79>:
.globl vector79
vector79:
  pushl $0
80106cdd:	6a 00                	push   $0x0
  pushl $79
80106cdf:	6a 4f                	push   $0x4f
  jmp alltraps
80106ce1:	e9 12 f7 ff ff       	jmp    801063f8 <alltraps>

80106ce6 <vector80>:
.globl vector80
vector80:
  pushl $0
80106ce6:	6a 00                	push   $0x0
  pushl $80
80106ce8:	6a 50                	push   $0x50
  jmp alltraps
80106cea:	e9 09 f7 ff ff       	jmp    801063f8 <alltraps>

80106cef <vector81>:
.globl vector81
vector81:
  pushl $0
80106cef:	6a 00                	push   $0x0
  pushl $81
80106cf1:	6a 51                	push   $0x51
  jmp alltraps
80106cf3:	e9 00 f7 ff ff       	jmp    801063f8 <alltraps>

80106cf8 <vector82>:
.globl vector82
vector82:
  pushl $0
80106cf8:	6a 00                	push   $0x0
  pushl $82
80106cfa:	6a 52                	push   $0x52
  jmp alltraps
80106cfc:	e9 f7 f6 ff ff       	jmp    801063f8 <alltraps>

80106d01 <vector83>:
.globl vector83
vector83:
  pushl $0
80106d01:	6a 00                	push   $0x0
  pushl $83
80106d03:	6a 53                	push   $0x53
  jmp alltraps
80106d05:	e9 ee f6 ff ff       	jmp    801063f8 <alltraps>

80106d0a <vector84>:
.globl vector84
vector84:
  pushl $0
80106d0a:	6a 00                	push   $0x0
  pushl $84
80106d0c:	6a 54                	push   $0x54
  jmp alltraps
80106d0e:	e9 e5 f6 ff ff       	jmp    801063f8 <alltraps>

80106d13 <vector85>:
.globl vector85
vector85:
  pushl $0
80106d13:	6a 00                	push   $0x0
  pushl $85
80106d15:	6a 55                	push   $0x55
  jmp alltraps
80106d17:	e9 dc f6 ff ff       	jmp    801063f8 <alltraps>

80106d1c <vector86>:
.globl vector86
vector86:
  pushl $0
80106d1c:	6a 00                	push   $0x0
  pushl $86
80106d1e:	6a 56                	push   $0x56
  jmp alltraps
80106d20:	e9 d3 f6 ff ff       	jmp    801063f8 <alltraps>

80106d25 <vector87>:
.globl vector87
vector87:
  pushl $0
80106d25:	6a 00                	push   $0x0
  pushl $87
80106d27:	6a 57                	push   $0x57
  jmp alltraps
80106d29:	e9 ca f6 ff ff       	jmp    801063f8 <alltraps>

80106d2e <vector88>:
.globl vector88
vector88:
  pushl $0
80106d2e:	6a 00                	push   $0x0
  pushl $88
80106d30:	6a 58                	push   $0x58
  jmp alltraps
80106d32:	e9 c1 f6 ff ff       	jmp    801063f8 <alltraps>

80106d37 <vector89>:
.globl vector89
vector89:
  pushl $0
80106d37:	6a 00                	push   $0x0
  pushl $89
80106d39:	6a 59                	push   $0x59
  jmp alltraps
80106d3b:	e9 b8 f6 ff ff       	jmp    801063f8 <alltraps>

80106d40 <vector90>:
.globl vector90
vector90:
  pushl $0
80106d40:	6a 00                	push   $0x0
  pushl $90
80106d42:	6a 5a                	push   $0x5a
  jmp alltraps
80106d44:	e9 af f6 ff ff       	jmp    801063f8 <alltraps>

80106d49 <vector91>:
.globl vector91
vector91:
  pushl $0
80106d49:	6a 00                	push   $0x0
  pushl $91
80106d4b:	6a 5b                	push   $0x5b
  jmp alltraps
80106d4d:	e9 a6 f6 ff ff       	jmp    801063f8 <alltraps>

80106d52 <vector92>:
.globl vector92
vector92:
  pushl $0
80106d52:	6a 00                	push   $0x0
  pushl $92
80106d54:	6a 5c                	push   $0x5c
  jmp alltraps
80106d56:	e9 9d f6 ff ff       	jmp    801063f8 <alltraps>

80106d5b <vector93>:
.globl vector93
vector93:
  pushl $0
80106d5b:	6a 00                	push   $0x0
  pushl $93
80106d5d:	6a 5d                	push   $0x5d
  jmp alltraps
80106d5f:	e9 94 f6 ff ff       	jmp    801063f8 <alltraps>

80106d64 <vector94>:
.globl vector94
vector94:
  pushl $0
80106d64:	6a 00                	push   $0x0
  pushl $94
80106d66:	6a 5e                	push   $0x5e
  jmp alltraps
80106d68:	e9 8b f6 ff ff       	jmp    801063f8 <alltraps>

80106d6d <vector95>:
.globl vector95
vector95:
  pushl $0
80106d6d:	6a 00                	push   $0x0
  pushl $95
80106d6f:	6a 5f                	push   $0x5f
  jmp alltraps
80106d71:	e9 82 f6 ff ff       	jmp    801063f8 <alltraps>

80106d76 <vector96>:
.globl vector96
vector96:
  pushl $0
80106d76:	6a 00                	push   $0x0
  pushl $96
80106d78:	6a 60                	push   $0x60
  jmp alltraps
80106d7a:	e9 79 f6 ff ff       	jmp    801063f8 <alltraps>

80106d7f <vector97>:
.globl vector97
vector97:
  pushl $0
80106d7f:	6a 00                	push   $0x0
  pushl $97
80106d81:	6a 61                	push   $0x61
  jmp alltraps
80106d83:	e9 70 f6 ff ff       	jmp    801063f8 <alltraps>

80106d88 <vector98>:
.globl vector98
vector98:
  pushl $0
80106d88:	6a 00                	push   $0x0
  pushl $98
80106d8a:	6a 62                	push   $0x62
  jmp alltraps
80106d8c:	e9 67 f6 ff ff       	jmp    801063f8 <alltraps>

80106d91 <vector99>:
.globl vector99
vector99:
  pushl $0
80106d91:	6a 00                	push   $0x0
  pushl $99
80106d93:	6a 63                	push   $0x63
  jmp alltraps
80106d95:	e9 5e f6 ff ff       	jmp    801063f8 <alltraps>

80106d9a <vector100>:
.globl vector100
vector100:
  pushl $0
80106d9a:	6a 00                	push   $0x0
  pushl $100
80106d9c:	6a 64                	push   $0x64
  jmp alltraps
80106d9e:	e9 55 f6 ff ff       	jmp    801063f8 <alltraps>

80106da3 <vector101>:
.globl vector101
vector101:
  pushl $0
80106da3:	6a 00                	push   $0x0
  pushl $101
80106da5:	6a 65                	push   $0x65
  jmp alltraps
80106da7:	e9 4c f6 ff ff       	jmp    801063f8 <alltraps>

80106dac <vector102>:
.globl vector102
vector102:
  pushl $0
80106dac:	6a 00                	push   $0x0
  pushl $102
80106dae:	6a 66                	push   $0x66
  jmp alltraps
80106db0:	e9 43 f6 ff ff       	jmp    801063f8 <alltraps>

80106db5 <vector103>:
.globl vector103
vector103:
  pushl $0
80106db5:	6a 00                	push   $0x0
  pushl $103
80106db7:	6a 67                	push   $0x67
  jmp alltraps
80106db9:	e9 3a f6 ff ff       	jmp    801063f8 <alltraps>

80106dbe <vector104>:
.globl vector104
vector104:
  pushl $0
80106dbe:	6a 00                	push   $0x0
  pushl $104
80106dc0:	6a 68                	push   $0x68
  jmp alltraps
80106dc2:	e9 31 f6 ff ff       	jmp    801063f8 <alltraps>

80106dc7 <vector105>:
.globl vector105
vector105:
  pushl $0
80106dc7:	6a 00                	push   $0x0
  pushl $105
80106dc9:	6a 69                	push   $0x69
  jmp alltraps
80106dcb:	e9 28 f6 ff ff       	jmp    801063f8 <alltraps>

80106dd0 <vector106>:
.globl vector106
vector106:
  pushl $0
80106dd0:	6a 00                	push   $0x0
  pushl $106
80106dd2:	6a 6a                	push   $0x6a
  jmp alltraps
80106dd4:	e9 1f f6 ff ff       	jmp    801063f8 <alltraps>

80106dd9 <vector107>:
.globl vector107
vector107:
  pushl $0
80106dd9:	6a 00                	push   $0x0
  pushl $107
80106ddb:	6a 6b                	push   $0x6b
  jmp alltraps
80106ddd:	e9 16 f6 ff ff       	jmp    801063f8 <alltraps>

80106de2 <vector108>:
.globl vector108
vector108:
  pushl $0
80106de2:	6a 00                	push   $0x0
  pushl $108
80106de4:	6a 6c                	push   $0x6c
  jmp alltraps
80106de6:	e9 0d f6 ff ff       	jmp    801063f8 <alltraps>

80106deb <vector109>:
.globl vector109
vector109:
  pushl $0
80106deb:	6a 00                	push   $0x0
  pushl $109
80106ded:	6a 6d                	push   $0x6d
  jmp alltraps
80106def:	e9 04 f6 ff ff       	jmp    801063f8 <alltraps>

80106df4 <vector110>:
.globl vector110
vector110:
  pushl $0
80106df4:	6a 00                	push   $0x0
  pushl $110
80106df6:	6a 6e                	push   $0x6e
  jmp alltraps
80106df8:	e9 fb f5 ff ff       	jmp    801063f8 <alltraps>

80106dfd <vector111>:
.globl vector111
vector111:
  pushl $0
80106dfd:	6a 00                	push   $0x0
  pushl $111
80106dff:	6a 6f                	push   $0x6f
  jmp alltraps
80106e01:	e9 f2 f5 ff ff       	jmp    801063f8 <alltraps>

80106e06 <vector112>:
.globl vector112
vector112:
  pushl $0
80106e06:	6a 00                	push   $0x0
  pushl $112
80106e08:	6a 70                	push   $0x70
  jmp alltraps
80106e0a:	e9 e9 f5 ff ff       	jmp    801063f8 <alltraps>

80106e0f <vector113>:
.globl vector113
vector113:
  pushl $0
80106e0f:	6a 00                	push   $0x0
  pushl $113
80106e11:	6a 71                	push   $0x71
  jmp alltraps
80106e13:	e9 e0 f5 ff ff       	jmp    801063f8 <alltraps>

80106e18 <vector114>:
.globl vector114
vector114:
  pushl $0
80106e18:	6a 00                	push   $0x0
  pushl $114
80106e1a:	6a 72                	push   $0x72
  jmp alltraps
80106e1c:	e9 d7 f5 ff ff       	jmp    801063f8 <alltraps>

80106e21 <vector115>:
.globl vector115
vector115:
  pushl $0
80106e21:	6a 00                	push   $0x0
  pushl $115
80106e23:	6a 73                	push   $0x73
  jmp alltraps
80106e25:	e9 ce f5 ff ff       	jmp    801063f8 <alltraps>

80106e2a <vector116>:
.globl vector116
vector116:
  pushl $0
80106e2a:	6a 00                	push   $0x0
  pushl $116
80106e2c:	6a 74                	push   $0x74
  jmp alltraps
80106e2e:	e9 c5 f5 ff ff       	jmp    801063f8 <alltraps>

80106e33 <vector117>:
.globl vector117
vector117:
  pushl $0
80106e33:	6a 00                	push   $0x0
  pushl $117
80106e35:	6a 75                	push   $0x75
  jmp alltraps
80106e37:	e9 bc f5 ff ff       	jmp    801063f8 <alltraps>

80106e3c <vector118>:
.globl vector118
vector118:
  pushl $0
80106e3c:	6a 00                	push   $0x0
  pushl $118
80106e3e:	6a 76                	push   $0x76
  jmp alltraps
80106e40:	e9 b3 f5 ff ff       	jmp    801063f8 <alltraps>

80106e45 <vector119>:
.globl vector119
vector119:
  pushl $0
80106e45:	6a 00                	push   $0x0
  pushl $119
80106e47:	6a 77                	push   $0x77
  jmp alltraps
80106e49:	e9 aa f5 ff ff       	jmp    801063f8 <alltraps>

80106e4e <vector120>:
.globl vector120
vector120:
  pushl $0
80106e4e:	6a 00                	push   $0x0
  pushl $120
80106e50:	6a 78                	push   $0x78
  jmp alltraps
80106e52:	e9 a1 f5 ff ff       	jmp    801063f8 <alltraps>

80106e57 <vector121>:
.globl vector121
vector121:
  pushl $0
80106e57:	6a 00                	push   $0x0
  pushl $121
80106e59:	6a 79                	push   $0x79
  jmp alltraps
80106e5b:	e9 98 f5 ff ff       	jmp    801063f8 <alltraps>

80106e60 <vector122>:
.globl vector122
vector122:
  pushl $0
80106e60:	6a 00                	push   $0x0
  pushl $122
80106e62:	6a 7a                	push   $0x7a
  jmp alltraps
80106e64:	e9 8f f5 ff ff       	jmp    801063f8 <alltraps>

80106e69 <vector123>:
.globl vector123
vector123:
  pushl $0
80106e69:	6a 00                	push   $0x0
  pushl $123
80106e6b:	6a 7b                	push   $0x7b
  jmp alltraps
80106e6d:	e9 86 f5 ff ff       	jmp    801063f8 <alltraps>

80106e72 <vector124>:
.globl vector124
vector124:
  pushl $0
80106e72:	6a 00                	push   $0x0
  pushl $124
80106e74:	6a 7c                	push   $0x7c
  jmp alltraps
80106e76:	e9 7d f5 ff ff       	jmp    801063f8 <alltraps>

80106e7b <vector125>:
.globl vector125
vector125:
  pushl $0
80106e7b:	6a 00                	push   $0x0
  pushl $125
80106e7d:	6a 7d                	push   $0x7d
  jmp alltraps
80106e7f:	e9 74 f5 ff ff       	jmp    801063f8 <alltraps>

80106e84 <vector126>:
.globl vector126
vector126:
  pushl $0
80106e84:	6a 00                	push   $0x0
  pushl $126
80106e86:	6a 7e                	push   $0x7e
  jmp alltraps
80106e88:	e9 6b f5 ff ff       	jmp    801063f8 <alltraps>

80106e8d <vector127>:
.globl vector127
vector127:
  pushl $0
80106e8d:	6a 00                	push   $0x0
  pushl $127
80106e8f:	6a 7f                	push   $0x7f
  jmp alltraps
80106e91:	e9 62 f5 ff ff       	jmp    801063f8 <alltraps>

80106e96 <vector128>:
.globl vector128
vector128:
  pushl $0
80106e96:	6a 00                	push   $0x0
  pushl $128
80106e98:	68 80 00 00 00       	push   $0x80
  jmp alltraps
80106e9d:	e9 56 f5 ff ff       	jmp    801063f8 <alltraps>

80106ea2 <vector129>:
.globl vector129
vector129:
  pushl $0
80106ea2:	6a 00                	push   $0x0
  pushl $129
80106ea4:	68 81 00 00 00       	push   $0x81
  jmp alltraps
80106ea9:	e9 4a f5 ff ff       	jmp    801063f8 <alltraps>

80106eae <vector130>:
.globl vector130
vector130:
  pushl $0
80106eae:	6a 00                	push   $0x0
  pushl $130
80106eb0:	68 82 00 00 00       	push   $0x82
  jmp alltraps
80106eb5:	e9 3e f5 ff ff       	jmp    801063f8 <alltraps>

80106eba <vector131>:
.globl vector131
vector131:
  pushl $0
80106eba:	6a 00                	push   $0x0
  pushl $131
80106ebc:	68 83 00 00 00       	push   $0x83
  jmp alltraps
80106ec1:	e9 32 f5 ff ff       	jmp    801063f8 <alltraps>

80106ec6 <vector132>:
.globl vector132
vector132:
  pushl $0
80106ec6:	6a 00                	push   $0x0
  pushl $132
80106ec8:	68 84 00 00 00       	push   $0x84
  jmp alltraps
80106ecd:	e9 26 f5 ff ff       	jmp    801063f8 <alltraps>

80106ed2 <vector133>:
.globl vector133
vector133:
  pushl $0
80106ed2:	6a 00                	push   $0x0
  pushl $133
80106ed4:	68 85 00 00 00       	push   $0x85
  jmp alltraps
80106ed9:	e9 1a f5 ff ff       	jmp    801063f8 <alltraps>

80106ede <vector134>:
.globl vector134
vector134:
  pushl $0
80106ede:	6a 00                	push   $0x0
  pushl $134
80106ee0:	68 86 00 00 00       	push   $0x86
  jmp alltraps
80106ee5:	e9 0e f5 ff ff       	jmp    801063f8 <alltraps>

80106eea <vector135>:
.globl vector135
vector135:
  pushl $0
80106eea:	6a 00                	push   $0x0
  pushl $135
80106eec:	68 87 00 00 00       	push   $0x87
  jmp alltraps
80106ef1:	e9 02 f5 ff ff       	jmp    801063f8 <alltraps>

80106ef6 <vector136>:
.globl vector136
vector136:
  pushl $0
80106ef6:	6a 00                	push   $0x0
  pushl $136
80106ef8:	68 88 00 00 00       	push   $0x88
  jmp alltraps
80106efd:	e9 f6 f4 ff ff       	jmp    801063f8 <alltraps>

80106f02 <vector137>:
.globl vector137
vector137:
  pushl $0
80106f02:	6a 00                	push   $0x0
  pushl $137
80106f04:	68 89 00 00 00       	push   $0x89
  jmp alltraps
80106f09:	e9 ea f4 ff ff       	jmp    801063f8 <alltraps>

80106f0e <vector138>:
.globl vector138
vector138:
  pushl $0
80106f0e:	6a 00                	push   $0x0
  pushl $138
80106f10:	68 8a 00 00 00       	push   $0x8a
  jmp alltraps
80106f15:	e9 de f4 ff ff       	jmp    801063f8 <alltraps>

80106f1a <vector139>:
.globl vector139
vector139:
  pushl $0
80106f1a:	6a 00                	push   $0x0
  pushl $139
80106f1c:	68 8b 00 00 00       	push   $0x8b
  jmp alltraps
80106f21:	e9 d2 f4 ff ff       	jmp    801063f8 <alltraps>

80106f26 <vector140>:
.globl vector140
vector140:
  pushl $0
80106f26:	6a 00                	push   $0x0
  pushl $140
80106f28:	68 8c 00 00 00       	push   $0x8c
  jmp alltraps
80106f2d:	e9 c6 f4 ff ff       	jmp    801063f8 <alltraps>

80106f32 <vector141>:
.globl vector141
vector141:
  pushl $0
80106f32:	6a 00                	push   $0x0
  pushl $141
80106f34:	68 8d 00 00 00       	push   $0x8d
  jmp alltraps
80106f39:	e9 ba f4 ff ff       	jmp    801063f8 <alltraps>

80106f3e <vector142>:
.globl vector142
vector142:
  pushl $0
80106f3e:	6a 00                	push   $0x0
  pushl $142
80106f40:	68 8e 00 00 00       	push   $0x8e
  jmp alltraps
80106f45:	e9 ae f4 ff ff       	jmp    801063f8 <alltraps>

80106f4a <vector143>:
.globl vector143
vector143:
  pushl $0
80106f4a:	6a 00                	push   $0x0
  pushl $143
80106f4c:	68 8f 00 00 00       	push   $0x8f
  jmp alltraps
80106f51:	e9 a2 f4 ff ff       	jmp    801063f8 <alltraps>

80106f56 <vector144>:
.globl vector144
vector144:
  pushl $0
80106f56:	6a 00                	push   $0x0
  pushl $144
80106f58:	68 90 00 00 00       	push   $0x90
  jmp alltraps
80106f5d:	e9 96 f4 ff ff       	jmp    801063f8 <alltraps>

80106f62 <vector145>:
.globl vector145
vector145:
  pushl $0
80106f62:	6a 00                	push   $0x0
  pushl $145
80106f64:	68 91 00 00 00       	push   $0x91
  jmp alltraps
80106f69:	e9 8a f4 ff ff       	jmp    801063f8 <alltraps>

80106f6e <vector146>:
.globl vector146
vector146:
  pushl $0
80106f6e:	6a 00                	push   $0x0
  pushl $146
80106f70:	68 92 00 00 00       	push   $0x92
  jmp alltraps
80106f75:	e9 7e f4 ff ff       	jmp    801063f8 <alltraps>

80106f7a <vector147>:
.globl vector147
vector147:
  pushl $0
80106f7a:	6a 00                	push   $0x0
  pushl $147
80106f7c:	68 93 00 00 00       	push   $0x93
  jmp alltraps
80106f81:	e9 72 f4 ff ff       	jmp    801063f8 <alltraps>

80106f86 <vector148>:
.globl vector148
vector148:
  pushl $0
80106f86:	6a 00                	push   $0x0
  pushl $148
80106f88:	68 94 00 00 00       	push   $0x94
  jmp alltraps
80106f8d:	e9 66 f4 ff ff       	jmp    801063f8 <alltraps>

80106f92 <vector149>:
.globl vector149
vector149:
  pushl $0
80106f92:	6a 00                	push   $0x0
  pushl $149
80106f94:	68 95 00 00 00       	push   $0x95
  jmp alltraps
80106f99:	e9 5a f4 ff ff       	jmp    801063f8 <alltraps>

80106f9e <vector150>:
.globl vector150
vector150:
  pushl $0
80106f9e:	6a 00                	push   $0x0
  pushl $150
80106fa0:	68 96 00 00 00       	push   $0x96
  jmp alltraps
80106fa5:	e9 4e f4 ff ff       	jmp    801063f8 <alltraps>

80106faa <vector151>:
.globl vector151
vector151:
  pushl $0
80106faa:	6a 00                	push   $0x0
  pushl $151
80106fac:	68 97 00 00 00       	push   $0x97
  jmp alltraps
80106fb1:	e9 42 f4 ff ff       	jmp    801063f8 <alltraps>

80106fb6 <vector152>:
.globl vector152
vector152:
  pushl $0
80106fb6:	6a 00                	push   $0x0
  pushl $152
80106fb8:	68 98 00 00 00       	push   $0x98
  jmp alltraps
80106fbd:	e9 36 f4 ff ff       	jmp    801063f8 <alltraps>

80106fc2 <vector153>:
.globl vector153
vector153:
  pushl $0
80106fc2:	6a 00                	push   $0x0
  pushl $153
80106fc4:	68 99 00 00 00       	push   $0x99
  jmp alltraps
80106fc9:	e9 2a f4 ff ff       	jmp    801063f8 <alltraps>

80106fce <vector154>:
.globl vector154
vector154:
  pushl $0
80106fce:	6a 00                	push   $0x0
  pushl $154
80106fd0:	68 9a 00 00 00       	push   $0x9a
  jmp alltraps
80106fd5:	e9 1e f4 ff ff       	jmp    801063f8 <alltraps>

80106fda <vector155>:
.globl vector155
vector155:
  pushl $0
80106fda:	6a 00                	push   $0x0
  pushl $155
80106fdc:	68 9b 00 00 00       	push   $0x9b
  jmp alltraps
80106fe1:	e9 12 f4 ff ff       	jmp    801063f8 <alltraps>

80106fe6 <vector156>:
.globl vector156
vector156:
  pushl $0
80106fe6:	6a 00                	push   $0x0
  pushl $156
80106fe8:	68 9c 00 00 00       	push   $0x9c
  jmp alltraps
80106fed:	e9 06 f4 ff ff       	jmp    801063f8 <alltraps>

80106ff2 <vector157>:
.globl vector157
vector157:
  pushl $0
80106ff2:	6a 00                	push   $0x0
  pushl $157
80106ff4:	68 9d 00 00 00       	push   $0x9d
  jmp alltraps
80106ff9:	e9 fa f3 ff ff       	jmp    801063f8 <alltraps>

80106ffe <vector158>:
.globl vector158
vector158:
  pushl $0
80106ffe:	6a 00                	push   $0x0
  pushl $158
80107000:	68 9e 00 00 00       	push   $0x9e
  jmp alltraps
80107005:	e9 ee f3 ff ff       	jmp    801063f8 <alltraps>

8010700a <vector159>:
.globl vector159
vector159:
  pushl $0
8010700a:	6a 00                	push   $0x0
  pushl $159
8010700c:	68 9f 00 00 00       	push   $0x9f
  jmp alltraps
80107011:	e9 e2 f3 ff ff       	jmp    801063f8 <alltraps>

80107016 <vector160>:
.globl vector160
vector160:
  pushl $0
80107016:	6a 00                	push   $0x0
  pushl $160
80107018:	68 a0 00 00 00       	push   $0xa0
  jmp alltraps
8010701d:	e9 d6 f3 ff ff       	jmp    801063f8 <alltraps>

80107022 <vector161>:
.globl vector161
vector161:
  pushl $0
80107022:	6a 00                	push   $0x0
  pushl $161
80107024:	68 a1 00 00 00       	push   $0xa1
  jmp alltraps
80107029:	e9 ca f3 ff ff       	jmp    801063f8 <alltraps>

8010702e <vector162>:
.globl vector162
vector162:
  pushl $0
8010702e:	6a 00                	push   $0x0
  pushl $162
80107030:	68 a2 00 00 00       	push   $0xa2
  jmp alltraps
80107035:	e9 be f3 ff ff       	jmp    801063f8 <alltraps>

8010703a <vector163>:
.globl vector163
vector163:
  pushl $0
8010703a:	6a 00                	push   $0x0
  pushl $163
8010703c:	68 a3 00 00 00       	push   $0xa3
  jmp alltraps
80107041:	e9 b2 f3 ff ff       	jmp    801063f8 <alltraps>

80107046 <vector164>:
.globl vector164
vector164:
  pushl $0
80107046:	6a 00                	push   $0x0
  pushl $164
80107048:	68 a4 00 00 00       	push   $0xa4
  jmp alltraps
8010704d:	e9 a6 f3 ff ff       	jmp    801063f8 <alltraps>

80107052 <vector165>:
.globl vector165
vector165:
  pushl $0
80107052:	6a 00                	push   $0x0
  pushl $165
80107054:	68 a5 00 00 00       	push   $0xa5
  jmp alltraps
80107059:	e9 9a f3 ff ff       	jmp    801063f8 <alltraps>

8010705e <vector166>:
.globl vector166
vector166:
  pushl $0
8010705e:	6a 00                	push   $0x0
  pushl $166
80107060:	68 a6 00 00 00       	push   $0xa6
  jmp alltraps
80107065:	e9 8e f3 ff ff       	jmp    801063f8 <alltraps>

8010706a <vector167>:
.globl vector167
vector167:
  pushl $0
8010706a:	6a 00                	push   $0x0
  pushl $167
8010706c:	68 a7 00 00 00       	push   $0xa7
  jmp alltraps
80107071:	e9 82 f3 ff ff       	jmp    801063f8 <alltraps>

80107076 <vector168>:
.globl vector168
vector168:
  pushl $0
80107076:	6a 00                	push   $0x0
  pushl $168
80107078:	68 a8 00 00 00       	push   $0xa8
  jmp alltraps
8010707d:	e9 76 f3 ff ff       	jmp    801063f8 <alltraps>

80107082 <vector169>:
.globl vector169
vector169:
  pushl $0
80107082:	6a 00                	push   $0x0
  pushl $169
80107084:	68 a9 00 00 00       	push   $0xa9
  jmp alltraps
80107089:	e9 6a f3 ff ff       	jmp    801063f8 <alltraps>

8010708e <vector170>:
.globl vector170
vector170:
  pushl $0
8010708e:	6a 00                	push   $0x0
  pushl $170
80107090:	68 aa 00 00 00       	push   $0xaa
  jmp alltraps
80107095:	e9 5e f3 ff ff       	jmp    801063f8 <alltraps>

8010709a <vector171>:
.globl vector171
vector171:
  pushl $0
8010709a:	6a 00                	push   $0x0
  pushl $171
8010709c:	68 ab 00 00 00       	push   $0xab
  jmp alltraps
801070a1:	e9 52 f3 ff ff       	jmp    801063f8 <alltraps>

801070a6 <vector172>:
.globl vector172
vector172:
  pushl $0
801070a6:	6a 00                	push   $0x0
  pushl $172
801070a8:	68 ac 00 00 00       	push   $0xac
  jmp alltraps
801070ad:	e9 46 f3 ff ff       	jmp    801063f8 <alltraps>

801070b2 <vector173>:
.globl vector173
vector173:
  pushl $0
801070b2:	6a 00                	push   $0x0
  pushl $173
801070b4:	68 ad 00 00 00       	push   $0xad
  jmp alltraps
801070b9:	e9 3a f3 ff ff       	jmp    801063f8 <alltraps>

801070be <vector174>:
.globl vector174
vector174:
  pushl $0
801070be:	6a 00                	push   $0x0
  pushl $174
801070c0:	68 ae 00 00 00       	push   $0xae
  jmp alltraps
801070c5:	e9 2e f3 ff ff       	jmp    801063f8 <alltraps>

801070ca <vector175>:
.globl vector175
vector175:
  pushl $0
801070ca:	6a 00                	push   $0x0
  pushl $175
801070cc:	68 af 00 00 00       	push   $0xaf
  jmp alltraps
801070d1:	e9 22 f3 ff ff       	jmp    801063f8 <alltraps>

801070d6 <vector176>:
.globl vector176
vector176:
  pushl $0
801070d6:	6a 00                	push   $0x0
  pushl $176
801070d8:	68 b0 00 00 00       	push   $0xb0
  jmp alltraps
801070dd:	e9 16 f3 ff ff       	jmp    801063f8 <alltraps>

801070e2 <vector177>:
.globl vector177
vector177:
  pushl $0
801070e2:	6a 00                	push   $0x0
  pushl $177
801070e4:	68 b1 00 00 00       	push   $0xb1
  jmp alltraps
801070e9:	e9 0a f3 ff ff       	jmp    801063f8 <alltraps>

801070ee <vector178>:
.globl vector178
vector178:
  pushl $0
801070ee:	6a 00                	push   $0x0
  pushl $178
801070f0:	68 b2 00 00 00       	push   $0xb2
  jmp alltraps
801070f5:	e9 fe f2 ff ff       	jmp    801063f8 <alltraps>

801070fa <vector179>:
.globl vector179
vector179:
  pushl $0
801070fa:	6a 00                	push   $0x0
  pushl $179
801070fc:	68 b3 00 00 00       	push   $0xb3
  jmp alltraps
80107101:	e9 f2 f2 ff ff       	jmp    801063f8 <alltraps>

80107106 <vector180>:
.globl vector180
vector180:
  pushl $0
80107106:	6a 00                	push   $0x0
  pushl $180
80107108:	68 b4 00 00 00       	push   $0xb4
  jmp alltraps
8010710d:	e9 e6 f2 ff ff       	jmp    801063f8 <alltraps>

80107112 <vector181>:
.globl vector181
vector181:
  pushl $0
80107112:	6a 00                	push   $0x0
  pushl $181
80107114:	68 b5 00 00 00       	push   $0xb5
  jmp alltraps
80107119:	e9 da f2 ff ff       	jmp    801063f8 <alltraps>

8010711e <vector182>:
.globl vector182
vector182:
  pushl $0
8010711e:	6a 00                	push   $0x0
  pushl $182
80107120:	68 b6 00 00 00       	push   $0xb6
  jmp alltraps
80107125:	e9 ce f2 ff ff       	jmp    801063f8 <alltraps>

8010712a <vector183>:
.globl vector183
vector183:
  pushl $0
8010712a:	6a 00                	push   $0x0
  pushl $183
8010712c:	68 b7 00 00 00       	push   $0xb7
  jmp alltraps
80107131:	e9 c2 f2 ff ff       	jmp    801063f8 <alltraps>

80107136 <vector184>:
.globl vector184
vector184:
  pushl $0
80107136:	6a 00                	push   $0x0
  pushl $184
80107138:	68 b8 00 00 00       	push   $0xb8
  jmp alltraps
8010713d:	e9 b6 f2 ff ff       	jmp    801063f8 <alltraps>

80107142 <vector185>:
.globl vector185
vector185:
  pushl $0
80107142:	6a 00                	push   $0x0
  pushl $185
80107144:	68 b9 00 00 00       	push   $0xb9
  jmp alltraps
80107149:	e9 aa f2 ff ff       	jmp    801063f8 <alltraps>

8010714e <vector186>:
.globl vector186
vector186:
  pushl $0
8010714e:	6a 00                	push   $0x0
  pushl $186
80107150:	68 ba 00 00 00       	push   $0xba
  jmp alltraps
80107155:	e9 9e f2 ff ff       	jmp    801063f8 <alltraps>

8010715a <vector187>:
.globl vector187
vector187:
  pushl $0
8010715a:	6a 00                	push   $0x0
  pushl $187
8010715c:	68 bb 00 00 00       	push   $0xbb
  jmp alltraps
80107161:	e9 92 f2 ff ff       	jmp    801063f8 <alltraps>

80107166 <vector188>:
.globl vector188
vector188:
  pushl $0
80107166:	6a 00                	push   $0x0
  pushl $188
80107168:	68 bc 00 00 00       	push   $0xbc
  jmp alltraps
8010716d:	e9 86 f2 ff ff       	jmp    801063f8 <alltraps>

80107172 <vector189>:
.globl vector189
vector189:
  pushl $0
80107172:	6a 00                	push   $0x0
  pushl $189
80107174:	68 bd 00 00 00       	push   $0xbd
  jmp alltraps
80107179:	e9 7a f2 ff ff       	jmp    801063f8 <alltraps>

8010717e <vector190>:
.globl vector190
vector190:
  pushl $0
8010717e:	6a 00                	push   $0x0
  pushl $190
80107180:	68 be 00 00 00       	push   $0xbe
  jmp alltraps
80107185:	e9 6e f2 ff ff       	jmp    801063f8 <alltraps>

8010718a <vector191>:
.globl vector191
vector191:
  pushl $0
8010718a:	6a 00                	push   $0x0
  pushl $191
8010718c:	68 bf 00 00 00       	push   $0xbf
  jmp alltraps
80107191:	e9 62 f2 ff ff       	jmp    801063f8 <alltraps>

80107196 <vector192>:
.globl vector192
vector192:
  pushl $0
80107196:	6a 00                	push   $0x0
  pushl $192
80107198:	68 c0 00 00 00       	push   $0xc0
  jmp alltraps
8010719d:	e9 56 f2 ff ff       	jmp    801063f8 <alltraps>

801071a2 <vector193>:
.globl vector193
vector193:
  pushl $0
801071a2:	6a 00                	push   $0x0
  pushl $193
801071a4:	68 c1 00 00 00       	push   $0xc1
  jmp alltraps
801071a9:	e9 4a f2 ff ff       	jmp    801063f8 <alltraps>

801071ae <vector194>:
.globl vector194
vector194:
  pushl $0
801071ae:	6a 00                	push   $0x0
  pushl $194
801071b0:	68 c2 00 00 00       	push   $0xc2
  jmp alltraps
801071b5:	e9 3e f2 ff ff       	jmp    801063f8 <alltraps>

801071ba <vector195>:
.globl vector195
vector195:
  pushl $0
801071ba:	6a 00                	push   $0x0
  pushl $195
801071bc:	68 c3 00 00 00       	push   $0xc3
  jmp alltraps
801071c1:	e9 32 f2 ff ff       	jmp    801063f8 <alltraps>

801071c6 <vector196>:
.globl vector196
vector196:
  pushl $0
801071c6:	6a 00                	push   $0x0
  pushl $196
801071c8:	68 c4 00 00 00       	push   $0xc4
  jmp alltraps
801071cd:	e9 26 f2 ff ff       	jmp    801063f8 <alltraps>

801071d2 <vector197>:
.globl vector197
vector197:
  pushl $0
801071d2:	6a 00                	push   $0x0
  pushl $197
801071d4:	68 c5 00 00 00       	push   $0xc5
  jmp alltraps
801071d9:	e9 1a f2 ff ff       	jmp    801063f8 <alltraps>

801071de <vector198>:
.globl vector198
vector198:
  pushl $0
801071de:	6a 00                	push   $0x0
  pushl $198
801071e0:	68 c6 00 00 00       	push   $0xc6
  jmp alltraps
801071e5:	e9 0e f2 ff ff       	jmp    801063f8 <alltraps>

801071ea <vector199>:
.globl vector199
vector199:
  pushl $0
801071ea:	6a 00                	push   $0x0
  pushl $199
801071ec:	68 c7 00 00 00       	push   $0xc7
  jmp alltraps
801071f1:	e9 02 f2 ff ff       	jmp    801063f8 <alltraps>

801071f6 <vector200>:
.globl vector200
vector200:
  pushl $0
801071f6:	6a 00                	push   $0x0
  pushl $200
801071f8:	68 c8 00 00 00       	push   $0xc8
  jmp alltraps
801071fd:	e9 f6 f1 ff ff       	jmp    801063f8 <alltraps>

80107202 <vector201>:
.globl vector201
vector201:
  pushl $0
80107202:	6a 00                	push   $0x0
  pushl $201
80107204:	68 c9 00 00 00       	push   $0xc9
  jmp alltraps
80107209:	e9 ea f1 ff ff       	jmp    801063f8 <alltraps>

8010720e <vector202>:
.globl vector202
vector202:
  pushl $0
8010720e:	6a 00                	push   $0x0
  pushl $202
80107210:	68 ca 00 00 00       	push   $0xca
  jmp alltraps
80107215:	e9 de f1 ff ff       	jmp    801063f8 <alltraps>

8010721a <vector203>:
.globl vector203
vector203:
  pushl $0
8010721a:	6a 00                	push   $0x0
  pushl $203
8010721c:	68 cb 00 00 00       	push   $0xcb
  jmp alltraps
80107221:	e9 d2 f1 ff ff       	jmp    801063f8 <alltraps>

80107226 <vector204>:
.globl vector204
vector204:
  pushl $0
80107226:	6a 00                	push   $0x0
  pushl $204
80107228:	68 cc 00 00 00       	push   $0xcc
  jmp alltraps
8010722d:	e9 c6 f1 ff ff       	jmp    801063f8 <alltraps>

80107232 <vector205>:
.globl vector205
vector205:
  pushl $0
80107232:	6a 00                	push   $0x0
  pushl $205
80107234:	68 cd 00 00 00       	push   $0xcd
  jmp alltraps
80107239:	e9 ba f1 ff ff       	jmp    801063f8 <alltraps>

8010723e <vector206>:
.globl vector206
vector206:
  pushl $0
8010723e:	6a 00                	push   $0x0
  pushl $206
80107240:	68 ce 00 00 00       	push   $0xce
  jmp alltraps
80107245:	e9 ae f1 ff ff       	jmp    801063f8 <alltraps>

8010724a <vector207>:
.globl vector207
vector207:
  pushl $0
8010724a:	6a 00                	push   $0x0
  pushl $207
8010724c:	68 cf 00 00 00       	push   $0xcf
  jmp alltraps
80107251:	e9 a2 f1 ff ff       	jmp    801063f8 <alltraps>

80107256 <vector208>:
.globl vector208
vector208:
  pushl $0
80107256:	6a 00                	push   $0x0
  pushl $208
80107258:	68 d0 00 00 00       	push   $0xd0
  jmp alltraps
8010725d:	e9 96 f1 ff ff       	jmp    801063f8 <alltraps>

80107262 <vector209>:
.globl vector209
vector209:
  pushl $0
80107262:	6a 00                	push   $0x0
  pushl $209
80107264:	68 d1 00 00 00       	push   $0xd1
  jmp alltraps
80107269:	e9 8a f1 ff ff       	jmp    801063f8 <alltraps>

8010726e <vector210>:
.globl vector210
vector210:
  pushl $0
8010726e:	6a 00                	push   $0x0
  pushl $210
80107270:	68 d2 00 00 00       	push   $0xd2
  jmp alltraps
80107275:	e9 7e f1 ff ff       	jmp    801063f8 <alltraps>

8010727a <vector211>:
.globl vector211
vector211:
  pushl $0
8010727a:	6a 00                	push   $0x0
  pushl $211
8010727c:	68 d3 00 00 00       	push   $0xd3
  jmp alltraps
80107281:	e9 72 f1 ff ff       	jmp    801063f8 <alltraps>

80107286 <vector212>:
.globl vector212
vector212:
  pushl $0
80107286:	6a 00                	push   $0x0
  pushl $212
80107288:	68 d4 00 00 00       	push   $0xd4
  jmp alltraps
8010728d:	e9 66 f1 ff ff       	jmp    801063f8 <alltraps>

80107292 <vector213>:
.globl vector213
vector213:
  pushl $0
80107292:	6a 00                	push   $0x0
  pushl $213
80107294:	68 d5 00 00 00       	push   $0xd5
  jmp alltraps
80107299:	e9 5a f1 ff ff       	jmp    801063f8 <alltraps>

8010729e <vector214>:
.globl vector214
vector214:
  pushl $0
8010729e:	6a 00                	push   $0x0
  pushl $214
801072a0:	68 d6 00 00 00       	push   $0xd6
  jmp alltraps
801072a5:	e9 4e f1 ff ff       	jmp    801063f8 <alltraps>

801072aa <vector215>:
.globl vector215
vector215:
  pushl $0
801072aa:	6a 00                	push   $0x0
  pushl $215
801072ac:	68 d7 00 00 00       	push   $0xd7
  jmp alltraps
801072b1:	e9 42 f1 ff ff       	jmp    801063f8 <alltraps>

801072b6 <vector216>:
.globl vector216
vector216:
  pushl $0
801072b6:	6a 00                	push   $0x0
  pushl $216
801072b8:	68 d8 00 00 00       	push   $0xd8
  jmp alltraps
801072bd:	e9 36 f1 ff ff       	jmp    801063f8 <alltraps>

801072c2 <vector217>:
.globl vector217
vector217:
  pushl $0
801072c2:	6a 00                	push   $0x0
  pushl $217
801072c4:	68 d9 00 00 00       	push   $0xd9
  jmp alltraps
801072c9:	e9 2a f1 ff ff       	jmp    801063f8 <alltraps>

801072ce <vector218>:
.globl vector218
vector218:
  pushl $0
801072ce:	6a 00                	push   $0x0
  pushl $218
801072d0:	68 da 00 00 00       	push   $0xda
  jmp alltraps
801072d5:	e9 1e f1 ff ff       	jmp    801063f8 <alltraps>

801072da <vector219>:
.globl vector219
vector219:
  pushl $0
801072da:	6a 00                	push   $0x0
  pushl $219
801072dc:	68 db 00 00 00       	push   $0xdb
  jmp alltraps
801072e1:	e9 12 f1 ff ff       	jmp    801063f8 <alltraps>

801072e6 <vector220>:
.globl vector220
vector220:
  pushl $0
801072e6:	6a 00                	push   $0x0
  pushl $220
801072e8:	68 dc 00 00 00       	push   $0xdc
  jmp alltraps
801072ed:	e9 06 f1 ff ff       	jmp    801063f8 <alltraps>

801072f2 <vector221>:
.globl vector221
vector221:
  pushl $0
801072f2:	6a 00                	push   $0x0
  pushl $221
801072f4:	68 dd 00 00 00       	push   $0xdd
  jmp alltraps
801072f9:	e9 fa f0 ff ff       	jmp    801063f8 <alltraps>

801072fe <vector222>:
.globl vector222
vector222:
  pushl $0
801072fe:	6a 00                	push   $0x0
  pushl $222
80107300:	68 de 00 00 00       	push   $0xde
  jmp alltraps
80107305:	e9 ee f0 ff ff       	jmp    801063f8 <alltraps>

8010730a <vector223>:
.globl vector223
vector223:
  pushl $0
8010730a:	6a 00                	push   $0x0
  pushl $223
8010730c:	68 df 00 00 00       	push   $0xdf
  jmp alltraps
80107311:	e9 e2 f0 ff ff       	jmp    801063f8 <alltraps>

80107316 <vector224>:
.globl vector224
vector224:
  pushl $0
80107316:	6a 00                	push   $0x0
  pushl $224
80107318:	68 e0 00 00 00       	push   $0xe0
  jmp alltraps
8010731d:	e9 d6 f0 ff ff       	jmp    801063f8 <alltraps>

80107322 <vector225>:
.globl vector225
vector225:
  pushl $0
80107322:	6a 00                	push   $0x0
  pushl $225
80107324:	68 e1 00 00 00       	push   $0xe1
  jmp alltraps
80107329:	e9 ca f0 ff ff       	jmp    801063f8 <alltraps>

8010732e <vector226>:
.globl vector226
vector226:
  pushl $0
8010732e:	6a 00                	push   $0x0
  pushl $226
80107330:	68 e2 00 00 00       	push   $0xe2
  jmp alltraps
80107335:	e9 be f0 ff ff       	jmp    801063f8 <alltraps>

8010733a <vector227>:
.globl vector227
vector227:
  pushl $0
8010733a:	6a 00                	push   $0x0
  pushl $227
8010733c:	68 e3 00 00 00       	push   $0xe3
  jmp alltraps
80107341:	e9 b2 f0 ff ff       	jmp    801063f8 <alltraps>

80107346 <vector228>:
.globl vector228
vector228:
  pushl $0
80107346:	6a 00                	push   $0x0
  pushl $228
80107348:	68 e4 00 00 00       	push   $0xe4
  jmp alltraps
8010734d:	e9 a6 f0 ff ff       	jmp    801063f8 <alltraps>

80107352 <vector229>:
.globl vector229
vector229:
  pushl $0
80107352:	6a 00                	push   $0x0
  pushl $229
80107354:	68 e5 00 00 00       	push   $0xe5
  jmp alltraps
80107359:	e9 9a f0 ff ff       	jmp    801063f8 <alltraps>

8010735e <vector230>:
.globl vector230
vector230:
  pushl $0
8010735e:	6a 00                	push   $0x0
  pushl $230
80107360:	68 e6 00 00 00       	push   $0xe6
  jmp alltraps
80107365:	e9 8e f0 ff ff       	jmp    801063f8 <alltraps>

8010736a <vector231>:
.globl vector231
vector231:
  pushl $0
8010736a:	6a 00                	push   $0x0
  pushl $231
8010736c:	68 e7 00 00 00       	push   $0xe7
  jmp alltraps
80107371:	e9 82 f0 ff ff       	jmp    801063f8 <alltraps>

80107376 <vector232>:
.globl vector232
vector232:
  pushl $0
80107376:	6a 00                	push   $0x0
  pushl $232
80107378:	68 e8 00 00 00       	push   $0xe8
  jmp alltraps
8010737d:	e9 76 f0 ff ff       	jmp    801063f8 <alltraps>

80107382 <vector233>:
.globl vector233
vector233:
  pushl $0
80107382:	6a 00                	push   $0x0
  pushl $233
80107384:	68 e9 00 00 00       	push   $0xe9
  jmp alltraps
80107389:	e9 6a f0 ff ff       	jmp    801063f8 <alltraps>

8010738e <vector234>:
.globl vector234
vector234:
  pushl $0
8010738e:	6a 00                	push   $0x0
  pushl $234
80107390:	68 ea 00 00 00       	push   $0xea
  jmp alltraps
80107395:	e9 5e f0 ff ff       	jmp    801063f8 <alltraps>

8010739a <vector235>:
.globl vector235
vector235:
  pushl $0
8010739a:	6a 00                	push   $0x0
  pushl $235
8010739c:	68 eb 00 00 00       	push   $0xeb
  jmp alltraps
801073a1:	e9 52 f0 ff ff       	jmp    801063f8 <alltraps>

801073a6 <vector236>:
.globl vector236
vector236:
  pushl $0
801073a6:	6a 00                	push   $0x0
  pushl $236
801073a8:	68 ec 00 00 00       	push   $0xec
  jmp alltraps
801073ad:	e9 46 f0 ff ff       	jmp    801063f8 <alltraps>

801073b2 <vector237>:
.globl vector237
vector237:
  pushl $0
801073b2:	6a 00                	push   $0x0
  pushl $237
801073b4:	68 ed 00 00 00       	push   $0xed
  jmp alltraps
801073b9:	e9 3a f0 ff ff       	jmp    801063f8 <alltraps>

801073be <vector238>:
.globl vector238
vector238:
  pushl $0
801073be:	6a 00                	push   $0x0
  pushl $238
801073c0:	68 ee 00 00 00       	push   $0xee
  jmp alltraps
801073c5:	e9 2e f0 ff ff       	jmp    801063f8 <alltraps>

801073ca <vector239>:
.globl vector239
vector239:
  pushl $0
801073ca:	6a 00                	push   $0x0
  pushl $239
801073cc:	68 ef 00 00 00       	push   $0xef
  jmp alltraps
801073d1:	e9 22 f0 ff ff       	jmp    801063f8 <alltraps>

801073d6 <vector240>:
.globl vector240
vector240:
  pushl $0
801073d6:	6a 00                	push   $0x0
  pushl $240
801073d8:	68 f0 00 00 00       	push   $0xf0
  jmp alltraps
801073dd:	e9 16 f0 ff ff       	jmp    801063f8 <alltraps>

801073e2 <vector241>:
.globl vector241
vector241:
  pushl $0
801073e2:	6a 00                	push   $0x0
  pushl $241
801073e4:	68 f1 00 00 00       	push   $0xf1
  jmp alltraps
801073e9:	e9 0a f0 ff ff       	jmp    801063f8 <alltraps>

801073ee <vector242>:
.globl vector242
vector242:
  pushl $0
801073ee:	6a 00                	push   $0x0
  pushl $242
801073f0:	68 f2 00 00 00       	push   $0xf2
  jmp alltraps
801073f5:	e9 fe ef ff ff       	jmp    801063f8 <alltraps>

801073fa <vector243>:
.globl vector243
vector243:
  pushl $0
801073fa:	6a 00                	push   $0x0
  pushl $243
801073fc:	68 f3 00 00 00       	push   $0xf3
  jmp alltraps
80107401:	e9 f2 ef ff ff       	jmp    801063f8 <alltraps>

80107406 <vector244>:
.globl vector244
vector244:
  pushl $0
80107406:	6a 00                	push   $0x0
  pushl $244
80107408:	68 f4 00 00 00       	push   $0xf4
  jmp alltraps
8010740d:	e9 e6 ef ff ff       	jmp    801063f8 <alltraps>

80107412 <vector245>:
.globl vector245
vector245:
  pushl $0
80107412:	6a 00                	push   $0x0
  pushl $245
80107414:	68 f5 00 00 00       	push   $0xf5
  jmp alltraps
80107419:	e9 da ef ff ff       	jmp    801063f8 <alltraps>

8010741e <vector246>:
.globl vector246
vector246:
  pushl $0
8010741e:	6a 00                	push   $0x0
  pushl $246
80107420:	68 f6 00 00 00       	push   $0xf6
  jmp alltraps
80107425:	e9 ce ef ff ff       	jmp    801063f8 <alltraps>

8010742a <vector247>:
.globl vector247
vector247:
  pushl $0
8010742a:	6a 00                	push   $0x0
  pushl $247
8010742c:	68 f7 00 00 00       	push   $0xf7
  jmp alltraps
80107431:	e9 c2 ef ff ff       	jmp    801063f8 <alltraps>

80107436 <vector248>:
.globl vector248
vector248:
  pushl $0
80107436:	6a 00                	push   $0x0
  pushl $248
80107438:	68 f8 00 00 00       	push   $0xf8
  jmp alltraps
8010743d:	e9 b6 ef ff ff       	jmp    801063f8 <alltraps>

80107442 <vector249>:
.globl vector249
vector249:
  pushl $0
80107442:	6a 00                	push   $0x0
  pushl $249
80107444:	68 f9 00 00 00       	push   $0xf9
  jmp alltraps
80107449:	e9 aa ef ff ff       	jmp    801063f8 <alltraps>

8010744e <vector250>:
.globl vector250
vector250:
  pushl $0
8010744e:	6a 00                	push   $0x0
  pushl $250
80107450:	68 fa 00 00 00       	push   $0xfa
  jmp alltraps
80107455:	e9 9e ef ff ff       	jmp    801063f8 <alltraps>

8010745a <vector251>:
.globl vector251
vector251:
  pushl $0
8010745a:	6a 00                	push   $0x0
  pushl $251
8010745c:	68 fb 00 00 00       	push   $0xfb
  jmp alltraps
80107461:	e9 92 ef ff ff       	jmp    801063f8 <alltraps>

80107466 <vector252>:
.globl vector252
vector252:
  pushl $0
80107466:	6a 00                	push   $0x0
  pushl $252
80107468:	68 fc 00 00 00       	push   $0xfc
  jmp alltraps
8010746d:	e9 86 ef ff ff       	jmp    801063f8 <alltraps>

80107472 <vector253>:
.globl vector253
vector253:
  pushl $0
80107472:	6a 00                	push   $0x0
  pushl $253
80107474:	68 fd 00 00 00       	push   $0xfd
  jmp alltraps
80107479:	e9 7a ef ff ff       	jmp    801063f8 <alltraps>

8010747e <vector254>:
.globl vector254
vector254:
  pushl $0
8010747e:	6a 00                	push   $0x0
  pushl $254
80107480:	68 fe 00 00 00       	push   $0xfe
  jmp alltraps
80107485:	e9 6e ef ff ff       	jmp    801063f8 <alltraps>

8010748a <vector255>:
.globl vector255
vector255:
  pushl $0
8010748a:	6a 00                	push   $0x0
  pushl $255
8010748c:	68 ff 00 00 00       	push   $0xff
  jmp alltraps
80107491:	e9 62 ef ff ff       	jmp    801063f8 <alltraps>
	...

80107498 <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80107498:	55                   	push   %ebp
80107499:	89 e5                	mov    %esp,%ebp
8010749b:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
8010749e:	8b 45 0c             	mov    0xc(%ebp),%eax
801074a1:	48                   	dec    %eax
801074a2:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
801074a6:	8b 45 08             	mov    0x8(%ebp),%eax
801074a9:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
801074ad:	8b 45 08             	mov    0x8(%ebp),%eax
801074b0:	c1 e8 10             	shr    $0x10,%eax
801074b3:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
801074b7:	8d 45 fa             	lea    -0x6(%ebp),%eax
801074ba:	0f 01 10             	lgdtl  (%eax)
}
801074bd:	c9                   	leave  
801074be:	c3                   	ret    

801074bf <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
801074bf:	55                   	push   %ebp
801074c0:	89 e5                	mov    %esp,%ebp
801074c2:	83 ec 04             	sub    $0x4,%esp
801074c5:	8b 45 08             	mov    0x8(%ebp),%eax
801074c8:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
801074cc:	8b 45 fc             	mov    -0x4(%ebp),%eax
801074cf:	0f 00 d8             	ltr    %ax
}
801074d2:	c9                   	leave  
801074d3:	c3                   	ret    

801074d4 <lcr3>:
  return val;
}

static inline void
lcr3(uint val)
{
801074d4:	55                   	push   %ebp
801074d5:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
801074d7:	8b 45 08             	mov    0x8(%ebp),%eax
801074da:	0f 22 d8             	mov    %eax,%cr3
}
801074dd:	5d                   	pop    %ebp
801074de:	c3                   	ret    

801074df <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
801074df:	55                   	push   %ebp
801074e0:	89 e5                	mov    %esp,%ebp
801074e2:	83 ec 28             	sub    $0x28,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpuid()];
801074e5:	e8 70 cc ff ff       	call   8010415a <cpuid>
801074ea:	89 c2                	mov    %eax,%edx
801074ec:	89 d0                	mov    %edx,%eax
801074ee:	c1 e0 02             	shl    $0x2,%eax
801074f1:	01 d0                	add    %edx,%eax
801074f3:	01 c0                	add    %eax,%eax
801074f5:	01 d0                	add    %edx,%eax
801074f7:	c1 e0 04             	shl    $0x4,%eax
801074fa:	05 c0 3a 11 80       	add    $0x80113ac0,%eax
801074ff:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
80107502:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107505:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
8010750b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010750e:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
80107514:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107517:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
8010751b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010751e:	8a 50 7d             	mov    0x7d(%eax),%dl
80107521:	83 e2 f0             	and    $0xfffffff0,%edx
80107524:	83 ca 0a             	or     $0xa,%edx
80107527:	88 50 7d             	mov    %dl,0x7d(%eax)
8010752a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010752d:	8a 50 7d             	mov    0x7d(%eax),%dl
80107530:	83 ca 10             	or     $0x10,%edx
80107533:	88 50 7d             	mov    %dl,0x7d(%eax)
80107536:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107539:	8a 50 7d             	mov    0x7d(%eax),%dl
8010753c:	83 e2 9f             	and    $0xffffff9f,%edx
8010753f:	88 50 7d             	mov    %dl,0x7d(%eax)
80107542:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107545:	8a 50 7d             	mov    0x7d(%eax),%dl
80107548:	83 ca 80             	or     $0xffffff80,%edx
8010754b:	88 50 7d             	mov    %dl,0x7d(%eax)
8010754e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107551:	8a 50 7e             	mov    0x7e(%eax),%dl
80107554:	83 ca 0f             	or     $0xf,%edx
80107557:	88 50 7e             	mov    %dl,0x7e(%eax)
8010755a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010755d:	8a 50 7e             	mov    0x7e(%eax),%dl
80107560:	83 e2 ef             	and    $0xffffffef,%edx
80107563:	88 50 7e             	mov    %dl,0x7e(%eax)
80107566:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107569:	8a 50 7e             	mov    0x7e(%eax),%dl
8010756c:	83 e2 df             	and    $0xffffffdf,%edx
8010756f:	88 50 7e             	mov    %dl,0x7e(%eax)
80107572:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107575:	8a 50 7e             	mov    0x7e(%eax),%dl
80107578:	83 ca 40             	or     $0x40,%edx
8010757b:	88 50 7e             	mov    %dl,0x7e(%eax)
8010757e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107581:	8a 50 7e             	mov    0x7e(%eax),%dl
80107584:	83 ca 80             	or     $0xffffff80,%edx
80107587:	88 50 7e             	mov    %dl,0x7e(%eax)
8010758a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010758d:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80107591:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107594:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
8010759b:	ff ff 
8010759d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075a0:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
801075a7:	00 00 
801075a9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075ac:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
801075b3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075b6:	8a 90 85 00 00 00    	mov    0x85(%eax),%dl
801075bc:	83 e2 f0             	and    $0xfffffff0,%edx
801075bf:	83 ca 02             	or     $0x2,%edx
801075c2:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801075c8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075cb:	8a 90 85 00 00 00    	mov    0x85(%eax),%dl
801075d1:	83 ca 10             	or     $0x10,%edx
801075d4:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801075da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075dd:	8a 90 85 00 00 00    	mov    0x85(%eax),%dl
801075e3:	83 e2 9f             	and    $0xffffff9f,%edx
801075e6:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801075ec:	8b 45 f4             	mov    -0xc(%ebp),%eax
801075ef:	8a 90 85 00 00 00    	mov    0x85(%eax),%dl
801075f5:	83 ca 80             	or     $0xffffff80,%edx
801075f8:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801075fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107601:	8a 90 86 00 00 00    	mov    0x86(%eax),%dl
80107607:	83 ca 0f             	or     $0xf,%edx
8010760a:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107610:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107613:	8a 90 86 00 00 00    	mov    0x86(%eax),%dl
80107619:	83 e2 ef             	and    $0xffffffef,%edx
8010761c:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107622:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107625:	8a 90 86 00 00 00    	mov    0x86(%eax),%dl
8010762b:	83 e2 df             	and    $0xffffffdf,%edx
8010762e:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107634:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107637:	8a 90 86 00 00 00    	mov    0x86(%eax),%dl
8010763d:	83 ca 40             	or     $0x40,%edx
80107640:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107646:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107649:	8a 90 86 00 00 00    	mov    0x86(%eax),%dl
8010764f:	83 ca 80             	or     $0xffffff80,%edx
80107652:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107658:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010765b:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80107662:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107665:	66 c7 80 88 00 00 00 	movw   $0xffff,0x88(%eax)
8010766c:	ff ff 
8010766e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107671:	66 c7 80 8a 00 00 00 	movw   $0x0,0x8a(%eax)
80107678:	00 00 
8010767a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010767d:	c6 80 8c 00 00 00 00 	movb   $0x0,0x8c(%eax)
80107684:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107687:	8a 90 8d 00 00 00    	mov    0x8d(%eax),%dl
8010768d:	83 e2 f0             	and    $0xfffffff0,%edx
80107690:	83 ca 0a             	or     $0xa,%edx
80107693:	88 90 8d 00 00 00    	mov    %dl,0x8d(%eax)
80107699:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010769c:	8a 90 8d 00 00 00    	mov    0x8d(%eax),%dl
801076a2:	83 ca 10             	or     $0x10,%edx
801076a5:	88 90 8d 00 00 00    	mov    %dl,0x8d(%eax)
801076ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076ae:	8a 90 8d 00 00 00    	mov    0x8d(%eax),%dl
801076b4:	83 ca 60             	or     $0x60,%edx
801076b7:	88 90 8d 00 00 00    	mov    %dl,0x8d(%eax)
801076bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076c0:	8a 90 8d 00 00 00    	mov    0x8d(%eax),%dl
801076c6:	83 ca 80             	or     $0xffffff80,%edx
801076c9:	88 90 8d 00 00 00    	mov    %dl,0x8d(%eax)
801076cf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076d2:	8a 90 8e 00 00 00    	mov    0x8e(%eax),%dl
801076d8:	83 ca 0f             	or     $0xf,%edx
801076db:	88 90 8e 00 00 00    	mov    %dl,0x8e(%eax)
801076e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076e4:	8a 90 8e 00 00 00    	mov    0x8e(%eax),%dl
801076ea:	83 e2 ef             	and    $0xffffffef,%edx
801076ed:	88 90 8e 00 00 00    	mov    %dl,0x8e(%eax)
801076f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801076f6:	8a 90 8e 00 00 00    	mov    0x8e(%eax),%dl
801076fc:	83 e2 df             	and    $0xffffffdf,%edx
801076ff:	88 90 8e 00 00 00    	mov    %dl,0x8e(%eax)
80107705:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107708:	8a 90 8e 00 00 00    	mov    0x8e(%eax),%dl
8010770e:	83 ca 40             	or     $0x40,%edx
80107711:	88 90 8e 00 00 00    	mov    %dl,0x8e(%eax)
80107717:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010771a:	8a 90 8e 00 00 00    	mov    0x8e(%eax),%dl
80107720:	83 ca 80             	or     $0xffffff80,%edx
80107723:	88 90 8e 00 00 00    	mov    %dl,0x8e(%eax)
80107729:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010772c:	c6 80 8f 00 00 00 00 	movb   $0x0,0x8f(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107733:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107736:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
8010773d:	ff ff 
8010773f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107742:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80107749:	00 00 
8010774b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010774e:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80107755:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107758:	8a 90 95 00 00 00    	mov    0x95(%eax),%dl
8010775e:	83 e2 f0             	and    $0xfffffff0,%edx
80107761:	83 ca 02             	or     $0x2,%edx
80107764:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
8010776a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010776d:	8a 90 95 00 00 00    	mov    0x95(%eax),%dl
80107773:	83 ca 10             	or     $0x10,%edx
80107776:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
8010777c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010777f:	8a 90 95 00 00 00    	mov    0x95(%eax),%dl
80107785:	83 ca 60             	or     $0x60,%edx
80107788:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
8010778e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107791:	8a 90 95 00 00 00    	mov    0x95(%eax),%dl
80107797:	83 ca 80             	or     $0xffffff80,%edx
8010779a:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
801077a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077a3:	8a 90 96 00 00 00    	mov    0x96(%eax),%dl
801077a9:	83 ca 0f             	or     $0xf,%edx
801077ac:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801077b2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077b5:	8a 90 96 00 00 00    	mov    0x96(%eax),%dl
801077bb:	83 e2 ef             	and    $0xffffffef,%edx
801077be:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801077c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077c7:	8a 90 96 00 00 00    	mov    0x96(%eax),%dl
801077cd:	83 e2 df             	and    $0xffffffdf,%edx
801077d0:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801077d6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077d9:	8a 90 96 00 00 00    	mov    0x96(%eax),%dl
801077df:	83 ca 40             	or     $0x40,%edx
801077e2:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801077e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077eb:	8a 90 96 00 00 00    	mov    0x96(%eax),%dl
801077f1:	83 ca 80             	or     $0xffffff80,%edx
801077f4:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
801077fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801077fd:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  lgdt(c->gdt, sizeof(c->gdt));
80107804:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107807:	83 c0 70             	add    $0x70,%eax
8010780a:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
80107811:	00 
80107812:	89 04 24             	mov    %eax,(%esp)
80107815:	e8 7e fc ff ff       	call   80107498 <lgdt>
}
8010781a:	c9                   	leave  
8010781b:	c3                   	ret    

8010781c <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
8010781c:	55                   	push   %ebp
8010781d:	89 e5                	mov    %esp,%ebp
8010781f:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107822:	8b 45 0c             	mov    0xc(%ebp),%eax
80107825:	c1 e8 16             	shr    $0x16,%eax
80107828:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
8010782f:	8b 45 08             	mov    0x8(%ebp),%eax
80107832:	01 d0                	add    %edx,%eax
80107834:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107837:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010783a:	8b 00                	mov    (%eax),%eax
8010783c:	83 e0 01             	and    $0x1,%eax
8010783f:	85 c0                	test   %eax,%eax
80107841:	74 14                	je     80107857 <walkpgdir+0x3b>
    pgtab = (pte_t*)P2V(PTE_ADDR(*pde));
80107843:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107846:	8b 00                	mov    (%eax),%eax
80107848:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010784d:	05 00 00 00 80       	add    $0x80000000,%eax
80107852:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107855:	eb 48                	jmp    8010789f <walkpgdir+0x83>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107857:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010785b:	74 0e                	je     8010786b <walkpgdir+0x4f>
8010785d:	e8 01 b4 ff ff       	call   80102c63 <kalloc>
80107862:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107865:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107869:	75 07                	jne    80107872 <walkpgdir+0x56>
      return 0;
8010786b:	b8 00 00 00 00       	mov    $0x0,%eax
80107870:	eb 44                	jmp    801078b6 <walkpgdir+0x9a>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107872:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107879:	00 
8010787a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107881:	00 
80107882:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107885:	89 04 24             	mov    %eax,(%esp)
80107888:	e8 09 d8 ff ff       	call   80105096 <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table
    // entries, if necessary.
    *pde = V2P(pgtab) | PTE_P | PTE_W | PTE_U;
8010788d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107890:	05 00 00 00 80       	add    $0x80000000,%eax
80107895:	83 c8 07             	or     $0x7,%eax
80107898:	89 c2                	mov    %eax,%edx
8010789a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010789d:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
8010789f:	8b 45 0c             	mov    0xc(%ebp),%eax
801078a2:	c1 e8 0c             	shr    $0xc,%eax
801078a5:	25 ff 03 00 00       	and    $0x3ff,%eax
801078aa:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
801078b1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078b4:	01 d0                	add    %edx,%eax
}
801078b6:	c9                   	leave  
801078b7:	c3                   	ret    

801078b8 <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
801078b8:	55                   	push   %ebp
801078b9:	89 e5                	mov    %esp,%ebp
801078bb:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;

  a = (char*)PGROUNDDOWN((uint)va);
801078be:	8b 45 0c             	mov    0xc(%ebp),%eax
801078c1:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801078c6:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
801078c9:	8b 55 0c             	mov    0xc(%ebp),%edx
801078cc:	8b 45 10             	mov    0x10(%ebp),%eax
801078cf:	01 d0                	add    %edx,%eax
801078d1:	48                   	dec    %eax
801078d2:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801078d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
801078da:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
801078e1:	00 
801078e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078e5:	89 44 24 04          	mov    %eax,0x4(%esp)
801078e9:	8b 45 08             	mov    0x8(%ebp),%eax
801078ec:	89 04 24             	mov    %eax,(%esp)
801078ef:	e8 28 ff ff ff       	call   8010781c <walkpgdir>
801078f4:	89 45 ec             	mov    %eax,-0x14(%ebp)
801078f7:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801078fb:	75 07                	jne    80107904 <mappages+0x4c>
      return -1;
801078fd:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107902:	eb 48                	jmp    8010794c <mappages+0x94>
    if(*pte & PTE_P)
80107904:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107907:	8b 00                	mov    (%eax),%eax
80107909:	83 e0 01             	and    $0x1,%eax
8010790c:	85 c0                	test   %eax,%eax
8010790e:	74 0c                	je     8010791c <mappages+0x64>
      panic("remap");
80107910:	c7 04 24 2c 88 10 80 	movl   $0x8010882c,(%esp)
80107917:	e8 38 8c ff ff       	call   80100554 <panic>
    *pte = pa | perm | PTE_P;
8010791c:	8b 45 18             	mov    0x18(%ebp),%eax
8010791f:	0b 45 14             	or     0x14(%ebp),%eax
80107922:	83 c8 01             	or     $0x1,%eax
80107925:	89 c2                	mov    %eax,%edx
80107927:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010792a:	89 10                	mov    %edx,(%eax)
    if(a == last)
8010792c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010792f:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107932:	75 08                	jne    8010793c <mappages+0x84>
      break;
80107934:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80107935:	b8 00 00 00 00       	mov    $0x0,%eax
8010793a:	eb 10                	jmp    8010794c <mappages+0x94>
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
    a += PGSIZE;
8010793c:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80107943:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
8010794a:	eb 8e                	jmp    801078da <mappages+0x22>
  return 0;
}
8010794c:	c9                   	leave  
8010794d:	c3                   	ret    

8010794e <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm(void)
{
8010794e:	55                   	push   %ebp
8010794f:	89 e5                	mov    %esp,%ebp
80107951:	53                   	push   %ebx
80107952:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
80107955:	e8 09 b3 ff ff       	call   80102c63 <kalloc>
8010795a:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010795d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107961:	75 0a                	jne    8010796d <setupkvm+0x1f>
    return 0;
80107963:	b8 00 00 00 00       	mov    $0x0,%eax
80107968:	e9 84 00 00 00       	jmp    801079f1 <setupkvm+0xa3>
  memset(pgdir, 0, PGSIZE);
8010796d:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107974:	00 
80107975:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010797c:	00 
8010797d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107980:	89 04 24             	mov    %eax,(%esp)
80107983:	e8 0e d7 ff ff       	call   80105096 <memset>
  if (P2V(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107988:	c7 45 f4 80 b4 10 80 	movl   $0x8010b480,-0xc(%ebp)
8010798f:	eb 54                	jmp    801079e5 <setupkvm+0x97>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start,
80107991:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107994:	8b 48 0c             	mov    0xc(%eax),%ecx
80107997:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010799a:	8b 50 04             	mov    0x4(%eax),%edx
8010799d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079a0:	8b 58 08             	mov    0x8(%eax),%ebx
801079a3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079a6:	8b 40 04             	mov    0x4(%eax),%eax
801079a9:	29 c3                	sub    %eax,%ebx
801079ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079ae:	8b 00                	mov    (%eax),%eax
801079b0:	89 4c 24 10          	mov    %ecx,0x10(%esp)
801079b4:	89 54 24 0c          	mov    %edx,0xc(%esp)
801079b8:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801079bc:	89 44 24 04          	mov    %eax,0x4(%esp)
801079c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801079c3:	89 04 24             	mov    %eax,(%esp)
801079c6:	e8 ed fe ff ff       	call   801078b8 <mappages>
801079cb:	85 c0                	test   %eax,%eax
801079cd:	79 12                	jns    801079e1 <setupkvm+0x93>
                (uint)k->phys_start, k->perm) < 0) {
      freevm(pgdir);
801079cf:	8b 45 f0             	mov    -0x10(%ebp),%eax
801079d2:	89 04 24             	mov    %eax,(%esp)
801079d5:	e8 1a 05 00 00       	call   80107ef4 <freevm>
      return 0;
801079da:	b8 00 00 00 00       	mov    $0x0,%eax
801079df:	eb 10                	jmp    801079f1 <setupkvm+0xa3>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (P2V(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
801079e1:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
801079e5:	81 7d f4 c0 b4 10 80 	cmpl   $0x8010b4c0,-0xc(%ebp)
801079ec:	72 a3                	jb     80107991 <setupkvm+0x43>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start,
                (uint)k->phys_start, k->perm) < 0) {
      freevm(pgdir);
      return 0;
    }
  return pgdir;
801079ee:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
801079f1:	83 c4 34             	add    $0x34,%esp
801079f4:	5b                   	pop    %ebx
801079f5:	5d                   	pop    %ebp
801079f6:	c3                   	ret    

801079f7 <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
801079f7:	55                   	push   %ebp
801079f8:	89 e5                	mov    %esp,%ebp
801079fa:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
801079fd:	e8 4c ff ff ff       	call   8010794e <setupkvm>
80107a02:	a3 e4 67 11 80       	mov    %eax,0x801167e4
  switchkvm();
80107a07:	e8 02 00 00 00       	call   80107a0e <switchkvm>
}
80107a0c:	c9                   	leave  
80107a0d:	c3                   	ret    

80107a0e <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
80107a0e:	55                   	push   %ebp
80107a0f:	89 e5                	mov    %esp,%ebp
80107a11:	83 ec 04             	sub    $0x4,%esp
  lcr3(V2P(kpgdir));   // switch to the kernel page table
80107a14:	a1 e4 67 11 80       	mov    0x801167e4,%eax
80107a19:	05 00 00 00 80       	add    $0x80000000,%eax
80107a1e:	89 04 24             	mov    %eax,(%esp)
80107a21:	e8 ae fa ff ff       	call   801074d4 <lcr3>
}
80107a26:	c9                   	leave  
80107a27:	c3                   	ret    

80107a28 <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
80107a28:	55                   	push   %ebp
80107a29:	89 e5                	mov    %esp,%ebp
80107a2b:	57                   	push   %edi
80107a2c:	56                   	push   %esi
80107a2d:	53                   	push   %ebx
80107a2e:	83 ec 1c             	sub    $0x1c,%esp
  if(p == 0)
80107a31:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80107a35:	75 0c                	jne    80107a43 <switchuvm+0x1b>
    panic("switchuvm: no process");
80107a37:	c7 04 24 32 88 10 80 	movl   $0x80108832,(%esp)
80107a3e:	e8 11 8b ff ff       	call   80100554 <panic>
  if(p->kstack == 0)
80107a43:	8b 45 08             	mov    0x8(%ebp),%eax
80107a46:	8b 40 08             	mov    0x8(%eax),%eax
80107a49:	85 c0                	test   %eax,%eax
80107a4b:	75 0c                	jne    80107a59 <switchuvm+0x31>
    panic("switchuvm: no kstack");
80107a4d:	c7 04 24 48 88 10 80 	movl   $0x80108848,(%esp)
80107a54:	e8 fb 8a ff ff       	call   80100554 <panic>
  if(p->pgdir == 0)
80107a59:	8b 45 08             	mov    0x8(%ebp),%eax
80107a5c:	8b 40 04             	mov    0x4(%eax),%eax
80107a5f:	85 c0                	test   %eax,%eax
80107a61:	75 0c                	jne    80107a6f <switchuvm+0x47>
    panic("switchuvm: no pgdir");
80107a63:	c7 04 24 5d 88 10 80 	movl   $0x8010885d,(%esp)
80107a6a:	e8 e5 8a ff ff       	call   80100554 <panic>

  pushcli();
80107a6f:	e8 1e d5 ff ff       	call   80104f92 <pushcli>
  mycpu()->gdt[SEG_TSS] = SEG16(STS_T32A, &mycpu()->ts,
80107a74:	e8 26 c7 ff ff       	call   8010419f <mycpu>
80107a79:	89 c3                	mov    %eax,%ebx
80107a7b:	e8 1f c7 ff ff       	call   8010419f <mycpu>
80107a80:	83 c0 08             	add    $0x8,%eax
80107a83:	89 c6                	mov    %eax,%esi
80107a85:	e8 15 c7 ff ff       	call   8010419f <mycpu>
80107a8a:	83 c0 08             	add    $0x8,%eax
80107a8d:	c1 e8 10             	shr    $0x10,%eax
80107a90:	89 c7                	mov    %eax,%edi
80107a92:	e8 08 c7 ff ff       	call   8010419f <mycpu>
80107a97:	83 c0 08             	add    $0x8,%eax
80107a9a:	c1 e8 18             	shr    $0x18,%eax
80107a9d:	66 c7 83 98 00 00 00 	movw   $0x67,0x98(%ebx)
80107aa4:	67 00 
80107aa6:	66 89 b3 9a 00 00 00 	mov    %si,0x9a(%ebx)
80107aad:	89 f9                	mov    %edi,%ecx
80107aaf:	88 8b 9c 00 00 00    	mov    %cl,0x9c(%ebx)
80107ab5:	8a 93 9d 00 00 00    	mov    0x9d(%ebx),%dl
80107abb:	83 e2 f0             	and    $0xfffffff0,%edx
80107abe:	83 ca 09             	or     $0x9,%edx
80107ac1:	88 93 9d 00 00 00    	mov    %dl,0x9d(%ebx)
80107ac7:	8a 93 9d 00 00 00    	mov    0x9d(%ebx),%dl
80107acd:	83 ca 10             	or     $0x10,%edx
80107ad0:	88 93 9d 00 00 00    	mov    %dl,0x9d(%ebx)
80107ad6:	8a 93 9d 00 00 00    	mov    0x9d(%ebx),%dl
80107adc:	83 e2 9f             	and    $0xffffff9f,%edx
80107adf:	88 93 9d 00 00 00    	mov    %dl,0x9d(%ebx)
80107ae5:	8a 93 9d 00 00 00    	mov    0x9d(%ebx),%dl
80107aeb:	83 ca 80             	or     $0xffffff80,%edx
80107aee:	88 93 9d 00 00 00    	mov    %dl,0x9d(%ebx)
80107af4:	8a 93 9e 00 00 00    	mov    0x9e(%ebx),%dl
80107afa:	83 e2 f0             	and    $0xfffffff0,%edx
80107afd:	88 93 9e 00 00 00    	mov    %dl,0x9e(%ebx)
80107b03:	8a 93 9e 00 00 00    	mov    0x9e(%ebx),%dl
80107b09:	83 e2 ef             	and    $0xffffffef,%edx
80107b0c:	88 93 9e 00 00 00    	mov    %dl,0x9e(%ebx)
80107b12:	8a 93 9e 00 00 00    	mov    0x9e(%ebx),%dl
80107b18:	83 e2 df             	and    $0xffffffdf,%edx
80107b1b:	88 93 9e 00 00 00    	mov    %dl,0x9e(%ebx)
80107b21:	8a 93 9e 00 00 00    	mov    0x9e(%ebx),%dl
80107b27:	83 ca 40             	or     $0x40,%edx
80107b2a:	88 93 9e 00 00 00    	mov    %dl,0x9e(%ebx)
80107b30:	8a 93 9e 00 00 00    	mov    0x9e(%ebx),%dl
80107b36:	83 e2 7f             	and    $0x7f,%edx
80107b39:	88 93 9e 00 00 00    	mov    %dl,0x9e(%ebx)
80107b3f:	88 83 9f 00 00 00    	mov    %al,0x9f(%ebx)
                                sizeof(mycpu()->ts)-1, 0);
  mycpu()->gdt[SEG_TSS].s = 0;
80107b45:	e8 55 c6 ff ff       	call   8010419f <mycpu>
80107b4a:	8a 90 9d 00 00 00    	mov    0x9d(%eax),%dl
80107b50:	83 e2 ef             	and    $0xffffffef,%edx
80107b53:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
  mycpu()->ts.ss0 = SEG_KDATA << 3;
80107b59:	e8 41 c6 ff ff       	call   8010419f <mycpu>
80107b5e:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  mycpu()->ts.esp0 = (uint)p->kstack + KSTACKSIZE;
80107b64:	e8 36 c6 ff ff       	call   8010419f <mycpu>
80107b69:	8b 55 08             	mov    0x8(%ebp),%edx
80107b6c:	8b 52 08             	mov    0x8(%edx),%edx
80107b6f:	81 c2 00 10 00 00    	add    $0x1000,%edx
80107b75:	89 50 0c             	mov    %edx,0xc(%eax)
  // setting IOPL=0 in eflags *and* iomb beyond the tss segment limit
  // forbids I/O instructions (e.g., inb and outb) from user space
  mycpu()->ts.iomb = (ushort) 0xFFFF;
80107b78:	e8 22 c6 ff ff       	call   8010419f <mycpu>
80107b7d:	66 c7 40 6e ff ff    	movw   $0xffff,0x6e(%eax)
  ltr(SEG_TSS << 3);
80107b83:	c7 04 24 28 00 00 00 	movl   $0x28,(%esp)
80107b8a:	e8 30 f9 ff ff       	call   801074bf <ltr>
  lcr3(V2P(p->pgdir));  // switch to process's address space
80107b8f:	8b 45 08             	mov    0x8(%ebp),%eax
80107b92:	8b 40 04             	mov    0x4(%eax),%eax
80107b95:	05 00 00 00 80       	add    $0x80000000,%eax
80107b9a:	89 04 24             	mov    %eax,(%esp)
80107b9d:	e8 32 f9 ff ff       	call   801074d4 <lcr3>
  popcli();
80107ba2:	e8 35 d4 ff ff       	call   80104fdc <popcli>
}
80107ba7:	83 c4 1c             	add    $0x1c,%esp
80107baa:	5b                   	pop    %ebx
80107bab:	5e                   	pop    %esi
80107bac:	5f                   	pop    %edi
80107bad:	5d                   	pop    %ebp
80107bae:	c3                   	ret    

80107baf <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
80107baf:	55                   	push   %ebp
80107bb0:	89 e5                	mov    %esp,%ebp
80107bb2:	83 ec 38             	sub    $0x38,%esp
  char *mem;

  if(sz >= PGSIZE)
80107bb5:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
80107bbc:	76 0c                	jbe    80107bca <inituvm+0x1b>
    panic("inituvm: more than a page");
80107bbe:	c7 04 24 71 88 10 80 	movl   $0x80108871,(%esp)
80107bc5:	e8 8a 89 ff ff       	call   80100554 <panic>
  mem = kalloc();
80107bca:	e8 94 b0 ff ff       	call   80102c63 <kalloc>
80107bcf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
80107bd2:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107bd9:	00 
80107bda:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107be1:	00 
80107be2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be5:	89 04 24             	mov    %eax,(%esp)
80107be8:	e8 a9 d4 ff ff       	call   80105096 <memset>
  mappages(pgdir, 0, PGSIZE, V2P(mem), PTE_W|PTE_U);
80107bed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bf0:	05 00 00 00 80       	add    $0x80000000,%eax
80107bf5:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80107bfc:	00 
80107bfd:	89 44 24 0c          	mov    %eax,0xc(%esp)
80107c01:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107c08:	00 
80107c09:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107c10:	00 
80107c11:	8b 45 08             	mov    0x8(%ebp),%eax
80107c14:	89 04 24             	mov    %eax,(%esp)
80107c17:	e8 9c fc ff ff       	call   801078b8 <mappages>
  memmove(mem, init, sz);
80107c1c:	8b 45 10             	mov    0x10(%ebp),%eax
80107c1f:	89 44 24 08          	mov    %eax,0x8(%esp)
80107c23:	8b 45 0c             	mov    0xc(%ebp),%eax
80107c26:	89 44 24 04          	mov    %eax,0x4(%esp)
80107c2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c2d:	89 04 24             	mov    %eax,(%esp)
80107c30:	e8 2a d5 ff ff       	call   8010515f <memmove>
}
80107c35:	c9                   	leave  
80107c36:	c3                   	ret    

80107c37 <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
80107c37:	55                   	push   %ebp
80107c38:	89 e5                	mov    %esp,%ebp
80107c3a:	83 ec 28             	sub    $0x28,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
80107c3d:	8b 45 0c             	mov    0xc(%ebp),%eax
80107c40:	25 ff 0f 00 00       	and    $0xfff,%eax
80107c45:	85 c0                	test   %eax,%eax
80107c47:	74 0c                	je     80107c55 <loaduvm+0x1e>
    panic("loaduvm: addr must be page aligned");
80107c49:	c7 04 24 8c 88 10 80 	movl   $0x8010888c,(%esp)
80107c50:	e8 ff 88 ff ff       	call   80100554 <panic>
  for(i = 0; i < sz; i += PGSIZE){
80107c55:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107c5c:	e9 a6 00 00 00       	jmp    80107d07 <loaduvm+0xd0>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
80107c61:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c64:	8b 55 0c             	mov    0xc(%ebp),%edx
80107c67:	01 d0                	add    %edx,%eax
80107c69:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107c70:	00 
80107c71:	89 44 24 04          	mov    %eax,0x4(%esp)
80107c75:	8b 45 08             	mov    0x8(%ebp),%eax
80107c78:	89 04 24             	mov    %eax,(%esp)
80107c7b:	e8 9c fb ff ff       	call   8010781c <walkpgdir>
80107c80:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107c83:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107c87:	75 0c                	jne    80107c95 <loaduvm+0x5e>
      panic("loaduvm: address should exist");
80107c89:	c7 04 24 af 88 10 80 	movl   $0x801088af,(%esp)
80107c90:	e8 bf 88 ff ff       	call   80100554 <panic>
    pa = PTE_ADDR(*pte);
80107c95:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107c98:	8b 00                	mov    (%eax),%eax
80107c9a:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107c9f:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
80107ca2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ca5:	8b 55 18             	mov    0x18(%ebp),%edx
80107ca8:	29 c2                	sub    %eax,%edx
80107caa:	89 d0                	mov    %edx,%eax
80107cac:	3d ff 0f 00 00       	cmp    $0xfff,%eax
80107cb1:	77 0f                	ja     80107cc2 <loaduvm+0x8b>
      n = sz - i;
80107cb3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cb6:	8b 55 18             	mov    0x18(%ebp),%edx
80107cb9:	29 c2                	sub    %eax,%edx
80107cbb:	89 d0                	mov    %edx,%eax
80107cbd:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107cc0:	eb 07                	jmp    80107cc9 <loaduvm+0x92>
    else
      n = PGSIZE;
80107cc2:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, P2V(pa), offset+i, n) != n)
80107cc9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ccc:	8b 55 14             	mov    0x14(%ebp),%edx
80107ccf:	8d 0c 02             	lea    (%edx,%eax,1),%ecx
80107cd2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107cd5:	05 00 00 00 80       	add    $0x80000000,%eax
80107cda:	8b 55 f0             	mov    -0x10(%ebp),%edx
80107cdd:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107ce1:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80107ce5:	89 44 24 04          	mov    %eax,0x4(%esp)
80107ce9:	8b 45 10             	mov    0x10(%ebp),%eax
80107cec:	89 04 24             	mov    %eax,(%esp)
80107cef:	e8 d5 a1 ff ff       	call   80101ec9 <readi>
80107cf4:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107cf7:	74 07                	je     80107d00 <loaduvm+0xc9>
      return -1;
80107cf9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107cfe:	eb 18                	jmp    80107d18 <loaduvm+0xe1>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
80107d00:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107d07:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d0a:	3b 45 18             	cmp    0x18(%ebp),%eax
80107d0d:	0f 82 4e ff ff ff    	jb     80107c61 <loaduvm+0x2a>
    else
      n = PGSIZE;
    if(readi(ip, P2V(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
80107d13:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107d18:	c9                   	leave  
80107d19:	c3                   	ret    

80107d1a <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80107d1a:	55                   	push   %ebp
80107d1b:	89 e5                	mov    %esp,%ebp
80107d1d:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
80107d20:	8b 45 10             	mov    0x10(%ebp),%eax
80107d23:	85 c0                	test   %eax,%eax
80107d25:	79 0a                	jns    80107d31 <allocuvm+0x17>
    return 0;
80107d27:	b8 00 00 00 00       	mov    $0x0,%eax
80107d2c:	e9 fd 00 00 00       	jmp    80107e2e <allocuvm+0x114>
  if(newsz < oldsz)
80107d31:	8b 45 10             	mov    0x10(%ebp),%eax
80107d34:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107d37:	73 08                	jae    80107d41 <allocuvm+0x27>
    return oldsz;
80107d39:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d3c:	e9 ed 00 00 00       	jmp    80107e2e <allocuvm+0x114>

  a = PGROUNDUP(oldsz);
80107d41:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d44:	05 ff 0f 00 00       	add    $0xfff,%eax
80107d49:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107d4e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
80107d51:	e9 c9 00 00 00       	jmp    80107e1f <allocuvm+0x105>
    mem = kalloc();
80107d56:	e8 08 af ff ff       	call   80102c63 <kalloc>
80107d5b:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
80107d5e:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107d62:	75 2f                	jne    80107d93 <allocuvm+0x79>
      cprintf("allocuvm out of memory\n");
80107d64:	c7 04 24 cd 88 10 80 	movl   $0x801088cd,(%esp)
80107d6b:	e8 51 86 ff ff       	call   801003c1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
80107d70:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d73:	89 44 24 08          	mov    %eax,0x8(%esp)
80107d77:	8b 45 10             	mov    0x10(%ebp),%eax
80107d7a:	89 44 24 04          	mov    %eax,0x4(%esp)
80107d7e:	8b 45 08             	mov    0x8(%ebp),%eax
80107d81:	89 04 24             	mov    %eax,(%esp)
80107d84:	e8 a7 00 00 00       	call   80107e30 <deallocuvm>
      return 0;
80107d89:	b8 00 00 00 00       	mov    $0x0,%eax
80107d8e:	e9 9b 00 00 00       	jmp    80107e2e <allocuvm+0x114>
    }
    memset(mem, 0, PGSIZE);
80107d93:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107d9a:	00 
80107d9b:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107da2:	00 
80107da3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107da6:	89 04 24             	mov    %eax,(%esp)
80107da9:	e8 e8 d2 ff ff       	call   80105096 <memset>
    if(mappages(pgdir, (char*)a, PGSIZE, V2P(mem), PTE_W|PTE_U) < 0){
80107dae:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107db1:	8d 90 00 00 00 80    	lea    -0x80000000(%eax),%edx
80107db7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dba:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
80107dc1:	00 
80107dc2:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107dc6:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107dcd:	00 
80107dce:	89 44 24 04          	mov    %eax,0x4(%esp)
80107dd2:	8b 45 08             	mov    0x8(%ebp),%eax
80107dd5:	89 04 24             	mov    %eax,(%esp)
80107dd8:	e8 db fa ff ff       	call   801078b8 <mappages>
80107ddd:	85 c0                	test   %eax,%eax
80107ddf:	79 37                	jns    80107e18 <allocuvm+0xfe>
      cprintf("allocuvm out of memory (2)\n");
80107de1:	c7 04 24 e5 88 10 80 	movl   $0x801088e5,(%esp)
80107de8:	e8 d4 85 ff ff       	call   801003c1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
80107ded:	8b 45 0c             	mov    0xc(%ebp),%eax
80107df0:	89 44 24 08          	mov    %eax,0x8(%esp)
80107df4:	8b 45 10             	mov    0x10(%ebp),%eax
80107df7:	89 44 24 04          	mov    %eax,0x4(%esp)
80107dfb:	8b 45 08             	mov    0x8(%ebp),%eax
80107dfe:	89 04 24             	mov    %eax,(%esp)
80107e01:	e8 2a 00 00 00       	call   80107e30 <deallocuvm>
      kfree(mem);
80107e06:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e09:	89 04 24             	mov    %eax,(%esp)
80107e0c:	e8 bc ad ff ff       	call   80102bcd <kfree>
      return 0;
80107e11:	b8 00 00 00 00       	mov    $0x0,%eax
80107e16:	eb 16                	jmp    80107e2e <allocuvm+0x114>
    return 0;
  if(newsz < oldsz)
    return oldsz;

  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
80107e18:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107e1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e22:	3b 45 10             	cmp    0x10(%ebp),%eax
80107e25:	0f 82 2b ff ff ff    	jb     80107d56 <allocuvm+0x3c>
      deallocuvm(pgdir, newsz, oldsz);
      kfree(mem);
      return 0;
    }
  }
  return newsz;
80107e2b:	8b 45 10             	mov    0x10(%ebp),%eax
}
80107e2e:	c9                   	leave  
80107e2f:	c3                   	ret    

80107e30 <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80107e30:	55                   	push   %ebp
80107e31:	89 e5                	mov    %esp,%ebp
80107e33:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
80107e36:	8b 45 10             	mov    0x10(%ebp),%eax
80107e39:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107e3c:	72 08                	jb     80107e46 <deallocuvm+0x16>
    return oldsz;
80107e3e:	8b 45 0c             	mov    0xc(%ebp),%eax
80107e41:	e9 ac 00 00 00       	jmp    80107ef2 <deallocuvm+0xc2>

  a = PGROUNDUP(newsz);
80107e46:	8b 45 10             	mov    0x10(%ebp),%eax
80107e49:	05 ff 0f 00 00       	add    $0xfff,%eax
80107e4e:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107e53:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80107e56:	e9 88 00 00 00       	jmp    80107ee3 <deallocuvm+0xb3>
    pte = walkpgdir(pgdir, (char*)a, 0);
80107e5b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e5e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107e65:	00 
80107e66:	89 44 24 04          	mov    %eax,0x4(%esp)
80107e6a:	8b 45 08             	mov    0x8(%ebp),%eax
80107e6d:	89 04 24             	mov    %eax,(%esp)
80107e70:	e8 a7 f9 ff ff       	call   8010781c <walkpgdir>
80107e75:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
80107e78:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107e7c:	75 14                	jne    80107e92 <deallocuvm+0x62>
      a = PGADDR(PDX(a) + 1, 0, 0) - PGSIZE;
80107e7e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e81:	c1 e8 16             	shr    $0x16,%eax
80107e84:	40                   	inc    %eax
80107e85:	c1 e0 16             	shl    $0x16,%eax
80107e88:	2d 00 10 00 00       	sub    $0x1000,%eax
80107e8d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107e90:	eb 4a                	jmp    80107edc <deallocuvm+0xac>
    else if((*pte & PTE_P) != 0){
80107e92:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e95:	8b 00                	mov    (%eax),%eax
80107e97:	83 e0 01             	and    $0x1,%eax
80107e9a:	85 c0                	test   %eax,%eax
80107e9c:	74 3e                	je     80107edc <deallocuvm+0xac>
      pa = PTE_ADDR(*pte);
80107e9e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107ea1:	8b 00                	mov    (%eax),%eax
80107ea3:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107ea8:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
80107eab:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107eaf:	75 0c                	jne    80107ebd <deallocuvm+0x8d>
        panic("kfree");
80107eb1:	c7 04 24 01 89 10 80 	movl   $0x80108901,(%esp)
80107eb8:	e8 97 86 ff ff       	call   80100554 <panic>
      char *v = P2V(pa);
80107ebd:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107ec0:	05 00 00 00 80       	add    $0x80000000,%eax
80107ec5:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
80107ec8:	8b 45 e8             	mov    -0x18(%ebp),%eax
80107ecb:	89 04 24             	mov    %eax,(%esp)
80107ece:	e8 fa ac ff ff       	call   80102bcd <kfree>
      *pte = 0;
80107ed3:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107ed6:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80107edc:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80107ee3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ee6:	3b 45 0c             	cmp    0xc(%ebp),%eax
80107ee9:	0f 82 6c ff ff ff    	jb     80107e5b <deallocuvm+0x2b>
      char *v = P2V(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
80107eef:	8b 45 10             	mov    0x10(%ebp),%eax
}
80107ef2:	c9                   	leave  
80107ef3:	c3                   	ret    

80107ef4 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
80107ef4:	55                   	push   %ebp
80107ef5:	89 e5                	mov    %esp,%ebp
80107ef7:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
80107efa:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80107efe:	75 0c                	jne    80107f0c <freevm+0x18>
    panic("freevm: no pgdir");
80107f00:	c7 04 24 07 89 10 80 	movl   $0x80108907,(%esp)
80107f07:	e8 48 86 ff ff       	call   80100554 <panic>
  deallocuvm(pgdir, KERNBASE, 0);
80107f0c:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107f13:	00 
80107f14:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
80107f1b:	80 
80107f1c:	8b 45 08             	mov    0x8(%ebp),%eax
80107f1f:	89 04 24             	mov    %eax,(%esp)
80107f22:	e8 09 ff ff ff       	call   80107e30 <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
80107f27:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107f2e:	eb 44                	jmp    80107f74 <freevm+0x80>
    if(pgdir[i] & PTE_P){
80107f30:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f33:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107f3a:	8b 45 08             	mov    0x8(%ebp),%eax
80107f3d:	01 d0                	add    %edx,%eax
80107f3f:	8b 00                	mov    (%eax),%eax
80107f41:	83 e0 01             	and    $0x1,%eax
80107f44:	85 c0                	test   %eax,%eax
80107f46:	74 29                	je     80107f71 <freevm+0x7d>
      char * v = P2V(PTE_ADDR(pgdir[i]));
80107f48:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107f4b:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80107f52:	8b 45 08             	mov    0x8(%ebp),%eax
80107f55:	01 d0                	add    %edx,%eax
80107f57:	8b 00                	mov    (%eax),%eax
80107f59:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107f5e:	05 00 00 00 80       	add    $0x80000000,%eax
80107f63:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
80107f66:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107f69:	89 04 24             	mov    %eax,(%esp)
80107f6c:	e8 5c ac ff ff       	call   80102bcd <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
80107f71:	ff 45 f4             	incl   -0xc(%ebp)
80107f74:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
80107f7b:	76 b3                	jbe    80107f30 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = P2V(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
80107f7d:	8b 45 08             	mov    0x8(%ebp),%eax
80107f80:	89 04 24             	mov    %eax,(%esp)
80107f83:	e8 45 ac ff ff       	call   80102bcd <kfree>
}
80107f88:	c9                   	leave  
80107f89:	c3                   	ret    

80107f8a <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80107f8a:	55                   	push   %ebp
80107f8b:	89 e5                	mov    %esp,%ebp
80107f8d:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
80107f90:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80107f97:	00 
80107f98:	8b 45 0c             	mov    0xc(%ebp),%eax
80107f9b:	89 44 24 04          	mov    %eax,0x4(%esp)
80107f9f:	8b 45 08             	mov    0x8(%ebp),%eax
80107fa2:	89 04 24             	mov    %eax,(%esp)
80107fa5:	e8 72 f8 ff ff       	call   8010781c <walkpgdir>
80107faa:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
80107fad:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107fb1:	75 0c                	jne    80107fbf <clearpteu+0x35>
    panic("clearpteu");
80107fb3:	c7 04 24 18 89 10 80 	movl   $0x80108918,(%esp)
80107fba:	e8 95 85 ff ff       	call   80100554 <panic>
  *pte &= ~PTE_U;
80107fbf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fc2:	8b 00                	mov    (%eax),%eax
80107fc4:	83 e0 fb             	and    $0xfffffffb,%eax
80107fc7:	89 c2                	mov    %eax,%edx
80107fc9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107fcc:	89 10                	mov    %edx,(%eax)
}
80107fce:	c9                   	leave  
80107fcf:	c3                   	ret    

80107fd0 <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
80107fd0:	55                   	push   %ebp
80107fd1:	89 e5                	mov    %esp,%ebp
80107fd3:	83 ec 48             	sub    $0x48,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80107fd6:	e8 73 f9 ff ff       	call   8010794e <setupkvm>
80107fdb:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107fde:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107fe2:	75 0a                	jne    80107fee <copyuvm+0x1e>
    return 0;
80107fe4:	b8 00 00 00 00       	mov    $0x0,%eax
80107fe9:	e9 f8 00 00 00       	jmp    801080e6 <copyuvm+0x116>
  for(i = 0; i < sz; i += PGSIZE){
80107fee:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80107ff5:	e9 cb 00 00 00       	jmp    801080c5 <copyuvm+0xf5>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
80107ffa:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ffd:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108004:	00 
80108005:	89 44 24 04          	mov    %eax,0x4(%esp)
80108009:	8b 45 08             	mov    0x8(%ebp),%eax
8010800c:	89 04 24             	mov    %eax,(%esp)
8010800f:	e8 08 f8 ff ff       	call   8010781c <walkpgdir>
80108014:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108017:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010801b:	75 0c                	jne    80108029 <copyuvm+0x59>
      panic("copyuvm: pte should exist");
8010801d:	c7 04 24 22 89 10 80 	movl   $0x80108922,(%esp)
80108024:	e8 2b 85 ff ff       	call   80100554 <panic>
    if(!(*pte & PTE_P))
80108029:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010802c:	8b 00                	mov    (%eax),%eax
8010802e:	83 e0 01             	and    $0x1,%eax
80108031:	85 c0                	test   %eax,%eax
80108033:	75 0c                	jne    80108041 <copyuvm+0x71>
      panic("copyuvm: page not present");
80108035:	c7 04 24 3c 89 10 80 	movl   $0x8010893c,(%esp)
8010803c:	e8 13 85 ff ff       	call   80100554 <panic>
    pa = PTE_ADDR(*pte);
80108041:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108044:	8b 00                	mov    (%eax),%eax
80108046:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010804b:	89 45 e8             	mov    %eax,-0x18(%ebp)
    flags = PTE_FLAGS(*pte);
8010804e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108051:	8b 00                	mov    (%eax),%eax
80108053:	25 ff 0f 00 00       	and    $0xfff,%eax
80108058:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if((mem = kalloc()) == 0)
8010805b:	e8 03 ac ff ff       	call   80102c63 <kalloc>
80108060:	89 45 e0             	mov    %eax,-0x20(%ebp)
80108063:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80108067:	75 02                	jne    8010806b <copyuvm+0x9b>
      goto bad;
80108069:	eb 6b                	jmp    801080d6 <copyuvm+0x106>
    memmove(mem, (char*)P2V(pa), PGSIZE);
8010806b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010806e:	05 00 00 00 80       	add    $0x80000000,%eax
80108073:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
8010807a:	00 
8010807b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010807f:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108082:	89 04 24             	mov    %eax,(%esp)
80108085:	e8 d5 d0 ff ff       	call   8010515f <memmove>
    if(mappages(d, (void*)i, PGSIZE, V2P(mem), flags) < 0)
8010808a:	8b 55 e4             	mov    -0x1c(%ebp),%edx
8010808d:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108090:	8d 88 00 00 00 80    	lea    -0x80000000(%eax),%ecx
80108096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108099:	89 54 24 10          	mov    %edx,0x10(%esp)
8010809d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801080a1:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801080a8:	00 
801080a9:	89 44 24 04          	mov    %eax,0x4(%esp)
801080ad:	8b 45 f0             	mov    -0x10(%ebp),%eax
801080b0:	89 04 24             	mov    %eax,(%esp)
801080b3:	e8 00 f8 ff ff       	call   801078b8 <mappages>
801080b8:	85 c0                	test   %eax,%eax
801080ba:	79 02                	jns    801080be <copyuvm+0xee>
      goto bad;
801080bc:	eb 18                	jmp    801080d6 <copyuvm+0x106>
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
801080be:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801080c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801080c8:	3b 45 0c             	cmp    0xc(%ebp),%eax
801080cb:	0f 82 29 ff ff ff    	jb     80107ffa <copyuvm+0x2a>
      goto bad;
    memmove(mem, (char*)P2V(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, V2P(mem), flags) < 0)
      goto bad;
  }
  return d;
801080d1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801080d4:	eb 10                	jmp    801080e6 <copyuvm+0x116>

bad:
  freevm(d);
801080d6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801080d9:	89 04 24             	mov    %eax,(%esp)
801080dc:	e8 13 fe ff ff       	call   80107ef4 <freevm>
  return 0;
801080e1:	b8 00 00 00 00       	mov    $0x0,%eax
}
801080e6:	c9                   	leave  
801080e7:	c3                   	ret    

801080e8 <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
801080e8:	55                   	push   %ebp
801080e9:	89 e5                	mov    %esp,%ebp
801080eb:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801080ee:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801080f5:	00 
801080f6:	8b 45 0c             	mov    0xc(%ebp),%eax
801080f9:	89 44 24 04          	mov    %eax,0x4(%esp)
801080fd:	8b 45 08             	mov    0x8(%ebp),%eax
80108100:	89 04 24             	mov    %eax,(%esp)
80108103:	e8 14 f7 ff ff       	call   8010781c <walkpgdir>
80108108:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
8010810b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010810e:	8b 00                	mov    (%eax),%eax
80108110:	83 e0 01             	and    $0x1,%eax
80108113:	85 c0                	test   %eax,%eax
80108115:	75 07                	jne    8010811e <uva2ka+0x36>
    return 0;
80108117:	b8 00 00 00 00       	mov    $0x0,%eax
8010811c:	eb 22                	jmp    80108140 <uva2ka+0x58>
  if((*pte & PTE_U) == 0)
8010811e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108121:	8b 00                	mov    (%eax),%eax
80108123:	83 e0 04             	and    $0x4,%eax
80108126:	85 c0                	test   %eax,%eax
80108128:	75 07                	jne    80108131 <uva2ka+0x49>
    return 0;
8010812a:	b8 00 00 00 00       	mov    $0x0,%eax
8010812f:	eb 0f                	jmp    80108140 <uva2ka+0x58>
  return (char*)P2V(PTE_ADDR(*pte));
80108131:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108134:	8b 00                	mov    (%eax),%eax
80108136:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010813b:	05 00 00 00 80       	add    $0x80000000,%eax
}
80108140:	c9                   	leave  
80108141:	c3                   	ret    

80108142 <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
80108142:	55                   	push   %ebp
80108143:	89 e5                	mov    %esp,%ebp
80108145:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80108148:	8b 45 10             	mov    0x10(%ebp),%eax
8010814b:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
8010814e:	e9 87 00 00 00       	jmp    801081da <copyout+0x98>
    va0 = (uint)PGROUNDDOWN(va);
80108153:	8b 45 0c             	mov    0xc(%ebp),%eax
80108156:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010815b:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
8010815e:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108161:	89 44 24 04          	mov    %eax,0x4(%esp)
80108165:	8b 45 08             	mov    0x8(%ebp),%eax
80108168:	89 04 24             	mov    %eax,(%esp)
8010816b:	e8 78 ff ff ff       	call   801080e8 <uva2ka>
80108170:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
80108173:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80108177:	75 07                	jne    80108180 <copyout+0x3e>
      return -1;
80108179:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010817e:	eb 69                	jmp    801081e9 <copyout+0xa7>
    n = PGSIZE - (va - va0);
80108180:	8b 45 0c             	mov    0xc(%ebp),%eax
80108183:	8b 55 ec             	mov    -0x14(%ebp),%edx
80108186:	29 c2                	sub    %eax,%edx
80108188:	89 d0                	mov    %edx,%eax
8010818a:	05 00 10 00 00       	add    $0x1000,%eax
8010818f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
80108192:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108195:	3b 45 14             	cmp    0x14(%ebp),%eax
80108198:	76 06                	jbe    801081a0 <copyout+0x5e>
      n = len;
8010819a:	8b 45 14             	mov    0x14(%ebp),%eax
8010819d:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
801081a0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801081a3:	8b 55 0c             	mov    0xc(%ebp),%edx
801081a6:	29 c2                	sub    %eax,%edx
801081a8:	8b 45 e8             	mov    -0x18(%ebp),%eax
801081ab:	01 c2                	add    %eax,%edx
801081ad:	8b 45 f0             	mov    -0x10(%ebp),%eax
801081b0:	89 44 24 08          	mov    %eax,0x8(%esp)
801081b4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081b7:	89 44 24 04          	mov    %eax,0x4(%esp)
801081bb:	89 14 24             	mov    %edx,(%esp)
801081be:	e8 9c cf ff ff       	call   8010515f <memmove>
    len -= n;
801081c3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801081c6:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
801081c9:	8b 45 f0             	mov    -0x10(%ebp),%eax
801081cc:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
801081cf:	8b 45 ec             	mov    -0x14(%ebp),%eax
801081d2:	05 00 10 00 00       	add    $0x1000,%eax
801081d7:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
801081da:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
801081de:	0f 85 6f ff ff ff    	jne    80108153 <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
801081e4:	b8 00 00 00 00       	mov    $0x0,%eax
}
801081e9:	c9                   	leave  
801081ea:	c3                   	ret    
