...
/* Our doubly linked lists have two header elements: the "head"
   ...
   last element.  The `prev' link of the front header is 0, as
...
{
  return elem != 0 && elem->prev == 0 && elem->next != 0;
}
...
{
  int output_run_cnt;        /* Number of runs output in current pass. */
...
/* Iterates through LIST and removes all but the first in each
   ...
   given auxiliary data AUX.  If DUPLICATES is non-0, then the
